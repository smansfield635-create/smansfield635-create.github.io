...
function buildAuthorityReceipt(input = {}) {
  ...
  return Object.freeze({
    shellSource: normalizeString(source.shellSource ?? source.bootSource, "/index.html"),
    runtimeSource: normalizeString(source.runtimeSource, "/world/runtime/world_runtime.js"),
    truthSource: normalizeString(source.truthSource, "/world/world_kernel.js"),
    structureSource: normalizeString(source.structureSource, "/world/planet_engine.js"),
    projectionOwner: normalizeString(source.projectionOwner, "/world/planet_surface_projector.js"),
    renderSource: normalizeString(source.renderSource, "/world/render.js"),
    controlSource: normalizeString(source.controlSource ?? source.inputOwner ?? source.orbitOwner, "/world/control.js"),
    inputOwner: normalizeString(source.inputOwner, "/world/control.js"),
    orbitSource: normalizeString(source.orbitSource ?? source.orbitOwner, "/world/control.js"),
    ...
    receiptWriter: normalizeString(source.receiptWriter, "/world/runtime/world_runtime.js")
  });
}
...
export function buildInstrumentState(input = {}) {
  ...
  const control = normalizeObject(source.control);
  const motionState = normalizeObject(control.motionState);
  const projectionSummary = normalizeObject(control.projectionSummary);
  const cameraState = normalizeObject(control.cameraState);

  const renderAudit = buildRenderReceipt(source.renderAudit);
  const orbitalHandshake = buildOrbitalHandshakeReceipt({
    ...normalizeObject(source.emissionReceipt),
    ...normalizeObject(source.placementReceipt),
    markerRequired: normalizeObject(source.placementReceipt).markerRequired
  });
  ...
  const authority = buildAuthorityReceipt({
    ...
    orbitSource: "/world/control.js",
    instrumentSource: "/assets/instruments.js",
    receiptWriter: "/world/runtime/world_runtime.js"
  });

  const pseudoSample = Object.freeze({
    latDeg: isFiniteNumber(projectionSummary.latDeg) ? projectionSummary.latDeg : 0,
    lonDeg: isFiniteNumber(projectionSummary.lonDeg) ? projectionSummary.lonDeg : 0,
    sampleX: isFiniteNumber(projectionSummary.sampleX) ? projectionSummary.sampleX : 0,
    sampleY: isFiniteNumber(projectionSummary.sampleY) ? projectionSummary.sampleY : 0,
    cellId: normalizeString(projectionSummary.cellId, "0:0"),
    terrainClass: normalizeString(source.world?.terrainClass, "NONE"),
    stabilityClass: normalizeString(source.world?.stabilityClass, "UNKNOWN"),
    biomeType: "NONE",
    surfaceMaterial: "NONE",
    continentId: "NONE",
    subRegion: "NONE",
    landMask: renderAudit.landFamilyCount > 0 ? 1 : 0,
    waterMask: renderAudit.waterFamilyCount > 0 ? 1 : 0,
    shoreline: renderAudit.shorelineCount > 0,
    plateauStrength: 0,
  ...
  const base = buildInstrumentReceipt({
    currentSample: pseudoSample,
    previousSample: null,
    tickIndex: 0,
    valueState: null,
    psychologyState: null,
    motionState: {
      ...
      pageVisible: true,
      pageRestored: true,
      yawVelocity: motionState.yawVelocity,
      pitchVelocity: motionState.pitchVelocity,
      orbitVelocity: motionState.yawVelocity,
      orbitPhase: motionState.orbitPhase,
  ...
  return Object.freeze({
    ...
    control,
    renderAudit,
    emissionReceipt: Object.freeze(normalizeObject(source.emissionReceipt)),
    placementReceipt: Object.freeze(normalizeObject(source.placementReceipt)),
    scope: Object.freeze(normalizeObject(source.scope)),
    lens: Object.freeze(normalizeObject(source.lens)),
    primitive: Object.freeze(normalizeObject(source.primitive)),
    topology: Object.freeze(normalizeObject(source.topology)),
    renderAuthority: Object.freeze(normalizeObject(source.renderAuthority)),
    density: Object.freeze(normalizeObject(source.density)),
    orbitalHandshake,
    cameraState: Object.freeze(cameraState),
    projectionSummary: Object.freeze(projectionSummary),
    world: Object.freeze({
      ...normalizeObject(base.world),
      phase: normalizeString(source.world?.phase, "RUNNING"),
      terrainClass: normalizeString(source.world?.terrainClass, normalizePrimitive(base.world?.terrainClass)),
      stabilityClass: normalizeString(source.world?.stabilityClass, normalizePrimitive(base.world?.stabilityClass))
    }),
    authority
  });
}
