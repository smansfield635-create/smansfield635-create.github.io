...
export const H_EARTH_GRATITUDE_REGION_CP2_PRESENTATION_PROFILE_ID =
  'H_EARTH_WATER_GPU_SEMANTIC_ATTRIBUTION_PROOF_v1';
...
void main(){
  bool waterAttributed = vPrimitiveIndex==uWaterPrimitiveIndices.x || vPrimitiveIndex==uWaterPrimitiveIndices.y || vPrimitiveIndex==uWaterPrimitiveIndices.z || vPrimitiveIndex==uWaterPrimitiveIndices.w;
  if(waterAttributed){
...
export function createHEarthRun8ER3CPersistentRenderer({ canvas, width = 640, height = 360 } = {}) {
  ...
  const waterPrimitiveIndices = WATER_PRIMITIVE_IDS.map((primitiveId) => renderPackage.primitiveSpans.find((span) => span.primitiveId === primitiveId)?.primitiveIndex);
  if (waterPrimitiveIndices.some((value) => !Number.isSafeInteger(value))) throw new Error('R3C_WATER_ATTRIBUTION_PRIMITIVE_ADDRESS_MISSING');
  ...
  function initialize(packet) {
    ...
    resources.uniforms = {
      ...
      distanceDesaturationStrength: uniform(resources.geometryProgram, 'uDistanceDesaturationStrength'), oceanProofPhase: uniform(resources.geometryProgram, 'uOceanProofPhase'), waterPrimitiveIndices: uniform(resources.geometryProgram, 'uWaterPrimitiveIndices'), depth: uniform(resources.depthProgram, 'uDepth')
    };
  ...
  function getResourceReceipt() {
    ...
    return {
      ...
      presentationProfile: {
        waterGpuSemanticAttributionProof: true, waterPrimitiveIds: WATER_PRIMITIVE_IDS, waterPrimitiveIndices,
