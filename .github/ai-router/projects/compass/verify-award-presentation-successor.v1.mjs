...
check('PURPOSE_FIRST_READ_ESTABLISHED',statement.includes("purpose.classList.add('compass-purpose-stage')")&&statement.includes('purpose.open=true'),'purpose is expanded when recomposed');
check('NARRATIVE_SEQUENCE',statement.includes('IDENTITY_EXPERIENCE_PURPOSE_SYSTEM_READINESS_EVIDENCE')&&statement.includes("instrument.insertAdjacentElement('afterend',purpose)")&&statement.includes("purpose.insertAdjacentElement('afterend',capabilities)"),'identity → experience → purpose → system → readiness → evidence');
