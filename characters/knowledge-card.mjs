...
export const CARD_FOOTER_ACTION_VARIANTS = deepFreeze({
  ...
  CROSS_ESTATE_ROUTE: {
    ...
    contrastRequired: true,
    nonColorCues: ['DISTINCT_TAB_SHAPE', 'ROUTE_GLYPH', 'EXPLICIT_LEAVES_PAGE_TEXT', 'ACCESSIBLE_DESTINATION_NAME']
  }
});
...
export const VERIFIED_CHARACTER_DESTINATIONS = deepFreeze([
  {
    verification: 'VERIFIED_CANONICAL_DESTINATION',
    characterId: 'JEEVES',
    destinationId: 'JEEVES_HEARTH_THRESHOLD',
    kind: 'CROSS_ESTATE_ROUTE',
    href: '/showroom/globe/hearth/jeeves/',
    provenance: 'MAIN_9CF64161:showroom/globe/hearth/jeeves/index.html:fe909379190431baaf825df1b776ec1d66c305f2'
  },
  {
    verification: 'VERIFIED_CANONICAL_DESTINATION',
    characterId: 'ELARA_SYLENE',
    destinationId: 'ELARA_ROOM',
    kind: 'CROSS_ESTATE_ROUTE',
    href: '/elara/',
    provenance: 'MAIN_9CF64161:elara/index.html:e6c8be34c76f40a4c279242511b63e833dfb42ce'
  },
  {
    verification: 'VERIFIED_CANONICAL_DESTINATION',
    characterId: 'AUREN',
    destinationId: 'AUREN_CHARACTER_DESTINATION',
    kind: 'CROSS_ESTATE_ROUTE',
    href: '/products/auren/',
    provenance: 'MAIN_9CF64161:products/auren/index.html:d6cd128ca9aa054655a8da71b6f29e8d1ea435b6'
  }
]);

export const CARDINAL_CHARACTER_CONVERSATION_BINDINGS = deepFreeze(Object.fromEntries(
  VERIFIED_CHARACTER_DESTINATIONS.map((destination) => [destination.characterId, {
    characterId: destination.characterId,
    status: 'VERIFIED_CANONICAL_DESTINATION',
    destination,
    exposed: true
  }])
));
...
export function resolveKnowledgeCardFooterActions(sceneState, { verifiedDestinations = VERIFIED_CHARACTER_DESTINATIONS } = {}) {
  if (sceneState?.version !== CARDINAL_SCENE_STATE_VERSION) throw new TypeError('INVALID_CARDINAL_SCENE_STATE');
  ...
  for (const destination of destinations) {
    const inPagePresenceSatisfied = destination.kind !== 'IN_PAGE_CHARACTER_INTERACTION'
      || (destination.characterId === characterId && presence.state === 'CHARACTER_PRESENT');
    if (!inPagePresenceSatisfied) continue;
    const name = CHARACTER_DISPLAY_NAMES[destination.characterId] ?? destination.characterId;
    const crossEstate = destination.kind === 'CROSS_ESTATE_ROUTE';
    actions.push(action({
      id: 'TALK_TO_CHARACTER',
      label: crossEstate ? `↗ Talk to ${name} · Leaves this page` : `Talk to ${name}`,
      variant: crossEstate ? 'CROSS_ESTATE_ROUTE' : 'CHARACTER_CONVERSATION',
      destination: deepFreeze({ ...destination }),
      description: crossEstate
        ? `Open the verified ${name} destination outside the current Characters page.`
        : `Begin the verified in-scene interaction with ${name}.`
    }));
  }

  const talkCount = actions.filter(({ id }) => id === 'TALK_TO_CHARACTER').length;
  const talkDisposition = talkCount > 0
    ? `EXPOSED_${talkCount}_VERIFIED_CHARACTER_DESTINATIONS`
    : presence.state === 'CHARACTER_PRESENT'
...
export function evaluateKnowledgeCardContract() {
  ...
  const defaultTalkActions = defaultFooter.actions.filter(({ id }) => id === 'TALK_TO_CHARACTER');
  const exactDefaultHrefs = defaultTalkActions.map(({ destination }) => destination.href).sort();
  const expectedDefaultHrefs = ['/elara/', '/products/auren/', '/showroom/globe/hearth/jeeves/'].sort();
  if (defaultTalkActions.length !== 3 || JSON.stringify(exactDefaultHrefs) !== JSON.stringify(expectedDefaultHrefs)) issues.push('THREE_VERIFIED_DESTINATIONS_NOT_EXPOSED');
  if (defaultTalkActions.some(({ label, variant }) => variant.variant !== 'CROSS_ESTATE_ROUTE' || !variant.nonColorCues?.includes('EXPLICIT_LEAVES_PAGE_TEXT') || !label.startsWith('↗ ') || !label.includes('Leaves this page'))) issues.push('VERIFIED_DESTINATION_ROUTE_DISTINCTION_FAILURE');
  ...
  if (!externalAction || externalAction.variant.variant !== 'CROSS_ESTATE_ROUTE' || !externalAction.variant.nonColorCues?.includes('EXPLICIT_LEAVES_PAGE_TEXT') || !externalAction.label.includes('Leaves this page')) issues.push('CROSS_ESTATE_ROUTE_NOT_DISTINCT');
  ...
  if (!siteOnlyFooter.actions.some(({ id }) => id === 'TALK_TO_CHARACTER')) issues.push('CROSS_ESTATE_ROUTE_IMPROPERLY_TIED_TO_LOCAL_PRESENCE');
  ...
  return deepFreeze({
    ...
    defaultConversationDestinationCount: VERIFIED_CHARACTER_DESTINATIONS.length,
