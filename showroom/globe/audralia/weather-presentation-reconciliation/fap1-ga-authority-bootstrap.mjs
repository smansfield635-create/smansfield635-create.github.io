import {createExteriorWeatherProjectionGA} from './exterior-weather.ga.mjs';
...
async function initialize(){
  ...
  if(receipt.exterior?.overlay){
    ...
    receipt.exterior.overlay.dataset.meteorologicalAuthority='RETIRED_VISIBLE_AUTHORITY';
  }
  if(receipt.canonicalWeather?.overlay){
    receipt.canonicalWeather.overlay.style.visibility='hidden';
    receipt.canonicalWeather.overlay.dataset.fap1GADiagnosticOnly='true';
    receipt.canonicalWeather.overlay.dataset.meteorologicalAuthority='REFERENCE_ONLY';
  }
  ...
  function frame(now){
    ...
    if(signature!==lastSignature||now-lastTimeRender>650){
      ga.render(camera);
  ...
  globalThis.__AUDRALIA_FAP1_GA_AUTHORITY__=Object.freeze({
    schema:'AUDRALIA_FAP1_GA_ONE_WEATHER_AUTHORITY_v1',
