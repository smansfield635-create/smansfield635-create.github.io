...
def main():
    ...
    g=mod.prep_raw(df); x=mod.agg_months(g)
    train=x[x.year<=2022].copy(); temporal=x[x.year>=2023].copy()
    train_transfer=x[(x.year<=2022)&x.STATE.isin(['KS','OR'])].copy()
    transfer=x[(x.year>=2023)&(x.STATE=='IN')].copy()
    ...
    if not evaluable:
        ...
        b0,b1,brel,bauc,p0,p1=mod.fit_eval(train,temporal)
        t0,t1,trel,tauc,_,_=mod.fit_eval(train_transfer,transfer)
        ...
        for s in mod.STATES:
            ...
            m0,m1,rel,ad,_,_=mod.fit_eval(train,te)
        ...
        findings.update({'temporal':{'base':b0,'aug':b1,'brier_rel_improve':brel,'auroc_delta':bauc},'transfer_IN':{'base':t0,'aug':t1,'brier_rel_improve':trel,'auroc_delta':tauc},'state_results':state_results,'stratified_residual_stat':obs,'stratified_residual_p':pp,'criteria':criteria,'verdict':'PASS' if all(criteria.values()) else 'FAIL'})
        out=temporal[['permit','STATE','month','year','Y','max_util']].copy(); out['p_base']=p0; out['p_aug']=p1; out.to_csv('wastewater_safe_envelope_temporal_predictions_v2.csv',index=False)
    Path('wastewater_safe_envelope_findings_v2.json').write_text(json.dumps(findings,indent=2,default=str))
    Path('WASTEWATER_SAFE_ENVELOPE_VERDICT_v2.txt').write_text(findings['verdict']+'\n'+json.dumps(findings,indent=2,default=str))
    x.sample(min(20000,len(x)),random_state=mod.SEED).to_csv('wastewater_safe_envelope_snapshot_sample_v2.csv',index=False)
