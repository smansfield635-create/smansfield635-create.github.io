...
function drawDisciplinedRegionLabels(ctx, state, projector) {
  ...
  for (const item of candidates) {
    ...
    const fontPx = Math.max(9, projector.radius(14, item.y));
...
export async function createScene(canvas, outputs) {
  ...
  function updateViewportOffset(forceSnap = false) {
    ...
    const followX = targetScreenX - (projectedPlayer.x + baseX);
    const followY = targetScreenY - (projectedPlayer.y + baseY);
  ...
  function worldPointFromClient(clientX, clientY) {
    ...
    return {
      x: local.x - worldViewportOffset.x,
      y: local.y - worldViewportOffset.y
    };
  }
