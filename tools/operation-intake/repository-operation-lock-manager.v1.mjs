...
const decodeContent=(content,source,producerId='CURRENT_LEDGER_DECODE')=>{if(typeof content!=='string'||!content.replace(/\s/g,''))throw err('LEDGER_CONTENT_UNAVAILABLE','content',source);const rawBytes=Buffer.from(content,'utf8'),decodedBytes=Buffer.from(content.replace(/\s/g,''),'base64'),diagnostic=historicalLedgerDiagnostic({source,objectSha:'UNBOUND',transport:producerId,encoding:'base64',rawBytes,decodedBytes});try{return ledger(JSON.parse(decodedBytes.toString('utf8')))}catch(e){if(e.code)throw e;throw err('LEDGER_JSON_DECODE_FAILURE','content',source,JSON.stringify({...diagnostic,producerId,parseError:e.message}))}};
async function readRemote({repository,lockRef=LOCK_REF,token}) {
  ...
  if(typeof f.content==='string'&&f.content.replace(/\s/g,''))return{blob,head,ledger:decodeContent(f.content,'contents-api','CURRENT_LEDGER_CONTENTS_API'),contentTransport:'CONTENTS_INLINE'};
  const g=await req(`${u}/git/blobs/${blob}`,{headers:H(token)});if(g.encoding!=='base64')throw err('LEDGER_BLOB_ENCODING_UNSUPPORTED','encoding','git-blob',String(g.encoding));return{blob,head,ledger:decodeContent(g.content,'git-blob','CURRENT_LEDGER_GIT_BLOB_FALLBACK'),contentTransport:'GIT_BLOB_FALLBACK'};
}
...
async function readGitLedgerBlob({repository,token,blobSha,source='resulting-ledger',commitSha=null}){const sha=dig(blobSha,40,'ledgerBlobSha',source);if(process.env.REPOSITORY_OPERATION_LOCAL_GIT_BLOB_READ==='1'){const tempDir=fs.mkdtempSync(path.join(process.env.RUNNER_TEMP||'/tmp','ledger-blob-')),tempPath=path.join(tempDir,'ledger.json'),fd=fs.openSync(tempPath,'w');try{execFileSync('git',['cat-file','blob',sha],{stdio:['ignore',fd,'inherit'],encoding:null,maxBuffer:1024});const raw=fs.readFileSync(tempPath,'utf8');if(raw.startsWith('Warning: truncated output')){const type=execFileSync('git',['cat-file','-t',sha],{encoding:'utf8',maxBuffer:1024}).trim(),size=execFileSync('git',['cat-file','-s',sha],{encoding:'utf8',maxBuffer:1024}).trim(),prefix=raw.slice(0,96).replace(/[\r\n]+/g,' ');throw err('LOCAL_GIT_BLOB_OUTPUT_DECORATED','content',source+'-git-object',`commitSha=${commitSha||'UNKNOWN'}:blobSha=${sha}:blobType=${type}:blobSize=${size}:blobPrefix=${JSON.stringify(prefix)}`)}return ledger(JSON.parse(raw))}catch(e){if(e.code)throw e;const rawBytes=Buffer.from(typeof raw==='string'?raw:'','utf8'),diagnostic=historicalLedgerDiagnostic({source:source+'-git-object',objectSha:sha,transport:'HISTORICAL_READ_GIT_BLOB',encoding:'raw',rawBytes,decodedBytes:rawBytes});throw err('LEDGER_JSON_DECODE_FAILURE','content',source+'-git-object',JSON.stringify({...diagnostic,producerId:'HISTORICAL_READ_GIT_BLOB',commitSha:commitSha||null,parseError:e.message}))}finally{fs.closeSync(fd);fs.rmSync(tempDir,{recursive:true,force:true})}}const g=await req(`${base(repository)}/git/blobs/${sha}`,{headers:{...H(token),Accept:'application/vnd.github.raw+json'}},[200],'LEDGER_BLOB_READ');let rawBytes;if(typeof g==='string')rawBytes=Buffer.from(g,'utf8');else if(typeof g?.raw==='string')rawBytes=Buffer.from(g.raw,'utf8');else if(g?.encoding==='base64'&&typeof g?.content==='string')rawBytes=Buffer.from(g.content.replace(/\s/g,''),'base64');else throw err('LEDGER_BLOB_ENCODING_UNSUPPORTED','encoding',source,'EXPECTED_SINGLE_BLOB_PAYLOAD');const actualGitBlobSha=createHash('sha1').update(Buffer.concat([Buffer.from(`blob ${rawBytes.length}\\0`,'utf8'),rawBytes])).digest('hex');if(actualGitBlobSha!==sha)throw err('LEDGER_BLOB_TRANSPORT_CORRESPONDENCE_FAILURE','content',source,JSON.stringify({requestedBlobSha:sha,actualGitBlobSha,rawByteLength:rawBytes.length,rawSha256:createHash('sha256').update(rawBytes).digest('hex'),rawFirst64Hex:rawBytes.subarray(0,64).toString('hex')}));if(rawBytes.subarray(0,32).toString('utf8').startsWith('Warning: truncated output'))throw err('LEDGER_BLOB_TRANSPORT_CORRESPONDENCE_FAILURE','content',source,JSON.stringify({requestedBlobSha:sha,actualGitBlobSha,rawByteLength:rawBytes.length,rawSha256:createHash('sha256').update(rawBytes).digest('hex'),rawFirst64Hex:rawBytes.subarray(0,64).toString('hex'),reason:'DECORATED_TRUNCATED_PAYLOAD'}));try{return ledger(JSON.parse(rawBytes.toString('utf8')))}catch(e){if(e.code)throw e;throw err('LEDGER_JSON_DECODE_FAILURE','content',source,e.message)}}
