...
try{
  ...
  assert.ok(o1.every(v=>v?.pass===true&&v?.cameraRelativeFallback===false),'C3C3_WORLD_SUN_LOCK_FAILED');
  assert.ok(projectedKeys.size>=2||visibilityStates.size>=2,`C3C3_SUN_DID_NOT_MOVE_WITH_CAMERA:${[...projectedKeys]}`);
  assert.equal(depthPass,true,'C3C3_DEPTH_HIERARCHY_FAILED');
  assert.ok(boundaryViews.length>=1,'C3C3_NO_CONNECTED_REGION_THRESHOLD_ENCOUNTERED');
  assert.ok(oceanViews.length>=1,'C3C3_NO_OPEN_OCEAN_VIEW_ENCOUNTERED');
