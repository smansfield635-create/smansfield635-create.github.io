/* /showroom/index.cosmos.js
   ...
   the accepted Showroom/ARCHCOIN static-base and burst-only sparkle corridor inside
...
(() => {
  ...
  const CONFIG = Object.freeze({
    ...
    sceneSelector: "[data-showroom-scene-field]",
  ...
  function installStyle() {
    ...
    style.textContent = `
      ...
      @media (max-width: 820px) {
        [${CANVAS_ATTRIBUTE}="${BASE_VALUE}"] { opacity: .82; }
      }
      @media (max-width: 560px) {
        [${CANVAS_ATTRIBUTE}="${BASE_VALUE}"] { opacity: .76; }
      }
  ...
  function bindEnvironment() {
    ...
    if (state.root && "MutationObserver" in globalThis) {
      ...
      state.motionObserver.observe(state.root, {
        attributes: true,
        attributeFilter: ["data-reduced-motion", "data-showroom-reduced-motion"]
      });
    }
