...
const retiredAutomaticRelease=read('.github/workflows/ai-entry-auto-release.yml');
...
check('policy-automatic-release-not-central',!Object.values(policy.checkoutLocality?.centralWorkflows||{}).includes('.github/workflows/ai-entry-auto-release.yml'));
check('policy-automatic-release-retired',policy.checkoutLocality?.retiredWorkflows?.automaticRelease?.status==='RETIRED_NON_MANDATORY');
check('policy-automatic-release-not-mandatory',policy.checkoutLocality?.retiredWorkflows?.automaticRelease?.localityInstrumentRequired===false);
check('policy-automatic-release-cannot-gate',policy.checkoutLocality?.retiredWorkflows?.automaticRelease?.mayGate===false);
check('retired-automatic-release-marker',retiredAutomaticRelease.includes('AI_ENTRY_AUTOMATIC_EXACT_HEAD_RELEASE=RETIRED'));
check('retired-automatic-release-dispatch-only',retiredAutomaticRelease.includes('workflow_dispatch:')&&!/^\s*push\s*:/m.test(retiredAutomaticRelease));
check('retired-automatic-release-no-pages-write',!retiredAutomaticRelease.includes('pages: write'));
