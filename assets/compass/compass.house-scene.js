...
const VERSION='mirror-manor-gothic-phase3-carousel-v2-visible';
...
function mount(canvas,{foreground=()=>true}={}){
 ...
 const api={fallback:false,setForeground:on=>{canvas.dataset.manorForeground=on?'true':'false';if(on)drawImpl()},draw:()=>drawImpl(),inspect:()=>({fallback:false,renderer:VERSION,contract:canvas.dataset.houseContract,foreground:foreground(),geometry:canvas.dataset.houseGeometry,carouselDistance:94,principalSpan:27,motion:'static-exhibit',continuity:CONTINUITY,ready,cullPolicy:canvas.dataset.houseCullPolicy,auditPass:canvas.dataset.houseAuditPass})};
