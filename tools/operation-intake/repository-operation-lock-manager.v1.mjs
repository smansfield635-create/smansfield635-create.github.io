...
export async function closeRemote(a) {
  ...
  let lineageCheckpoint=null;
  try { const checkpointFile=await req(`${base(a.repository)}/contents/${LINEAGE_CHECKPOINT_PATH}?ref=main`,{headers:H(a.token)},[200],'AUTHORITY_LINEAGE_CHECKPOINT_READ'); lineageCheckpoint=JSON.parse(Buffer.from(checkpointFile.content||'','base64').toString('utf8')); } catch(e) { if(e?.code==='AUTHORITY_LEDGER_LINEAGE_UNTRUSTED')throw e; throw err('AUTHORITY_LEDGER_LINEAGE_UNTRUSTED','lineageCheckpoint','authority-lineage',e.message); }
