...
// AUDRALIA_G2_5_PARENT_ENGINE_CHILD_COMPOSITION_TNT_v1
// Parent mount/composition engine for Audralia clean-canvas stack.
// Owns: mount, canvas creation, child loading, pixel composition, FORM_VISIBLE compatibility, receipt aggregation.
// Children:
//   /assets/audralia/clean/engine/continents.js
//   /assets/audralia/clean/engine/motion.js
//   /assets/audralia/clean/engine/sky.js
// Does not own: route bridge HTML, route bridge JS, parent Globe, Characters, Gauges, Showroom, or navigation.

(() => {
  "use strict";

  const CONTRACT = "AUDRALIA_G2_5_PARENT_ENGINE_CHILD_COMPOSITION_TNT_v1";
  const RECEIPT = "AUDRALIA_G2_5_PARENT_ENGINE_CHILD_COMPOSITION_RECEIPT_v1";
  const PREVIOUS_CONTRACT = "AUDRALIA_G2_4_AXIS_ROTATION_FINGER_DRAG_NON_BLOB_CONTINENT_ENGINE_TNT_v1";
  const ROUTE = "/showroom/globe/audralia/";
  const VERSION = "2026-05-20.audralia-g2-5-parent-engine-child-composition-v1";

  const CHILDREN = Object.freeze({
    continents: Object.freeze({
      path: "/assets/audralia/clean/engine/continents.js",
      globalName: "AUDRALIA_CONTINENTS_ENGINE",
      fallbackName: "fallback-continents"
    }),
    motion: Object.freeze({
      path: "/assets/audralia/clean/engine/motion.js",
      globalName: "AUDRALIA_MOTION_ENGINE",
      fallbackName: "fallback-motion"
    }),
    sky: Object.freeze({
      path: "/assets/audralia/clean/engine/sky.js",
      globalName: "AUDRALIA_SKY_ENGINE",
      fallbackName: "fallback-sky"
    })
  });
  ...
  function fallbackContinentsEngine() {
    ...
    function classifySurface(worldNormal) {
      ...
      const bandA = Math.exp(-Math.pow((u + 0.38) / 0.28, 2) - Math.pow((v + 0.08) / 0.20, 2));
      const bandB = Math.exp(-Math.pow((u - 0.24) / 0.28, 2) - Math.pow((v - 0.03) / 0.22, 2));
      const bandC = Math.exp(-Math.pow((u + 0.12) / 0.24, 2) - Math.pow((v - 0.30) / 0.18, 2));
      const bandD = Math.exp(-Math.pow((u - 0.20) / 0.22, 2) - Math.pow((v + 0.42) / 0.15, 2));
      const north = Math.exp(-Math.pow(u / 0.42, 2) - Math.pow((v - 0.78) / 0.17, 2));

      const terrainCandidate = clamp01(Math.max(bandA, bandB, bandC, bandD, north));
      const summit = clamp01(Math.pow(Math.max(0, dot3(n, normalize3({ x: 0.12, y: 0.36, z: 0.92 }))), 28));
      const elevation = clamp01(terrainCandidate * 0.68 + summit * 0.22);
      const southPole = v < -0.68;
      const seaLevel = PLANET.seaLevel;

      const exposedLand = !southPole && terrainCandidate > 0.42 && elevation > seaLevel;
      const drownedContinent = !exposedLand && terrainCandidate > 0.47;
      const nearSeaShelf = !exposedLand && terrainCandidate > 0.32 && !drownedContinent;
      const deepOcean = !exposedLand && !drownedContinent && !nearSeaShelf;
      const southIce = southPole ? 0.85 : 0;
      const northIce = v > 0.78 ? 0.45 : 0;
    ...
    function sampleRatios() {
      return Object.freeze({
        exposedLandRatio: 0.18,
        shelfRatio: 0.16,
        drownedContinentRatio: 0.18,
        deepOceanRatio: 0.48,
        iceRatio: 0.10
      });
    }
  ...
  function fallbackMotionEngine() {
    ...
    return Object.freeze({
      ...
      projectViewToWorld(viewNormal, motionState) {
        return motionState?.projectViewToWorld
  ...
  function fallbackSkyEngine() {
    ...
    function applySkyColor(color, surface = {}, lighting = null, context = {}) {
      ...
      const raw = dot3(n, light);
      const lit = clamp01(raw * 0.74 + 0.44);
  ...
  function loadAsClassicScript(path, globalName) {
    return new Promise((resolve) => {
      ...
      if (existing) {
        setTimeout(() => resolve(win()[globalName] || null), 30);
        return;
      }
      ...
      script.onload = () => resolve(win()[globalName] || null);
  ...
  async function loadChild(name, fallbackFactory) {
    ...
    let loaded = win()[child.globalName] || null;

    if (!loaded) {
      loaded = await importAsModule(child.path, child.globalName);
    }
    ...
    if (!loaded) {
      ...
      state.childStatus[name] = "loaded";
    }

    state.children[name] = loaded;
    state.childContracts[name] = loaded?.contract || child.fallbackName;
    ...
    for (let y = 0; y < size; y += 1) {
      for (let x = 0; x < size; x += 1) {
        ...
        if (sky.applySkyColor) {
          color = sky.applySkyColor(color, skySurface, null, {
            viewNormal,
  ...
  function updateLabel() {
    ...
    label.textContent = `Audralia G2.5 · child stack active · lon ${lon} · land ${land} · ocean ${ocean}`;
  }
