...
function installTensorCacheAlias() {
  ...
  function aliasUrl(input) {
    ...
    if (url.origin === window.location.origin && url.pathname === MODEL_MANIFEST_ROOT + "tensor-cache.json") {
...
function shouldUseFirstPartySiteContract(prompt) {
  const text = normalize(prompt).toLowerCase();
  if (!text) return false;
  const asksAboutFirstParty = /\b(diamond\s*gate\s*bridge|diamondgatebridge\.com|on your side aai|native chat|this website|this site|this page|this build|public talk|jeeves)\b/.test(text);
  const asksForDescription = /\b(what is|what are|tell me|explain|describe|about|can you tell|can you help|why did|how does|what can|role specifically|other characters)\b/.test(text);
