/*
  ...
  PURPOSE=
  CREATE_HYDRATION_ENGINE_FOR_PLANET_1
  MAKE_WATER_ADHERE_TO_TERRAIN
  PROVIDE_OCEAN_DEPTH_SHORELINE_RIVERS_LAKES_GLACIERS_BASINS
  SUPPORT_TERRAIN_ENGINE_WITH_SHARED_LAND_MASKS
  ...
  function createHydrationDefs(uid) {
    return `
      ...
      <radialGradient id="${uid}_lakeDepth" cx="38%" cy="32%" r="72%">
        <stop offset="0%" stop-color="rgba(88,210,231,0.58)"></stop>
        <stop offset="48%" stop-color="rgba(17,112,151,0.44)"></stop>
        <stop offset="100%" stop-color="rgba(4,34,56,0.70)"></stop>
      </radialGradient>

      <linearGradient id="${uid}_glacier" x1="0" y1="0" x2="1" y2="1">
        <stop offset="0%" stop-color="rgba(245,252,255,0.86)"></stop>
        <stop offset="52%" stop-color="rgba(184,220,238,0.58)"></stop>
        <stop offset="100%" stop-color="rgba(110,165,205,0.34)"></stop>
      </linearGradient>
    `;
  }

  function useLandStrokes(land, color, width, opacity, label) {
    return `
      <g aria-label="${label}" opacity="${opacity}">
  ...
  function createHydrationLayer(uid, options) {
    ...
    return `
      ...
      >
        ...
        ${useLandStrokes(land, "rgba(32,151,191,0.58)", 26, "0.34", "continental shelf water depth")}
        ${useLandStrokes(land, "rgba(145,219,226,0.62)", 13, "0.34", "shoreline wetness blend")}
        ${useLandStrokes(land, "rgba(246,226,161,0.50)", 5, "0.34", "beach and erosion transition")}

        <g aria-label="rivers cut into land" filter="url(#${uid}_waterInset)">
          <path d="M420 202 C446 253 462 300 482 352 C504 409 522 455 552 514" fill="none" stroke="rgba(92,209,233,0.46)" stroke-width="6" stroke-linecap="round"></path>
          <path d="M514 371 C489 412 466 449 454 503 C444 548 416 593 383 636" fill="none" stroke="rgba(92,209,233,0.36)" stroke-width="5" stroke-linecap="round"></path>
          <path d="M315 401 C359 435 396 470 440 507 C478 539 521 567 582 602" fill="none" stroke="rgba(92,209,233,0.34)" stroke-width="4.8" stroke-linecap="round"></path>
          <path d="M720 421 C756 458 781 501 802 552" fill="none" stroke="rgba(92,209,233,0.32)" stroke-width="4.5" stroke-linecap="round"></path>
          <path d="M190 454 C226 493 252 540 278 584" fill="none" stroke="rgba(92,209,233,0.32)" stroke-width="4.5" stroke-linecap="round"></path>
        </g>

        <g aria-label="lake catchment basins" filter="url(#${uid}_waterInset)">
          ...
          <path d="M396 602 C442 578 500 586 552 618 C520 650 462 655 410 632Z" fill="rgba(23,91,112,0.42)" opacity="0.62"></path>
          <path d="M252 518 C281 490 322 492 349 520 C319 548 279 547 252 518Z" fill="rgba(23,91,112,0.38)" opacity="0.56"></path>
        </g>

        <g aria-label="glacier fields attached to cold terrain">
          <use href="#${land.northPole}" fill="url(#${uid}_glacier)" opacity="0.72"></use>
          <use href="#${land.southPole}" fill="url(#${uid}_glacier)" opacity="0.68"></use>
          <path d="M482 84 C520 80 562 86 602 106 C584 128 538 136 496 128 C466 122 447 108 482 84Z" fill="url(#${uid}_glacier)" opacity="0.58"></path>
          <path d="M420 886 C474 870 540 874 596 902 C556 928 482 932 421 910Z" fill="url(#${uid}_glacier)" opacity="0.50"></path>
        </g>
      </g>
    `;
  }
