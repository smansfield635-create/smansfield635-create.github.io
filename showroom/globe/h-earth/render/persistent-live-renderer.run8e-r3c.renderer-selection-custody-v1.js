...
export const H_EARTH_GRATITUDE_REGION_CP2_PRESENTATION_PROFILE_ID =
  'H_EARTH_RENDERER_SELECTION_CUSTODY_PROOF_v1';
...
const WATER_PRIMITIVE_IDS = Object.freeze([
...
uniform uvec4 uWaterPrimitiveIndices;
uniform uint uExactWaterSpanDiagnostic;
uniform uint uRendererSelectionCustody;
out vec4 outColor;
...
void main(){
  if(uRendererSelectionCustody==1u && gl_FragCoord.x < 72.0 && gl_FragCoord.y < 72.0){
    float checker=mod(floor(gl_FragCoord.x/9.0)+floor(gl_FragCoord.y/9.0),2.0);
    outColor=vec4(mix(vec3(1.0,0.0,0.78),vec3(1.0,1.0,0.0),checker),1.0);
    return;
  }
  if(uExactWaterSpanDiagnostic==1u){
    float diagnosticStripe=step(0.5,fract((vWorldPosition.x+vWorldPosition.z)*0.075));
    vec3 diagnostic=mix(vec3(1.0,0.0,0.72),vec3(1.0,1.0,1.0),diagnosticStripe*0.58);
    outColor=vec4(diagnostic,1.0);
    return;
  }
  ...
  if(vRoleCode==1u){
    ...
    float broadWave=stableWave(dot(waterWorld,vec2(0.052,0.031))+uOceanProofPhase*0.72);
    float crossWave=stableWave(dot(waterWorld,vec2(-0.087,0.063))-uOceanProofPhase*0.49+1.7);
    float capillary=stableWave(dot(waterWorld,vec2(0.19,-0.14))+uOceanProofPhase*0.31+4.2);
    float waterNoise=noise2(waterWorld*0.026+vec2(uOceanProofPhase*0.015,-uOceanProofPhase*0.011));
    float compositeWave=clamp(broadWave*0.46+crossWave*0.31+capillary*0.11+waterNoise*0.12,0.0,1.0);
    float shorelineDepth=smoothstep(-96.0,-176.0,vWorldPosition.z);
...
export function createHEarthRun8ER3CPersistentRenderer({ canvas, width = 640, height = 360 } = {}) {
  ...
  const waterPrimitiveSpans = WATER_PRIMITIVE_IDS.map((primitiveId) => renderPackage.primitiveSpans.find((span) => span.primitiveId === primitiveId));
  if (waterPrimitiveSpans.some((span) => !span || !Number.isSafeInteger(span.indexStart) || !Number.isSafeInteger(span.indexCount) || span.indexCount <= 0)) throw new Error('R3C_WATER_ATTRIBUTION_INDEX_SPAN_MISSING');
  const waterPrimitiveIndices = waterPrimitiveSpans.map((span) => span.primitiveIndex);
  const waterSpanByStart = new Map(waterPrimitiveSpans.map((span) => [span.indexStart, span]));
  ...
  function initialize(packet) {
    ...
    resources.uniforms = {
      ...
      distanceDesaturationStrength: uniform(resources.geometryProgram, 'uDistanceDesaturationStrength'), oceanProofPhase: uniform(resources.geometryProgram, 'uOceanProofPhase'), waterPrimitiveIndices: uniform(resources.geometryProgram, 'uWaterPrimitiveIndices'), exactWaterSpanDiagnostic: uniform(resources.geometryProgram, 'uExactWaterSpanDiagnostic'), rendererSelectionCustody: uniform(resources.geometryProgram, 'uRendererSelectionCustody'), depth: uniform(resources.depthProgram, 'uDepth')
    };
    ...
    gl.uniform4ui(resources.uniforms.waterPrimitiveIndices, ...waterPrimitiveIndices);
    gl.uniform1ui(resources.uniforms.exactWaterSpanDiagnostic, 0);
  ...
  function renderFrame(packet) {
    ...
    for (const range of packet.drawRanges) {
      ...
      let cursor = range.indexStart;
      while (cursor < rangeEnd) {
        const waterSpan = waterSpanByStart.get(cursor);
        if (waterSpan) {
          gl.uniform1ui(resources.uniforms.exactWaterSpanDiagnostic, 1);
          gl.drawElements(gl.TRIANGLES, waterSpan.indexCount, gl.UNSIGNED_INT, waterSpan.indexStart * 4);
          counters.geometryDrawCallCount += 1; counters.totalDrawnIndexCount += waterSpan.indexCount;
          cursor += waterSpan.indexCount;
          continue;
        }
        const nextWaterStart = waterPrimitiveSpans
          .map((span) => span.indexStart)
          .filter((start) => start > cursor && start < rangeEnd)
          .sort((a,b) => a-b)[0] ?? rangeEnd;
        const normalCount = nextWaterStart - cursor;
        if (normalCount > 0) {
          gl.uniform1ui(resources.uniforms.exactWaterSpanDiagnostic, 0);
          gl.drawElements(gl.TRIANGLES, normalCount, gl.UNSIGNED_INT, cursor * 4);
          counters.geometryDrawCallCount += 1; counters.totalDrawnIndexCount += normalCount;
          cursor += normalCount;
        }
      }
    }
  ...
  function getResourceReceipt() {
    ...
    return {
      ...
      presentationProfile: {
        rendererSelectionCustodyProof: true, rendererSelectionMarker: '72PX_MAGENTA_YELLOW_CHECKER_BOTTOM_LEFT',
        waterExactIndexSpanAttributionProof: true, waterGpuSemanticAttributionProof: false, waterPrimitiveIds: WATER_PRIMITIVE_IDS, waterPrimitiveIndices,
        waterPrimitiveSpans: waterPrimitiveSpans.map(({primitiveId,indexStart,indexCount,primitiveIndex})=>({primitiveId,indexStart,indexCount,primitiveIndex})),
        diagnosticWaterTreatment: 'MAGENTA_WHITE_STRIPE', oceanGraphicalExpressionProof: false, continuousWaterExpression: false, multiScaleDeterministicSurface: false,
        shorelineTruthMutated: false, waterTruthMutated: false, fluidSimulationClaimed: false,
