...
function wire(renderer,sky,exterior,canonical,fap1){
  const pointers=new Map();let gesture=null,wheelTimer=0,settleFastTimer=0,settleFullTimer=0;
  ...
  const motion={startedAt:0,lastAt:0,lastPoint:null,velocity:Infinity,stage:'REST',lastPartialAt:-Infinity,lastLateAt:-Infinity};
  const evidence={schema:'AUDRALIA_INTERACTION_QUALITY_LADDER_v1',stage:'REST',fastFrames:0,partialFrames:0,lateFrames:0,fullSettles:0,lastVelocity:0};
  ...
  const renderFast=()=>{renderer.render();evidence.fastFrames++;};
  const renderPartial=now=>{renderer.render();if(now-motion.lastPartialAt<180){evidence.fastFrames++;return;}const camera=cameraFrame(renderer);sky.render(camera);exterior.render(camera);motion.lastPartialAt=now;evidence.partialFrames++;};
  const renderLate=now=>{renderer.render();if(now-motion.lastLateAt<320){renderPartial(now);return;}const camera=cameraFrame(renderer);sky.render(camera);exterior.render(camera);fap1.render(camera);motion.lastLateAt=now;motion.lastPartialAt=now;evidence.lateFrames++;};
  const cancelSettle=()=>{clearTimeout(settleFastTimer);clearTimeout(settleFullTimer);settleFastTimer=0;settleFullTimer=0;};
  ...
  const updateMotion=point=>{const now=performance.now(),dt=Math.max(1,now-motion.lastAt),d=motion.lastPoint?Math.hypot(point.x-motion.lastPoint.x,point.y-motion.lastPoint.y):0,v=d/dt;motion.velocity=motion.velocity===Infinity?v:motion.velocity*.68+v*.32;motion.lastAt=now;motion.lastPoint=point;evidence.lastVelocity=motion.velocity;const age=now-motion.startedAt;if(age<170||motion.velocity>.24){motion.stage='INITIAL';evidence.stage='INITIAL';renderFast();}else if(motion.velocity>.075||age<650){motion.stage='LATE_MOTION';evidence.stage='LATE_MOTION';renderPartial(now);}else{motion.stage='PRE_SETTLE';evidence.stage='PRE_SETTLE';renderLate(now);}};
  const settle=()=>{cancelSettle();motion.stage='RELEASE';evidence.stage='RELEASE';renderer.render();settleFastTimer=setTimeout(()=>{const camera=cameraFrame(renderer);sky.render(camera);exterior.render(camera);evidence.stage='SHARPEN';},60);settleFullTimer=setTimeout(()=>{fap1.endInteraction();exterior.endInteraction();canonical.endInteraction();renderAll();motion.stage='REST';evidence.stage='REST';evidence.fullSettles++;},190);};
  ...
  canvas.addEventListener('wheel',event=>{event.preventDefault();fap1.beginInteraction();exterior.beginInteraction();canonical.beginInteraction();renderer.zoom(event.deltaY);renderFast();clearTimeout(wheelTimer);wheelTimer=setTimeout(settle,120);},{passive:false});
