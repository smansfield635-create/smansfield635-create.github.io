...
// Immediate visible proof that v2 loaded.
// Loads current Audralia lattice, landmap, climate, surface, and canvas chain with cache-busted URLs.
// Mounts only window.AUDRALIA_CANVAS.mount(...).
// Guards against legacy Gratitude v10 route status overwriting the new shell.
...
(() => {
  ...
  const VERSION = "2026-05-15.audralia-route-canonical-loader-v2";
  ...
  function setNotice(text, tone = "pending") {
    authoritativeNoticeText = String(text || "");
    const { notice } = nodes();
    if (!notice) return;

    notice.textContent = authoritativeNoticeText;
  ...
  async function init() {
    ...
    setStatus("AUDRALIA_ROUTE_CANONICAL_LOADER_TNT_v2 is active. Legacy Gratitude v10 route status is no longer current truth.");
