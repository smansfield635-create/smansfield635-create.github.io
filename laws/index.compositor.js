/* /laws/index.compositor.js
   LAW COMPASS dedicated camera and visual compositor.
   ...
   Source status:
   LAW_COMPASS_COMPOSITOR_CAMERA_DEPTH_STANDARD
   !=
   RUNTIME_PASS
   !=
   VISUAL_PASS
...
(() => {
  ...
  const CONTRACT = Object.freeze({
    ...
    receiptId:
      "DGB_LAWS_COMPOSITOR_RECEIPT_v1",
    ...
    controllerContainsLawContent:
      false,

    compositorContainsLawContent:
      false
  });
  ...
  function createCanvas(definition) {
    const existing =
      qs(
        `canvas[data-laws-compositor-layer="${definition.datasetValue}"]`,
        state.field
      );

    invariant(
      !existing,
      `LAWS_COMPOSITOR_DUPLICATE_LAYER:${definition.id}`
    );
  ...
  function requireController() {
    ...
    invariant(
      controller,
      "LAWS_COMPOSITOR_CONTROLLER_REQUIRED"
    );
    ...
    invariant(
      controller.moduleId ===
        CONTRACT
          .requiredControllerModuleId,
      "LAWS_COMPOSITOR_CONTROLLER_MODULE_INVALID",
      {
        expected:
          CONTRACT
            .requiredControllerModuleId,
    ...
    invariant(
      controller.moduleVersion ===
        CONTRACT
          .requiredControllerModuleVersion,
      "LAWS_COMPOSITOR_CONTROLLER_VERSION_INVALID",
      {
        expected:
          CONTRACT
            .requiredControllerModuleVersion,
    ...
    invariant(
      typeof controller
        .getFrameState ===
        "function",
      "LAWS_COMPOSITOR_CONTROLLER_FRAME_SURFACE_REQUIRED"
    );
    ...
    invariant(
      typeof controller
        ...
        "function",
      "LAWS_COMPOSITOR_CONTROLLER_LAW_REGISTRY_SURFACE_REQUIRED"
    );
    ...
    invariant(
      typeof controller
        ...
        "function",
      "LAWS_COMPOSITOR_CONTROLLER_ROUTE_REGISTRY_SURFACE_REQUIRED"
    );
  ...
  function resolveDom() {
    ...
    invariant(
      state.root,
      "LAWS_COMPOSITOR_ROOT_NOT_FOUND"
    );
    ...
    invariant(
      state.scene,
      "LAWS_COMPOSITOR_SCENE_NOT_FOUND"
    );
    ...
    invariant(
      state.field,
      "LAWS_COMPOSITOR_FIELD_NOT_FOUND"
    );
    ...
    invariant(
      state.compassMount,
      "LAWS_COMPOSITOR_COMPASS_MOUNT_NOT_FOUND"
    );
    ...
    invariant(
      state.semanticLayer,
      "LAWS_COMPOSITOR_SEMANTIC_LAYER_NOT_FOUND"
    );
  }
  ...
  function stackingContextFacts(element) {
    const style =
      globalThis
        .getComputedStyle(
          element
  ...
  function validateStacking() {
    invariant(
      state.rearCanvas &&
      state.frontCanvas,
      "LAWS_COMPOSITOR_LAYERS_REQUIRED"
    );
    ...
    invariant(
      state.rearCanvas
        .parentElement ===
        state.field,
      "LAWS_COMPOSITOR_REAR_LAYER_PARENT_INVALID"
    );
    ...
    invariant(
      state.frontCanvas
        .parentElement ===
        state.field,
      "LAWS_COMPOSITOR_FRONT_LAYER_PARENT_INVALID"
    );
    ...
    invariant(
      globalThis
        ...
        .pointerEvents ===
        "none",
      "LAWS_COMPOSITOR_REAR_POINTER_EVENTS_INVALID"
    );
    ...
    invariant(
      globalThis
        ...
        .pointerEvents ===
        "none",
      "LAWS_COMPOSITOR_FRONT_POINTER_EVENTS_INVALID"
    );
    ...
    invariant(
      Number(
        ...
        LAYERS.REAR.zIndex,
      "LAWS_COMPOSITOR_REAR_Z_INDEX_INVALID"
    );
    ...
    invariant(
      ...
      "LAWS_COMPOSITOR_COMPASS_Z_INDEX_INVALID"
    );
    ...
    invariant(
      Number(
        ...
        LAYERS.FRONT.zIndex,
      "LAWS_COMPOSITOR_FRONT_Z_INDEX_INVALID"
    );
    ...
    invariant(
      ...
      "LAWS_COMPOSITOR_SEMANTIC_Z_INDEX_INVALID"
    );
    ...
    state.root.dataset
      .lawsCompositeStackingValidated =
      "true";
  ...
  function normalizePresentation(frame) {
    if (
      !frame ||
      frame.held
    ) {
  ...
  function resolveCameraPreset(presentation) {
    const mobile =
      state.cssWidth /
      Math.max(
        1,
  ...
  function updateCamera(
    ...
    invariant(
      ...
      "LAWS_COMPOSITOR_COMPASS_PLANE_INVALID"
    );
  }
  ...
  function getViewMatrix() {
    invariant(
      state.viewMatrix,
      "LAWS_COMPOSITOR_VIEW_MATRIX_NOT_READY"
    );
  ...
  function getProjectionMatrix() {
    invariant(
      state.projectionMatrix,
      "LAWS_COMPOSITOR_PROJECTION_MATRIX_NOT_READY"
    );
  ...
  function getCompassFieldBounds() {
    invariant(
      state.field &&
      state.compassMount,
      "LAWS_COMPOSITOR_COMPASS_BOUNDS_NOT_AVAILABLE"
    );
  ...
  function projectWorldPoint(
    ...
    invariant(
      ...
      state.projectionMatrix &&
      state.viewProjectionMatrix,
      "LAWS_COMPOSITOR_FRAME_NOT_BEGUN"
    );
  ...
  function partitionNodes(options = {}) {
    ...
    invariant(
      typeof options
        .getWorldCenter ===
        "function",
      "LAWS_COMPOSITOR_WORLD_CENTER_CALLBACK_REQUIRED"
    );
  ...
  function renderComposite(options = {}) {
    ...
    invariant(
      typeof options
        .drawRear ===
        "function",
      "LAWS_COMPOSITOR_REAR_DRAW_CALLBACK_REQUIRED"
    );
    ...
    invariant(
      typeof options
        .drawFront ===
        "function",
      "LAWS_COMPOSITOR_FRONT_DRAW_CALLBACK_REQUIRED"
    );
  ...
  function getRearLayer() {
    invariant(
      state.rearCanvas,
      "LAWS_COMPOSITOR_REAR_LAYER_NOT_READY"
    );
  ...
  function getFrontLayer() {
    invariant(
      state.frontCanvas,
      "LAWS_COMPOSITOR_FRONT_LAYER_NOT_READY"
    );
  ...
  function publishReceipt(extra = {}) {
    ...
    globalThis
      .DGB_LAWS_COMPOSITOR_RECEIPT =
      frozen;

    if (state.root) {
      state.root.dataset
        .lawsCompositorReceipt =
        JSON.stringify(
          frozen
        );

      state.root.dataset
        .lawsCompositorStatus =
        state.status;

      state.root.dataset
        .lawsCompositorVersion =
        CONTRACT.moduleVersion;

      state.root.dataset
        .lawsCompositorControllerVersion =
        CONTRACT
          .requiredControllerModuleVersion;

      state.root.dataset
        .lawsCompassPlaneViewDepth =
        String(
          state.compassPlaneViewDepth
        );

      state.root.dataset
        .lawsRearVisibleNodeCount =
        String(
          state
            .rearVisibleNodeCount
        );
      ...
      state.root.dataset
        .lawsCompositorContainsLawContent =
        "false";
    }
  ...
  function initialize() {
    ...
    invariant(
      !state.disposed,
      "LAWS_COMPOSITOR_ALREADY_DISPOSED"
    );
    ...
    try {
      ...
      state.root.dataset
        .lawsCompositorCameraOwner =
        "true";

      state.root.dataset
        .lawsCompositorCompassLogicOwned =
        "false";

      state.root.dataset
        .lawsCompositorCompassOverlapMeasurementOwned =
        "true";

      state.root.dataset
        .lawsCompositorCrystalGeometryOwned =
        "false";

      state.root.dataset
        .lawsCompositorDepthClassificationOwned =
        "true";

      state.root.dataset
        .lawsCompositorRouteAuthorityOwned =
        "false";

      state.root.dataset
        .lawsCompositorSelectionAuthorityOwned =
        "false";
...
/*
DGB_LAWS_COMPOSITOR_CAMERA_DEPTH_RESULT_v1
...
Conversion:
 - archcoin root selectors converted to laws root selectors
 - archcoin scene selectors converted to laws scene selectors
 - archcoin canvas data attributes converted to laws canvas data attributes
 - controller dependency converted from DGB_ARCHCOIN_CONTROLLER to DGB_LAWS_CONTROLLER
 - crystals compatibility anchor converted from DGB_ARCHCOIN_CRYSTALS to DGB_LAWS_CRYSTALS
 - outerCardinalsActive read converted to outerCategoriesActive
 - activeRoomCluster read converted to activeLawCluster
 - archcoin receipts converted to laws receipts

Preserved:
 - public compositor surface
 - camera presets
 - camera interpolation
 - viewport and pixel-ratio logic
 - view and projection mathematics
 - Compass-plane depth calculation
 - projected Compass overlap measurement
 - depth hysteresis
 - rear/front classification
 - rear/front canvas construction
 - layer ordering
 - composite-pass orchestration
 - style rollback
 - owned-layer disposal
 - projectWorldPoint() return fields:
   x
   y
   normalizedX
   normalizedY
...
Compositor contains law content:
 FALSE
