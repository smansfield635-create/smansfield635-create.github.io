...
export function buildHEarthMapWideEnvironmentPreviewObserverReceipt(meshStatistics = null, waterStatistics = null, continuationStatistics = null) {
  ...
  const checks = {
    ...
    atriumCrownPreparedAndProtected: atrium.valid === true && (atrium.sitePreparation?.zoneWeights?.atrium ?? 0) > 0.9 && atrium.sitePreparation?.revision6ShapeProtected === true,
    estateSaddlePreserved: saddle.valid === true && (saddle.sitePreparation?.zoneWeights?.connectiveSpine ?? 0) > 0.9,
    largeHillInterfacePreserved: hillInterface.valid === true && (hillInterface.sitePreparation?.zoneWeights?.hillInterface ?? 0) > 0.9,
    ...
    enclosedNaturalizedReservoirPresent: reservoir.valid === true && (reservoir.hydrology?.reservoirWeight ?? 0) > 0.9 && reservoir.hydrology?.reservoirOutlineClass === 'IRREGULAR_TERRAIN_CONFORMING_MOUNTAIN_TOE_BASIN' && reservoir.hydrology?.enclosedReservoir === true && reservoir.hydrology?.visibleDrainageToCoast === false,
    broadWaterfallCorridorPresent: waterfall.valid === true && (waterfall.hydrology?.waterfallWeight ?? 0) > 0.5 && H_EARTH_MAP_WIDE_ENVIRONMENT_REDEVELOPMENT_HYDROLOGY.waterfall.halfWidth >= 12,
    ...
    futureRegionMountainContinuationPresent: futureRegion.valid === true && futureRegion.presentationElevation > futureRegion.boundaryElevation && futureRegion.canonicalRun8BExtensionClaimed === false && futureRegion.liveTraversalAuthorized === false,
    restoredInlandBayPresent: bayCenter < Math.min(bayWest, bayEast) - 18,
    terrainConformingBeachPresent: beach.valid === true && (beach.coastline?.beachWeight ?? 0) > 0.5 && beach.coastline?.terrainConformingBeach === true && beach.coastline?.restoredBay === true,
    sandbarPresent: sandbar.valid === true && (sandbar.coastline?.sandbarWeight ?? 0) > 0.5 && sandbar.presentationElevation > H_EARTH_MAP_WIDE_ENVIRONMENT_REDEVELOPMENT_HYDROLOGY.seaLevelY,
    materialReliefWitnessPresent: reliefWitnesses.some((sample) => sample.valid === true && Math.abs(sample.presentationReliefOffset) >= 4),
    inspectorDenseEnoughForTerrainReview: meshStatistics === null || (meshStatistics.validSampleCount >= 4900 && meshStatistics.singlePassTerrainSampling === true && meshStatistics.neighborResamplingRemoved === true && meshStatistics.sitePreparationSampleCount > 0 && meshStatistics.reservoirSampleCount > 0 && meshStatistics.waterfallSampleCount > 0 && meshStatistics.cavernReserveSampleCount > 0 && meshStatistics.beachSampleCount > 0 && meshStatistics.sandbarSampleCount > 0 && meshStatistics.restoredBaySampleCount > 0 && meshStatistics.terrainConformingBeachPresentation === true && meshStatistics.irregularReservoirPresentation === true),
    staticWaterContextPresent: waterStatistics === null || (waterStatistics.oceanTriangleCount >= 128 && waterStatistics.reservoirTriangleCount > 0 && waterStatistics.waterfallTriangleCount > 0 && waterStatistics.restoredBayBoundary === true && waterStatistics.terrainConformingBeachContext === true && waterStatistics.irregularReservoirOutline === true && waterStatistics.liveWaterMutation === false),
    continuationPreviewPresent: continuationStatistics === null || (continuationStatistics.validSampleCount > 0 && continuationStatistics.flatRearBoundaryRemoved === true && continuationStatistics.canonicalRun8BExtensionClaimed === false && continuationStatistics.liveTraversalAuthorized === false),
  ...
  return freeze({
    ...
    reliefWitnesses,
    boundaries: freeze({ nonpublicPreviewOnly: true, mapAuthoringIteration: true, userDifferentialRecorded: false, role5RatifiedForSuccessor: false, liveIntegrationAuthorized: false, mergeAuthorized: false, deploymentAuthorized: false, releaseAuthorized: false })
  });
}
