...
function build(){
  const g=mesh();
  const bark=WOOD_MATERIAL.TRUNK,dark=WOOD_MATERIAL.ROOT,network=[.055,.18,.22,1],coreGold=[.82,.58,.18,1],springGreen=[.08,.28,.12,1],summerGreen=[.03,.20,.11,1],water=[.08,.42,.72,.92],autumn=[.72,.35,.07,1],winter=[.64,.77,.82,1],returnBlue=[.04,.22,.34,1];
  const O=V(0,-.28,0);
...
void main(){
  ...
  if(feed>.5){
    ...
    if(u_reduced>.5){travelT=1.0;dockT=1.0;}
  ...
  if(zoneEnv>.5&&u_reduced<.5){
    ...
    if(a_kind>16.5&&a_kind<17.5){
      p.x+=sin(u_time*.42*speed+a_phase*31.0)*amp*.70;
      p.y+=cos(u_time*.35*speed+a_phase*23.0)*amp*.62;
      p.z+=sin(u_time*.28*speed+a_phase*19.0)*amp*.34;
    }else if(a_kind>17.5&&a_kind<18.5){
      float burst=.62+.38*(.5+.5*sin(u_time*.18+a_phase*17.0));
      p.x+=sin(u_time*.74*speed+a_phase*37.0)*amp*burst;
      p.y-=abs(sin(u_time*.92*speed+a_phase*29.0))*amp*.90;
    }else if(a_kind>18.5&&a_kind<19.5){
      p.x+=sin(u_time*.58*speed+a_phase*41.0)*amp*1.05;
      p.y+=cos(u_time*.51*speed+a_phase*33.0)*amp*.86;
      p.z+=sin(u_time*.39*speed+a_phase*27.0)*amp*.42;
    }else{
      float gust=.55+.45*(.5+.5*sin(u_time*.16+a_phase*13.0));
      p.x+=sin(u_time*.44*speed+a_phase*43.0)*amp*gust;
      p.y-=abs(cos(u_time*.36*speed+a_phase*35.0))*amp*.52;
      p.z+=sin(u_time*.27*speed+a_phase*21.0)*amp*.24;
    }
  }
...
void main(){
  ...
  float glow=(reached*.18+front*.72)*u_activity+network*reached*.10+core*(.22+.12*circ)+contrib*reached*(.30+.20*circ)+renew*u_renewal*(.24+.38*front)+star*reached*.35+u_retained*(1.0-ss(.22,.40,v_q))*.18+feed*(.18+.20*circ)+zoneEnv*(.09+.12*envPulse);
...
function mount(host){
  ...
  const publishReceipt=(failure=null,extra={})=>{globalThis.DGB_COMMUNITY_LIFECYCLE_3D_RECEIPT=Object.freeze({
    ...
    environmentMode:CONTRACT.environment,environmentSectorCount:4,environmentDepthPlaneCount:CONTRACT.environmentDepthPlanes.length,permanentQuadrants:true,feedOrbitRevolutions:0,feedPathMode:CONTRACT.feedPath,feedingStopsAtSeconds:16.5,guideRemnantsAfterLock:false,postLockFeeding:false,protectedCenter:true,rimTaper:null,
