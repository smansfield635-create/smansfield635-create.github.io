...
export const FAP1_W5_LOCAL_RAYMARCH_SCHEMA='FAP1_W5_LOCAL_RAYMARCH_SURFACE_v3_L5_DIRECT';
...
export function createW5LocalRayMarchSurface({worldCanvas,parent=worldCanvas?.parentElement}={}){
  ...
  function render({camera,spatialState,sunDirection=[.42,.78,.46],quality=null}={}){
    ...
    if(!refinement.active||!activeEntry||localWeight<W5_RAYMARCH_MIN_LOCAL_WEIGHT){
      ...
      lastReceipt=freeze({schema:FAP1_W5_LOCAL_RAYMARCH_SCHEMA,active:false,weatherId:null,localWeight:0,macroRendererMutation:false,lightingApplied:true,l5LightingActive:true,l5LightingModel:'DIRECT_SUN_TRANSMITTANCE_ONLY',quality:effectiveQuality,lightSteps,extinctionSplit:true,compositeAuthority:'BOUNDED_HANDOFF_PARTICIPANT'});
      return lastReceipt;
    }
    ...
    gl.uniform1f(U.uAspect,canvas.width/Math.max(1,canvas.height));gl.uniform1f(U.uTanHalfFov,Math.tan(55*Math.PI/360));gl.uniform1f(U.uLocalWeight,clamp(localWeight,0,1));gl.uniform1f(U.uDensityScale,1);gl.uniform1f(U.uExtinction,.018);gl.uniform1f(U.uLightExtinction,L5_EXTINCTION);gl.uniform1i(U.uStepCount,interaction?20:W5_RAYMARCH_MAX_STEPS);gl.uniform1i(U.uLightStepCount,lightSteps);
...
export function verifyW5LocalRayMarchReceipt(receipt){
  ...
  return freeze({schema:'FAP1_W5_LOCAL_RAYMARCH_INVARIANTS_v3_L5_DIRECT',pass:failures.length===0,failures:freeze(failures),weatherId:receipt?.weatherId??null});
}
