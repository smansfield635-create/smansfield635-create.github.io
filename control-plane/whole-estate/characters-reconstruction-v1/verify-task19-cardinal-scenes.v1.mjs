...
record('CANONICAL_ADMISSION_BOUND', contract.admission.canonicalResult === 'ADMITTED_AND_LOCKED' && contract.admission.lockGeneration === 1915);
record('ROUTER_PASS_BOUND', contract.admission.routerResult === 'PASS' && /^[a-f0-9]{64}$/.test(contract.admission.routerReceiptSha256));
record('EXACT_ALLOWED_PATH_COUNT', Array.isArray(contract.allowedPaths) && contract.allowedPaths.length === 14, `count=${contract.allowedPaths?.length ?? 0}`);
...
record('SUCCESSOR_PRODUCT_MUTATION_RECORDED', contract.productMutationPerformedAtThisLayer === true);
record('THREE_VERIFIED_DESTINATIONS_BOUND', contract.verifiedConversationDestinations?.count === 3 && contract.verifiedConversationDestinations?.bindings?.length === 3);

try {
  const knowledgeCardModule = await import(pathToFileURL(path.join(root, 'characters/knowledge-card.mjs')).href);
  const receipt = knowledgeCardModule.evaluateKnowledgeCardContract();
  record('KNOWLEDGE_CARD_EXECUTABLE_CONTRACT_PASS', receipt.eligible === true, receipt.result, 'PRODUCT');
  record('KNOWLEDGE_CARD_EXECUTABLE_DESTINATION_COUNT', receipt.defaultConversationDestinationCount === 3, `count=${receipt.defaultConversationDestinationCount}`, 'PRODUCT');
} catch (error) {
  record('KNOWLEDGE_CARD_EXECUTABLE_CONTRACT_PASS', false, String(error), 'PRODUCT');
}
...
if (base && candidate) {
  ...
  if (!productConstructionStarted) {
    ...
    const workflow = readAt(candidate, '.github/workflows/characters-night-world-state-proof.yml');
    ...
    const destinationHrefs = contract.verifiedConversationDestinations.bindings.map(({ href }) => href);
    record('THREE_DESTINATION_HREFS_EXACT', destinationHrefs.every(href => cards.includes(href)), destinationHrefs.join(','), 'PRODUCT');
    record('THREE_DESTINATION_SOURCE_BLOBS_EXACT', contract.verifiedConversationDestinations.bindings.every(({ sourcePath, sourceBlob }) => blobAt(candidate, sourcePath) === sourceBlob), '', 'PRODUCT');
    record('EXTERNAL_ROUTE_NONCOLOR_DISTINCTION', /DISTINCT_TAB_SHAPE/.test(cards) && /ROUTE_GLYPH/.test(cards) && /EXPLICIT_LEAVES_PAGE_TEXT/.test(cards) && /ACCESSIBLE_DESTINATION_NAME/.test(cards), '', 'PRODUCT');
    record('EXTERNAL_ROUTES_NOT_LOCAL_PRESENCE_CLAIMS', /CROSS_ESTATE_ROUTE_IMPROPERLY_TIED_TO_LOCAL_PRESENCE/.test(cards) && /IN_PAGE_CHARACTER_INTERACTION/.test(cards), '', 'PRODUCT');
    ...
    record('TASK19_WORKFLOW_REPLACES_LEGACY_SIGNAL_SELECTOR', !/\.signal\[data-id=/.test(workflow) && /intro-skip/.test(workflow) && /coast-map/.test(workflow) && /knowledge-card/.test(workflow), '', 'PRODUCT');
    record('TASK19_WORKFLOW_ASSERTS_THREE_DESTINATIONS', destinationHrefs.every(href => workflow.includes(href)), destinationHrefs.join(','), 'PRODUCT');
  }
}
