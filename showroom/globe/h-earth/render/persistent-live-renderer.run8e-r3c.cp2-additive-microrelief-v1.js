...
vec3 perturbTerrainNormal(
  ...
  float reliefDx=dFdx(reliefSignal);
  float reliefDy=dFdy(reliefSignal);
  ...
  return normalize(
    ...
    strength*orientation*surfaceGradient
  );
}
void main(){
  ...
  float rim=pow(1.0-max(dot(geometricNormal,viewDirection),0.0),2.2);
  ...
  if(vRoleCode==1u){
    ...
    float reliefSignal =
      ...
      (crestSignal - terraceSignal) * 0.12;

    float reliefFootprint =
      1.0 - smoothstep(0.06, 0.18, fwidth(reliefSignal));

    float reliefEnvelope = clamp(
      nearDetail *
      mix(0.28, 1.0, slopeResponse) *
      reliefFootprint,
      0.0,
      1.0
    );

    vec3 reliefNormal = perturbTerrainNormal(
      geometricNormal,
      vWorldPosition,
      reliefSignal,
      0.30
  ...
  float specular=pow(max(dot(shadingNormal,halfDirection),0.0),24.0)*specularScale;
  float ambient=0.26+0.16*clamp(geometricNormal.y,0.0,1.0)+0.05*materialSignal;
