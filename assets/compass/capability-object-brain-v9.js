...
function build(gl){
 ...
 function spline(c,steps=3){const o=[];for(let i=0;i<c.length-1;i++){const a=c[M.max(0,i-1)],b=c[i],d=c[i+1],e=c[M.min(c.length-1,i+2)];for(let s=0;s<steps;s++)o.push(cat(a,b,d,e,s/steps))}o.push(c[c.length-1]);return o}
 ...
 function serpent(side,center,tangent,length=.22,r=.026,seed=0,color=neutral,amp=.052){
  ...
  tube(spline(ctrl,3),u=>r*(.90+.17*M.sin(PI*u)),7,color);
 }
 ...
 // The cerebrum is populated in explicit medial, intermediate and lateral 3-D banks. No shell or envelope function exists.
 const superior=[];
 ...
 // Deep intermediate gyri provide real volumetric continuity rather than a hidden core.
 const deep=[];
 ...
 for(const side of[-1,1]){
  addSeeds(side,superior,10);addSeeds(side,frontal,70);addSeeds(side,precentral,130,light);addSeeds(side,postcentral,150,light);addSeeds(side,parietal,180);addSeeds(side,temporal,260);addSeeds(side,occipital,360);addSeeds(side,medial,470,dark);addSeeds(side,deep,540,dark);
 }

 // G3: compact cerebellum built from short folia in three depth banks.
 function cerebellum(side){
  ...
  // Peduncle fibers bridge cerebellum to pons.
 ...
 pons.forEach((c,i)=>tube(spline(c,4),.020+i*.001,8,light));
 const stem=[[0,-.18,-.17],[0,-.245,-.14],[.004,-.315,-.118],[.008,-.385,-.113],[.006,-.455,-.122],[.002,-.530,-.142],[-.003,-.605,-.165],[-.006,-.680,-.188],[0,-.755,-.205]];
 for(let k=-4;k<=4;k++){const c=stem.map((p,i)=>[p[0]+k*.008,p[1]+.003*M.sin(i*.8+k),p[2]+.004*M.sin(i*1.15+k*.45)]);tube(spline(c,3),u=>lerp(.017,.010,u),7,k%2?dark:neutral)}
