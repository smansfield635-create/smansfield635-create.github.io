...
export async function closeRemote(a) {
  ...
  return stable(u.ok?{...x.receipt,schema:'REPOSITORY_OPERATION_REMOTE_CLOSURE_RECEIPT_v1',result:'TERMINAL_CLOSURE_COMMITTED',authorityVerification,observedLedgerBlobSha:o.blob,observedBranchHead:o.head,committedLedgerBlobSha:u.blob,closureCommitSha:u.commit,contentTransport:o.contentTransport}:{schema:'REPOSITORY_OPERATION_REMOTE_CLOSURE_RECEIPT_v1',result:'LEDGER_COMPARE_AND_SWAP_CONFLICT',errorCode:u.errorCode,httpStatus:u.httpStatus,operationId:a.operationId,lockScope:canonScope(a.lockScope),lockGeneration:Number(a.lockGeneration),terminalDisposition:a.terminalDisposition,lockReleased:false,authorityVerification,observedLedgerBlobSha:o.blob,observedBranchHead:o.head,contentTransport:o.contentTransport});
}

function cli(v){const a={};for(let i=0;i<v.length;i++){if(!v[i].startsWith('--'))throw err('UNKNOWN_ARGUMENT',v[i],'cli');a[v[i].slice(2)]=v[++i]??null}return a}
async function main(){const a=cli(process.argv.slice(2));try{const mode=a.mode||'acquire',token=process.env.GITHUB_TOKEN;if(mode==='ensure'){console.log(text(await ensureRef({repository:a.repository,lockRef:a['lock-ref']||LOCK_REF,baseHead:a['base-head'],token})));return}if(mode==='acquire'){console.log(text(await acquireRemote({repository:a.repository,lockRef:a['lock-ref']||LOCK_REF,token,operationId:a['operation-id'],lockScope:a['lock-scope'],governingHead:a['governing-head'],requestDigest:a['request-digest'],procedureLocatorDigest:a['procedure-locator-digest'],preWriteDelayMs:a['pre-write-delay-ms'],readyFile:a['ready-file'],barrierFile:a['barrier-file'],barrierTimeoutMs:a['barrier-timeout-ms']})));return}if(mode==='close'){console.log(text(await closeRemote({repository:a.repository,lockRef:a['lock-ref']||LOCK_REF,token,operationId:a['operation-id'],lockScope:a['lock-scope'],lockGeneration:Number(a['lock-generation']),terminalDisposition:a['terminal-disposition']})));return}throw err('UNKNOWN_MODE','mode','cli',mode)}catch(e){console.error(text({schema:'REPOSITORY_OPERATION_LOCK_ERROR_v1',result:'FAIL_CLOSED',errorCode:e.code||'UNEXPECTED_ERROR',field:e.field||null,sourceDocument:e.sourceDocument||null,detail:e.detail||e.message||null}));process.exitCode=1}}
