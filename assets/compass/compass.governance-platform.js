...
function build(host,options={}){
 ...
 const trigger=q('[data-governance-panel-trigger]')||document.createElement('button');trigger.type='button';trigger.className='governance-panel-trigger';trigger.textContent=trigger.textContent.trim()||options.label||'Governance Panel';trigger.setAttribute('title','Open Governance Panel');trigger.setAttribute('aria-expanded','false');trigger.setAttribute('aria-controls','governance-panel');trigger.setAttribute('data-governance-panel-trigger','');
 ...
 if(!trigger.isConnected)host.append(trigger);document.body.append(panel);
 ...
 trigger.addEventListener('click',toggle);q('.governance-panel__close',panel).addEventListener('click',close);
 let touchX=0,touchY=0,touchActive=false;
 const SWIPE_MIN=72,EDGE=48;
 document.addEventListener('touchstart',e=>{if(panel.classList.contains('is-open')||!e.touches.length)return;const t=e.touches[0];touchX=t.clientX;touchY=t.clientY;touchActive=touchX<=EDGE||touchX>=innerWidth-EDGE;},{passive:true});
 document.addEventListener('touchend',e=>{if(panel.classList.contains('is-open')||!touchActive||!e.changedTouches.length){touchActive=false;return;}const t=e.changedTouches[0],dx=t.clientX-touchX,dy=t.clientY-touchY;touchActive=false;if(Math.abs(dx)>=SWIPE_MIN&&Math.abs(dx)>Math.abs(dy)*1.35&&(touchX<=EDGE&&dx>0||touchX>=innerWidth-EDGE&&dx<0))open();},{passive:true});panel.addEventListener('click',e=>{if(e.target===panel)close();});
