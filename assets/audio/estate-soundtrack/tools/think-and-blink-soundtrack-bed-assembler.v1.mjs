...
function selfTest(){requireCommand('ffmpeg');requireCommand('ffprobe');const checks=[
...
];return{schema:'THINK_AND_BLINK_SOUNDTRACK_BED_ASSEMBLER_SELF_TEST_v1',result:checks.every(Boolean)?'PASS_CLOSED':'FAIL_CLOSED',checks,passed:checks.filter(Boolean).length,failed:checks.filter(x=>!x).length,networkFetchPerformed:false,repositoryMutationPerformed:false};}
...
try{if(args['self-test']){const r=selfTest();if(args.output)writeJson(args.output,r);else process.stdout.write(JSON.stringify(r,null,2)+'\n');if(r.result!=='PASS_CLOSED')process.exitCode=1;}else await assemble(args);}catch(e){const r={schema:SCHEMA,result:'FAIL_CLOSED',errorCode:e.code||'UNEXPECTED_ERROR',detail:e.detail??e.message,mainMutationPerformed:false,mergePerformed:false,deploymentPerformed:false,publicationPerformed:false};if(args.receipt)writeJson(args.receipt,r);else process.stderr.write(JSON.stringify(r,null,2)+'\n');process.exitCode=1;}
