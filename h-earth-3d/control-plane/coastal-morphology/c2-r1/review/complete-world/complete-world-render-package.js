/**
 * H_EARTH_C2_R1_COMPLETE_WORLD_RENDER_PACKAGE_v1
 *
 * Isolated package-level binding of the accepted C2-R1 coastal component into
 * the canonical complete H-Earth package. The canonical package and all closed
 * coastal sources are consumed read-only. No public route or renderer source is
...
async function resolveDependencies(options) {
  ...
  return { canonicalPackage, sampleCoastalTerrain, sampleCoastalSurfaceFrame, sampleCandidateMaterial };
}
...
export async function buildHEarthC2R1CompleteWorldRenderPackage(options = {}) {
  ...
  if (!validation.eligible) {
    return freeze({
      ...
      contractId: H_EARTH_C2_R1_COMPLETE_WORLD_PACKAGE_CONTRACT_ID,
      issues: freeze(validation.issues)
    });
  }
  ...
  for (let vertexIndex = 0; vertexIndex < validation.vertexCount; vertexIndex += 1) {
    ...
    const material = dependencies.sampleCandidateMaterial(worldX, worldZ, {
      timeSeconds: Number(options.timeSeconds ?? 0)
    });
    ...
    if (role === H_EARTH_C2_R1_COMPLETE_WORLD_BINDING.roleCodes.TERRAIN) {
      ...
      set4(buffers.baseColorsLinear, vertexIndex, [
        material.material.colorLinear[0],
        material.material.colorLinear[1],
        material.material.colorLinear[2],
        priorColor[3]
      ]);
  ...
  if (counters.boundTerrainVertexCount === 0) {
    return freeze({
      ...
      status: 'H_EARTH_C2_R1_COMPLETE_WORLD_PACKAGE_REJECTED',
      contractId: H_EARTH_C2_R1_COMPLETE_WORLD_PACKAGE_CONTRACT_ID,
      issues: freeze(['NO_TERRAIN_VERTEX_BOUND_TO_ACCEPTED_COASTAL_COMPONENT']),
  ...
  if (counters.candidateSampleFailureCount !== 0) {
    return freeze({
      ...
      status: 'H_EARTH_C2_R1_COMPLETE_WORLD_PACKAGE_REJECTED',
      contractId: H_EARTH_C2_R1_COMPLETE_WORLD_PACKAGE_CONTRACT_ID,
      issues: freeze([`CANDIDATE_SAMPLE_FAILURES:${counters.candidateSampleFailureCount}`]),
      counters: freeze(counters)
    });
  }
...
export function evaluateHEarthC2R1CompleteWorldRenderPackage(result, canonicalPackage) {
  const issues = [];
  if (result?.eligible !== true) issues.push(...(result?.issues ?? ['COMPLETE_WORLD_PACKAGE_NOT_ELIGIBLE']));
