...
function mountProductsConstellation(root){
  ...
  const variant=root.dataset.productsSpatialVariant==='explore'?'explore':'door';const canvas=document.createElement('canvas');canvas.className='products-constellation-v2-canvas';canvas.setAttribute('aria-hidden','true');root.prepend(canvas);const ctx=canvas.getContext('2d',{alpha:true});if(!ctx)throw new Error('PRODUCTS_SPATIAL_CANVAS_UNAVAILABLE');const reduced=matchMedia?.('(prefers-reduced-motion: reduce)')?.matches??false;let active=true,raf=0,last=0,start=performance.now();
  function frame(now){if(!active&&!reduced)return;if(!reduced&&now-last<30){raf=requestAnimationFrame(frame);return}last=now;const rect=root.getBoundingClientRect(),dpr=Math.min(1.25,devicePixelRatio||1),W=Math.max(1,Math.round(rect.width*dpr)),H=Math.max(1,Math.round(rect.height*dpr));if(canvas.width!==W||canvas.height!==H){canvas.width=W;canvas.height=H;canvas.style.width=`${rect.width}px`;canvas.style.height=`${rect.height}px`}ctx.setTransform(dpr,0,0,dpr,0,0);const state=getProductsSpatialState(now-start,variant,reduced);render(ctx,rect.width,rect.height,state,variant);root.dataset.productsSpatialPhase=state.phase;if(active&&!reduced)raf=requestAnimationFrame(frame)}
  ...
  const api=Object.freeze({specimenId:SPECIMEN_ID,variant,cycleMs:PRODUCTS_SPATIAL_CYCLE_MS,reducedMotion:reduced,gemCount:GEMS.length,facesPerGem:GEM.faces.length,destroy(){active=false;cancelAnimationFrame(raf);io?.disconnect();canvas.remove();root.dataset.productsSpatialMounted='false'}});root.dataset.productsSpatialMounted='true';root.__productsSpatialV2=api;return api;
}
