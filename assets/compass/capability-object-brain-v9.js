...
function build(gl){
 ...
 const palette={
  frontal:[.92,.53,.56],parietal:[.86,.45,.50],temporal:[.78,.34,.42],occipital:[.71,.29,.38],
  cerebellum:[.62,.28,.36],stem:[.65,.30,.34],pons:[.72,.35,.39]
 };
 ...
 function tube(points,radius,color,sides=9){
  ...
  for(let i=0;i<points.length;i++){
   ...
   for(let s=0;s<sides;s++){
    const a=s/sides*TAU;
    const radial=add(mul(n,M.cos(a)),mul(b,M.sin(a)));
 ...
 function corticalColor(z,y){
  if(y<-.23)return palette.temporal;
  if(z>.34)return palette.frontal;
  if(z<-.47)return palette.occipital;
  return palette.parietal;
 }

 function corticalHemisphere(side){
  const bands=25,samples=34;
  for(let band=0;band<bands;band++){
   const q=band/(bands-1);
   const a=-1.18+q*2.36;
   const pts=[];
   for(let i=0;i<samples;i++){
    const u=i/(samples-1);
    const z=-.78+1.56*u;
    const crown=M.sqrt(M.max(0,1-M.pow(z/.93,2)));
    const phase=band*1.73;
    const localA=a+.105*M.sin(u*TAU*2.15+phase)+.045*M.sin(u*TAU*5.1-phase*.4);
    const rx=.58*crown*(1-.08*M.max(0,-z));
    const ry=.62*crown*(1-.10*M.max(0,-z));
    const x=side*(.075+rx*M.cos(localA));
    const y=.055+ry*M.sin(localA)+.028*M.sin(u*TAU*7.0+phase)+.012*M.sin(u*TAU*13.0-phase);
    const zz=z+.035*M.sin(u*TAU*4.0+phase*.55)+.018*M.sin(u*TAU*9.0-phase*.3);
    pts.push([x,y,zz]);
   }
   const mid=pts[M.floor(samples*.58)];
  ...
  for(let r=0;r<crossRows;r++){
   ...
   for(let i=0;i<18;i++){
    ...
    const phase=r*2.21;
    const aa=a+.095*M.sin(u*TAU*2.6+phase);
    const x=side*(.078+.555*crown*M.cos(aa));
    const y=.055+.595*crown*M.sin(aa)+.024*M.sin(u*TAU*5.0+phase);
    const zz=z+.045*M.sin(u*TAU*3.0+phase*.6);
    pts.push([x,y,zz]);
   }
 ...
 function cerebellum(side){
  for(let r=0;r<17;r++){
   ...
   for(let i=0;i<22;i++){
    ...
    const z=-.57+.23*sweep-.055*sweep*sweep+.018*M.sin(u*TAU*4.0+r*.55);
    pts.push([x,yy,z]);
   }
  ...
  for(let c=0;c<6;c++){
   ...
   for(let i=0;i<15;i++){
    const u=i/14;
    const y=-.48+u*.27;
    const x=side*(.12+.17*(c/5))+.014*M.sin(u*TAU*2+c);
    const z=-.60+.12*(c/5)+.025*M.sin(u*TAU*3+c*.7);
    pts.push([x,y,z]);
   }
 ...
 function pons(){
  ...
  for(let i=0;i<10;i++){
   const u=i/9;
   pts.push([0,-.405+u*.12,-.13+.025*M.sin(u*PI)]);
  }
  tube(pts,(u)=>.083+.028*M.sin(u*PI),palette.pons,12);
 }

 corticalHemisphere(-1);corticalHemisphere(1);
 cerebellum(-1);cerebellum(1);
 brainstem();pons();

 const vertexCount=pos.length/3;
 if(vertexCount>65535)throw Error('Brain V9 tubular vertex budget exceeded: '+vertexCount);
 ...
 document.querySelectorAll('[data-capability-brain-v9]').forEach(canvas=>{
  const api=stage.mount(canvas,{
   ...
   dataset:{
    ...
    brainContract:'COMPASS_COHERISCOPE_TUBULAR_COMPOSITE_v1',
    brainMaterial:'NATIVE_ROSE_FLESH_V7',
    brainDepthModel:'TRUE_WEBGL_GEOMETRY',
    brainComponents:'tubular-cortical-gyri,longitudinal-fissure,natural-sulcal-gaps,paired-cerebellar-folia,pons,medulla,brainstem',
    brainConstruction:'EMERGENT_TUBULAR_COMPOSITE',
