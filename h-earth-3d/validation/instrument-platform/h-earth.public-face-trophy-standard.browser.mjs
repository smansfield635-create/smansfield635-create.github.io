...
const waitForSettledCardDepth = async (page, selector, index, minOpacity, maxOpacity, minBlur = 3) => page.waitForFunction(({ selector, index, minOpacity, maxOpacity, minBlur }) => {
  const node = document.querySelectorAll(selector)[index];
  if (!(node instanceof HTMLElement)) return false;
  const style = getComputedStyle(node);
  const opacity = Number.parseFloat(style.opacity);
  const blur = Number.parseFloat(style.getPropertyValue('--content-blur'));
  return Number.isFinite(opacity) && opacity >= minOpacity && opacity <= maxOpacity && Number.isFinite(blur) && blur >= minBlur;
}, { selector, index, minOpacity, maxOpacity, minBlur }, { timeout: 5000 });
...
try {
  ...
  await waitForSettledCardDepth(awards, '[data-achievement-stage] [data-story]', 1, .40, .56);
  ...
  await waitForSettledCardDepth(awards, '[data-trophy-stage] [data-lens-key]', 1, .40, .52);
  ...
  await waitForSettledCardDepth(mobileAwards, '[data-achievement-stage] [data-story]', 1, .40, .56);
