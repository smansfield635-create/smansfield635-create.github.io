...
const OPERATION='COMPASS_FRONT_PAGE_CORRECTIVE_GEOMETRY_PROVENANCE_BRAIN_PERFORMANCE_20260817_004';
const LOCK=1524;
const GOVERNING='a7b13dd5cebeafe6ce126d7df64c4088db7feb48';
const OUTPUT=process.env.COMPASS_PERFORMANCE_OUTPUT||'/tmp/compass-gen1524-performance.json';
...
check('GEN1524_OPERATION_BINDING',OPERATION.endsWith('_004')&&LOCK===1524&&GOVERNING.length===40,{OPERATION,LOCK,GOVERNING});
...
check('SPACECRAFT_DPR_BOUNDED',
  t.identity.includes('Math.min(devicePixelRatio||1,1)')&&
  t.identity.includes("powerPreference:'low-power'")&&
  t.identity.includes('preserveDrawingBuffer:false')&&
  t.identity.includes("performancePolicy='desktop-only-30fps-dpr1'"),
  'DPR 1 hard cap, low-power WebGL context, no preserved backbuffer');
check('SPACECRAFT_IDLE_GATED',
  t.identity.includes('document.hidden||reduce.matches||now-lastDraw<33')&&
  t.identity.includes('innerWidth<=720')&&
  t.identity.includes('state.nextLaunch')&&
  t.identity.includes('18000+')&&
  t.identity.includes("performancePolicy='desktop-only-30fps-dpr1'"),
  'document/reduced-motion/mobile suspension plus 30fps flight throttle and bounded relaunch cadence');
check('SPATIAL_COMPASS_IDLE_SETTLED',
  t.identity.includes('firstEnhancedFrameCompleted')&&
  t.identity.includes('renderer.stop?.(receipt.lastMountedInstanceId)')&&
  t.identity.includes("compassSpatialIdlePolicy='first-enhanced-frame-then-stop'")&&
  t.identity.includes("compassSpatialRendererStatus='settled-static-enhanced-frame'"),
  'true-3D Compass renders its enhanced frame, then stops continuous idle work');

check('SPATIAL_COMPASS_REUSES_EXISTING_GEOMETRY',
  t.identity.includes('/assets/compass/upstream-compass.geometry.js?v=3.0.0-fixed-center-independent-sibling')&&
  t.identity.includes('/assets/compass/upstream-compass.renderer.js?v=3.1.1-generic-lifecycle-receipt')&&
  t.geometry.includes('dimensionality: 3')&&
  t.geometry.includes('occupiedTotalDepth: 0.48'),
  'existing true-3D geometry/renderer reused');
check('SPATIAL_COMPASS_RESOURCE_POLICY',
  t.renderer.includes('normalDevicePixelRatioCap')&&
  t.renderer.includes('lowPowerDevicePixelRatioCap')&&
  t.renderer.includes('bloomDisableWidthPx')&&
  t.renderer.includes('preserveDrawingBuffer')&&
  t.renderer.includes('false'),
  'renderer DPR/bloom/backbuffer controls retained');
...
const allowed=new Set([
  ...
  'laws/index.spacecraft.js'
]);
const changed=git(['diff','--name-only',`${GOVERNING}...HEAD`]);
if(changed.status===0){
  const list=changed.stdout.trim().split(/\r?\n/).filter(Boolean);
  check('EXACT_13_PATH_BOUNDARY',list.length>0&&list.every(p=>allowed.has(p)),{changed:list,allowedCount:allowed.size});
  check('PROTECTED_CONTROLLER_CAROUSEL_UNTOUCHED',list.every(p=>!['assets/compass/compass.controller.js','assets/compass/compass.crystals.js','assets/compass/compass.capability-carousel.js','assets/compass/compass.capability-carousel.css'].includes(p)),list);
}else check('EXACT_13_PATH_BOUNDARY',false,changed.stderr);

const failures=checks.filter(c=>!c.pass);
const receipt={schema:'COMPASS_FRONT_PAGE_CORRECTIVE_GEN1524_PERFORMANCE_RECEIPT_v1',operationId:OPERATION,lockGeneration:LOCK,governingHead:GOVERNING,candidateHead:head,targetFrameBudgetMs:16.7,result:failures.length?'FAIL_CLOSED':'PASS_CLOSED',checks,failures:failures.map(x=>x.id)};
