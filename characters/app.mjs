...
import {step9Frame,step9ShorelineZ,step9TerrainHeight,resolveStep9Site,resolveStep9Camera,step9MapPosition,STEP9_DESTINATION_BINDINGS} from './step9-regional-geography.mjs';
import {resolveCameraSiteAnchor} from './gratitude-geography.adapter.mjs';
import {applyCardinalSceneEvent,createCardinalSceneState} from './cardinal-scene-state.mjs';
import {CARDINAL_DISCOVERIES} from './cardinal-scenes.data.mjs';
...
const tabletProfile=Math.min(innerWidth,innerHeight)<=1024&&(navigator.maxTouchPoints||0)>0;
...
const WORLD_MOON_POSITION=Object.freeze([frame.xMinimum+200,frame.seaLevelY+780,frame.zMinimum-1400]);
const ALARIC_SITE_ID='WATCHFIRE_OVERLOOK';
const ALARIC_OPERATION_ID='CHARACTERS_GRATITUDE_CARDINAL_INTEGRATION_RECOVERY_ALARIC_PROOF_20260909_001_REBIND_001';
...
out vec4 outColor;
void main(){vec3 n=normalize(vNormal);vec3 moon=normalize(uMoonWorld-vPos);float light=.24+.76*max(dot(n,moon),0.0);vec3 color=uBaseColor*light+uBaseColor*uEmissive*.38;outColor=vec4(color,1.0);}`;
const siteProgram=makeProgram(VS,SITE_FS);
...
function meshTerrain(){const N=compact?96:150,z0=frame.zMinimum,z1=frame.zMaximum,verts=[],idx=[];for(let j=0;j<=N;j++){const z=mix(z0,z1,j/N);for(let i=0;i<=N;i++){const x=mix(frame.xMinimum,frame.xMaximum,i/N),y=terrainHeight(x,z),eps=3,dx=terrainHeight(x+eps,z)-terrainHeight(x-eps,z),dz=terrainHeight(x,z+eps)-terrainHeight(x,z-eps),n=norm([-dx,2*eps,-dz]);verts.push(x,y,z,...n);}}const row=N+1;for(let j=0;j<N;j++)for(let i=0;i<N;i++){const a=j*row+i,b=a+1,c=a+row,d=c+1;idx.push(a,c,b,b,c,d);}return{verts:new Float32Array(verts),idx:new Uint32Array(idx)};}
function meshWater(){const S=compact?96:180,R=compact?4:6,marginX=180,farZ=frame.zMaximum+180,y=frame.seaLevelY-1.4,verts=[],idx=[];for(let i=0;i<=S;i++){const x=mix(frame.xMinimum-marginX,frame.xMaximum+marginX,i/S),shore=shorelineZ(x)+1.5;for(let r=0;r<=R;r++){const t=r/R,z=mix(shore,farZ,t);verts.push(x,y,z,0,1,0);}}const row=R+1;for(let i=0;i<S;i++)for(let r=0;r<R;r++){const a=i*row+r,b=(i+1)*row+r,c=a+1,d=b+1;idx.push(a,c,b,b,c,d);}return{verts:new Float32Array(verts),idx:new Uint32Array(idx)};}
...
function makeCardinalVAO(source,anchor){const verts=new Float32Array((source.positions.length/3)*6);for(let i=0,o=0;i<source.positions.length;i+=3){verts[o++]=source.positions[i]+anchor.x;verts[o++]=source.positions[i+1]+anchor.y;verts[o++]=source.positions[i+2]+anchor.z;verts[o++]=source.normals[i];verts[o++]=source.normals[i+1];verts[o++]=source.normals[i+2];}return makeVAO({verts,idx:new Uint32Array(source.indices)});}
const terrain=makeVAO(meshTerrain()),water=makeVAO(meshWater());
const alaricDiscoveries=CARDINAL_DISCOVERIES.filter(discovery=>discovery.siteId===ALARIC_SITE_ID);
const ALARIC_STORY_STATE=Object.freeze({storyReceiptId:'TASK19_FROZEN_CARDINAL_DISCOVERY_PUBLIC_COMPOSITION_v1',chronologyState:'TASK18_FROZEN_ARCHITECTURE_PUBLIC_COMPOSITION',discoveryAvailabilityById:Object.freeze(Object.fromEntries(alaricDiscoveries.map(discovery=>[discovery.id,Object.freeze({state:'AVAILABLE',predicateReceiptId:`TASK19_FROZEN_DISCOVERY_AVAILABLE:${discovery.id}`,chronologyState:'TASK18_FROZEN_ARCHITECTURE_PUBLIC_COMPOSITION'})])))});
let cardinalState=createCardinalSceneState({storyState:ALARIC_STORY_STATE});
const cardinalPhaseLog=['SURVEY_HUB'];
function applyAlaricEvent(event){const result=applyCardinalSceneEvent(cardinalState,event);if(result.receipt.accepted){cardinalState=result.state;cardinalPhaseLog.push(cardinalState.phase);}return result.receipt;}
function closeAlaricCardinalState(){if(cardinalState.phase!=='SURVEY_HUB')applyAlaricEvent({type:'RETURN_TO_HUB'});}
const alaricGeometry=buildCardinalSiteGeometry(ALARIC_SITE_ID,tabletProfile?'REGIONAL':'LOCAL');
const alaricAnchor=alaricGeometry.geography.canonicalWorldReference;
const alaricCamera=resolveCameraSiteAnchor(ALARIC_SITE_ID);
const alaricSiteMeshes=alaricGeometry.components.map(component=>({mesh:makeCardinalVAO(component.mesh,alaricAnchor),material:component.mesh.lightingResponse}));
const D=(id,data)=>{const binding=STEP9_DESTINATION_BINDINGS[id];if(!binding)throw new Error(`STEP9_DESTINATION_BINDING_MISSING:${id}`);const site=resolveStep9Site(binding.siteId),survey=resolveStep9Camera(binding.siteId);return{id,siteId:binding.siteId,bindingRole:binding.role,pos:[site.world.x,site.world.y+data.height,site.world.z],eye:[survey.eye.x,survey.eye.y,survey.eye.z],look:[site.world.x,data.lookY??site.world.y,site.world.z],...data};};
const DESTINATIONS=[
D('crossing',{height:34,lookY:22,signal:'Something happened here',kind:'The shore remembers',title:'The Crossing',copy:'The first team crossed into Mirrorland. The return path never opened. What began as discovery became a responsibility no one could hand back.',more:['Dextrion found the anomaly from Earth. Others crossed because the possibility mattered enough to risk the unknown.','The failure of the return path changed every decision that followed. Mirrorland stopped being somewhere they could visit and became somewhere they had to prepare.'],connections:['dextrion','manor','clock']}),
D('dextrion',{height:44,lookY:30,signal:'A transmission from Earth',kind:'A voice from the other side',title:'Dextrion',copy:'He opened the path and stayed behind. Every person sent through the one-way crossing remains part of the choice he made.',more:['His Earth-side lab becomes command center, workshop and countdown room. He cannot enter the world he is responsible for helping save.','His problem is no longer whether the anomaly is real. It is whether the door can become something more than a wound.'],connections:['crossing','manor','clock']}),
D('alaric',{height:42,lookY:35,signal:'Someone is watching the route',kind:'Trust the warning before the proof',title:'Alaric',copy:'He reads danger before proof arrives. Waiting until everyone agrees can mean waiting until the safe route is already gone.',more:['Alaric carries orientation as responsibility. He studies what changes first: routes, pressure, silence, the things people ignore because they have not become obvious yet.','His conflict is simple to state and difficult to live with: how do you ask people to trust a warning they cannot yet see?'],connections:['tarian','soren','crossing']}),
D('tarian',{height:38,lookY:30,signal:'The waterline is changing',kind:'Survival has a physical limit',title:'Tarian',copy:'He keeps the mission physically honest. A future is meaningless if the people trying to reach it cannot keep standing.',more:['Water, movement and endurance are not background systems to him. They are the difference between a plan that exists and people who can survive long enough to carry it.','When ambition outruns the body, Tarian is the one who notices the cost.'],connections:['alaric','manor','soren']}),
D('manor',{height:96,lookY:70,signal:'A light farther inland',kind:'The first standing anchor',title:'Mirror Manor',copy:'The Manor stands because the crossing became permanent. Shelter is not the end of the problem. It is where preparation begins.',more:['The people who crossed needed more than protection from weather. They needed a place capable of holding knowledge, people and decisions while the world around them was still becoming known.','The Manor can protect, but protection can become isolation. The question is whether a sanctuary can remain connected to the world it was built to help.'],connections:['tarian','elara','auren','jeeves','clock']}),
D('elara',{height:40,lookY:34,signal:'A signal beyond the dunes',kind:'A future has to become visible',title:'Elara',copy:'She makes a possible future visible before the world believes in it. Hope has to become something people can actually see.',more:['Visibility is not decoration. It changes what people believe is reachable. Elara carries that burden every time she makes possibility public.','But exposure has a cost. Showing people the future can also show danger where to look.'],connections:['manor','soren','clock']}),
D('soren',{height:52,lookY:45,signal:'The ground does not agree',kind:'Damage does not become progress by renaming it',title:'Soren',copy:'He refuses fake restoration. Saving a world by hiding what was damaged only prepares the next collapse.',more:['Soren looks for the hidden cost: contamination, displaced pressure, the consequence everyone wants to call acceptable because the alternative is inconvenient.','His severity exists for a reason. A repaired surface is not the same thing as a healed system.'],connections:['alaric','tarian','elara','clock']}),
D('auren',{height:58,lookY:55,signal:'A path toward shelter',kind:'Protection can become a cage',title:'Auren Vale',copy:'He keeps the Manor from becoming a beautiful cage. Every protected life makes the sanctuary more necessary—and harder to keep hidden.',more:['Auren turns land, rooms, misdirection and stewardship into protection. His gift is custody; his danger is overprotection.','The question surrounding him is whether safety can remain open enough to serve the people it was meant to protect.'],connections:['manor','jeeves','clock']}),
D('jeeves',{height:72,lookY:62,signal:'The house is listening',kind:'Not every truth belongs at the door',title:'Jeeves',copy:'He decides how much the Manor reveals at once. Too much truth can break people. Too little can send them into the wrong room.',more:['The Manor is more than architecture when knowledge itself can overwhelm. Jeeves turns complexity into thresholds people can survive.','His problem is not secrecy for its own sake. It is timing: what must someone know now, and what can wait until they are ready?'],connections:['manor','auren','clock']}),
D('clock',{height:60,lookY:52,signal:'The horizon is still moving',kind:'Time is not waiting for agreement',title:'The Clock',copy:'The transition will happen. Preparation is the variable. What this world becomes is not separate from what eventually happens to Earth.',more:['Mirrorland is not an escape from consequence. It is a place where possible futures can become visible before they harden.','ZIONTS is consequence. H-Earth is survival. Audralia is possibility. The responsibility is deciding which direction people are willing to build toward.'],connections:['crossing','dextrion','manor','elara','soren']}),
D('remote',{height:48,lookY:48,signal:'Lights beyond the estate',kind:'Survival has to travel',title:'Beyond the Manor',copy:'If what works here cannot leave the estate, the Manor is only a bunker. Survival has to become something communities can carry elsewhere.',more:['The remote teams turn isolated restoration into distributed response. They carry what is learned here into places without the Manor’s protection.','The story gets larger at this boundary: the future cannot be saved from one house.'],connections:['manor','tarian','clock']})];
const byId=new Map(DESTINATIONS.map(d=>[d.id,d]));
const alaricDestination=byId.get('alaric');
alaricDestination.pos=[alaricAnchor.x,alaricAnchor.y+42,alaricAnchor.z];
alaricDestination.eye=[alaricCamera.eye.x,alaricCamera.eye.y,alaricCamera.eye.z];
alaricDestination.look=[alaricCamera.look.x,alaricCamera.look.y,alaricCamera.look.z];
...
Object.defineProperty(window,'__DGB_ALARIC_PROOF__',{value:Object.freeze({operationId:ALARIC_OPERATION_ID,siteId:ALARIC_SITE_ID,geometryIdentity:alaricGeometry.geometryIdentity,moonFrame:'WORLD_SPACE_CONSTANT',clouds:'DISABLED',get phase(){return cardinalState.phase;},get phases(){return [...cardinalPhaseLog];},get selectedSiteId(){return cardinalState.selectedSiteId;},get activeSiteId(){return cardinalState.activeSiteId;}}),configurable:false});

const proofExpressions={};
for(const [id,className] of [['crossing','crossing-scar'],['dextrion','earth-transmission'],['tarian','waterline-station'],['manor','manor-mass'],['elara','signal-lantern'],['soren','restoration-boundary'],['auren','shelter-path'],['jeeves','threshold-light'],['clock','clock-anomaly'],['remote','distant-settlement']]){const el=document.createElement('div');el.className=`proof-expression ${className}`;el.setAttribute('aria-hidden','true');proofLayer.appendChild(el);proofExpressions[id]=el;}
for(const d of DESTINATIONS){const b=document.createElement('button');b.type='button';b.className='signal';if(d.id==='clock')b.classList.add('clock-star');b.dataset.id=d.id;b.setAttribute('aria-label',d.signal);b.innerHTML=`<span>${d.signal}</span>`;b.addEventListener('click',()=>travelTo(d));signalLayer.appendChild(b);d.button=b;
 const m=document.createElement('button');m.type='button';m.className='map-node';m.dataset.id=d.id;m.setAttribute('aria-label',`Travel to ${d.title}`);const map=step9MapPosition(d.siteId);m.style.left=`${8+map.u*84}%`;m.style.top=`${10+map.v*78}%`;m.innerHTML=`<span>${d.title}</span>`;m.addEventListener('click',()=>{closeMap();travelTo(d,true);});mapGrid.appendChild(m);d.mapButton=m;}
...
function render(now){const {w,h}=resize();updateTransition(now);const sceneTime=reducedMotion?0:now*.001;const vp=mul(perspective(52*Math.PI/180,w/h,1,5000),lookAt(camera.eye,camera.look));gl.disable(gl.BLEND);const clear=worldState.environment.phase==='deep-night'?[.004,.008,.022,1]:GRATITUDE_COAST_NIGHT.sky.clear;gl.clearColor(clear[0],clear[1],clear[2],clear[3]);gl.clear(gl.COLOR_BUFFER_BIT|gl.DEPTH_BUFFER_BIT);drawSky();drawMoon(vp);gl.enable(gl.DEPTH_TEST);draw(water,1,vp,sceneTime);draw(terrain,0,vp,sceneTime);drawAlaricSite(vp);updateSignals(vp);updateConstellations(vp);updateProofExpressions(vp);requestAnimationFrame(render);}
