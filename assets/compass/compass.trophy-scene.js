...
function mesh(gl){
  ...
  function textLine(name,text,cx,cy,width,height){const chars=[...text],advance=1.18,units=chars.reduce((s,ch)=>s+(ch===' '?advance*.65:advance),0)-.18,scale=width/units,gh=height,gw=scale;let x=cx-width*.5;for(const ch of chars){if(ch===' '){x+=gw*advance*.65;continue}for(const [x0,y0,x1,y1] of glyphStrokes(ch)){stroke(`${name}-${ch}`,x+x0*gw,cy-gh*.5+y0*gh,x+x1*gw,cy-gh*.5+y1*gh,gw*.13,.326,.337,letter)}x+=gw*advance}}
  ...
  box('nameplate-mount',-.315,.315,-.665,-.555,.286,.302,black);
  box('nameplate-face',-.295,.295,-.65,-.57,.302,.323,plaque);
  box('nameplate-edge-top',-.305,.305,-.57,-.558,.300,.326,plaqueEdge);
  box('nameplate-edge-bottom',-.305,.305,-.662,-.65,.300,.326,plaqueEdge);
  box('nameplate-edge-left',-.307,-.295,-.65,-.57,.300,.326,plaqueEdge);
  box('nameplate-edge-right',.295,.307,-.65,-.57,.300,.326,plaqueEdge);
  textLine('nameplate-line-1','DIAMOND GATE',0,-.595,.51,.024);
  textLine('nameplate-line-2','BRIDGE ?',0,-.628,.34,.024);
...
function mount(canvas,{foreground=()=>true}={}){
  ...
  const api=Object.freeze({canvas,fallback:false,activate:()=>false,capture:()=>{draw();return{triangles:m.triangles,parts:m.parts}},restore:()=>{draw();return true},inspect:()=>({fallback:false,foreground:foreground(),motion:'static-exhibit',triangles:m.triangles,parts:m.parts,material:'differentiated-aged-polished-gold-bronze-nameplate',nameplate:'raised-geometry:DIAMOND GATE BRIDGE ?'})});scenes.set(canvas,api);primary=api;return api
}
window.CompassTrophyScene=Object.freeze({version:'procedural-webgl-trophy-v5-mounted-lettering',mount,activate:()=>false,capture:()=>primary?.capture?.()||null,restore:()=>primary?.restore?.()||false,inspect:()=>primary?.inspect?.()||null});
})();
