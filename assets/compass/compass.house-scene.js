...
const VERSION='mirror-manor-gothic-phase3-carousel-v6-material-detail-final';
...
const surfaceClass=m=>{const id=m?.id||'',role=m?.role||'',material=m?.material||'';if(/^R-/.test(id)||/roof|crown/i.test(role)||/SLATE/i.test(material))return 2;if(/^(FC|TR-W|TR-E|IC)$/.test(id)||/court|terrace/i.test(role))return 3;return 1;};
function mount(canvas,{foreground=()=>true}={}){
 ...
 canvas.hidden=false;canvas.style.display='block';canvas.style.width='100%';canvas.style.height='100%';canvas.style.filter='brightness(1.24) saturate(.96) contrast(1.12) drop-shadow(0 22px 24px rgba(0,0,0,.34))';
 Object.assign(canvas.dataset,{houseRenderer:VERSION,houseContract:'MIRROR_MANOR_GOTHIC_ARCHITECTURAL_DETAIL_PHASE3_v1',houseGeometry:'canonical-phase3-principal-estate-mesh,carousel-detached-foreground-omitted,carousel-gatehouse-omitted,carousel-formal-garden-plinth-omitted,true-facade-openings,open-pointed-portals,true-dormer-apertures,tower-crowns,owned-gothic-enrichment,procedural-slate-shingle-courses,engraved-stone-court',houseMotion:'static-exhibit',houseRoofWindows:'none',houseWindowFloors:'two',houseCarouselDistance:String(CAROUSEL_DISTANCE),housePrincipalSpan:'27',houseCarouselComposition:'principal-estate-only-lowered-final-framing',houseSurfaceDetail:'slate-shingle-courses,engraved-compass-court,facade-relief-lighting',objectContinuity:CONTINUITY,legacyContinuity:LEGACY_CONTINUITY,houseCullPolicy:'two-sided-mixed-winding'});
 ...
 (async()=>{
  try{
   ...
   const vs=`attribute vec3 aPosition;attribute vec3 aNormal;attribute vec3 aColor;attribute float aSurface;uniform mat4 uMVP;varying float vLight;varying vec3 vColor;varying vec3 vPosition;varying float vSurface;void main(){vec3 N=normalize(aNormal);vec3 L=normalize(vec3(-.48,.82,.52));float key=max(0.0,dot(N,L));float side=max(0.0,dot(N,normalize(vec3(.62,.22,-.75))));vLight=.54+.40*key+.10*side;vColor=aColor;vPosition=aPosition;vSurface=aSurface;gl_Position=uMVP*vec4(aPosition,1.0);}`;
   const fs=`precision mediump float;varying float vLight;varying vec3 vColor;varying vec3 vPosition;varying float vSurface;void main(){vec3 c=vColor*vLight;if(vSurface>1.5&&vSurface<2.5){float course=step(.88,fract((vPosition.y+20.0)*1.28));float stagger=mod(floor((vPosition.y+20.0)*1.28),2.0)*.47;float joint=step(.92,fract((vPosition.x+vPosition.z*.32+stagger)*.82));float seam=max(course,joint);c*=mix(1.0,.58,seam);c*=vec3(.80,.84,.92);}else if(vSurface>2.5){vec2 p=vec2(vPosition.x,(vPosition.z-7.5)*.82);float r=length(p);float ring=step(abs(fract(r*.42)-.5),.035);float cross=max(step(abs(p.x),.075)*step(r,5.8),step(abs(p.y),.075)*step(r,5.8));float diag1=step(abs(p.x-p.y),.09)*step(r,4.4);float diag2=step(abs(p.x+p.y),.09)*step(r,4.4);float engr=max(ring*.78,max(cross,max(diag1,diag2)*.5));c*=mix(1.0,.55,engr);c+=vec3(.025,.02,.015);}else{float relief=.96+.04*fract((vPosition.y+vPosition.x*.17)*.43);c*=relief;}gl_FragColor=vec4(c,1.0);}`;
   ...
   const build=p3.buildPhase3DetailMesh();
   for(const m of neutral.buildNeutralMesh({omitFaceMap:p1b.OMIT_FACE_MAP}).meshes.filter(m=>m.id!==build.suppressedNeutralRoofId&&!omittedFromCarousel(m.id)&&!detachedForegroundMesh(m))){const s=surfaceClass(m);const c=s===2?[.15,.16,.19]:s===3?[.36,.35,.34]:[.43,.42,.41];add(m,c,s)}
   for(const m of build.replacementRoof.filter(m=>!omittedFromCarousel(m.id)&&!detachedForegroundMesh(m)))add(m,[.15,.16,.19],2);
   for(const m of build.meshes.filter(m=>!omittedFromCarousel(m.id)&&!detachedForegroundMesh(m))){const z=p3.PHASE3_MATERIALS[m.material]||p2.PHASE2_MATERIALS[m.material]||p1.MATERIAL_ZONES[m.material]||p1.MATERIAL_ZONES.GRAND_STONE;const s=surfaceClass(m);add(m,s===2?[.15,.16,.19]:z.rgb,s)}
