...
async function readGitLedgerBlob({repository,token,blobSha,source='resulting-ledger',commitSha=null}){const sha=dig(blobSha,40,'ledgerBlobSha',source),url=`${base(repository)}/git/blobs/${sha}`;const response=await fetch(url,{headers:{Accept:'application/vnd.github+json',Authorization:`Bearer ${str(token,'GITHUB_TOKEN','environment')}`,'X-GitHub-Api-Version':'2022-11-28'}});if(response.status!==200)throw err('LEDGER_BLOB_READ_FAILED','content',source,`status=${response.status}:commitSha=${commitSha||'UNKNOWN'}:blobSha=${sha}`);const q=await response.text();let g;try{g=q?JSON.parse(q):null}catch{g=null}let rawBytes;if(g?.encoding==='base64'&&typeof g?.content==='string')rawBytes=Buffer.from(g.content.replace(/\s/g,''),'base64');else if(g&&typeof g==='object'&&!Array.isArray(g))rawBytes=Buffer.from(q,'utf8');else throw err('LEDGER_BLOB_ENCODING_UNSUPPORTED','encoding',source,'EXPECTED_GIT_BLOB_JSON_OR_RAW_OBJECT');const actualGitBlobSha=createHash('sha1').update(Buffer.concat([Buffer.from(`blob ${rawBytes.length}\0`,'utf8'),rawBytes])).digest('hex');if(actualGitBlobSha!==sha)throw err('LEDGER_BLOB_TRANSPORT_CORRESPONDENCE_FAILURE','content',source,JSON.stringify({requestedBlobSha:sha,actualGitBlobSha,rawByteLength:rawBytes.length,rawSha256:createHash('sha256').update(rawBytes).digest('hex'),rawFirst64Hex:rawBytes.subarray(0,64).toString('hex'),commitSha:commitSha||null}));if(rawBytes.subarray(0,32).toString('utf8').startsWith('Warning: truncated output'))throw err('LEDGER_BLOB_TRANSPORT_CORRESPONDENCE_FAILURE','content',source,JSON.stringify({requestedBlobSha:sha,actualGitBlobSha,rawByteLength:rawBytes.length,rawSha256:createHash('sha256').update(rawBytes).digest('hex'),rawFirst64Hex:rawBytes.subarray(0,64).toString('hex'),commitSha:commitSha||null,reason:'DECORATED_TRUNCATED_PAYLOAD'}));try{return ledger(JSON.parse(rawBytes.toString('utf8')))}catch(e){if(e.code)throw e;throw err('LEDGER_JSON_DECODE_FAILURE','content',source,JSON.stringify({requestedBlobSha:sha,actualGitBlobSha,rawByteLength:rawBytes.length,commitSha:commitSha||null,parseError:e.message}))}}
