...
// Scientific-delta ingress must remain a governed evidence update rather than a claim upgrade.
...
// Hurricane delta: exact bounded standing, no scientific upgrade.
...
eq(hurricane.results.terminalDisposition, 'SIGNAL_PRESENT_INCONCLUSIVE', 'hurricane_terminal');
near(hurricane.results.stateEnvironmentAuc, 0.6525, 1e-12, 'hurricane_base_auc');
near(hurricane.results.combinedAuc, 0.6765, 1e-12, 'hurricane_combined_auc');
near(hurricane.results.incrementalAuc, 0.024, 1e-12, 'hurricane_incremental_auc');
ok(hurricane.results.stormClusterBootstrap.ci95[0] < 0 && hurricane.results.stormClusterBootstrap.ci95[1] > 0, 'hurricane_ci_must_cross_zero');
near(hurricane.results.COnlyAllEligibleAuc, 0.6738567730802415, 1e-12, 'hurricane_c_only_auc');
eq(hurricane.results.matchedStateContrast.pairs, 69, 'hurricane_pairs');
ok(hurricane.adjudication.newProjectEmpiricalResult === true, 'hurricane_new_result');
eq(hurricane.adjudication.staticHurdat2ProxyRemains, 'NEGATIVE', 'hurdat_negative_preserved');
eq(hurricane.adjudication.structuralMetadataStanding, 'POSITIVE_SIGNAL_INCONCLUSIVE', 'hurricane_standing');
ok(hurricane.adjudication.incrementalPredictionSupportThresholdCleared === false, 'hurricane_support_threshold');
...
ok(hurricane.trajectorySpecificationBoundary.specificationDifferencePresent === true, 'trajectory_spec_boundary');
ok(hurricane.checkpoint7Impact.selectedAsFirstVerticalCase === false, 'hurricane_not_first_vertical');

// Candidate selection must be non-performance-driven inside PR709 and must preserve source standing.
...
const hurricaneCandidate = candidates.candidates.find(x => x.candidateId === 'HURRICANE_PR734_TC_RADAR_METADATA');
...
eq(hurricaneCandidate.selectionDisposition, 'ELIGIBLE_NOT_SELECTED_FIRST', 'hurricane_candidate_disposition');
ok(hurricaneCandidate.scientificRejection === false, 'hurricane_not_rejected');
...
eq(h.exactHead, 'f6a7267e010a42cce94a7ea292af8e1974591ae2', 'pr734_head');
for (const f of s.files) {
  eq(git('rev-parse', `${s.exactHead}:${f.path}`), f.blob, `source_blob:${f.path}`);
}
eq(git('rev-parse', `${h.exactHead}:${h.executionProtocolPath}`), h.executionProtocolBlob, 'hurricane_protocol_blob');
eq(git('rev-parse', `${h.exactHead}:${h.executionScriptPath}`), h.executionScriptBlob, 'hurricane_script_blob');
...
const remoteProtocol = showJson(h.exactHead, h.executionProtocolPath);
eq(remoteProtocol.momentum.formula, 'C_t - previous same-storm C_t', 'hurricane_remote_momentum_formula');
eq(remoteProtocol.momentum.eligible_previous_interval_hours, [6,36], 'hurricane_remote_interval');
eq(remoteProtocol.claim_ceiling, hurricane.delta.CLAIM_CEILING, 'hurricane_remote_claim_ceiling');

// Source ledger itself may not authorize source mutation or merges.
...
console.log(JSON.stringify({
  result: 'PASS_FIRST_SAFE_VERTICAL_CASE_SELECTION_v1',
