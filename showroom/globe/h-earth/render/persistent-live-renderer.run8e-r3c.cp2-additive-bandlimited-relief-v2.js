...
const LOGICAL_ID = H_EARTH_RUN_8E_GEN957_PACKAGE_SUCCESSOR_CONTRACT_ID;
...
export function createHEarthRun8ER3CPersistentRenderer({ canvas, width = 640, height = 360 } = {}) {
  ...
  const renderPackage = getHEarthRun8EGen957SuccessorLiveRenderPackage();
  const packageEvaluation = evaluateHEarthRun8EGen957SuccessorLiveRenderPackage(renderPackage);
  if (packageEvaluation.eligible !== true) throw new Error(`R3C_SUCCESSOR_PACKAGE_REJECTED:${packageEvaluation.issues.join(',')}`);
  const uploadViews = createHEarthRun8EGen957SuccessorGPUUploadViews(renderPackage);
