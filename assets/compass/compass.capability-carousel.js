(()=>{
  ...
  function mountCapability(){
    ...
    const brainPointer=event=>event.composedPath?.().some(node=>node instanceof Element&&node.matches?.('[data-human-brain], [data-human-brain] *'));stage.addEventListener('pointerdown',event=>{if(brainPointer(event))stage.dataset.brainPointer='true'},true);const clearBrainPointer=()=>delete stage.dataset.brainPointer;stage.addEventListener('pointerup',clearBrainPointer,true);stage.addEventListener('pointercancel',clearBrainPointer,true);
    window.CompassOrbitInput?.claimSwipe(stage,rotateCard,{disabled:()=>mode!=='orbit'||stage.dataset.brainPointer==='true'});window.CompassOrbitInput?.claimSwipe(houseOrbit,rotateMember,{disabled:()=>mode!=='house'});
