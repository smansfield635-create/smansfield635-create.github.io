...
function renderReceipt(receipt){if(!receipt?.stages)return;let progress=0,stage='',label='Preparing browser-native world runtime',failedStage=null;for(const [name,status] of Object.entries(receipt.stages)){if(status==='PASS'){progress=Math.max(progress,PROGRESS[name]??0);stage=name;label=LABEL[name]??label;}else if(status==='FAIL'){failedStage=name;break;}}verifiedProgress=Math.max(verifiedProgress,progress);displayedProgress=Math.max(displayedProgress,verifiedProgress);updateLoaderVisual();if(failedStage){clearInterval(activityTimer);loader.dataset.failed='true';loader.querySelector('.h-earth-experience-loader__headline').textContent='Initialization failed.';statusNode.textContent=`FAILED AT: ${failedStage} · ${receipt.failureClass??'UNCLASSIFIED'}`;const diagnostic={firstFailureStage:receipt.firstFailureStage??failedStage,failureClass:receipt.failureClass??null,exceptionName:receipt.exceptionName??null,exceptionMessage:receipt.exceptionMessage??null,shaderLog:receipt.shaderLog??null,programLinkLog:receipt.programLinkLog??null,webglError:receipt.webglError??null,framebufferStatus:receipt.framebufferStatus??null,contextLost:receipt.contextLost??false};diagnosticNode.textContent=JSON.stringify(diagnostic,null,2);return;}statusNode.textContent=label;if(stage&&stage!==activeStage)startBoundedActivity(stage);if(verifiedProgress>=100){clearInterval(activityTimer);displayedProgress=100;updateLoaderVisual();clearTimeout(readyHideTimer);readyHideTimer=setTimeout(()=>{loader.dataset.ready='true';setTimeout(()=>loader.remove(),700);},reducedMotion?0:420);}}
