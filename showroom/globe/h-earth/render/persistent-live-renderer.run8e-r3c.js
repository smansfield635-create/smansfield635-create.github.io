...
void main(){
  ...
  if(vRoleCode==1u){
    ...
    float cavernGroundContact=max(
      cavernContact,
      max(cavernOuterContact*0.82,max(cavernRelationContact*0.72,cavernApproachEdge))
    );
    ...
    palette*=mix(1.0,0.37,cavernGroundContact*(0.38+0.48*cavernFracture));
    palette*=mix(0.66,1.30,cavernApproachStrata*cavernApproach*0.86+(1.0-cavernApproach)*0.50);
    ...
    palette+=vec3(-0.032,0.044,0.062)*(cavernApproachStrata-0.5)*cavernApproach;
    presentationContact=max(presentationContact,cavernGroundContact);
    presentationHighlight=max(presentationHighlight,cavernOuterContact*0.34+cavernRelationContact*0.26+cavernApproachEdge*0.42);
  ...
  function getResourceReceipt() {
    ...
    return {
      ...
      presentationProfile: {
        ...
        cavernQuantizationScaleIncrement: true,
