...
.h-earth-experience-loader__actions{display:none;gap:8px;margin-top:12px}.h-earth-experience-loader[data-failed="true"] .h-earth-experience-loader__actions{display:flex}
...
function renderReceipt(receipt){if(!receipt?.stages)return;const readyPublished=receipt.stages.READY_PUBLISHED==='PASS';let progress=0,stage='',label='Preparing browser-native world runtime',failedStage=null;for(const [name,status] of Object.entries(receipt.stages)){if(status==='PASS'){progress=Math.max(progress,PROGRESS[name]??0);stage=name;label=LABEL[name]??label;}else if(status==='FAIL'&&!readyPublished){failedStage=name;break;}}verifiedProgress=Math.max(verifiedProgress,readyPublished?100:progress);displayedProgress=Math.max(displayedProgress,verifiedProgress);updateLoaderVisual();if(readyPublished){clearInterval(activityTimer);delete loader.dataset.failed;loader.querySelector('.h-earth-experience-loader__headline').textContent='Entering the coast.';statusNode.textContent=LABEL.READY_PUBLISHED;displayedProgress=100;updateLoaderVisual();clearTimeout(readyHideTimer);readyHideTimer=setTimeout(()=>{loader.dataset.ready='true';setTimeout(()=>loader.remove(),700);},reducedMotion?0:420);return;}if(failedStage){clearInterval(activityTimer);loader.dataset.failed='true';loader.querySelector('.h-earth-experience-loader__headline').textContent='Initialization failed.';statusNode.textContent=`FAILED AT: ${failedStage} · ${receipt.failureClass??'UNCLASSIFIED'}`;const diagnostic={firstFailureStage:receipt.firstFailureStage??failedStage,failureClass:receipt.failureClass??null,exceptionName:receipt.exceptionName??null,exceptionMessage:receipt.exceptionMessage??null,shaderLog:receipt.shaderLog??null,programLinkLog:receipt.programLinkLog??null,webglError:receipt.webglError??null,framebufferStatus:receipt.framebufferStatus??null,contextLost:receipt.contextLost??false};diagnosticNode.textContent=JSON.stringify(diagnostic,null,2);return;}statusNode.textContent=label;if(stage&&stage!==activeStage)startBoundedActivity(stage);}
...
copyButton.addEventListener('click',()=>navigator.clipboard?.writeText(diagnosticNode.textContent||''));retryButton.addEventListener('click',()=>location.reload());
setTimeout(()=>{if(loader.isConnected&&loader.dataset.ready!=='true'&&loader.dataset.failed!=='true'){const receipt=window.H_EARTH_RENDERER_STARTUP_DIAGNOSTICS?.getReceipt?.()??{};const routeReady=document.getElementById('h-earth-functional-landscape-route')?.dataset.run8eReady==='true';const observerReady=receipt.stages?.READY_PUBLISHED==='PASS';if(routeReady||observerReady){renderReceipt({...receipt,stages:{...(receipt.stages??{}),READY_PUBLISHED:'PASS'}});return;}const synthetic={...receipt,stages:{...(receipt.stages??{}),READY_PUBLISHED:'FAIL'},firstFailureStage:receipt.firstFailureStage??'READY_PUBLISHED',failureClass:receipt.failureClass??'STARTUP_TIMEOUT',exceptionMessage:receipt.exceptionMessage??'H-Earth did not publish READY within 20 seconds.'};renderReceipt(synthetic);}},20000);
