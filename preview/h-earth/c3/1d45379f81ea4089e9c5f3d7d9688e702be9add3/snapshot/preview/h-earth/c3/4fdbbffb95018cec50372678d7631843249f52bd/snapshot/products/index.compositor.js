/* /products/index.compositor.js
   PRODUCTS dedicated camera and visual compositor.

   Module:
   DGB_PRODUCTS_COMPOSITOR
   1.0.0-camera-depth-layer-orchestration

   Controller anchor:
   DGB_PRODUCTS_CONTROLLER
   7.0.0-controller-interaction-semantic-priority

   Crystals compatibility anchor:
   DGB_PRODUCTS_CRYSTALS
...
(() => {
  ...
  const CONTRACT = Object.freeze({
    ...
    receiptId:
      "DGB_PRODUCTS_COMPOSITOR_RECEIPT_v2",
    ...
    compatibleCrystalsModuleVersion:
      "2.0.0-products-compositor-coordinated-renderer",
  ...
  function createCanvas(
    ...
    const existing =
      qs(
        `canvas[data-products-compositor-layer="${definition.datasetValue}"]`,
        state.field
      );

    invariant(
      !existing,
      `PRODUCTS_COMPOSITOR_DUPLICATE_LAYER:${definition.id}`
    );
    ...
    canvas.dataset
      .productsCompositorLayer =
      definition.datasetValue;

    canvas.dataset
      .productsCrystalsCanvas =
      definition.datasetValue;
  ...
  function requireController() {
    const controller = globalThis.DGB_PRODUCTS_CONTROLLER;
    invariant(controller, "PRODUCTS_COMPOSITOR_CONTROLLER_REQUIRED");
    invariant(
      typeof controller.getFrameState === "function",
      "PRODUCTS_COMPOSITOR_CONTROLLER_FRAME_SURFACE_REQUIRED"
    );
    return controller;
  }
  ...
  function resolveDom() {
    ...
    invariant(
      state.root,
      "PRODUCTS_COMPOSITOR_ROOT_NOT_FOUND"
    );
    ...
    invariant(
      state.scene,
      "PRODUCTS_COMPOSITOR_SCENE_NOT_FOUND"
    );

    state.field =
      qs(
        "[data-products-scene-field]",
        state.scene
      ) ||
      qs(
        ".products-scene",
    ...
    invariant(
      state.field,
      "PRODUCTS_COMPOSITOR_FIELD_NOT_FOUND"
    );
    ...
    invariant(
      state.compassMount,
      "PRODUCTS_COMPOSITOR_COMPASS_MOUNT_NOT_FOUND"
    );
    ...
    invariant(
      state.semanticLayer,
      "PRODUCTS_COMPOSITOR_SEMANTIC_LAYER_NOT_FOUND"
    );
  }
  ...
  function validateStacking() {
    invariant(
      state.rearCanvas &&
      state.frontCanvas,
      "PRODUCTS_COMPOSITOR_LAYERS_REQUIRED"
    );
    ...
    invariant(
      state.rearCanvas
        .parentElement ===
        state.field,
      "PRODUCTS_COMPOSITOR_REAR_LAYER_PARENT_INVALID"
    );
    ...
    invariant(
      state.frontCanvas
        .parentElement ===
        state.field,
      "PRODUCTS_COMPOSITOR_FRONT_LAYER_PARENT_INVALID"
    );
    ...
    invariant(
      globalThis
        ...
        .pointerEvents ===
        "none",
      "PRODUCTS_COMPOSITOR_REAR_POINTER_EVENTS_INVALID"
    );
    ...
    invariant(
      globalThis
        ...
        .pointerEvents ===
        "none",
      "PRODUCTS_COMPOSITOR_FRONT_POINTER_EVENTS_INVALID"
    );
    ...
    invariant(
      Number(
        ...
        LAYERS.REAR.zIndex,
      "PRODUCTS_COMPOSITOR_REAR_Z_INDEX_INVALID"
    );
    ...
    invariant(
      ...
      "PRODUCTS_COMPOSITOR_COMPASS_Z_INDEX_INVALID"
    );
    ...
    invariant(
      Number(
        ...
        LAYERS.FRONT.zIndex,
      "PRODUCTS_COMPOSITOR_FRONT_Z_INDEX_INVALID"
    );
    ...
    invariant(
      ...
      "PRODUCTS_COMPOSITOR_SEMANTIC_Z_INDEX_INVALID"
    );
    ...
    state.root.dataset
      .productsCompositeStackingValidated =
      "true";

    state.root.dataset
      .productsCompositeOrder =
      "REAR_COMPASS_FRONT_SEMANTIC";

    state.root.dataset
      .productsCompositeFieldFacts =
      JSON.stringify(
  ...
  function resize() {
    invariant(
      state.initialized &&
      !state.disposed,
      "PRODUCTS_COMPOSITOR_NOT_AVAILABLE"
    );
  ...
  function normalizePresentation(frame) {
    if (!frame || frame.held || frame.state === "HELD") {
      return PRESENTATIONS.HELD;
    }
    if (frame.state === "PRIMARY_ENTRY") {
      return PRESENTATIONS.CONSTELLATION;
    }
    if (frame.state === "CLUSTER_OPEN" || frame.state === "PRODUCT_SELECTED") {
  ...
  function updateCamera(
    ...
    invariant(
      ...
      "PRODUCTS_COMPOSITOR_COMPASS_PLANE_INVALID"
    );
  }
  ...
  function getViewMatrix() {
    invariant(
      state.viewMatrix,
      "PRODUCTS_COMPOSITOR_VIEW_MATRIX_NOT_READY"
    );
  ...
  function getProjectionMatrix() {
    invariant(
      state.projectionMatrix,
      "PRODUCTS_COMPOSITOR_PROJECTION_MATRIX_NOT_READY"
    );
  ...
  function getCompassFieldBounds() {
    invariant(
      state.field &&
      state.compassMount,
      "PRODUCTS_COMPOSITOR_COMPASS_BOUNDS_NOT_AVAILABLE"
    );
  ...
  function projectWorldPoint(
    ...
    invariant(
      ...
      state.projectionMatrix &&
      state.viewProjectionMatrix,
      "PRODUCTS_COMPOSITOR_FRAME_NOT_BEGUN"
    );
  ...
  function partitionNodes(options = {}) {
    ...
    invariant(
      typeof options
        .getWorldCenter ===
        "function",
      "PRODUCTS_COMPOSITOR_WORLD_CENTER_CALLBACK_REQUIRED"
    );
  ...
  function renderComposite(options = {}) {
    ...
    invariant(
      typeof options
        .drawRear ===
        "function",
      "PRODUCTS_COMPOSITOR_REAR_DRAW_CALLBACK_REQUIRED"
    );
    ...
    invariant(
      typeof options
        .drawFront ===
        "function",
      "PRODUCTS_COMPOSITOR_FRONT_DRAW_CALLBACK_REQUIRED"
    );
  ...
  function getRearLayer() {
    invariant(
      state.rearCanvas,
      "PRODUCTS_COMPOSITOR_REAR_LAYER_NOT_READY"
    );
  ...
  function getFrontLayer() {
    invariant(
      state.frontCanvas,
      "PRODUCTS_COMPOSITOR_FRONT_LAYER_NOT_READY"
    );
  ...
  function publishReceipt(extra = {}) {
    ...
    globalThis
      .DGB_PRODUCTS_COMPOSITOR_RECEIPT =
      frozen;

    if (state.root) {
      state.root.dataset
        .productsCompositorReceipt =
        JSON.stringify(
          frozen
        );

      state.root.dataset
        .productsCompositorStatus =
        state.status;

      state.root.dataset
        .productsCompositorVersion =
        CONTRACT.moduleVersion;

      state.root.dataset
        .productsCompositorControllerVersion =
        CONTRACT
          .requiredControllerModuleVersion;

      state.root.dataset
        .productsCompassPlaneViewDepth =
        String(
          state.compassPlaneViewDepth
        );

      state.root.dataset
        .productsRearVisibleNodeCount =
        String(
          state
            .rearVisibleNodeCount
        );
  ...
  function initialize() {
    ...
    invariant(
      !state.disposed,
      "PRODUCTS_COMPOSITOR_ALREADY_DISPOSED"
    );
    ...
    try {
      ...
      state.root.dataset
        .productsCompositorCameraOwner =
        "true";

      state.root.dataset
        .productsCompositorCompassLogicOwned =
        "false";

      state.root.dataset
        .productsCompositorCompassOverlapMeasurementOwned =
        "true";

      state.root.dataset
        .productsCompositorCrystalGeometryOwned =
        "false";

      state.root.dataset
        .productsCompositorDepthClassificationOwned =
        "true";
...
/*
AUDRALIA_PRODUCTS_COMPOSITOR_BOUNDED_CONTROLLER_7_INTEGRATION_RESULT_v2
...
- changed data-products-crystals-canvas value to rear/front layer identity
