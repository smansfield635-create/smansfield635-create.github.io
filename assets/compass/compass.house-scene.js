...
const VERSION='mirror-manor-gothic-phase3-carousel-v5-final-framing';
...
function mount(canvas,{foreground=()=>true}={}){
 ...
 (async()=>{
  try{
   ...
   const build=p3.buildPhase3DetailMesh();for(const m of neutral.buildNeutralMesh({omitFaceMap:p1b.OMIT_FACE_MAP}).meshes.filter(m=>m.id!==build.suppressedNeutralRoofId&&!omittedFromCarousel(m.id)&&!detachedForegroundMesh(m)))add(m,[.39,.40,.42]);for(const m of build.replacementRoof.filter(m=>!omittedFromCarousel(m.id)&&!detachedForegroundMesh(m)))add(m,p2.PHASE2_MATERIALS.DORMER_SLATE.rgb);for(const m of build.meshes.filter(m=>!omittedFromCarousel(m.id)&&!detachedForegroundMesh(m))){const z=p3.PHASE3_MATERIALS[m.material]||p2.PHASE2_MATERIALS[m.material]||p1.MATERIAL_ZONES[m.material]||p1.MATERIAL_ZONES.GRAND_STONE;add(m,z.rgb)}
