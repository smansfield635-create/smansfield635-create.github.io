...
import {
  createHEarthRun8ER3CPersistentRenderer as createNeutralRenderer,
  H_EARTH_GRATITUDE_REGION_CP7D_NEUTRAL_CONTROL_FIELD_PROFILE_ID
...
function apply(state, intent) {
  ...
  if (result?.ok !== true) throw new Error(`CP7D_NAVIGATION_REJECTED:${intent.action}:${result?.status}:${result?.issues?.join(',')}`);
  return result.state;
}
...
function stateForView(camera, target) {
  ...
  if (initial?.ok !== true) throw new Error('CP7D_INITIAL_NAVIGATION_STATE_REJECTED');
  ...
  if (cameraTerrain?.valid !== true || targetTerrain?.valid !== true) throw new Error('CP7D_TERRAIN_SAMPLE_INVALID');
  ...
  if (evaluation.eligible !== true) throw new Error(`CP7D_SCENE_STATE_INELIGIBLE:${evaluation.issues.join(',')}`);
  return state;
}
function readRgba(canvas) {
  ...
  if (error !== gl.NO_ERROR) throw new Error(`CP7D_COLOR_READBACK_ERROR:${error}`);
  return bytes;
}
function readDepthMask(renderer, canvas) {
  ...
  const error = gl.getError();
  if (error !== gl.NO_ERROR) throw new Error(`CP7D_DEPTH_READBACK_ERROR:${error}`);
  const mask = new Uint8Array(canvas.width * canvas.height);
  for (let index = 0; index < mask.length; index += 1) mask[index] = rgba[index * 4] > 0 ? 1 : 0;
  return mask;
}

export async function createCp7dNeutralRendererSuite({ acceptedCanvas, neutralCanvas, authority }) {
  ...
  const neutralRenderer = createNeutralRenderer({ canvas: neutralCanvas, width: viewport.width, height: viewport.height });
  const contextLoss = { accepted: 0, neutral: 0 };
  acceptedCanvas.addEventListener('webglcontextlost', () => { contextLoss.accepted += 1; });
  neutralCanvas.addEventListener('webglcontextlost', () => { contextLoss.neutral += 1; });
  acceptedRenderer.initialize(initialPacket);
  neutralRenderer.initialize(initialPacket);
  const records = [];
  ...
  for (const scene of authority.scenes) {
    ...
    neutralRenderer.renderFrame(packet);
    neutralRenderer.presentColorFrame();
    const neutralBytes = readRgba(neutralCanvas);
    const neutralDepth = readDepthMask(neutralRenderer, neutralCanvas);
    neutralRenderer.renderFrame(packet);
    neutralRenderer.presentColorFrame();
    const repeatedBytes = readRgba(neutralCanvas);
    ...
    records.push({
      ...
      neutralHash: fnv(neutralBytes),
      repeatedNeutralHash: fnv(repeatedBytes),
      colorBytesExact: exactBytes(acceptedBytes, neutralBytes),
      depthMaskExact: exactBytes(acceptedDepth, neutralDepth),
      neutralFixedFrameExact: exactBytes(neutralBytes, repeatedBytes),
  ...
  return clone({
    profile: neutralRenderer.getResourceReceipt().presentationProfileId,
    expectedProfile: H_EARTH_GRATITUDE_REGION_CP7D_NEUTRAL_CONTROL_FIELD_PROFILE_ID,
    scenes: records,
    contextLoss,
    receipts: {
      accepted: acceptedRenderer.getResourceReceipt(),
      neutral: neutralRenderer.getResourceReceipt()
    }
  });
}
