...
import { resolveBoundaryPrimitive } from "./render/boundary_primitive_engine.js";
...
/*
  ...
  Downstream render engines plug into resolvePrimitiveInstruction(...).

  Policy:
  - hub owns orchestration
  - downstream owns primitive choice
  - fallback remains full-diamond if downstream output is invalid
*/
...
function resolvePrimitiveInstruction({
  ...
  if (typeof resolveBoundaryPrimitive !== "function") {
    return fallback;
  }

  const resolved = resolveBoundaryPrimitive({
    sample,
    signalCell,
    x,
    y,
  ...
  if (!resolved || !Array.isArray(resolved.primitivePoints) || resolved.primitivePoints.length < 3) {
    return fallback;
  }

  return {
    primitiveType: typeof resolved.primitiveType === "string" ? resolved.primitiveType : fallback.primitiveType,
    primitiveScale: isFiniteNumber(resolved.primitiveScale) ? resolved.primitiveScale : fallback.primitiveScale,
    primitivePoints: resolved.primitivePoints,
    boundaryClass: typeof resolved.boundaryBandClass === "string" ? resolved.boundaryBandClass : fallback.boundaryClass,
    subdivisionTier: isFiniteNumber(resolved.subdivisionTier) ? resolved.subdivisionTier : fallback.subdivisionTier,
    orientation: typeof resolved.orientation === "string" ? resolved.orientation : fallback.orientation,
    approxSpanPx: isFiniteNumber(resolved.approxSpanPx) ? resolved.approxSpanPx : measurePolygonSpan(resolved.primitivePoints)
  };
}
...
function drawForwardSignalMesh(ctx, grid, topologyGrid, projectPoint, projectionState) {
  ...
  for (let y = rowStride; y < rowCount - rowStride; y += rowStride) {
    ...
    for (let x = 0; x < rowLength; x += colStride) {
      ...
      maxSubdivisionTier = Math.max(maxSubdivisionTier, renderResult.primitiveInstruction.subdivisionTier ?? projectionState.lensTier);

      if (renderResult.primitiveInstruction.primitiveType !== "FULL_DIAMOND") {
        sawBoundaryPrimitive = true;
      }
    }
  }
  ...
  if (sawBoundaryPrimitive) {
    primitiveMode = "BOUNDARY_SIGNAL";
    primitivePath = "boundaryPrimitiveEngine";
  }
  ...
  return {
    ...
    centerAnchored: true,
    rowColumnPathActive: true,
    sectorBandPathActive: sawBoundaryPrimitive,
    topologyMode: "HYBRID",
    neighborLaw: "CROSS_GRID",
    renderReadsScope: true,
    renderReadsLens: true,
