...
const checks = Object.freeze({
  ...
  PAGE_SPECIFIC_STAGE_CSS_PRESENT: /Laws child-stage integration checkpoint/.test(cssText) && /--stage-height:\s*clamp\(720px, 82svh, 940px\)/.test(cssText),
  MOBILE_STAGE_CONTINUITY_PRESENT: /--stage-height:\s*calc\(100svh - 66px\)/.test(cssText),
  FOUR_FAMILY_CANONICAL_ORDER_RING_PRESENT: /FAMILY_OVERVIEW_REGIONS/.test(extensionText) && /CANONICAL_ORDER_WITHOUT_DIRECTIONAL_AUTHORITY/.test(extensionText),
  CURVED_MODEL_TRAJECTORY_PRESENT: /CURVED_CANONICAL_ARC/.test(extensionText) && /arcDistance \* 1\.15/.test(extensionText),
