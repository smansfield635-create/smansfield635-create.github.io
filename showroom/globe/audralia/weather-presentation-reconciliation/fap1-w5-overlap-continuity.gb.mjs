...
function fbm(x,y,z){
  let value=noise3(x,y,z)*.62;
  value+=noise3(x*2.07+5.3,y*2.07+1.7,z*2.07+9.2)*(.62*.48);
  return value;
}
...
export function sampleFAP1MacroDensity(object,worldPoint,{canonicalTimeHours=0}={}){
  ...
  const radius2=x*x+vertical*vertical+y*y;
  ...
  if(g<2.5){
    ...
    const cells=.5+.5*Math.sin(qx*2.7+n*2.4)*Math.cos(qy*2.3-time*.012);
    ...
    const taper=mix(1,.48,smooth(.08,.95,z)),cell=.5+.5*Math.sin(qx*2.6+seed*9)*Math.cos(qy*2.35-time*.014);
...
function worldPointFromLocal(object,q){
  ...
  return [
    c[0]+object.V_i.axisU[0]*q[0]*r[0]+object.V_i.axisUp[0]*q[1]*r[1]+object.V_i.axisV[0]*q[2]*r[2],
    c[1]+object.V_i.axisU[1]*q[0]*r[0]+object.V_i.axisUp[1]*q[1]*r[1]+object.V_i.axisV[1]*q[2]*r[2],
    c[2]+object.V_i.axisU[2]*q[0]*r[0]+object.V_i.axisUp[2]*q[1]*r[1]+object.V_i.axisV[2]*q[2]*r[2]
  ];
}
...
export function evaluateMacroLocalContinuity(object,{canonicalTimeHours=0,resolution=CONTINUITY_SAMPLE_RESOLUTION}={}){
  ...
  for(let z=0;z<resolution;z++)for(let y=0;y<resolution;y++)for(let x=0;x<resolution;x++){
    const q=[-1+(x+.5)*span,-1+(y+.5)*span,-1+(z+.5)*span],radius=Math.hypot(...q);
    ...
    const macroOccupied=macro>W5_EMPTY_THRESHOLD,localOccupied=local>W5_EMPTY_THRESHOLD;
    if(macroOccupied===localOccupied)occupiedAgreement++;
    if(radius>=CONTINUITY_BOUNDARY_INNER_RADIUS&&radius<=CONTINUITY_BOUNDARY_OUTER_RADIUS){boundaryCount++;boundaryError+=Math.abs(local-macro);}
    count++;
  }
  const normalization=Math.max(.08,(macroMass+localMass)/(2*Math.max(count,1))),normalizedMae=(absoluteError/Math.max(count,1))/normalization;
  const boundaryMae=(boundaryError/Math.max(boundaryCount,1))/normalization;
  const massRatio=localMass/Math.max(macroMass,1e-9),occupancyAgreement=occupiedAgreement/Math.max(count,1);
  ...
  return freeze({
    schema:FAP1_W5_OVERLAP_CONTINUITY_SCHEMA,
    weatherId:object.ID_i,
    persistentWeatherIdentity:true,
    sharedSamplePositions:true,
    sampleCount:count,
    ...
    boundaryMae,
    occupancyAgreement,
    thresholds:freeze({maxNormalizedMae:CONTINUITY_MAX_NORMALIZED_MAE,maxBoundaryMae:CONTINUITY_MAX_BOUNDARY_MAE,minMassRatio:CONTINUITY_MIN_MASS_RATIO,maxMassRatio:CONTINUITY_MAX_MASS_RATIO}),
    pass,
    handoffAuthority:pass?'ELIGIBLE_FOR_BOUNDED_HANDOFF_CONSTRUCTION':'HELD',
    macroRendererMutation:false,
    localRendererMutation:false,
