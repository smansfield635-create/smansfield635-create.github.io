...
import {
  createHEarthRun8ER3CPersistentRenderer as createRound2Renderer,
  H_EARTH_GRATITUDE_REGION_CP2_ROUND2_PRESENTATION_PROFILE_ID
} from '../../../showroom/globe/h-earth/render/persistent-live-renderer.run8e-r3c.cp2-round2-candidate.js';
...
function apply(state, intent) {
  ...
  if (result?.ok !== true) throw new Error(`CP5_NAVIGATION_REJECTED:${intent.action}:${result?.status}:${result?.issues?.join(',')}`);
  return result.state;
}
...
function stateForView(camera, target) {
  ...
  if (initial?.ok !== true) throw new Error('CP5_INITIAL_NAVIGATION_STATE_REJECTED');
  ...
  if (cameraTerrain?.valid !== true || targetTerrain?.valid !== true) throw new Error('CP5_PRESENTATION_TERRAIN_SAMPLE_INVALID');
  ...
  if (evaluation.eligible !== true) throw new Error(`CP5_SCENE_STATE_INELIGIBLE:${evaluation.issues.join(',')}`);
  return state;
}
...
function readRgba(canvas) {
  ...
  if (error !== gl.NO_ERROR) throw new Error(`CP5_COLOR_READBACK_ERROR:${error}`);
  return bytes;
}
function readDepthMask(renderer, canvas) {
  ...
  gl.readPixels(0, 0, canvas.width, canvas.height, gl.RGBA, gl.UNSIGNED_BYTE, bytes);
  const error = gl.getError();
  if (error !== gl.NO_ERROR) throw new Error(`CP5_DEPTH_READBACK_ERROR:${error}`);
  const mask = new Uint8Array(canvas.width * canvas.height);
  for (let index = 0; index < mask.length; index += 1) mask[index] = bytes[index * 4] > 0 ? 1 : 0;
  return mask;
}
...
function normalizedLuminance(bytes, mask, width, height, outputWidth = 256, outputHeight = 256) {
  ...
  for (let oy = 0; oy < outputHeight; oy += 1) {
    ...
    for (let ox = 0; ox < outputWidth; ox += 1) {
      ...
      values[cell] = eligibleCount > 0 ? rectangle(sum, x0, y0, x1, y1) / eligibleCount : 0;
    }
  }
  return { values, mask: outputMask, width: outputWidth, height: outputHeight };
}
...
function repetitionMetric(bytes, depthMask, width, height, control) {
  const normalized = normalizedLuminance(bytes, depthMask, width, height, 256, 256);
  ...
  const scores = {
    micro: directionalCorrelationScore(micro, normalized.mask, 256, 256, control.orientationsDegrees, control.lagsPixels),
    meso: directionalCorrelationScore(meso, normalized.mask, 256, 256, control.orientationsDegrees, control.lagsPixels),
    macro: directionalCorrelationScore(macro, normalized.mask, 256, 256, control.orientationsDegrees, control.lagsPixels)
  };
...
export async function createCp5Round2Suite({ cp2Canvas, candidateCanvas, control }) {
  if (!(cp2Canvas instanceof HTMLCanvasElement) || !(candidateCanvas instanceof HTMLCanvasElement)) throw new TypeError('CP5_CANVASES_REQUIRED');
  const viewport = control.viewport;
  const initialState = stateForView(control.scenes[0].camera, control.scenes[0].target);
  const initialPacket = createHEarthRun8ER3AFrameUniformPacket({ navigationState: initialState, viewport, frameSequence: 1 });
  const cp2Renderer = createCp2Renderer({ canvas: cp2Canvas, width: viewport.width, height: viewport.height });
  const candidateRenderer = createRound2Renderer({ canvas: candidateCanvas, width: viewport.width, height: viewport.height });
  ...
  const sceneRecords = [];
  const contextLoss = { cp2: 0, candidate: 0 };
  ...
  const renderWith = (renderer, canvas, packet) => {
    ...
    const depthMask = readDepthMask(renderer, canvas);
    return { responseMs, bytes, metrics, depthMask };
  };

  const renderScene = (sceneId) => {
    const scene = sceneMap.get(sceneId);
    if (!scene) throw new Error(`CP5_SCENE_UNKNOWN:${sceneId}`);
    ...
    const maskIdentity = fnv(cp2.depthMask) === fnv(candidate.depthMask);
    const cp2Repetition = repetitionMetric(cp2.bytes, cp2.depthMask, cp2Canvas.width, cp2Canvas.height, control);
    const candidateRepetition = repetitionMetric(candidate.bytes, cp2.depthMask, candidateCanvas.width, candidateCanvas.height, control);
    const record = {
      scene: clone(scene),
      ...
      cp2: { metrics: cp2.metrics, repetition: cp2Repetition, responseMs: cp2.responseMs },
      candidate: { metrics: candidate.metrics, repetition: candidateRepetition, responseMs: candidate.responseMs, repeatedHash },
      fixedFrameDeterministic: repeatedHash === candidate.metrics.byteHash,
      depthMaskIdentity: maskIdentity,
      repetitionRatio: candidateRepetition.sceneScore / Math.max(1e-9, cp2Repetition.sceneScore),
  ...
  const buildMotionPackets = () => {
    ...
    for (let index = 0; index < control.motion.frameCount; index += 1) {
      ...
      packets.push(createHEarthRun8ER3AFrameUniformPacket({ navigationState: state, viewport, frameSequence: ++sequence }));
    }
    return packets;
  };

  const executeMotion = () => {
    const packets = buildMotionPackets();
    ...
    replay(cp2Renderer, cp2Canvas, false);
    replay(candidateRenderer, candidateCanvas, false);
    const cp2Performance = replay(cp2Renderer, cp2Canvas, false);
    const candidatePerformance = replay(candidateRenderer, candidateCanvas, false);
    const candidateReplayA = replay(candidateRenderer, candidateCanvas, true);
    const candidateReplayB = replay(candidateRenderer, candidateCanvas, true);
    return {
      frameCount: packets.length,
      deterministicReplay: JSON.stringify(candidateReplayA.hashes) === JSON.stringify(candidateReplayB.hashes),
      firstReplayHashes: candidateReplayA.hashes,
      secondReplayHashes: candidateReplayB.hashes,
      cp2: { medianMs: median(cp2Performance.times), p95Ms: percentile(cp2Performance.times, 0.95), samples: cp2Performance.times.length },
      ...
      medianRatio: median(candidatePerformance.times) / Math.max(0.0001, median(cp2Performance.times)),
      p95Ratio: percentile(candidatePerformance.times, 0.95) / Math.max(0.0001, percentile(cp2Performance.times, 0.95))
    };
  };

  const finalize = () => {
    const motion = executeMotion();
    ...
    const cp2RepetitionMean = average(sceneRecords.map((record) => record.cp2.repetition.sceneScore));
    const candidateRepetitionMean = average(sceneRecords.map((record) => record.candidate.repetition.sceneScore));
    return clone({
      ...
      aggregates: {
        ...
        cp2RepetitionMean,
        candidateRepetitionMean,
        repetitionRatio: candidateRepetitionMean / Math.max(1e-9, cp2RepetitionMean),
      ...
      motion,
      contextLoss,
      receipts: { cp2: cp2Receipt, candidate: candidateReceipt }
    });
  };
  ...
  return Object.freeze({
    ...
    getIdentity: () => ({ checkpoint: 5, profile: H_EARTH_GRATITUDE_REGION_CP2_ROUND2_PRESENTATION_PROFILE_ID, sceneCount: sceneMap.size })
  });
}
