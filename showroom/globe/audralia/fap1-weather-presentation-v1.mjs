const POLICY_ID='AUDRALIA_FAP1_ORGANIZED_WEATHER_PRESENTATION_v7_GLOBAL_ADVANCED_ONLY';
...
vec3 fap1OrganizedWeather(vec3 radial,float h,float lat,float lon){
  ...
  // Proven long frontal donor and geographically distinct descendants.
  ...
  // Proven long high-cloud / jet-band grammar distributed to both hemispheres.
  ...
  // Exact successful cyclone donor plus one separated organized descendant.
...
`;
  ...
  next=next.replace(
    ...
    'vec3 fap1=fap1OrganizedWeather(radial,h,lat,lon);float clearCorridor=fap1ClearCorridor(lat,lon);float background=0.0*(1.0-clearCorridor);float iceMass=fap1.y,precipMass=fap1.z,mass=fap1.x;'
  );
