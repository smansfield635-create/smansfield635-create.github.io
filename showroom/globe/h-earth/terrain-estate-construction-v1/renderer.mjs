...
const COASTAL_WET_OUTER_OFFSET = 7;
const COASTAL_WET_X_MIN = APERTURE.uMin;
const COASTAL_WET_X_MAX = APERTURE.uMax;
const COASTAL_WET_SEGMENTS = 384;
const COASTAL_WET_OFFSETS = freeze([0.25, 2, 4.5, 7]);
...
export const AUDRALIA_OPEN_WORLD_AUTHORING_CONTRACT = freeze({
  ...
  coherenceOperation: COHERENCE_OPERATION,
  ...
  coastalRibbonReconstructed: false, coastalRibbonLandwardEdgeContinuous: false,
  coastalTerrainMaterialIntegrated: true, independentBeachVisualAuthority: false,
  landwardBeachRibbonConstructed: false, seawardWetTransitionPreserved: true,
  outerCoastalShapePreserved: true,
  sharedHarborCoastAuthority: true, transitionCoastlineAnchoredToSharedHarborFunction: true,
  ...
  canonicalGeographicIdentityGateConstructed: true,
...
function tangentCoordinatesFromDirection(direction) {
  ...
  if (horizontal < 1e-12) return freeze({ u: 0, v: 0 });
...
function harborTransitionSignedField(u, v) { return gratitudeCoastalBoundaryZ(u) - (v + LOCAL_CENTER_Z); }
const HARBOR_BINDING_XS = freeze([-256, -192, -96, 0, 96, 192, 256]);
const COAST_TRANSITION_BINDING_XS = freeze([-384, -320, -256, -192, -128, -96, -64, 0, 64, 96, 128, 192, 256, 320, 384]);

function buildGratitudeCoastControlPoints() {
  const transition = COAST_TRANSITION_BINDING_XS.map((u) => freeze([u, gratitudeCoastalBoundaryV(u)]));
  return freeze([
    freeze([-1710, -270]), freeze([-1540, -20]), freeze([-1280, 150]), freeze([-1010, 170]),
    freeze([-770, 95]), freeze([-590, 235]), freeze([-430, 145]), ...transition,
...
function macroCoastalMaterialInfluence(u,v,coast,elevation){
  if(!coast.inside)return 0;
  const distanceWeight=1-smooth(10,62,coast.distance);
  const elevationWeight=1-smooth(HYDRO.seaLevelY+8,HYDRO.seaLevelY+18,elevation);
  const variation=clamp(.58+.22*Math.sin(u*.017+v*.009)+.16*Math.sin(u*.031-v*.014),.18,1);
  return c01(distanceWeight*elevationWeight*variation);
}
function gratitudeMacroSurfaceAtUV(u,v){
  ...
  const coastalSand=macroCoastalMaterialInfluence(u,v,coast,elevation);color=mix3(color,PALETTE.dune,coastalSand*.34);color=mix3(color,PALETTE.beach,coastalSand*.34);
  return{direction,field:coast.signedDistance,land:coast.land,elevation,color,coast,coastalSand};
}
...
function localColor(terrain,elevation,x,z){
  const wet=c01(terrain.coastline?.wetSandWeight??0),sand=c01(terrain.coastline?.beachWeight??0),site=c01(terrain.sitePreparation?.weight??0),high=c01((elevation-25)/42),low=c01((28-elevation)/22),preview=previewSandbarSample(x,z),source=c01(terrain.coastline?.sandbarWeight??0)*.22,sandbar=Math.max(source,preview.weight);
  let color=PALETTE.meadow;color=mix3(color,PALETTE.coastal,low*.48);color=mix3(color,PALETTE.dune,sand*.56);color=mix3(color,PALETTE.beach,sand*.74);color=mix3(color,PALETTE.wet,wet*.34);color=mix3(color,PALETTE.beach,sandbar*.96);color=mix3(color,PALETTE.upland,high*.34);color=mix3(color,PALETTE.rock,high*.42);if(terrain.insideReservedEstateEnvelope)color=mix3(color,PALETTE.estate,.38);return mix3(color,PALETTE.earth,site*.42);
}
function localTerrainSignedField(terrain,x,z){const mainland=gratitudeCoastalBoundaryZ(x)-z,preview=previewSandbarSample(x,z),source=c01(terrain.coastline?.sandbarWeight??0)*.30,sandbar=Math.max(source,preview.weight);return Math.max(mainland,(sandbar-.20)*18);}
...
function buildGratitudeDetailMesh(){
  ...
  return freeze({vertices:new Float32Array(vertices),indices:new Uint32Array(indices),samples,statistics:freeze({validSampleCount:samples.length,triangleCount:indices.length/3,renderedTerrainTriangles:counters.renderedTriangles,clippedCoastlineTriangleCount:counters.clippedTriangles,omittedOceanTriangles:counters.omittedTriangles,transparentTerrainTriangleCount:0,topologyClippedAtCoastline:true,coastlineTopology:'SUBCELL_SCALAR_FIELD_CLIPPED',minimumElevation,maximumElevation,beachSampleCount:beachSamples,terrainIntegratedSandSampleCount:terrainIntegratedSandSamples,terrainIntegratedSand:true,independentBeachVisualAuthority:false,gratitudeHighResolution:true,revision10MigrationSourcePreserved:true,localWidthAuthoringUnits:LOCAL_DOMAIN.width,localDepthAuthoringUnits:LOCAL_DOMAIN.depth,localArcScaleOneToOne:true,localScaleCompressed:false,localPreviewReliefNormalizationApplied:true,sourceTerrainMutation:false,renderColumns:LOCAL_RENDER_COLS,renderRows:LOCAL_RENDER_ROWS,rectangularBoundaryVisible:false,authoringRegionIsWorldBoundary:false,singleSurfaceOceanUsesPlanetaryMesh:true,localOceanOverlayConstructed:false,trueCoastalHarborBinding:true,previewSandbarCount:PREVIEW_SANDBARS.length,previewSandbarDiversityConstructed:true,sharedHarborCoastAuthority:true,liveTerrainMutation:false})});
}
...
function buildStitchMesh(detailMesh){
  ...
  return freeze({vertices:new Float32Array(vertices),indices:new Uint32Array(indices),statistics:freeze({vertexCount:vertices.length/10,triangleCount:indices.length/3,renderedTriangles:counters.renderedTriangles,clippedCoastlineTriangleCount:counters.clippedTriangles,omittedOceanTriangles:counters.omittedTriangles,stitchWidthAuthoringUnits:STITCH_WIDTH,layerStepAuthoringUnits:STITCH_LAYER_STEP,maximumTriangleEdgeLength:maximumEdgeLength,boundedTriangleEdges:maximumEdgeLength<=40,maximumLocalBoundaryPositionError,localBoundarySharedGeometrically:maximumLocalBoundaryPositionError<1e-6,maximumOuterBoundaryMacroElevationError,outerBoundaryConvergesToMacro:maximumOuterBoundaryMacroElevationError<1e-9,alignedRectangularGrid:true,scalarFieldCoastlineClipping:true,sharedHarborCoastAuthority:true,explicitAnnulusConstructed:true,separateMesh:true})});
}

function seawardWetTransitionColor(offset){const t=smooth(.25,COASTAL_WET_OUTER_OFFSET,offset);const near=mix3(PALETTE.wet,PALETTE.beach,.28);return mix3(near,PALETTE.ocean,t*.62);}
function seawardWetTransitionElevation(offset){return HYDRO.seaLevelY+mix(.16,.025,smooth(.25,COASTAL_WET_OUTER_OFFSET,offset));}
function buildSeawardWetTransitionMesh(){
  const vertices=[],indices=[],rows=COASTAL_WET_OFFSETS.length,columns=COASTAL_WET_SEGMENTS+1;
  for(let column=0;column<columns;column+=1){const x=mix(COASTAL_WET_X_MIN,COASTAL_WET_X_MAX,column/COASTAL_WET_SEGMENTS),shore=gratitudeCoastalBoundaryZ(x);for(let row=0;row<rows;row+=1){const offset=COASTAL_WET_OFFSETS[row],z=shore+offset,elevation=seawardWetTransitionElevation(offset),position=tangentPosition(x,z-LOCAL_CENTER_Z,elevation),normal=tangentDirection(x,z-LOCAL_CENTER_Z),color=seawardWetTransitionColor(offset);vertices.push(...position,...normal,color[0],color[1],color[2],1);}}
  ...
  return freeze({vertices:new Float32Array(vertices),indices:new Uint32Array(indices),statistics:freeze({vertexCount:vertices.length/10,triangleCount:indices.length/3,coastalRibbonConstructed:false,landwardEdgeContinuous:false,seawardEdgeContinuous:true,independentBeachVisualAuthority:false,landwardBeachRibbonConstructed:false,seawardWetTransitionConstructed:true,terrainIntegratedSand:true,outerCoastalShapePreserved:true,xMin:COASTAL_WET_X_MIN,xMax:COASTAL_WET_X_MAX,inlandOffset:null,seawardOffset:COASTAL_WET_OUTER_OFFSET,lateralLayerCount:rows,segmentCount:COASTAL_WET_SEGMENTS,sharesHarborShorelineFunction:true,sharedHarborCoastAuthority:true})});
}
...
function buildOW01Evidence(planetMesh,continentMesh,stitchMesh,detailMesh,coastalWetMesh){
  ...
  return freeze({schema:'H_EARTH_AUDRALIA_OPEN_WORLD_OW01_GEOGRAPHIC_EVIDENCE_v7',operationId:OPERATION_ID,coherenceOperation:COHERENCE_OPERATION,checkpoint:CHECKPOINT,lockGeneration:LOCK_GENERATION,governingHead:GOVERNING_HEAD,revision10Source:REVISION10_SOURCE,trueCoastalHarborBinding:maximumCoastalBindingError<1e-9,maximumCoastalBindingError,coastalBindingSampleCount:coastalBindingSamples.length,coastalBindingSamples:freeze(coastalBindingSamples),fullScaleLocalGratitudePreserved:true,coastlineRepresentation:'DELIBERATE_CLOSED_COASTLINE_CONTOUR_V1',coastlineUnionOfEllipses:false,coastlineTopology:'SUBCELL_SCALAR_FIELD_CLIPPED',coastlineControlPointCount:GRATITUDE_COAST_CONTROL_POINTS.length,coastlineSampleCount:GRATITUDE_COAST_CONTOUR.length,coastlinePlanarArea:polygonArea(GRATITUDE_COAST_CONTOUR),coastlinePlanarPerimeter:polygonPerimeter(GRATITUDE_COAST_CONTOUR),planetaryGratitudeLandRemoved:planetMesh.statistics.planetaryGratitudeLandVertices===0,gratitudeContinentalMeshSeparate:continentMesh.statistics.continentalMeshSeparateFromPlanetaryBase===true,continentalApertureConstructed:continentMesh.statistics.continentalApertureConstructed===true,continentalApertureClear:continentMesh.statistics.apertureClear===true,continentalApertureIntrusionTriangleCount:continentMesh.statistics.apertureIntrusionTriangleCount,explicitStitchAnnulusConstructed:stitchMesh.statistics.explicitAnnulusConstructed===true,stitchGridAligned:stitchMesh.statistics.alignedRectangularGrid===true,stitchScalarFieldCoastlineClipping:stitchMesh.statistics.scalarFieldCoastlineClipping===true,stitchMaximumTriangleEdgeLength:stitchMesh.statistics.maximumTriangleEdgeLength,stitchTrianglesBounded:stitchMesh.statistics.boundedTriangleEdges===true,stitchLocalBoundarySharedGeometrically:stitchMesh.statistics.localBoundarySharedGeometrically===true,stitchOuterBoundaryConvergesToMacro:stitchMesh.statistics.outerBoundaryConvergesToMacro===true,sharedHarborCoastAuthority:stitchMesh.statistics.sharedHarborCoastAuthority===true&&detailMesh.statistics.sharedHarborCoastAuthority===true&&coastalWetMesh.statistics.sharedHarborCoastAuthority===true,transitionBindingPointCount:COAST_TRANSITION_BINDING_XS.length,localTerrainTopologyClippedAtCoastline:detailMesh.statistics.topologyClippedAtCoastline===true,localCoastlineClipTriangleCount:detailMesh.statistics.clippedCoastlineTriangleCount,localTransparentTerrainTriangleCount:detailMesh.statistics.transparentTerrainTriangleCount,localPreviewReliefNormalizationApplied:detailMesh.statistics.localPreviewReliefNormalizationApplied===true,sourceTerrainMutation:false,continentalReliefHierarchy:continentMesh.statistics.continentalReliefHierarchy,reliefLowlandFraction:relief.lowlandFraction,reliefUplandFraction:relief.uplandFraction,reliefMountainFraction:relief.mountainFraction,reliefLowlandMajority:relief.lowlandMajority,reliefMountainCoverageBounded:relief.mountainCoverageBounded,planetaryOceanSingleSurface:true,localOceanOverlayConstructed:false,primaryInlandMountainWatershedAxes:true,primaryInlandAxisCount:PRIMARY_INLAND_AXES.length,climateReadyReliefHierarchy:true,detailedClimateModelConstructed:false,ow02DetailedContinuationConstructed:false,otherEightContinentsRemainNoncanonical:true,mechanicalPassIsNotUserAcceptance:true,coastalRibbonConstructed:false,coastalRibbonLandwardEdgeContinuous:false,coastalTerrainMaterialIntegrated:detailMesh.statistics.terrainIntegratedSand===true,independentBeachVisualAuthority:false,landwardBeachRibbonConstructed:false,seawardWetTransitionConstructed:coastalWetMesh.statistics.seawardWetTransitionConstructed===true,outerCoastalShapePreserved:true,previewSandbarCount:PREVIEW_SANDBARS.length,previewSandbarDiversityConstructed:true,unresolvedContinentPresentation:'ANISOTROPIC_WARPED_CONTOUR_PREVIEW',liveProductMutation:false});
}

function resolveCanonicalFeaturePosition(anchor){
  if(!anchor||typeof anchor.id!=='string')return null;
  if(anchor.id==='WATERFALL_CREST')return freeze({x:HYDRO.waterfall.visibleCrest.x,z:HYDRO.waterfall.visibleCrest.z});
  if(anchor.id==='WATERFALL_BASIN_CENTER')return freeze({x:HYDRO.reservoir.center.x,z:HYDRO.reservoir.center.z});
  if(anchor.id==='RESERVOIR_EAST_EDGE')return resolveHEarthMapWideReservoirBoundaryPoint(0);
  if(anchor.id==='RESERVOIR_WEST_EDGE')return resolveHEarthMapWideReservoirBoundaryPoint(Math.PI);
  if(anchor.class==='COASTLINE'||anchor.class==='HEADLAND')return freeze({x:anchor.x,z:gratitudeCoastalBoundaryZ(anchor.x)});
  if(anchor.id==='ESTATE_REFERENCE_POINT')return freeze({x:80,z:-172});
  return freeze({x:anchor.x,z:anchor.z});
}
function evaluateCanonicalGeographicIdentity(manifest){
  const tolerance=Number(manifest?.roundTripToleranceAuthoringUnits??1e-6),anchors=Array.isArray(manifest?.anchors)?manifest.anchors:[],results=[];let maximumRoundTripError=0,maximumFeatureBindingError=0;
  for(const anchor of anchors){const u=Number(anchor.x),v=Number(anchor.z)-LOCAL_CENTER_Z,direction=tangentDirection(u,v),inverse=tangentCoordinatesFromDirection(direction),planetaryX=inverse.u,planetaryZ=inverse.v+LOCAL_CENTER_Z,localError=Math.hypot(u-Number(anchor.x),(v+LOCAL_CENTER_Z)-Number(anchor.z)),regionError=Math.hypot(u-Number(anchor.x),(v+LOCAL_CENTER_Z)-Number(anchor.z)),continentError=Math.hypot(inverse.u-u,inverse.v-v),planetaryError=Math.hypot(planetaryX-Number(anchor.x),planetaryZ-Number(anchor.z)),roundTripError=Math.max(localError,regionError,continentError,planetaryError),feature=resolveCanonicalFeaturePosition(anchor),featureBindingError=feature?Math.hypot(feature.x-Number(anchor.x),feature.z-Number(anchor.z)):Infinity;maximumRoundTripError=Math.max(maximumRoundTripError,roundTripError);maximumFeatureBindingError=Math.max(maximumFeatureBindingError,featureBindingError);results.push(freeze({id:anchor.id,canonical:freeze({x:Number(anchor.x),z:Number(anchor.z)}),local:freeze({x:u,z:v+LOCAL_CENTER_Z}),region:freeze({u,v}),continent:freeze({u:inverse.u,v:inverse.v}),planetary:freeze({direction:freeze([...direction]),inverseX:planetaryX,inverseZ:planetaryZ}),feature:feature?freeze({x:feature.x,z:feature.z}):null,roundTripError,featureBindingError,pass:roundTripError<=tolerance&&featureBindingError<=tolerance}));}
  const canonicalPositionalIdentityPassed=anchors.length===12&&results.every(result=>result.pass===true)&&maximumRoundTripError<=tolerance&&maximumFeatureBindingError<=tolerance;
  return freeze({schema:'H_EARTH_OW01_CANONICAL_POSITIONAL_IDENTITY_GATE_v1',coherenceOperation:COHERENCE_OPERATION,manifestSchema:manifest?.schema??null,anchorCount:anchors.length,requiredRepresentationCount:4,toleranceAuthoringUnits:tolerance,maximumRoundTripError,maximumFeatureBindingError,canonicalPositionalIdentityPassed,results:freeze(results)});
}
...
export function createMapWideEnvironmentRenderer(canvas){
  ...
  function render(){resize();gl.enable(gl.DEPTH_TEST);gl.clearColor(...PALETTE.sky,1);gl.clear(gl.COLOR_BUFFER_BIT|gl.DEPTH_BUFFER_BIT);const cam=camera(),vp=drawTerrain(planetMesh,planetBuffers,cam,3600,11800);drawTerrain(continentMesh,continentBuffers,cam,1700,7200,[2,2]);drawTerrain(stitchMesh,stitchBuffers,cam,950,4700,[1.2,1.2]);drawTerrain(gratitudeMesh,gratitudeBuffers,cam,900,4400,[.25,.25]);drawTerrain(beachMesh,beachBuffers,cam,900,4400,[-.15,-.15]);if(waterMesh.indices.length>0){gl.enable(gl.BLEND);gl.blendFunc(gl.SRC_ALPHA,gl.ONE_MINUS_SRC_ALPHA);gl.depthMask(false);gl.useProgram(waterProgram);gl.uniformMatrix4fv(gl.getUniformLocation(waterProgram,'uVP'),false,vp);gl.bindVertexArray(localWaterBuffers.vao);gl.drawElements(gl.TRIANGLES,waterMesh.indices.length,gl.UNSIGNED_INT,0);gl.depthMask(true);gl.disable(gl.BLEND);}state.renderedFrames+=1;}
