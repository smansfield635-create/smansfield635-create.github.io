...
// AUDRALIA_DIAGNOSTIC_PROBE_CANVAS_SURFACE_TRUTH_F8_3D_TNT_v6
...
//
// Renewal purpose:
// - Restore F8 as surface-truth observer, not lifecycle interpreter.
// - Preserve target-frame, route, runtime-global, dataset, canvas, owner,
//   issue, and lifecycle-evidence separation.
// - Keep lifecycle milestones observable but nonblocking.
...
runtimeEvidenceReadableProvesReady: false,
lifecycleEvidenceBlocksSurfaceTruth: false,
readyClaimed: false,
...
function issue(code, path, detail, severity) {
return {
code: String(code || "ISSUE"),
path: String(path || "$"),
detail: String(detail || code || "ISSUE"),
severity: String(severity || "BLOCKING")
};
}
...
doc.querySelector("iframe[data-diagnostic-target-frame]") ||
...
firstString(
stringFromDataset(body, "audraliaPlanetActiveRendererBackend"),
stringFromDataset(html, "audraliaPlanetActiveRendererBackend"),
stringFromDataset(body, "audraliaPlanetRendererBackend"),
stringFromDataset(html, "audraliaPlanetRendererBackend")
),
...
firstNumber(
numberFromDataset(body, "audraliaPlanetRuntimeRenderSubmissionCount"),
numberFromDataset(html, "audraliaPlanetRuntimeRenderSubmissionCount")
),

drawCount:
firstNumber(
numberFromDataset(body, "audraliaPlanetRuntimeDrawCount"),
numberFromDataset(html, "audraliaPlanetRuntimeDrawCount")
),
...
result.runtimeEvidenceReadable =
Boolean(
isObject(result.runtimeStatus) ||
isObject(result.receiptLight) ||
isObject(result.receipt) ||
isObject(result.datasetStatus)
);
} catch (error) {
result.readError =
error && error.message
...
var runtimeEvidenceReadable =
firstBoolean(
liveFrame && liveFrame.runtimeEvidenceReadable,
...
var renderSubmissionCount =
numericStatusValue(runtimeStatus, "renderSubmissionCount");

var drawCount =
numericStatusValue(runtimeStatus, "drawCount");
...
var geometryHash =
stringStatusValue(runtimeStatus, "geometryHash");

var activeRendererBackend =
firstString(
stringStatusValue(runtimeStatus, "activeRendererBackend"),
stringStatusValue(runtimeStatus, "rendererBackend"),
...
runtimeEvidenceReadable === true;

var lifecycleMilestonesComplete =
mounted === true &&
...
var lifecycleEvidencePending =
runtimeGlobalPresent === true &&
runtimeEvidenceReadable === true &&
lifecycleMilestonesComplete !== true;
...
} else if (runtimeEvidenceReadable !== true) {
lifecycleClass =
...
lifecycleClass =
"FALLBACK_VISIBLE";
} else if (lifecycleEvidencePending) {
lifecycleClass =
"SURFACE_OBSERVED_WITH_LIFECYCLE_PENDING";
} else {
lifecycleClass =
"SURFACE_OBSERVED_RUNTIME_LIFECYCLE_UNCLASSIFIED";
}
...
runtimeEvidenceReadable: runtimeEvidenceReadable,
mounted: mounted,
running: running,
...
renderSubmissionCount: renderSubmissionCount,
drawCount: drawCount,
geometryHash: geometryHash,
activeRendererBackend: activeRendererBackend,
width: width,
height: height,
...
lifecycleMilestonesComplete: lifecycleMilestonesComplete,
lifecycleEvidencePending: lifecycleEvidencePending,
runtimeLifecyclePending: lifecycleEvidencePending,
surfaceTruthObserved: surfaceTruthObserved,
surfaceTruthAdmitted: surfaceTruthObserved,
surfaceAdmissionBasis: "FRAME_ROUTE_SAME_ORIGIN_RUNTIME_GLOBAL_RUNTIME_EVIDENCE",
...
if (surface.runtimeGlobalPresent !== true || surface.runtimeEvidenceReadable !== true) {
return {
ownerType: "PUBLIC_AUDRALIA_RUNTIME_PUBLICATION",
...
component: "RUNTIME_GLOBAL_AND_RECEIPT_PUBLICATION"
};
}
...
function buildBlockingIssues(surface) {
var issues = [];
...
if (surface.runtimeEvidenceReadable !== true) {
issues.push(
issue(
"TARGET_RUNTIME_EVIDENCE_UNAVAILABLE",
"$.target.runtimeStatus",
"The Audralia runtime global was present, but no usable status, receipt, or dataset evidence was readable.",
"BLOCKING"
)
);
...
function buildLifecycleIssues(surface) {
var issues = [];
...
return buildBlockingIssues(surface);
}
...
case "SURFACE_OBSERVED_WITH_LIFECYCLE_PENDING":
return "CANVAS_SURFACE_TRUTH_ADMITTED_RUNTIME_EVIDENCE_WITH_LIFECYCLE_PENDING";

case "SURFACE_OBSERVED_RUNTIME_LIFECYCLE_UNCLASSIFIED":
return "CANVAS_SURFACE_TRUTH_ADMITTED_RUNTIME_EVIDENCE_OBSERVED";

default:
return surface.surfaceTruthAdmitted
? "CANVAS_SURFACE_TRUTH_ADMITTED_RUNTIME_EVIDENCE_OBSERVED"
: "CANVAS_SURFACE_TRUTH_HELD_SURFACE_EVIDENCE_INCOMPLETE";
}
}
...
var blockingIssues =
buildBlockingIssues(surface);

var lifecycleIssues =
buildLifecycleIssues(surface);
...
runtimeEvidenceReadable: surface.runtimeEvidenceReadable
}
),

observation(
"CANVAS_SURFACE_LIFECYCLE_EVIDENCE",
"OBSERVED",
{
lifecycleEvidenceIsAdmissionGate: false,
runtimeEvidenceAvailable: surface.runtimeEvidenceAvailable,
mode: surface.mode,
activeRendererBackend: surface.activeRendererBackend,
...
geometryHash: surface.geometryHash,
webGL: surface.webGL,
...
renderSubmissionCount: surface.renderSubmissionCount,
drawCount: surface.drawCount,
...
surfaceTruthObserved: surface.surfaceTruthObserved,
surfaceTruthAdmitted: surface.surfaceTruthAdmitted,
surfaceAdmissionBasis: surface.surfaceAdmissionBasis,
primaryVisible: surface.primaryVisible,
fallbackVisible: surface.fallbackVisible,
lifecycleMilestonesComplete: surface.lifecycleMilestonesComplete,
lifecycleEvidencePending: surface.lifecycleEvidencePending,
lifecycleEvidenceBlocksAdmission: false,
blockingIssueCount: blockingIssues.length,
lifecycleIssueCount: lifecycleIssues.length,
...
{
id: "CANVAS_SURFACE_LIFECYCLE_ISSUES_NONBLOCKING",
kind: "OBSERVED",
nonblocking: true,
issueCount: lifecycleIssues.length,
...
kind: "DERIVED",
passed: false,
surfaceTruthAdmitted: false,
blockingIssueCount: blockingIssues.length,
lifecycleIssueCount: lifecycleIssues.length,
firstIssueCode:
blockingIssues[0] && blockingIssues[0].code
? blockingIssues[0].code
: null,
lifecycleClass: surface.lifecycleClass
}
]),
...
surfaceTruthObserverRole: true,
lifecycleEvidenceObservable: true,
lifecycleEvidenceAdmissionGate: false,
...
buildBlockingIssues: buildBlockingIssues,
buildLifecycleIssues: buildLifecycleIssues,
hash: hash,
noClaims: NO_CLAIMS
});
