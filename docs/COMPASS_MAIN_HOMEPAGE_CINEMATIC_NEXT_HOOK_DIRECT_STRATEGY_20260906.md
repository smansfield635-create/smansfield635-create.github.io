...
Date: 2026-09-06  
Exact strategy base: `99025b828629548be1ea066cd2e01dd16d855a03`
...
Current exact `main` at strategy freeze:

`99025b828629548be1ea066cd2e01dd16d855a03`

This is the merge of PR #2801, **Publish pre-rendered Compass homepage cinematic**.
...
`Diamond Gate Bridge bridge/title dissolve → text appears → image follows → text dissolves first → image remains briefly → next text appears before visual rhythm fully resets → next image follows`

The intended invariant is:

`TEXT IN < IMAGE IN < TEXT OUT < IMAGE OUT`
...
That distinction is now resolved.
...
`ENTRY_READY → Play selected → button begins fragmenting → fragmentation propagates into its containing entry card/stage → the actual visible entry surface disappears cell-by-cell → corresponding fragments become mobile → fragments migrate toward Compass/star targets → successor field is revealed underneath → Compass coheres → first MP4 frame is ready/presentable → final remnants clear → zero-blank-frame handoff → MP4 clock starts → PLAYING`

The key improvement is not "more particles." It is a one-to-one source/material relationship:

`SOURCE CELL DISAPPEARS ↔ MATCHING FRAGMENT BECOMES MOBILE`
...
Treat `main@99025b828629548be1ea066cd2e01dd16d855a03` and the PR #2801 master/player as the live inspection baseline.
