...
export const H_EARTH_RUN_8E_R3F2_CONTROL = freeze({
  ...
  predecessor: freeze({
    ...
    status: 'PASS_CLOSED',
    exactHead: '3642f3a561d787d37d988a8a66f2270d0b13bd45',
    pullRequest: 250,
    ...
    finalWorkflowRun: 30314790631,
    finalWorkflowJob: 90137966794,
    finalArtifactId: 8671739669,
  ...
  previewValidationEvidence: freeze({
    ...
    launcherHttpStatus: null,
    routeApiReady: null,
    iframeSameOriginAccess: null,
    launcherInstrumentationReady: null,
  ...
  physicalReferenceDeviceExecution: freeze({
    ...
    evidenceClassRequired: 'PHYSICAL_LOCAL_OR_REMOTE_PHYSICAL',
...
export function evaluateHEarthRun8ER3F2Control(candidate = H_EARTH_RUN_8E_R3F2_CONTROL) {
  ...
  if (candidate?.currentStatus !== 'PREVIEW_CONSTRUCTION_PENDING') {
    ...
    if (execution.routeHttpStatus !== 200 || execution.launcherHttpStatus !== 200 || execution.routeApiReady !== true || execution.iframeSameOriginAccess !== true || execution.launcherInstrumentationReady !== true) issues.push('R3F2_PREVIEW_VALIDATION_NOT_PASS');
  }
  ...
  return freeze({
    ...
    status: issues.length === 0
      ? (candidate?.currentStatus === 'PREVIEW_CONSTRUCTION_PENDING'
          ? 'RUN_8E_R3F2_PREVIEW_CONSTRUCTION_ELIGIBLE'
          : candidate?.currentStatus === 'PREVIEW_READY_PHYSICAL_EXECUTION_PENDING'
            ? 'RUN_8E_R3F2_PREVIEW_READY_PHYSICAL_EXECUTION_PENDING'
            : 'RUN_8E_R3F2_PASS_CLOSED')
      : 'RUN_8E_R3F2_CONTROL_FAIL',
    issues
  });
}
