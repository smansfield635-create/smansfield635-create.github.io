...
function radialFromLatLon(latitudeDeg,longitudeDeg){
  ...
  return [NORTH[0]*sl+(MERIDIAN[0]*co+EAST[0]*si)*cl,NORTH[1]*sl+(MERIDIAN[1]*co+EAST[1]*si)*cl,NORTH[2]*sl+(MERIDIAN[2]*co+EAST[2]*si)*cl];
}
...
async function waitForAuthority(){
  for(let i=0;i<320;i++){
    ...
    if(authority?.meteorologicalAuthority==='FAP1_ONLY'&&typeof authority.renderNow==='function'&&receipt?.setCameraStateForTest)return{authority,receipt};
...
function proofControl(control){globalThis.__AUDRALIA_FAP1_GA_PROOF_CONTROL__=freeze({canonicalTimeHoursOverride:0,...control});}
...
async function settle(authority,caseId){authority.renderNow();await waitForCase(authority,caseId);await sleep(90);}
function weatherCanvas(){return document.querySelector('canvas[data-fap1-ga="true"],canvas[data-fap1-g-a="true"]');}
function captureVisualMetric(){
  ...
  for(let i=0;i<data.length;i+=4){const alpha=data[i+3]/255;alphaSum+=alpha;alphaMax=Math.max(alphaMax,alpha);if(alpha>1/255)nonzero++;const luma=(.2126*data[i]+.7152*data[i+1]+.0722*data[i+2])/255;lumaAlpha+=luma*alpha;}
...
async function run(){
  ...
  proofControl({caseId:'PROOF_COMPLETE'});authority.renderNow();
  const result=freeze({schema:'AUDRALIA_FAP1_GA_RUNTIME_NEGATIVE_PROOF_v2',candidateHead:'e5fd753ca67777c4a8ba216245fbc16d3c86f36d',executedAt:new Date().toISOString(),thresholds:freeze({A1KnockoutAlphaRatioMax:.90,A2MaxAlphaMax:1/255,A3ClearAlphaRatioMax:.35,A4OriginAlphaRatioMax:.35,A4DestinationGainMin:2,A5MaxAlphaMax:1/255}),A1:freeze({cases:freeze(a1),pass:a1.every(item=>item.pass)}),A2:a2,A3:a3,A4:a4,A5:a5,pass:a1.every(item=>item.pass)&&a2.pass&&a3.pass&&a4.pass&&a5.pass,disposition:'G_A_RUNTIME_NEGATIVE_PROOF_ONLY_NO_G_B_AUTHORITY'});
  globalThis.__AUDRALIA_FAP1_GA_NEGATIVE_PROOF__=result;
  document.documentElement.setAttribute('data-fap1-ga-negative-proof',result.pass?'PASS':'FAIL');
  console.info('AUDRALIA_FAP1_GA_NEGATIVE_PROOF',result);
  return result;
}

run().catch(error=>{
  const failure=freeze({schema:'AUDRALIA_FAP1_GA_RUNTIME_NEGATIVE_PROOF_v2',pass:false,error:error instanceof Error?error.message:String(error),executedAt:new Date().toISOString()});
  globalThis.__AUDRALIA_FAP1_GA_NEGATIVE_PROOF__=failure;
  document.documentElement.setAttribute('data-fap1-ga-negative-proof','ERROR');
