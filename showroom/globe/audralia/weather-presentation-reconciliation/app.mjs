...
function wire(renderer,sky,exterior,canonical,fap1){
  const pointers=new Map();let gesture=null,wheelTimer=0,weatherTimer=0,settleFastTimer=0,settleFullTimer=0,weatherRevision=0;
  ...
  const evidence={schema:'AUDRALIA_INTERACTION_QUALITY_LADDER_v2',stage:'REST',fastFrames:0,partialFrames:0,lateFrames:0,fullSettles:0,lastVelocity:0,weatherSuperseded:0,weatherJobsRun:0,pointerWeatherDecoupled:true};
  ...
  const updateMotion=point=>{const now=performance.now(),dt=Math.max(1,now-motion.lastAt),d=motion.lastPoint?Math.hypot(point.x-motion.lastPoint.x,point.y-motion.lastPoint.y):0,v=d/dt;motion.velocity=motion.velocity===Infinity?v:motion.velocity*.68+v*.32;motion.lastAt=now;motion.lastPoint=point;evidence.lastVelocity=motion.velocity;renderFast();const age=now-motion.startedAt;if(age<170||motion.velocity>.24){motion.stage='INITIAL';evidence.stage='INITIAL';cancelWeather();}else if(motion.velocity>.075||age<650){motion.stage='LATE_MOTION';evidence.stage='LATE_MOTION';scheduleWeather('PARTIAL',110);}else{motion.stage='PRE_SETTLE';evidence.stage='PRE_SETTLE';scheduleWeather('LATE',140);}};
  const settle=()=>{cancelSettle();motion.stage='RELEASE';evidence.stage='RELEASE';renderer.render();settleFastTimer=setTimeout(()=>{const camera=cameraFrame(renderer);sky.render(camera);exterior.render(camera);evidence.stage='SHARPEN';},55);settleFullTimer=setTimeout(()=>{scheduleWeather('FULL',0);},185);};
  ...
  canvas.addEventListener('wheel',event=>{event.preventDefault();fap1.beginInteraction();exterior.beginInteraction();canonical.beginInteraction();renderer.zoom(event.deltaY);renderFast();cancelWeather();clearTimeout(wheelTimer);wheelTimer=setTimeout(settle,120);},{passive:false});
  ...
  window.addEventListener('resize',()=>{cancelWeather();renderer.render();fap1.endInteraction();exterior.endInteraction();canonical.endInteraction();renderAll();});
