...
// SHOWROOM_DIAMOND_LATTICE_RESTORATION_REBASE_JS_TNT_v1
// Pair: SHOWROOM_DIAMOND_LATTICE_RESTORATION_REBASE_HTML_TNT_v1
// Scope: active /showroom/ Diamond renderer only.
// Purpose:
// - Restore the raised WebGL Crown Cut 256 Lattice Diamond.
// - Preserve crystal/lattice dual-lens behavior.
// - Preserve 16 radial compass metric and 256 lattice logic.
// - Correct camera/stage scale so the diamond is not clipped and does not collide with the instruction layer.
// - No Audralia inheritance. No planet-template inheritance. No generated image. No GraphicBox.

const SHOWROOM_DIAMOND_STATE = Object.freeze({
  contract: "SHOWROOM_DIAMOND_LATTICE_RESTORATION_REBASE_JS_TNT_v1",
  pairedHtmlContract: "SHOWROOM_DIAMOND_LATTICE_RESTORATION_REBASE_HTML_TNT_v1",
  pairContract: "SHOWROOM_DIAMOND_LATTICE_RESTORATION_REBASE_TNT_v1",
  route: "/showroom/",
  role: "showroom-restored-raised-webgl-crown-cut-256-lattice-diamond",
  ...
  latticeStates: 256,
  stageCorrection: "restored-raised-above-instruction-box",
  clipped: false,
...
const LENS_COPY = Object.freeze({
  crystal: {
    title: "Crystal Form",
    route: "Crystal Form → WebGL true 3D Crown Cut Diamond · restored raised inspection view",
  ...
  lattice: {
    ...
    route: "Lattice Structure → WebGL 16-point compass geometry · restored raised lattice view",
...
const state = {
  ...
  pointProgram: null,
  crystalMesh: null,
  crystalEdgeLines: null,
  crystalPoints: null,
  latticeGhostMesh: null,
  latticeLines: null,
  latticePoints: null,
  errors: []
};
...
function scaleMat4(out, a, value) {
  out[0] = a[0] * value;
  out[1] = a[1] * value;
  out[2] = a[2] * value;
  out[3] = a[3] * value;
...
function buildLatticeGeometry() {
  ...
  ringDefs.forEach(([radius, y], ringIndex) => {
    ...
    for (let i = 0; i < RADIAL_POINTS; i += 1) {
      ...
      const compassCut = i % 2 === 0 ? 1 : 0.93;
      const point = makePoint(Math.cos(angle) * radius * compassCut, y, Math.sin(angle) * radius * compassCut);
      ring.push(point);
      points.push(point);
    }
...
function initPrograms(gl) {
  ...
  const lineVertex = `
    ...
    void main() {
      ...
      vDepth = clamp((mv.z + 6.0) / 8.0, 0.0, 1.0);
...
function resizeCanvas(canvas, gl) {
  ...
  const height = Math.max(560, Math.floor((box.height || 720) * dpr));
...
function makeMatrices(width, height) {
  ...
  perspective(projection, CAMERA.fovRadians, width / height, 0.1, 100);

  identity(model);
  rotateY(model, model, state.yaw);
  rotateX(model, model, state.pitch);
  rotateZ(model, model, Math.sin(state.sparkle * 0.16) * 0.025);
  scaleMat4(model, model, CAMERA.scale);

  identity(view);
  translate(view, view, [0, CAMERA.y, CAMERA.z]);
...
function step(time) {
  ...
  state.pitch = clamp(state.pitch, CAMERA.pitchMin, CAMERA.pitchMax);
...
function bindPointer(stage) {
  stage.addEventListener("pointerdown", (event) => {
    ...
    if (timestamp - state.lastTap < 320) {
      resetDiamond();
    }
  ...
  stage.addEventListener("pointermove", (event) => {
    ...
    state.yaw += dx * 0.0082;
    state.pitch = clamp(state.pitch + dy * 0.0058, CAMERA.pitchMin, CAMERA.pitchMax);
  ...
  window.DGBShowroomDiamond = Object.freeze({
    ...
    status() {
      return Object.freeze({
        ...
        correctedPhysicalRead: "restored-raised-webgl-true-3d-not-flat-paper",
        flatBadgeBlocked: true,
        tableClear: true,
        visibleCrown: true,
        facetedGirdle: true,
