...
export const RULES=freeze({
  trueDormerRoofSeating:true,slopedLowerCheeks:true,explicitRoofCurb:true,dormerFaceGlazingOnly:true,
...
function slopeStrip(id,x0,x1,zRear,zFront,yRear,yFront,height,material,role){
  const front=[V(x0,yFront,zFront),V(x1,yFront,zFront),V(x1,yFront+height,zFront),V(x0,yFront+height,zFront)];
  const back=[V(x0,yRear,zRear),V(x1,yRear,zRear),V(x1,yRear+height,zRear),V(x0,yRear+height,zRear)];
  return quadPrism(id,front,back,material,role);
}
...
function correctedDormerMeshes(d){
  ...
  // Body side walls and rear/front walls have a lower boundary that follows the host roof at each z edge.
  out.push(slopeStrip(`${d.id}-BODY`,xL,xR,d.zRear,d.zFront,d.rearBase,d.frontBase,d.topY-Math.max(d.rearBase,d.frontBase),'DORMER_STONE','slope-seated-dormer-body'));
  // Explicit roof curb follows both side edges and closes at front/rear; this is the owned roof-cut junction.
