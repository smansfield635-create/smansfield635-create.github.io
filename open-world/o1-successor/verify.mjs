...
const checks={
  ...
  independentRenderer:/getContext\('webgl'/.test(files.app)&&/drawElements\(gl\.TRIANGLES/.test(files.app),
  independentCamera:/lookAt\(/.test(files.app)&&/perspective\(/.test(files.app)&&/state=\{lat:/.test(files.app),
  continuousTraversal:/travel\(dx,dy\)/.test(files.app)&&/pointermove/.test(files.app),
  scaleTraversal:/scaleName\(alt\)/.test(files.app)&&/planetary/.test(files.world)&&/continental/.test(files.world)&&/regional/.test(files.world)&&/local/.test(files.world),
  ocean:/C\.a<\.45/.test(files.app)&&/\.025,.16/.test(files.app),
  atmosphereHorizon:/haze=pow\(rim/.test(files.app)&&/radial-gradient/.test(files.css),
  ...
  noConstructionBeyondFive:['index.html','styles.css','world.mjs','app.mjs'].every(n=>fs.existsSync(path.join(root,n)))
};
const pass=Object.values(checks).every(Boolean);console.log(JSON.stringify({schema:'OPEN_WORLD_O1_SUCCESSOR_SPECIMEN_VERIFICATION_RECEIPT_v1',result:pass?'PASS_INSPECTABLE_ENVIRONMENTAL_SUCCESSOR':'FAIL_CLOSED',checks,productionMergeAuthorized:false,correspondenceC1Authorized:false,userInspectionRequired:true},null,2));if(!pass)process.exitCode=1;
