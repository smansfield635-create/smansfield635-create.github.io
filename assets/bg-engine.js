/* TNT — /assets/bg-engine.js
   CORE L1 — BG ENGINE (CATEGORY 4/4)
   PURPOSE: cinematic dusk field + moon + soft clouds + lantern drift + subtle dial rings
   BUILD: CORE_BG_L1
   RULES:
   - mounts into #gd-field if present (else body)
   - single canvas id=gd_bg_canvas
   - singleton guard
   - no gd_* keys
   - NO DRAGON LOGIC
*/
...
function mount(opts){
  ...
  var clouds=[];
  (function(){
    var n=(opts.cloud_count!=null)?opts.cloud_count:20;
    for(var i=0;i<n;i++){
      var layer=(i%3);
      clouds.push({
        layer:layer,
        x:crnd()*W,
        y:lerp(H*0.42,H*0.95,crnd()),
  ...
  var lanterns=[];
  (function(){
    var n=(opts.lantern_count!=null)?opts.lantern_count:22;
    for(var i=0;i<n;i++){
      lanterns.push({
        x:Math.random()*W,
  ...
  function drawSky(){
    ...
    g.addColorStop(0.22,"rgba(120,20,20,0.12)");
    g.addColorStop(0.58,"rgba(180,55,30,0.16)");
    g.addColorStop(1.00,"rgba(0,0,0,0.12)");
    ctx.fillStyle=g;
  ...
  function drawCloudShape(x,y,w,h,a){
    ctx.save();
    ctx.globalAlpha=a;
    ctx.globalCompositeOperation="screen";
  ...
  function drawLanterns(dtMs){
    ...
    for(var i=0;i<lanterns.length;i++){
      ...
      g.addColorStop(0,"rgba(255,210,140,0.50)");
      g.addColorStop(0.35,"rgba(255,140,90,0.20)");
      g.addColorStop(1,"rgba(0,0,0,0)");
      ctx.fillStyle=g;
...
window.GD_BG = { version:"CORE_BG_L1", mount: mount };
