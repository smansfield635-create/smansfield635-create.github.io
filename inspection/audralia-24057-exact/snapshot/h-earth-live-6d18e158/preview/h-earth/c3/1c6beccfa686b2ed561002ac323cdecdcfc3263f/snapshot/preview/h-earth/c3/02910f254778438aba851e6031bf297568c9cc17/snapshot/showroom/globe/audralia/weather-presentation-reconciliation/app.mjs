...
function renderDiagnostics(runtime,renderer,exterior){
  if(summaryNode)summaryNode.textContent=`${runtime?.invariants?.pass?'ARCHITECTURE PASS':'ARCHITECTURE FAIL'} · ${renderer.getViewScale()} · ${runtime?.spatial?.activeLocalCount??0}/${runtime?.spatial?.maxLocalCount??0} local · exterior cutoff ${exterior.overlay.dataset.nearCutoff||'—'}`;
...
function wire(renderer,sky,exterior,canonical){
  ...
  const renderAll=()=>{const camera=cameraFrame(renderer);sky.render(camera);exterior.render(camera);const runtime=canonical.render(camera);renderDiagnostics(runtime,renderer,exterior);window.__AUDRALIA_WEATHER_PRESENTATION_RECONCILIATION_RUNTIME__=runtime;return runtime;};
  canvas.addEventListener('pointerdown',event=>{canvas.setPointerCapture(event.pointerId);pointers.set(event.pointerId,{x:event.clientX,y:event.clientY});exterior.beginInteraction();canonical.beginInteraction();if(pointers.size===2)beginTwo();else if(pointers.size>2)gesture=null;});
  ...
  const clear=event=>{pointers.delete(event.pointerId);refresh();if(pointers.size===0){exterior.endInteraction();canonical.endInteraction();renderAll();}else{exterior.beginInteraction();canonical.beginInteraction();}};
  ...
  canvas.addEventListener('wheel',event=>{event.preventDefault();exterior.beginInteraction();canonical.beginInteraction();renderer.zoom(event.deltaY);renderAll();clearTimeout(wheelTimer);wheelTimer=setTimeout(()=>{exterior.endInteraction();canonical.endInteraction();renderAll();},140);},{passive:false});
  canvas.addEventListener('dblclick',()=>{renderer.focusGratitude();exterior.endInteraction();canonical.endInteraction();renderAll();});focusButton?.addEventListener('click',()=>{renderer.focusGratitude();exterior.endInteraction();canonical.endInteraction();renderAll();});
  ...
  window.addEventListener('resize',()=>{renderer.render();exterior.endInteraction();canonical.endInteraction();renderAll();});
...
async function initialize(){
  try{
    ...
    setStatus('building…','AUDRALIA_WEATHER_RECONCILIATION_BUILDING');if(loaderStage)loaderStage.textContent='Reconciling sky and weather…';
    ...
    const canonical=createCanonicalWeatherProjectionLayer({renderer,worldCanvas:canvas,objects,onDiagnostics:runtime=>renderDiagnostics(runtime,renderer,exterior)});
    const controls=wire(renderer,sky,exterior,canonical);
    ...
    const receipt=Object.freeze({
      ...
      renderer,canonicalObjects:objects,sky,exterior,canonicalWeather:canonical,
      classification:EXTERIOR_COMPONENT_CLASSIFICATION,invariants:RECONCILIATION_INVARIANTS,
      getCameraFrame:()=>cameraFrame(renderer),getRuntime:()=>window.__AUDRALIA_WEATHER_PRESENTATION_RECONCILIATION_RUNTIME__,
      setCameraStateForTest:partial=>{Object.assign(renderer.state,partial||{});renderer.render();return controls.renderAll();}
    });
    window.__H_EARTH_AUDRALIA_OPEN_WORLD_OW01_PREVIEW__=Object.freeze({operationId:RECONCILIATION_SCHEMA,renderer,atmosphereEvidence:sky.getEvidence(),cloudEvidence:Object.freeze({exterior:exterior.getEvidence(),canonical:canonical.getEvidence?.()||null})});
