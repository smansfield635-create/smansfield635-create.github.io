/** H_EARTH_RUN_8E_WORLD_MANIFOLD_FRAME_AND_RENDER_INTEGRATION_v3 */
...
export const H_EARTH_RUN_8E_RENDER_INTEGRATION_CONTRACT_ID='H_EARTH_RUN_8E_WORLD_MANIFOLD_FRAME_AND_RENDER_INTEGRATION_v3';
...
export function constructHEarthRun8ESuccessorEnvironmentFrame({camera,viewport={width:320,height:180,pixelRatio:1},timeOfDayHours=15.25,frameOccurrenceId='H_EARTH_RUN_8E_SUCCESSOR_FRAME_OCCURRENCE_001',transferOccurrenceId='H_EARTH_RUN_8E_PACKET_002_TRANSFER_OCCURRENCE_001'}={}){
  ...
  const worldFarPlane=finite(camera?.farPlane)?camera.farPlane:RUN8C_PRESENTATION_DISTANCE_MAX;
  const run8CPresentationFarPlane=Math.min(worldFarPlane,RUN8C_PRESENTATION_DISTANCE_MAX);
  const presentation=issues.length===0?buildHEarthRun8CTerrainMaterialLightingPresentation({timeOfDayHours,cameraWorld:camera.position,viewportWidth:viewport.width,viewportHeight:viewport.height,cameraFarPlane:run8CPresentationFarPlane}):null;
  ...
  if(issues.length)return freeze({ok:false,status:'RUN_8E_WORLD_MANIFOLD_FRAME_REJECTED',contractId:H_EARTH_RUN_8E_RENDER_INTEGRATION_CONTRACT_ID,worldFarPlane,run8CPresentationFarPlane,issues});
