const POLICY_ID='AUDRALIA_FAP1_ORBITAL_SUPPORT_TUNING_v2_VISIBLE_EARTHLIKE';
...
const CLIMATE_BAND_REPLACEMENT='float climateBand(float lat,float center,float halfWidth){return 1.0-smoothstep(halfWidth*.78,halfWidth*1.72,abs(lat-center));}';
const BROKEN_TARGET='float broken=smoothstep(.50,.70,broad*.72+detail*.28+.075*lonWave);';
const BROKEN_REPLACEMENT='float broken=smoothstep(.37,.63,broad*.72+detail*.28+.075*lonWave);';
const SUPPORT_TARGET='return clamp(climate*broken*clearSlot*.38,0.0,.34);';
const SUPPORT_REPLACEMENT='float regionalVariance=.76+.24*(.5+.5*sin(lon*2.4+lat*3.1+t*.21));return clamp(climate*broken*clearSlot*regionalVariance*.64,0.0,.52);';
