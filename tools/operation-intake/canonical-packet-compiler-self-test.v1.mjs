...
function fixture() {
  const allowedPaths = ['tools/example/fixture.txt'];
  const artifactPaths = ['issue:test', 'review:test'];
  ...
  const request = {
    ...
    workflowPath: 'SELF_TEST_NO_EXECUTION',
    artifactPaths,
    fingerprintDomain,
    errorPrecedence,
  ...
  const procedure = {
    ...
    workflowAndArtifactPackagingPaths: { workflowPath: request.workflowPath, artifactPaths },
...
const { request, procedure } = fixture();
...
const badProcedure = clone(procedure);
delete badProcedure.workflowAndArtifactPackagingPaths.workflowPath;
...
const negatives = [
  ...
  expectFailure('35113108135_INPUT_INCOMPLETE_WORKFLOW_PATH', () => compileCanonicalPacket(request, badProcedure), 'CONSTRUCTION_PROCEDURE_INVALID'),
