...
const importProduct=rel=>import(`${pathToFileURL(path.join(ROOT,rel)).href}?gen2002=1`);

const EXPECTED={
 schema:'POST_GEN2001_CANOPY_ONLY_CONTINUITY_REPAIR_CONTRACT_v1',
 ...
 admissionRun:34238070797,
 routerRun:34237712409,
 psalm:5586633039,
 evidence:5586588434,
 closure:5586618955,
...
check('PREDECESSOR_BOUND',CONTRACT.gen2001PartialSuccess?.evidenceCommentId===EXPECTED.evidence&&CONTRACT.gen2001PartialSuccess?.terminalClosureCommentId===EXPECTED.closure&&CONTRACT.gen2001PartialSuccess?.lockReleased===true&&CONTRACT.gen2001PartialSuccess?.hydrology?.risk===false);
...
const forbidden=['WIDE_COAST','OPEN_TERRAIN','WOODLAND_EDGE','FOREST_INTERIOR_INDEPENDENT','MIRROR_MANOR_APPROACH','SHORELINE_WETLAND_TRANSITION','WIDE_RETURN','compactMobile','V2_RESULT_LOCATION','riskMaxEmptyCorridorCellsAbove','riskLargestComponentRatioBelow','riskOccupiedCellRatioBelow','5579294638','GEN2000_POST_GEN1998_CANOPY_HYDROLOGY_RELATION_DIAGNOSTIC'];
...
check('GENERIC_COVERAGE_TOPOLOGY_REFINEMENT_PRESENT',populationSource.includes('refineCoverageTopology')&&populationSource.includes('coverageGapScore')&&populationSource.includes('redundancyScore'));
...
const receipt={schema:'POST_GEN2001_CANOPY_ONLY_CONTINUITY_REPAIR_VERIFICATION_RECEIPT_v1',operationId:CONTRACT.operationId,lockGeneration:CONTRACT.lockGeneration,governingHead:CONTRACT.governingHead,partialSuccessFoundation:CONTRACT.partialSuccessFoundation,candidateHead,evaluationToolingHead:CONTRACT.evaluationTooling.head,result:failed.length?'FAIL_CLOSED':'PASS_CLOSED',productMechanicalState:failed.length?'NON_CLEAR':'ALL_FOUR_V2_RISK_FAMILIES_CLEAR',materialDispositionAuthority:false,materialDisposition:'UNASSIGNED_PENDING_FRESH_VISUAL_EVIDENCE_AND_READ_ONLY_ADJUDICATION',environmentFloorFreezeAuthority:false,mergeAuthority:false,deploymentAuthority:false,publicationAuthority:false,evidence:{changedPaths:changed,population:{canopyCount:population.instanceCount,understoryCount:understory.instanceCount,understoryBlob:blobSha(read('characters/vegetation-understory.mjs'))},v2:{metricDigest:v2.metricDigest,familyRisk,failedCheckIds:failedIds,hydrologyResponse:hydro,canopy:states.map(s=>({state:s.state,...s.canopyFieldContinuity}))}},checkCount:checks.length,passCount:checks.length-failed.length,failCount:failed.length,checks};
