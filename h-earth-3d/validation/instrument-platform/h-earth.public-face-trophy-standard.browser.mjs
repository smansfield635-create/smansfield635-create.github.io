...
try {
  ...
  check('SIX_AWARD_LENSES', await awards.locator('.h-earth-award-lens').count() === 6);
  check('ALL_AWARD_LENSES_CLOSED_ON_LOAD', await awards.locator('.h-earth-award-lens[open]').count() === 0);
  check('FIVE_TARGET_PROGRAMS', await awards.locator('[data-award-program]').count() === 5);
  check('AWARDS_INTERACTIVE_NOW_VISIBLE', await awards.getByText('Interactive now', { exact: true }).first().isVisible());
  check('AWARDS_IN_DEVELOPMENT_VISIBLE', await awards.getByText('In active development', { exact: true }).first().isVisible());
