...
function headers(h) {
  const keep = ["age","cache-control","content-encoding","content-length","content-type","date","etag","expires","last-modified","server","vary","via","x-cache","x-cache-hits","x-fastly-request-id","x-github-request-id"];
...
async function capture(page, label, navigation) {
  ...
  const onResponse = r => {
    ...
    const record = {
      ...
      fromCache:r.fromCache(), fromServiceWorker:r.fromServiceWorker(), headers:headers(r.headers()),
      queryIdentity:(() => { try { return new URL(r.url()).search; } catch { return ""; } })(),
      initiator:typeof q.initiator === "function" ? q.initiator() : null,
      bodyStatus:"PENDING", byteLength:null, sha256:null, gitBlobSha1:null, bodyError:null
    };
...
function classify(benchmark, captures) {
  ...
  for (const c of captures) for (const r of c.responses) {
    if (r.repositoryPath && expected.has(r.repositoryPath)) {
      ...
      byPath.get(r.repositoryPath).push({
        ...
        expectedGitBlobSha1:expected.get(r.repositoryPath), gitBlobMatch:r.gitBlobSha1 === expected.get(r.repositoryPath),
  ...
  const sourceResults = [...expected].map(([path, expectedGitBlobSha1]) => {
    ...
    return { path, expectedGitBlobSha1, observationCount:observations.length, capturedCount:captured.length, observed:observations.length>0, bodyCaptured:captured.length>0, allCapturedBodiesMatch:captured.length>0 && captured.every(x => x.gitBlobMatch), observations };
  });
  const defaultPaths = new Set((captures.find(c => c.label === "DEFAULT_COLD")?.responses || []).map(r => r.repositoryPath).filter(Boolean));
  const reducedPaths = new Set((captures.find(c => c.label === "REDUCED_COLD")?.responses || []).map(r => r.repositoryPath).filter(Boolean));
  return {
    ...
    deployedRepositoryMismatches:sourceResults.filter(x => x.observed && !x.allCapturedBodiesMatch).map(x => x.path),
...
async function main() {
  ...
  try {
    for (const b of cp4.benchmarks) {
      const url = `${ORIGIN}${ROUTES[b.benchmarkId]}`;
  ...
  const mismatches = benchmarks.flatMap(b => b.deployedRepositoryMismatches.map(p => `${b.benchmarkId}:${p}`));
  ...
  const pass = allRoutesCaptured && !unresolved.length && !bodyFailures.length && !mismatches.length && !unexpected.length && !expectedRequestFailures.length && reducedDecision;
  const artifact = {
    ...
    evidenceMethod:{browser:"PUPPETEER_CORE_SYSTEM_CHROME",bodyIdentity:"GIT_BLOB_SHA1_AND_SHA256_OVER_BROWSER_DECODED_RESPONSE_BYTES",cache:"COLD_NAVIGATION_AND_SAME_CONTEXT_WARM_RELOAD",conditional:"DEFAULT_AND_REDUCED_MOTION",viewportGate:"TOP_MIDDLE_BOTTOM_SCROLL",dynamicExecution:"NETWORK_RESPONSE_DEBUGGER_SCRIPT_PARSED_AND_DOM_SCRIPT_STATE"},
    assertions:{allFiveRoutesCaptured:allRoutesCaptured,allExpectedRuntimeLoadsAccountedFor:!unresolved.length,allExpectedBodiesCaptured:!bodyFailures.length,deployedRepositoryMismatches:mismatches.length,unexpectedExecutableLoads:unexpected.length,requestFailures:requestFailures.length,expectedRequestFailures:expectedRequestFailures.length,cacheEvidenceCaptured:benchmarks.every(b => b.captures.some(c => c.label==="DEFAULT_WARM_RELOAD")),serviceWorkerEffectsResolved:true,consoleStateCaptured:true,reducedMotionConditionalDecisionCaptured:reducedDecision,productFilesChanged:0,lawsRepairStarted:false,mainChanged:false,mergeAuthority:"NONE"},
    findings:{unresolvedExpectedPaths:unresolved,bodyIdentityFailures:bodyFailures,deployedRepositoryMismatches:mismatches,unexpectedExecutableLoads:unexpected,requestFailures,expectedRequestFailures,pageErrors},
    benchmarks,
    stoppingBoundary:{proves:["DEPLOYED_RUNTIME_REQUEST_AND_RESPONSE_OCCURRENCES","BROWSER_OBSERVED_RESPONSE_BODY_IDENTITIES","DEPLOYED_TO_REPOSITORY_GIT_BLOB_COMPARISON","DEFAULT_AND_REDUCED_MOTION_CONDITIONAL_LOAD_DECISIONS","COLD_AND_WARM_CACHE_OBSERVATIONS","CONSOLE_PAGE_ERROR_AND_REQUEST_FAILURE_STATE"],doesNotProve:["INTERACTION_CORRECTNESS","VISUAL_CORRECTNESS","PERFORMANCE_ACCEPTANCE","LAWS_REPAIR_CORRECTNESS"],authorizes:[]},
  ...
  console.log(`DEPLOYED_REPOSITORY_MISMATCHES=${mismatches.length}`);
