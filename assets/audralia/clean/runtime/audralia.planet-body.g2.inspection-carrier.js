...
// AUDRALIA_PLANET_BODY_G2_INSPECTION_CARRIER_NEW_FILE_TNT_v1
//
// Previous generation reference:
// /assets/audralia/clean/runtime/audralia.planet-body.inspection-carrier.js
//
// CCR basis:
// AUDRALIA_PLANET_BODY_G2_INSPECTION_CARRIER_NEW_FILE_CCR_v1
//
// Purpose:
// - Create a new G2 carrier file instead of continuing to rewrite the G1 carrier.
// - Keep the existing parent terrain baseline as the source of truth.
// - Treat older downstream visual children as obsolete for this G2 visible-form pass.
// - Convert the dry terrain parent packet into a strong, continuous, centered, readable planet.
// - Preserve carrier-only visible rendering, drag/zoom inspection, and carrier-compatible lens controls.
// - Keep hydration held, active water false, and final visual pass false.
// - Do not mutate terrain truth, hydration truth, route shell, HTML, or parent files.

(function () {
  "use strict";

  var CONTRACT = "AUDRALIA_PLANET_BODY_G2_INSPECTION_CARRIER_NEW_FILE_TNT_v1";
  var CCR = "AUDRALIA_PLANET_BODY_G2_INSPECTION_CARRIER_NEW_FILE_CCR_v1";
  var SPEC_OPS = "AUDRALIA_PLANET_BODY_G2_INSPECTION_CARRIER_NEW_FILE_SPEC_OPS_v1";
  var NEWS = "AUDRALIA_PLANET_BODY_G2_INSPECTION_CARRIER_NEW_FILE_NEWS_v1";

  var FILE = "/assets/audralia/clean/runtime/audralia.planet-body.g2.inspection-carrier.js";
  var LEGACY_FILE = "/assets/audralia/clean/runtime/audralia.planet-body.inspection-carrier.js";
  ...
  var CLASS_COLORS = Object.freeze({
    summit_pressure_zone: Object.freeze({ r: 223, g: 196, b: 125 }),
    mountain_belt: Object.freeze({ r: 184, g: 159, b: 108 }),
    ridge_chain: Object.freeze({ r: 164, g: 144, b: 92 }),
    trench_corridor: Object.freeze({ r: 49, g: 42, b: 38 }),
    dry_basin_floor: Object.freeze({ r: 76, g: 91, b: 67 }),
    valley_corridor: Object.freeze({ r: 91, g: 113, b: 73 }),
    shelf_terrace: Object.freeze({ r: 121, g: 127, b: 84 }),
    escarpment_rim: Object.freeze({ r: 130, g: 99, b: 69 }),
    planetary_highland: Object.freeze({ r: 146, g: 137, b: 94 }),
    upland_plateau: Object.freeze({ r: 113, g: 130, b: 78 }),
    former_seabed: Object.freeze({ r: 45, g: 65, b: 65 }),
    lowland_gap: Object.freeze({ r: 57, g: 75, b: 62 }),
    stable_craton: Object.freeze({ r: 96, g: 122, b: 77 })
  });
  ...
  if (
    window.__AUDRALIA_PLANET_BODY_G2_INSPECTION_CARRIER_CONTROLLER__ &&
    typeof window.__AUDRALIA_PLANET_BODY_G2_INSPECTION_CARRIER_CONTROLLER__.stop === "function"
  ) {
    try {
      window.__AUDRALIA_PLANET_BODY_G2_INSPECTION_CARRIER_CONTROLLER__.stop();
    } catch (_error) {}
  }
  ...
  function fbm(x, y, seed, octaves) {
    ...
    for (var i = 0; i < (octaves || 4); i += 1) {
      total += noise2(x * frequency, y * frequency, seed + i * 31.77) * amplitude;
  ...
  function inverseRotateFromScreen(nx, ny, z) {
    var xRot = nx;
    var yRot = -ny;
    var zRot = z;
    ...
    var y0 = yRot * cp + zRot * sp;
    var z1 = -yRot * sp + zRot * cp;

    var cy = Math.cos(state.yaw);
    var sy = Math.sin(state.yaw);

    var x0 = xRot * cy - z1 * sy;
    var z0 = xRot * sy + z1 * cy;
  ...
  function detectDryTerrain() {
    ...
    if (!state.dryTerrainDetected) {
      state.dryFailureReason = "dry revealed physical terrain parent missing";
      return false;
    }

    if (!state.dryTerrainApiComplete) {
      state.dryFailureReason = "dry revealed physical terrain parent API incomplete";
      return false;
    }

    state.dryTerrainStatus = safeCall("dryTerrain", api, "status");
    state.dryCarrierPacket = safeCall("dryTerrain", api, "getCarrierTerrainPacket", "audralia-g2-inspection-carrier", { compact: false });
    state.dryTerrainPacket = safeCall("dryTerrain", api, "getDryRevealedTerrainPacket", "audralia-g2-inspection-carrier", { compact: false });
    ...
    state.dryFailureReason = state.dryTerrainValidated ? "" : "dry terrain validation failed or 256-node parent packet unavailable";
  ...
  function sampleTerrainAtLonLat(lon, lat) {
    ...
    var safeLat = clamp(lat, -80, 80);
    var gx = ((lon + 180) / 360) * RADIAL_NODES - 0.5;
    var gy = ((80 - safeLat) / 160) * (FIBONACCI_BANDS - 1) - 0.5;
  ...
  function colorForTerrain(sample, lens, grain, fracture, contour) {
    ...
    color = mixColor(color, { r: 229, g: 202, b: 130 }, ridge * 0.34);
    color = mixColor(color, { r: 49, g: 62, b: 52 }, basin * 0.24);
    color = mixColor(color, { r: 34, g: 29, b: 28 }, trench * 0.38);
    color = mixColor(color, { r: 134, g: 128, b: 84 }, shelf * 0.20);
    color = mixColor(color, { r: 79, g: 61, b: 43 }, escarpment * 0.24);
    color = mixColor(color, { r: 36, g: 56, b: 60 }, gap * 0.34);
    color = mixColor(color, { r: 58, g: 77, b: 76 }, carving * 0.13);

    var heightShade = (elevation - 0.50) * 48;
    var reliefShade = (ridge - basin - trench * 0.48) * 32;
    var grainShade = (grain - 0.5) * 22 + (fracture - 0.5) * 16;
    var contourShade = contour * 22;

    color = shadeColor(color, heightShade + reliefShade + grainShade + contourShade);

    if (lens === "body") {
      color = mixColor(color, { r: 35, g: 45, b: 42 }, 0.16);
      color = shadeColor(color, 2);
    } else if (lens === "surface") {
      color = shadeColor(color, 16);
      color = mixColor(color, { r: 235, g: 214, b: 145 }, ridge * 0.13);
    } else if (lens === "hydration") {
      color = mixColor(color, { r: 45, g: 84, b: 98 }, clamp(gap * 0.43 + carving * 0.20, 0, 0.50));
      color = mixColor(color, { r: 21, g: 30, b: 32 }, 0.12);
    } else if (lens === "sixth-sense") {
      color = mixColor(color, { r: 141, g: 216, b: 255 }, clamp((ridge + gap + escarpment) * 0.18, 0, 0.34));
      color = shadeColor(color, 8);
    } else if (lens === "lattice") {
      color = mixColor(color, { r: 15, g: 25, b: 42 }, 0.42);
    } else if (lens === "receipt") {
      color = mixColor(color, { r: 20, g: 29, b: 38 }, 0.34);
    }
  ...
  function buildTerrainTexture(lens) {
    ...
    for (var y = 0; y < TEXTURE_HEIGHT; y += 1) {
      ...
      for (var x = 0; x < TEXTURE_WIDTH; x += 1) {
        ...
        if (!sample) {
          color = { r: 20, g: 34, b: 40 };
        } else {
          var grain = fbm(u * 22.0 + 0.13, v * 13.0 - 0.07, 9101, 4);
          var fracture = fbm(u * 52.0 - 0.31, v * 27.0 + 0.19, 9107, 3);
          var contourRaw = Math.abs(fract(sample.dryElevation * 9.0 + fracture * 0.12) - 0.5);
          var contour = smoothstep(0.455, 0.500, contourRaw) * 0.16;

          color = colorForTerrain(sample, mode, grain, fracture, contour);
          ...
          contrastTotal += Math.abs(sample.dryElevation - 0.5) + relief * 0.38;
        }
    ...
    state.dryTerrainFormStrength = round(
      clamp(
        ...
        state.textureContrastAverage * 0.44,
        0,
        1
      ),
      4
    ...
    var texture = {
      ...
      lens: mode,
      contract: CONTRACT,
      source: "dry-terrain-parent-packet",
      activeHydration: false,
      activeWater: false,
      finalVisualPassClaim: false
    };
  ...
  function drawBackground() {
    ...
    var bg = ctx.createRadialGradient(m.cx, m.cy, m.r * 0.12, m.cx, m.cy, Math.max(state.width, state.height) * 0.74);
    bg.addColorStop(0.00, "rgba(20,39,54,0.92)");
    ...
    var glow = ctx.createRadialGradient(m.cx, m.cy, m.r * 0.36, m.cx, m.cy, m.r * 1.65);
    glow.addColorStop(0.00, "rgba(143,240,195,0.030)");
    glow.addColorStop(0.58, "rgba(141,216,255,0.078)");
    glow.addColorStop(0.86, "rgba(243,200,111,0.052)");
    glow.addColorStop(1.00, "rgba(141,216,255,0)");

    ctx.fillStyle = glow;
    ctx.beginPath();
    ctx.arc(m.cx, m.cy, m.r * 1.65, 0, TAU);
    ctx.fill();
  }
  ...
  function drawSphere() {
    ...
    var m = metrics();
    var diameter = Math.max(240, Math.min(MAX_SPHERE_PIXELS, Math.floor(m.r * 2)));
    ...
    for (var y = 0; y < diameter; y += 1) {
      ...
      for (var x = 0; x < diameter; x += 1) {
        ...
        var original = inverseRotateFromScreen(nx, ny, z);

        var lat = Math.asin(clamp(original.y, -1, 1));
        var lon = Math.atan2(original.z, original.x);
        ...
        if (mode === "hydration") {
          ...
          var trace = smoothstep(0.70, 0.92, diagnostic) * 0.09;
          r = mix(r, 94, trace);
          g = mix(g, 169, trace);
          b = mix(b, 188, trace);
        }
        ...
        if (mode === "sixth-sense") {
          ...
          var pulse = smoothstep(0.72, 0.95, sense) * 0.12;
          r = mix(r, 141, pulse);
          g = mix(g, 216, pulse);
          b = mix(b, 255, pulse);
        }
  ...
  function drawAtmosphereRim() {
    ...
    halo.addColorStop(0.82, "rgba(141,216,255,0.125)");
    ...
    ctx.beginPath();
    ctx.arc(m.cx, m.cy, m.r * 1.002, 0, TAU);
    ctx.strokeStyle = "rgba(190,232,255,0.23)";
  ...
  function drawLatticeLayer(layer) {
    ...
    for (var j = 0; j < state.latticeSeats.length; j += 1) {
      var seatItem = state.latticeSeats[j];
      var p = project(seatItem);
      var front = p.z >= 0;
      ...
      ctx.beginPath();
      ctx.arc(p.x, p.y, Math.max(0.62, state.dpr * (seatItem.major ? 1.50 : seatItem.secondary ? 1.05 : 0.78) * p.scale), 0, TAU);
      ctx.fillStyle = layer === "front"
        ? (seatItem.major ? "rgba(244,207,131,0.72)" : "rgba(141,216,255,0.42)")
        : (seatItem.major ? "rgba(141,216,255,0.080)" : "rgba(141,216,255,0.040)");
      ctx.fill();
    }
  ...
  function drawReceipt() {
    ...
    ctx.fillText("G2 INSPECTION CARRIER · NEW FILE", m.cx, y + h * 0.075);

    ctx.font = "900 " + Math.max(7, 8.2 * state.dpr) + "px ui-monospace, monospace";

    var rows = [
      "CONTRACT " + CONTRACT,
      "DRY PARENT VALIDATED " + String(state.dryTerrainValidated).toUpperCase() + " · NODES " + state.dryTerrainNodes.length,
      ...
      "G2 CONSUMES DRY PARENT ONLY · LEGACY CHILDREN OBSOLETE FOR FORM",
      "BODY/SURFACE GRID SUPPRESSED · RAW 256 ONLY IN LATTICE",
      "CARRIER INVENTS TERRAIN FALSE · SOURCE TERRAIN MUTATED FALSE",
      "HYDRATION HELD TRUE · ACTIVE WATER FALSE",
      "HTML CUTOVER NOT CLAIMED BY THIS FILE",
  ...
  function drawPendingParentNotice() {
    ...
    ctx.fillStyle = "rgba(10,22,34,0.94)";
    ctx.fill();

    ctx.beginPath();
    ctx.arc(m.cx, m.cy, m.r * 1.002, 0, TAU);
    ctx.strokeStyle = "rgba(141,216,255,0.22)";
    ctx.lineWidth = Math.max(1, state.dpr);
    ctx.stroke();

    ctx.font = "900 " + Math.max(11, 13 * state.dpr) + "px ui-monospace, monospace";
    ctx.fillStyle = "rgba(244,207,131,.88)";
    ctx.fillText("G2 CARRIER WAITING FOR DRY TERRAIN PARENT", m.cx, m.cy - m.r * 0.05);

    ctx.font = "800 " + Math.max(8, 9 * state.dpr) + "px ui-monospace, monospace";
    ctx.fillStyle = "rgba(238,244,255,.66)";
    ctx.fillText(state.dryFailureReason, m.cx, m.cy + m.r * 0.08);
  ...
  function setLens(value) {
    ...
    if (label) {
      if (lens === "body") {
        label.innerHTML = "<strong>Body</strong> → G2 continuous dry physical form; normal grid suppressed";
      } else if (lens === "surface") {
        label.innerHTML = "<strong>Surface</strong> → dry parent terrain converted into readable planetary surface";
      } else if (lens === "hydration") {
        label.innerHTML = "<strong>Hydration</strong> → held / future-fill diagnostic only; active water false";
      } else if (lens === "sixth-sense") {
        label.innerHTML = "<strong>Sixth Sense</strong> → diagnostic scaffold over G2 dry terrain form";
      } else if (lens === "lattice") {
        label.innerHTML = "<strong>Lattice</strong> → raw 256 inspection only; not a second planet";
      } else if (lens === "receipt") {
        label.innerHTML = "<strong>Receipt</strong> → G2 new-file carrier proof";
      }
    }
  ...
  function refreshDownstreamPackets() {
    ...
    state.textureCache = Object.create(null);
  ...
  function getDryTerrainFormReceipt() {
    return {
      ...
      news: NEWS,
      target: FILE,
      previousCarrierFile: LEGACY_FILE,

      g2NewFileCarrierActive: true,
      g1CarrierNotRewrittenByThisFile: true,
      g1ParentsBaselinePreserved: true,
      legacyChildrenObsoleteForG2VisibleForm: true,
      g2ConsumesOnlyDryParentForVisibleForm: true,
  ...
  function publishStatus(options) {
    ...
    var payload = {
      ...
      htmlCutoverClaimedByThisFile: false,
      htmlRewriteRequiredToLoadThisFile: true,
      ...
      dryTerrainFormReceipt: dryReceipt,
      ...
      deployMarker: "AUDRALIA_PLANET_BODY_G2_INSPECTION_CARRIER_NEW_FILE_DEPLOY_MARKER_v1"
    };

    state.lastStatusPublishedAt = time;
    state.lastStatusPayload = payload;

    window.AUDRALIA_PLANET_BODY_G2_INSPECTION_CARRIER_STATUS = payload;
    window.AUDRALIA_PLANET_BODY_G2_INSPECTION_CARRIER_NEW_FILE_STATUS = payload;
    ...
    try {
      ...
      document.documentElement.dataset.audraliaG2InspectionCarrierNewFileActive = "true";
      document.documentElement.dataset.audraliaG2ConsumesOnlyDryParentForVisibleForm = "true";
      document.documentElement.dataset.audraliaLegacyChildrenObsoleteForG2VisibleForm = "true";
      document.documentElement.dataset.audraliaG1ParentsBaselinePreserved = "true";
      document.documentElement.dataset.audraliaHtmlCutoverClaimedByThisFile = "false";
      document.documentElement.dataset.audraliaHtmlRewriteRequiredToLoadThisFile = "true";
  ...
  function init() {
    ...
    state.canvas.setAttribute("data-target-file", FILE);
    state.canvas.setAttribute("data-previous-carrier-file", LEGACY_FILE);
    state.canvas.setAttribute("data-g2-inspection-carrier-new-file-active", "true");
    state.canvas.setAttribute("data-g1-carrier-not-rewritten-by-this-file", "true");
    state.canvas.setAttribute("data-g1-parents-baseline-preserved", "true");
    state.canvas.setAttribute("data-legacy-children-obsolete-for-g2-visible-form", "true");
    state.canvas.setAttribute("data-g2-consumes-only-dry-parent-for-visible-form", "true");
    ...
    state.canvas.setAttribute("aria-label", "Audralia G2 inspection carrier planet renderer");
  ...
  window.__AUDRALIA_PLANET_BODY_G2_INSPECTION_CARRIER_CONTROLLER__ = controller;
  window.__AUDRALIA_G2_PLANET_BODY_CLEAN_PAIR_CONTROLLER__ = controller;
