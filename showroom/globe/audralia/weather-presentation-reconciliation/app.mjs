...
function wire(renderer,sky,exterior,canonical,fap1){
  const pointers=new Map();let gesture=null,wheelTimer=0,renderFrame=0;
  ...
  const renderAllNow=()=>{if(renderFrame){cancelAnimationFrame(renderFrame);renderFrame=0;}return renderAll();};
  const scheduleRenderAll=()=>{if(renderFrame)return;renderFrame=requestAnimationFrame(()=>{renderFrame=0;renderAll();});};
  ...
  return Object.freeze({renderAll:renderAllNow});
}
