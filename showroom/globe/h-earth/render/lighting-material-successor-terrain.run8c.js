 ...
 *
 * Creates an immutable renderer-consumable presentation record from the Run 8B
 * South-neutral terrain/mountain primitive, Run 8C successor material projection,
 * and the accepted Run 7C atmosphere state/presentation. It does not mutate the
 * renderer, geometry, camera, route, admission corridor, or deployment state.
 */
...
export const H_EARTH_RUN_8C_NORMAL_LIGHT_MATERIAL_PROFILE = freeze({
  ...
  controllingInterfaceContractId:
    H_EARTH_RUN_8A_NORMAL_LIGHT_AND_MATERIAL_INTERFACE_CONTRACT.contractId,
  sourceNeutralGeometryContractId:
    H_EARTH_RUN_8B_SUCCESSOR_NEUTRAL_GEOMETRY_CONTRACT_ID,
  sourceSurfaceMaterialContractId:
    H_EARTH_RUN_8C_SUCCESSOR_SURFACE_MATERIAL_CONTRACT_ID,
  sourceAtmosphereStateContractId: H_EARTH_ATMOSPHERE_STATE_CONTRACT_ID,
  sourceAtmospherePresentationContractId:
    H_EARTH_ATMOSPHERE_PRESENTATION_CONTRACT_ID,
  realizationModel:
    'WORLD_NORMAL_DIFFUSE_AMBIENT_SLOPE_CURVATURE_WETNESS_ROUGHNESS_REFLECTANCE_AND_DISTANCE_HAZE',
  owns: {
...
export function projectHEarthRun8CVertexMaterialLighting({
  world,
  normal,
  surfaceMaterial,
  atmosphereState,
  ...
  if (
    !world || !normal || !cameraWorld ||
    ![world.x, world.y, world.z, normal.x, normal.y, normal.z,
      cameraWorld.x, cameraWorld.y, cameraWorld.z].every(finite) ||
    ...
    return freeze({
      eligible: false,
      status: 'RUN_8C_VERTEX_LIGHT_MATERIAL_REJECTED',
      issues: ['INVALID_VERTEX_LIGHT_MATERIAL_INPUT']
    });
  }
  ...
  const ambientLightFactor = clamp(
    0.15 + atmosphereState.sunIntensity * 0.33 + Math.max(0, normal.y) * 0.12,
    0.14,
    0.62
  );
  const slopeShadeFactor = clamp(0.66 + Math.max(0, normal.y) * 0.34, 0.48, 1);
  const curvatureOcclusionFactor = clamp(
    1 - Math.max(0, -surfaceMaterial.curvature) * 0.72,
    0.72,
    1
  );
  ...
  const reflectanceResponse = clamp01(
    surfaceMaterial.reflectance *
    (1 - surfaceMaterial.roughness) *
    diffuseLightFactor *
    (0.08 + surfaceMaterial.wetness * 0.22)
  );

  const lightFactor = clamp(
    (ambientLightFactor + diffuseLightFactor * 0.86) *
    slopeShadeFactor *
  ...
  const litRgba = [
    linearToSrgb8(litLinear.r),
    linearToSrgb8(litLinear.g),
    linearToSrgb8(litLinear.b),
    255
  ];
  const cameraDistance = distance3(world, cameraWorld);
  const atmospheric = applyHEarthAtmosphericDistanceToColor({
    baseColor: litRgba,
  ...
  if (atmospheric.eligible !== true) {
    return freeze({
      eligible: false,
      status: 'RUN_8C_VERTEX_ATMOSPHERIC_APPLICATION_REJECTED',
      issues: atmospheric.issues
    });
  }
...
function summarizeVertexAttributes(attributes) {
  const classCounts = {};
  let minimumDiffuse = Infinity;
  let maximumDiffuse = -Infinity;
  let minimumLuminance = Infinity;
  let maximumLuminance = -Infinity;
  let minimumHaze = Infinity;
  let maximumHaze = -Infinity;
  let luminanceSum = 0;

  for (const attribute of attributes) {
    classCounts[attribute.surfaceClass] =
      (classCounts[attribute.surfaceClass] ?? 0) + 1;
...
export function buildHEarthRun8CTerrainMaterialLightingPresentation({
  ...
  const atmosphereState = sampleHEarthAtmosphereState({
    ...
    viewDistance: cameraFarPlane
  });
  const atmospherePresentation = buildHEarthAtmospherePresentation(
    atmosphereState,
    { viewportWidth, viewportHeight, cameraFarPlane }
  );
  const issues = [];

  if (geometryResult?.ok !== true || !geometryResult.primitive) {
    issues.push('RUN_8B_SUCCESSOR_NEUTRAL_GEOMETRY_INVALID');
  }
  if (evaluateHEarthAtmosphereStateSample(atmosphereState).eligible !== true) {
    issues.push('RUN_7C_ATMOSPHERE_STATE_INVALID');
  }
  if (evaluateHEarthAtmospherePresentation(atmospherePresentation).eligible !== true) {
    issues.push('RUN_7C_ATMOSPHERE_PRESENTATION_INVALID');
  }
  if (!cameraWorld || ![cameraWorld.x, cameraWorld.y, cameraWorld.z].every(finite)) {
    issues.push('CAMERA_OBSERVATION_INPUT_INVALID');
  }

  if (issues.length > 0) {
    return freeze({
      eligible: false,
      status: 'RUN_8C_TERRAIN_LIGHT_MATERIAL_PRESENTATION_REJECTED',
      contractId: H_EARTH_RUN_8C_NORMAL_LIGHT_MATERIAL_CONTRACT_ID,
      issues
    });
  }
  ...
  for (let index = 0; index < vertices.length; index += 1) {
    ...
    const projected = projectHEarthRun8CVertexMaterialLighting({
      world,
      normal,
      surfaceMaterial,
      atmosphereState,
    ...
    if (projected.eligible !== true) {
      issues.push(`VERTEX_LIGHT_MATERIAL_FAILED:${index}`);
      break;
    }
    vertexAttributes.push(freeze({ index, ...projected }));
  }

  if (vertexAttributes.length !== vertices.length) {
    issues.push('VERTEX_ATTRIBUTE_COUNT_MISMATCH');
  }
  ...
  return freeze({
    ...
    status: issues.length === 0
      ? 'RUN_8C_TERRAIN_LIGHT_MATERIAL_PRESENTATION_COMPLETE'
      : 'RUN_8C_TERRAIN_LIGHT_MATERIAL_PRESENTATION_FAILED',
    contractId: H_EARTH_RUN_8C_NORMAL_LIGHT_MATERIAL_CONTRACT_ID,
    controllingInterfaceContractId:
      H_EARTH_RUN_8A_NORMAL_LIGHT_AND_MATERIAL_INTERFACE_CONTRACT.contractId,
    sourceNeutralGeometryContractId:
...
export function evaluateHEarthRun8CTerrainMaterialLightingPresentation(record) {
  ...
  if (record?.contractId !== H_EARTH_RUN_8C_NORMAL_LIGHT_MATERIAL_CONTRACT_ID) {
    issues.push('RUN_8C_PRESENTATION_CONTRACT_ID_MISMATCH');
  }
  if (record?.sourceNeutralGeometryContractId !==
      H_EARTH_RUN_8B_SUCCESSOR_NEUTRAL_GEOMETRY_CONTRACT_ID) {
    issues.push('RUN_8B_NEUTRAL_GEOMETRY_SOURCE_MISMATCH');
  }
  if (!Array.isArray(record?.vertexAttributes) ||
      record.vertexAttributes.length !== record.vertexAttributeCount ||
      record.vertexAttributeCount <= 0) {
    issues.push('RUN_8C_VERTEX_ATTRIBUTES_INVALID');
  }
  if (record?.summary?.surfaceClassCount < 5) {
    issues.push('RUN_8C_MATERIAL_CLASS_VARIATION_INSUFFICIENT');
  }
  if (!finite(record?.summary?.diffuseRange) || record.summary.diffuseRange <= 0.05) {
    issues.push('RUN_8C_NORMAL_DRIVEN_LIGHT_VARIATION_INSUFFICIENT');
  }
  if (!finite(record?.summary?.luminanceRange) || record.summary.luminanceRange <= 5) {
    issues.push('RUN_8C_VISIBLE_MATERIAL_RESPONSE_VARIATION_INSUFFICIENT');
  }
  if (!finite(record?.summary?.hazeRange) || record.summary.hazeRange <= 0) {
    issues.push('RUN_8C_DISTANCE_HAZE_VARIATION_INSUFFICIENT');
  }
  if (record?.sourceNeutralPrimitiveMutated !== false ||
      record?.rendererMutation !== false ||
      record?.geometryMutation !== false ||
      record?.WestAdmissionExecuted !== false ||
      record?.packet002TransferExecuted !== false ||
      record?.publicRouteMutation !== false ||
      record?.deployment !== false ||
      record?.visualImprovementClaim !== false) {
    issues.push('RUN_8C_BOUNDARY_VIOLATION');
  }

  return freeze({
    eligible: issues.length === 0,
    status: issues.length === 0
      ? 'RUN_8C_TERRAIN_LIGHT_MATERIAL_PRESENTATION_PASS'
      : 'RUN_8C_TERRAIN_LIGHT_MATERIAL_PRESENTATION_FAIL',
    issues
  });
}
