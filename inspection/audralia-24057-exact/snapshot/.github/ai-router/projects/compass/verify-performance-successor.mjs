...
check('HOUSE_VISIBILITY_SUSPENSION',house.includes("houseMotion:'static-exhibit'")&&house.includes('setForeground=on=>')&&!house.includes('requestAnimationFrame'),{staticExhibit:house.includes("houseMotion:'static-exhibit'"),foregroundGate:house.includes('setForeground=on=>'),continuousRaf:house.includes('requestAnimationFrame')});
check('HOUSE_FOREGROUND_SUSPENSION',house.includes('foreground()')&&house.includes('setForeground'),true);
check('REDUCED_MOTION_BOUND',cap.includes("matchMedia('(prefers-reduced-motion: reduce)')")&&cap.includes('const settleDelay=reduce.matches?0:140')&&house.includes("houseMotion:'static-exhibit'"),{capabilityReducedMotion:true,houseStatic:true});
