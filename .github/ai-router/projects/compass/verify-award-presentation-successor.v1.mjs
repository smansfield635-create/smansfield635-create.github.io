...
const allowed=new Set([
  ...
  '.github/ai-router/projects/compass/verify-award-presentation-successor.v1.mjs',
...
check('PURPOSE_FIRST_READ_ESTABLISHED',statement.includes("purpose.classList.add('compass-purpose-stage')")&&statement.includes('purpose.open=true')&&statement.includes('compass-purpose-first-read')&&statement.includes('Explore the fuller context'),'purpose is expanded, simplified for first read, and preserves deeper context');
const narrativeToken=statement.includes("root.dataset.awardNarrative='identity-experience-purpose-system-readiness-evidence'")||statement.includes('IDENTITY_EXPERIENCE_PURPOSE_SYSTEM_READINESS_EVIDENCE');
const purposeAfterExperience=statement.includes('instrument.after(purpose)')||statement.includes("instrument.insertAdjacentElement('afterend',purpose)");
const capabilityAfterPurpose=statement.includes('purpose.after(capabilities)')||statement.includes("purpose.insertAdjacentElement('afterend',capabilities)");
const readinessAfterCapability=statement.includes('capabilities.after(readiness)')||statement.includes("capabilities.insertAdjacentElement('afterend',readiness)");
check('NARRATIVE_SEQUENCE',narrativeToken&&purposeAfterExperience&&capabilityAfterPurpose&&readinessAfterCapability,'identity → experience → purpose → system → readiness → evidence');
check('TRL_TRA_SHARED_STAGE',statement.includes("readiness.dataset.readinessStage='trl-tra'")&&statement.includes('Technology Readiness Assessment (TRA)')&&index.includes('Software TRL 7'),'shared readiness stage retains TRL 7 and adds TRA assessment context');
check('TRL_TRA_BOUNDARY_LANGUAGE',statement.includes('It is not a second maturity score')&&statement.includes('does not raise Software TRL 7')&&statement.includes('creates no external certification or endorsement'),'TRA is assessment, not a new level or endorsement');
...
check('KEYBOARD_SEMANTIC_ORDER',capability.includes("e.key==='ArrowRight'||e.key==='ArrowLeft'")&&statement.includes("event.key==='ArrowRight'||event.key==='ArrowLeft'") ,'keyboard carousel controls retained');
...
const receipt={
  schema:'COMPASS_AWARD_PRESENTATION_STATIC_RECEIPT_v1',
  operationId:'COMPASS_AWARD_PRESENTATION_SUCCESSOR_20260818_v3',
  lockGeneration:1549,
  governingHead:'4e2ab91a18ab6a36fd47b83f794267a896d86fde',
  candidateHead:head,
  baseHead:BASE,
