...
function mesh(gl){
  ...
  const gold=[.72,.43,.095],bright=[.98,.73,.24],aged=[.56,.31,.07],bronze=[.42,.22,.055],dark=[.22,.13,.045],black=[.065,.055,.045],plaque=[.78,.52,.16],plaqueEdge=[1.0,.78,.30],letter=[1.0,.94,.66];
  ...
  function textLine(name,text,cx,cy,width,height){const chars=[...text],advance=1.18,units=chars.reduce((s,ch)=>s+(ch===' '?advance*.65:advance),0)-.18,scale=width/units,gh=height,gw=scale;let x=cx-width*.5;for(const ch of chars){if(ch===' '){x+=gw*advance*.65;continue}for(const [x0,y0,x1,y1] of glyphStrokes(ch)){stroke(`${name}-${ch}`,x+x0*gw,cy-gh*.5+y0*gh,x+x1*gw,cy-gh*.5+y1*gh,gw*.16,.382,.410,letter)}x+=gw*advance}}
  ...
  box('plinth-body',-.46,.46,-.705,-.565,-.295,.295,dark);
  box('plinth-foot',-.49,.49,-.755,-.700,-.315,.315,black);
  box('nameplate-mount',-.425,.425,-.708,-.505,.330,.360,black);
  box('nameplate-face',-.405,.405,-.690,-.520,.360,.386,plaque);
  box('nameplate-edge-top',-.415,.415,-.520,-.505,.356,.392,plaqueEdge);
  box('nameplate-edge-bottom',-.415,.415,-.708,-.690,.356,.392,plaqueEdge);
  box('nameplate-edge-left',-.419,-.405,-.690,-.520,.356,.392,plaqueEdge);
  box('nameplate-edge-right',.405,.419,-.690,-.520,.356,.392,plaqueEdge);
  textLine('nameplate-line-1','DIAMOND GATE',0,-.575,.72,.052);
  textLine('nameplate-line-2','BRIDGE ?',0,-.642,.54,.056);
...
function mount(canvas,{foreground=()=>true}={}){
  ...
  const draw=(t=0)=>{resize();gl.clearColor(0,0,0,0);gl.clear(gl.COLOR_BUFFER_BIT|gl.DEPTH_BUFFER_BIT);gl.enable(gl.DEPTH_TEST);gl.enable(gl.BLEND);gl.blendFunc(gl.SRC_ALPHA,gl.ONE_MINUS_SRC_ALPHA);gl.disable(gl.CULL_FACE);gl.useProgram(pr);lastYaw=M.sin(t*.00045)*.11;gl.uniform1f(L.aspect,canvas.width/M.max(1,canvas.height));gl.uniform1f(L.yaw,lastYaw);bind(m.p,L.p);bind(m.n,L.n);bind(m.c,L.c);gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER,m.i);gl.drawElements(gl.TRIANGLES,m.count,gl.UNSIGNED_SHORT,0);frames++;canvas.dataset.trophyFrames=String(frames);canvas.dataset.trophyYaw=lastYaw.toFixed(4)};
  const reduced=matchMedia?.('(prefers-reduced-motion: reduce)')?.matches;
  let raf=0;const animate=t=>{if(foreground())draw(reduced?0:t);raf=requestAnimationFrame(animate)};draw(0);if(!reduced)raf=requestAnimationFrame(animate);new ResizeObserver(()=>draw(performance.now())).observe(canvas);
  const api=Object.freeze({canvas,fallback:false,activate:()=>true,capture:()=>{draw(performance.now());return{triangles:m.triangles,parts:m.parts,yaw:lastYaw}},restore:()=>{draw(0);return true},inspect:()=>({fallback:false,foreground:foreground(),motion:reduced?'static-reduced-motion':'slow-showcase-yaw',yaw:lastYaw,triangles:m.triangles,parts:m.parts,material:'differentiated-aged-polished-gold-bronze-nameplate',nameplate:'visibly-mounted-geometry:DIAMOND GATE BRIDGE ?'})});scenes.set(canvas,api);primary=api;return api
}
window.CompassTrophyScene=Object.freeze({version:'procedural-webgl-trophy-v7-front-facing-motion',mount,activate:()=>primary?.activate?.()||false,capture:()=>primary?.capture?.()||null,restore:()=>primary?.restore?.()||false,inspect:()=>primary?.inspect?.()||null});
})();