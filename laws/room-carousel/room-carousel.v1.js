(() => {
  ...
  const SEMANTIC_RULES = Object.freeze([
    ...
    { re:/\b(keep detection)\b/i, lead:(s,m)=>`${s} decides whether a declared ${m} event has been observed at all. It is the threshold-crossing or event-presence decision that turns measured input into a bounded yes/no detection before any category is assigned.`, why:(s,m)=>`Detection answers whether the ${m} event is present under the declared observation and threshold rules. Keeping that decision separate prevents a later category label from being mistaken for proof that the event was detected in the first place.`, engineering:(s,m)=>`Bind ${s} to the detector inputs, declared event definition, decision threshold or rule, evaluation horizon, and binary or scored output that records whether the ${m} event crossed the detection boundary.`, evidence:(s,m)=>`Use detector outputs tied to labeled event-presence records, including threshold, timestamp, and false-positive/false-negative behavior, to show that the ${m} event was or was not detected under the declared rule.`, failure:(s)=>`${s} fails when a category label is treated as detection evidence, when the threshold is moved after outcome inspection, or when absence of a detection record is silently converted into a negative event.`, limits:(s,m)=>`${s} establishes event presence under the declared ${m} detection rule; it does not assign the detected observation to a class, explain cause, or establish operational action authority.` },
    { re:/\b(keep classification)\b/i, lead:(s,m)=>`${s} assigns an already-detected ${m} observation to a declared category, state, or label. It operates after event presence is established and asks what kind of detected observation it is, not whether anything was detected.`, why:(s,m)=>`Classification answers which declared ${m} category best describes an observation that has already crossed the detection boundary. Separating it from detection preserves the difference between event presence and category identity.`, engineering:(s,m)=>`Bind ${s} to the eligible detected-observation set, class vocabulary, feature representation, decision boundaries or scoring rule, tie and unknown handling, and the versioned mapping from detector output to ${m} category.`, evidence:(s,m)=>`Use labeled detected observations with a confusion matrix, per-class errors, or equivalent category-assignment records showing how the same detected ${m} cases were assigned among the declared classes.`, failure:(s)=>`${s} fails when undetected cases are forced into categories, when class definitions overlap or drift without versioning, or when category accuracy is reported as event-detection performance.`, limits:(s,m)=>`${s} establishes category identity for detected ${m} observations; it does not establish that the event was detected correctly, why the event occurred, or whether the class label warrants an operational response.` },
    { re:/\b(keep causal explanation|causal explanation|causal attribution|mechanism explanation)\b/i, lead:(s,m)=>`${s} asks what produced the detected or classified ${m} event and through what mechanism. Detection and classification can establish that an event was observed and labeled without establishing why it occurred.`, why:(s,m)=>`A predictive or associated ${m} signal can be useful without identifying its cause. Keeping causal explanation separate prevents correlation, ranking, or category assignment from being promoted into a mechanism claim.`, engineering:(s,m)=>`Bind ${s} to a hypothesized cause and mechanism, temporal ordering, competing explanations and confounders, the identification assumptions that separate cause from association, and an intervention, counterfactual, or other causal design appropriate to ${m}.`, evidence:(s,m)=>`Use controlled intervention, randomized or quasi-experimental comparison, natural-experiment evidence, mechanism-specific observations, or another declared causal-identification design that can distinguish the proposed ${m} cause from plausible alternatives.`, failure:(s)=>`${s} fails when prediction, correlation, detection, or classification is restated as cause; when reverse causality remains open; or when material confounding and competing mechanisms are not addressed.`, limits:(s,m)=>`${s} supports only the bounded causal relation justified by the declared ${m} identification assumptions; it does not by itself authorize an operational response or action policy.` },
    { re:/\b(keep action policy|action policy|response policy|decision policy|intervention policy)\b/i, lead:(s,m)=>`${s} defines what response is permitted, required, or prohibited after ${m} reaches a declared detected or classified state. It is a decision rule layered on evidence, not an explanation of why the signal occurred.`, why:(s,m)=>`The same ${m} state can justify different actions when costs, harms, tolerances, authority, or operating context differ. Separating policy from causal explanation keeps response authority from being smuggled into the signal itself.`, engineering:(s,m)=>`Bind ${s} to the triggering state or threshold, eligible action set, decision maker and authority, timing, safety constraints, cost or utility criterion, deployment population, escalation conditions, and rollback or hold behavior for ${m}.`, evidence:(s,m)=>`Use decision-rule calibration, intervention outcomes, utility or loss analysis, safety evidence, and operational validation in the intended ${m} population to show that the declared response performs acceptably under the policy's own criteria.`, failure:(s)=>`${s} fails when action is inferred directly from a signal without an explicit rule or authority, when thresholds drift after outcomes are seen, or when foreseeable costs, harms, and deployment constraints are omitted.`, limits:(s,m)=>`${s} authorizes only the bounded response encoded for the declared ${m} state and operating context; it does not establish why the signal occurred or prove a causal mechanism.` },
    { re:/\b(declare observation state|observation state|receiver state|measurement state)\b/i, lead:(s,m)=>`${s} fixes the condition in which the ${m} return is observed. It records the receiver or measured-system state at the declared observation instant rather than the quantity carried by the return.`, why:(s,m)=>`The same ${m} returned quantity can mean different things under different receiver conditions, so ${s.toLowerCase()} must preserve observation context independently from the payload itself.`, engineering:(s,m)=>`Bind ${s} to the receiver or measured-system state vector at the declared sampling instant: operating mode, relevant control state, baseline or prior state, validity flags, and the conditions required to interpret ${m}.`, evidence:(s,m)=>`Use a timestamped state snapshot, receiver telemetry, controller-state record, or equivalent observation record showing the condition in which ${m} was observed independently of the returned quantity.`, failure:(s)=>`${s} fails when surrounding system condition is inferred from the returned value, reconstructed only after response, or mixed across incompatible operating states.`, limits:(s,m)=>`${s} establishes the context in which ${m} was observed; it does not establish the returned quantity or category, the sampling location, or the action taken from that observation.` },
    { re:/\b(declare returned value|returned value|feedback payload|return payload)\b/i, lead:(s,m)=>`${s} records the quantity, category, or bounded payload that actually came back through the ${m} path. It identifies the content of the return rather than the condition of the system that received it.`, why:(s,m)=>`Receiver state alone cannot establish what ${m} returned. ${s} must therefore preserve magnitude, sign, category, units, and correspondence to the originating output without confusing payload with observation context.`, engineering:(s,m)=>`Bind ${s} to the exact ${m} payload at the declared return boundary: value or category, units and scale, sign or direction where applicable, encoding, validity or quality flag, and the source-output identity to which it corresponds.`, evidence:(s,m)=>`Use the received ${m} record, decoded payload, loop trace, or paired send/return receipt that preserves the returned quantity or category independently from the receiver's surrounding state.`, failure:(s)=>`${s} fails when payload is inferred from the receiver's reaction, units or scale are missing, a transformed downstream value is substituted for the received return, or absence is treated as zero.`, limits:(s,m)=>`${s} establishes what came back through the ${m} path; it does not establish the receiver's observation state, why that value occurred, or whether a controller was authorized to act on it.` },
    { re:/\b(specify decision function|decision function|control decision function)\b/i, lead:(s,m)=>`${s} defines the rule that converts observed ${m} inputs into a selected command or response. It describes the mapping from inputs to choice, not the controller's current internal condition.`, why:(s,m)=>`A controller can occupy the same state while choosing different outputs for different returned inputs, and the same input can yield a different output when the decision rule changes. ${s} therefore has to preserve selection logic separately from controller state.`, engineering:(s,m)=>`Bind ${s} to the declared input variables, thresholds and branch conditions, precedence or tie handling, selected command or action output, and the versioned logic used by ${m} at the decision instant.`, evidence:(s,m)=>`Use a decision trace, rule evaluation record, or reproducible test showing that declared ${m} inputs traverse the specified branch logic and produce the recorded command.`, failure:(s)=>`${s} fails when the chosen output is inferred from controller state alone, when branch logic is undocumented or changed after inspection, or when downstream execution is substituted for the decision itself.`, limits:(s,m)=>`${s} establishes how ${m} inputs are mapped to a selected response; it does not establish the controller's current state, whether the command was executed, or whether the controller had authority to act.` },
    { re:/\b(specify state|controller state|control state|internal state)\b/i, lead:(s,m)=>`${s} records the controller's internal condition at the ${m} decision instant. It captures mode, memory, active configuration, and accumulated context rather than the rule that selects an output.`, why:(s,m)=>`Identical ${m} returns can be interpreted differently when controller mode, stored history, setpoints, timers, or integrator values differ. State supplies decision context without defining the decision function itself.`, engineering:(s,m)=>`Bind ${s} to the contemporaneous controller snapshot: operating mode, memory or history variables, active setpoints and configuration, timers or flags, integrator or accumulator values, software or rule version, and timestamp.`, evidence:(s,m)=>`Use controller telemetry, a state-vector snapshot, register or configuration record, or equivalent contemporaneous trace that independently records the condition present when the ${m} decision was made.`, failure:(s)=>`${s} fails when internal condition is reconstructed from the chosen action, mixed across different decision times or modes, or when mutable memory that can alter behavior is omitted.`, limits:(s,m)=>`${s} establishes the controller condition at the ${m} decision instant; it does not establish the decision function, the selected command, the received return value, or execution of the resulting action.` },
    { re:/\b(record the commanded change|commanded change|commanded adjustment|issued command)\b/i, lead:(s,m)=>`${s} records the specific control instruction selected for ${m} after the return is interpreted. It captures the requested change in target, actuator, allocation, or setting before execution is measured.`, why:(s,m)=>`A commanded ${m} change is an intended intervention, not proof that the system moved or that the instruction stayed inside its permitted envelope. Keeping the command explicit separates what was asked from what was allowed and what actually happened.`, engineering:(s,m)=>`Bind ${s} to the controller output at the decision instant: command identity, destination actuator or parameter, requested value or delta, units, direction, issue time, and the return state that triggered the instruction.`, evidence:(s,m)=>`Use a controller-output trace, command log, message receipt, or setpoint record showing the exact ${m} instruction that was issued before downstream execution.`, failure:(s)=>`${s} fails when the requested adjustment is reconstructed only from the later system state, when multiple commands are conflated, or when an executed value is substituted for the command that was actually issued.`, limits:(s,m)=>`${s} establishes the requested ${m} adjustment; it does not establish the permitted bounds, whether limiting changed the request, whether the command executed, or what next state resulted.` },
    { re:/\b(record bounds|adjustment bounds|command bounds|permitted range|control envelope)\b/i, lead:(s,m)=>`${s} defines the admissible envelope within which a ${m} adjustment may operate. It records limits on magnitude, rate, direction, duration, or reachable state rather than the particular command selected inside that envelope.`, why:(s,m)=>`Two identical ${m} commands can have different consequences when their permitted ranges differ. Declaring ${s.toLowerCase()} makes constraint authority visible and prevents a requested change from being mistaken for an unconstrained instruction.`, engineering:(s,m)=>`Bind ${s} to explicit lower and upper limits, rate or slew limits, directional prohibitions, saturation or safety constraints, units, applicable controller mode, and the rule used when a requested ${m} command exceeds the envelope.`, evidence:(s,m)=>`Use configuration records, limiter settings, safety constraints, controller specifications, or clamp events demonstrating the permitted ${m} range independently from any one issued command.`, failure:(s)=>`${s} fails when constraints are inferred from observed outputs, when bounds silently change by mode or time, or when clipping is mistaken for the originally requested adjustment.`, limits:(s,m)=>`${s} establishes the permissible envelope for ${m}; it does not establish which command was selected, whether a limiter activated on a particular event, or whether the resulting adjustment executed successfully.` },
