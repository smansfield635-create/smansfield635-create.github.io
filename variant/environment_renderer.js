...
function drawGlobalSkyWash(ctx, phaseLabel, runtime) {
  const width = runtime?.width ?? 1180;
  const height = runtime?.height ?? 1240;

  const gradient = ctx.createLinearGradient(0, 0, 0, height);

  if (phaseLabel === "CLEAR_WINDOW") {
    gradient.addColorStop(0, "rgba(255,252,244,0.10)");
    gradient.addColorStop(0.30, "rgba(210,238,255,0.05)");
    gradient.addColorStop(0.72, "rgba(116,180,232,0.02)");
    gradient.addColorStop(1, "rgba(20,42,72,0.00)");
  } else if (phaseLabel === "LOCKDOWN") {
    gradient.addColorStop(0, "rgba(206,214,228,0.05)");
    gradient.addColorStop(0.34, "rgba(132,154,188,0.06)");
    gradient.addColorStop(0.72, "rgba(70,96,134,0.04)");
    gradient.addColorStop(1, "rgba(20,32,54,0.02)");
  } else if (phaseLabel === "SEVERE") {
    gradient.addColorStop(0, "rgba(226,228,234,0.05)");
    gradient.addColorStop(0.34, "rgba(154,182,212,0.05)");
    gradient.addColorStop(0.72, "rgba(82,132,184,0.03)");
    gradient.addColorStop(1, "rgba(20,42,72,0.01)");
  } else {
    gradient.addColorStop(0, "rgba(246,244,236,0.06)");
    gradient.addColorStop(0.34, "rgba(182,220,244,0.03)");
    gradient.addColorStop(0.72, "rgba(88,150,204,0.02)");
    gradient.addColorStop(1, "rgba(20,42,72,0.00)");
  }
...
function drawUpperSkyBloom(ctx, phaseLabel, runtime) {
  const width = runtime?.width ?? 1180;
  const height = runtime?.height ?? 1240;
  ...
  gradient.addColorStop(0, `rgba(228,244,255,${alpha})`);
  gradient.addColorStop(0.36, `rgba(166,216,248,${alpha * 0.45})`);
  gradient.addColorStop(1, "rgba(166,216,248,0.00)");

  ctx.fillStyle = gradient;
  ctx.fillRect(0, 0, width, height);
}

function drawPlanetLimbHaze(ctx, projector, phaseLabel) {
  ...
  haze.addColorStop(0, "rgba(180,220,255,0.00)");
  haze.addColorStop(0.30, `rgba(170,220,252,${0.022 * intensity})`);
  haze.addColorStop(0.52, `rgba(206,238,255,${0.040 * intensity})`);
  ...
  if (phaseLabel === "LOCKDOWN" || phaseLabel === "SEVERE") {
    ...
    coolVeil.addColorStop(0, "rgba(188,214,255,0.00)");
    coolVeil.addColorStop(0.50, `rgba(176,204,244,${0.026 * intensity})`);
    coolVeil.addColorStop(1, "rgba(176,204,244,0.00)");

    ctx.fillStyle = coolVeil;
    ctx.fillRect(
      body.centerX - (body.radius * 1.30),
      body.centerY - (body.radius * 1.30),
      body.radius * 2.60,
      body.radius * 2.60
    );
  }
...
function drawWaterBodyAtmosphere(ctx, points, tick, waterClass, phaseLabel, projector) {
  ...
  if (phaseLabel === "LOCKDOWN") {
    ...
    gradient.addColorStop(0.45, "rgba(132,164,212,0.04)");
    gradient.addColorStop(1, "rgba(40,66,108,0.01)");
  } else if (phaseLabel === "SEVERE" || phaseLabel === "UNSTABLE") {
    if (waterClass === "harbor") {
      gradient.addColorStop(0, `rgba(224,246,255,${0.08 + (pulse * 0.03)})`);
      gradient.addColorStop(0.45, "rgba(134,206,244,0.05)");
      gradient.addColorStop(1, "rgba(38,88,142,0.00)");
    } else {
      gradient.addColorStop(0, `rgba(204,236,255,${0.06 + (pulse * 0.02)})`);
      gradient.addColorStop(0.5, "rgba(126,184,220,0.04)");
      gradient.addColorStop(1, "rgba(36,74,116,0.00)");
    }
  } else if (phaseLabel === "CLEAR_WINDOW") {
    if (waterClass === "harbor") {
      gradient.addColorStop(0, `rgba(236,250,255,${0.10 + (pulse * 0.03)})`);
      gradient.addColorStop(0.45, "rgba(156,220,250,0.06)");
      gradient.addColorStop(1, "rgba(30,78,132,0.00)");
    } else {
      gradient.addColorStop(0, `rgba(214,246,255,${0.08 + (pulse * 0.02)})`);
      gradient.addColorStop(0.5, "rgba(138,196,224,0.04)");
      gradient.addColorStop(1, "rgba(34,74,110,0.00)");
    }
  } else {
    if (waterClass === "harbor") {
      gradient.addColorStop(0, `rgba(210,246,255,${0.07 + (pulse * 0.02)})`);
      gradient.addColorStop(0.45, "rgba(124,196,232,0.04)");
      gradient.addColorStop(1, "rgba(30,78,132,0.00)");
    } else {
      gradient.addColorStop(0, `rgba(196,236,248,${0.05 + (pulse * 0.015)})`);
      gradient.addColorStop(0.5, "rgba(126,178,204,0.03)");
      gradient.addColorStop(1, "rgba(34,74,110,0.00)");
    }
  }
...
function drawWaterBodyShimmer(ctx, points, tick, waterClass, phaseLabel, projector) {
  ...
  const shimmerBands = waterClass === "harbor"
    ? [
        { x: cx - 150, y: cy - 30, w: 180, h: 18, a: (0.04 + (pulse * 0.025)) * alphaScale },
        { x: cx - 40, y: cy + 8, w: 140, h: 16, a: (0.03 + (pulse * 0.02)) * alphaScale },
        { x: cx + 40, y: cy - 56, w: 120, h: 14, a: (0.026 + (pulse * 0.016)) * alphaScale }
      ]
    : [
        { x: cx - 74, y: cy - 14, w: 92, h: 12, a: (0.03 + (pulse * 0.016)) * alphaScale },
        { x: cx - 14, y: cy + 10, w: 80, h: 10, a: (0.024 + (pulse * 0.014)) * alphaScale }
      ];
...
function drawHarborMist(ctx, kernel, phaseLabel, projector) {
  ...
  if (phaseLabel === "CLEAR_WINDOW") {
    ...
    mist.addColorStop(0.4, "rgba(198,228,246,0.018)");
    mist.addColorStop(1, "rgba(188,222,244,0.00)");
  } else if (phaseLabel === "LOCKDOWN" || phaseLabel === "SEVERE") {
    mist.addColorStop(0, "rgba(236,244,255,0.10)");
    mist.addColorStop(0.4, "rgba(194,220,246,0.05)");
    mist.addColorStop(1, "rgba(188,222,244,0.00)");
  } else {
    mist.addColorStop(0, "rgba(232,244,255,0.06)");
    mist.addColorStop(0.4, "rgba(188,222,244,0.03)");
    mist.addColorStop(1, "rgba(188,222,244,0.00)");
  }
...
function drawSeaHazardAtmosphere(ctx, kernel, phaseLabel, projector) {
  ...
  for (const hazard of seaHazardsById.values()) {
    ...
    if (hazard.hazardClass === "reef") {
      ...
      gradient.addColorStop(0.5, `rgba(110,182,168,${0.03 * multiplier})`);
      gradient.addColorStop(1, "rgba(110,182,168,0.00)");
    } else {
      gradient.addColorStop(0, `rgba(176,214,255,${0.06 * multiplier})`);
      gradient.addColorStop(0.55, `rgba(116,156,220,${0.03 * multiplier})`);
...
function drawPhaseRadiationVeil(ctx, phaseLabel, tick, runtime) {
  ...
  if (phaseLabel === "CLEAR_WINDOW") {
    const glow = ctx.createRadialGradient(width * 0.50, height * 0.20, 60, width * 0.50, height * 0.20, width * 0.62);
    glow.addColorStop(0, "rgba(255,250,236,0.08)");
    glow.addColorStop(0.32, "rgba(214,238,255,0.04)");
  ...
  const alphaA = (0.016 + (pulse * 0.010)) * strength;
  const alphaB = (0.012 + (pulse * 0.008)) * strength;

  const veilA = ctx.createLinearGradient(0, 0, width, height * 0.70);
...
function drawChargedCloudBands(ctx, phaseLabel, tick, runtime) {
  if (
    phaseLabel !== "BUILDING" &&
    phaseLabel !== "UNSTABLE" &&
    phaseLabel !== "SEVERE" &&
  ...
  const bands = [
    { x: 110 + drift, y: 114, w: 280, h: 54, a: 0.024 + (phaseStrength * 0.020) },
    { x: 386 - (drift * 0.4), y: 170, w: 360, h: 62, a: 0.022 + (phaseStrength * 0.018) },
    { x: 728 + (drift * 0.7), y: 128, w: 270, h: 58, a: 0.022 + (phaseStrength * 0.020) },
    { x: 218 - (drift * 0.6), y: 246, w: 430, h: 72, a: 0.018 + (phaseStrength * 0.016) }
  ];
  ...
  for (const band of bands) {
    ...
    gradient.addColorStop(0.5, `rgba(162,186,220,${band.a * 1.05})`);
    gradient.addColorStop(1, "rgba(98,128,170,0.00)");
    ctx.fillStyle = gradient;
    ctx.fillRect(band.x, band.y, Math.min(band.w, width), band.h);
  }
  ctx.restore();
}

function drawStormChargeFlashes(ctx, phaseLabel, tick, runtime) {
  ...
  if (pulseA > 0.90) {
    ctx.fillStyle = `rgba(228,242,255,${(pulseA - 0.90) * 0.16})`;
    ctx.fillRect(0, 0, width, height * 0.72);
  }

  if (pulseB > 0.94) {
    const gradient = ctx.createRadialGradient(width * 0.70, height * 0.18, 20, width * 0.70, height * 0.18, width * 0.28);
    gradient.addColorStop(0, `rgba(244,250,255,${(pulseB - 0.94) * 1.1})`);
    gradient.addColorStop(0.3, `rgba(210,232,255,${(pulseB - 0.94) * 0.56})`);
    gradient.addColorStop(1, "rgba(210,232,255,0.00)");
    ctx.fillStyle = gradient;
    ctx.fillRect(width * 0.44, 0, width * 0.44, height * 0.40);
  }
}

function drawDistanceVeil(ctx, phaseLabel, runtime) {
  const width = runtime?.width ?? 1180;
  const height = runtime?.height ?? 1240;

  const topVeil = ctx.createLinearGradient(0, 0, 0, height * 0.34);
  if (phaseLabel === "CLEAR_WINDOW") {
    topVeil.addColorStop(0, "rgba(255,255,255,0.02)");
    topVeil.addColorStop(1, "rgba(255,255,255,0.00)");
  } else if (phaseLabel === "LOCKDOWN" || phaseLabel === "SEVERE") {
    topVeil.addColorStop(0, "rgba(248,250,255,0.05)");
    topVeil.addColorStop(1, "rgba(255,255,255,0.00)");
  } else {
...
function drawVignette(ctx, phaseLabel, runtime) {
  const width = runtime?.width ?? 1180;
  const height = runtime?.height ?? 1240;
  const radius = phaseLabel === "CLEAR_WINDOW" ? width * 0.92 : width * 0.86;
  ...
  const gradient = ctx.createRadialGradient(width * 0.50, height * 0.50, width * 0.20, width * 0.50, height * 0.50, radius);
...
export function createEnvironmentRenderer() {
  ...
  function draw(ctx, runtime) {
    ...
    drawPhaseRadiationVeil(ctx, phaseLabel, tick, runtime);
    drawChargedCloudBands(ctx, phaseLabel, tick, runtime);
    drawPlanetLimbHaze(ctx, projector, phaseLabel);
