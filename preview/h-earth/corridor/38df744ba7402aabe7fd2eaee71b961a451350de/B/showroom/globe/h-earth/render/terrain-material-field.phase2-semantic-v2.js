...
export const H_EARTH_PHASE2_SEMANTIC_MATERIAL_FIELD_ID = 'H_EARTH_UNIQUE_BAKED_LANDFORM_MATERIAL_FIELD_v1';
...
export async function loadHEarthPhase2SemanticMaterialField() {
  if (cachedPromise === null) cachedPromise = (async () => {
    ...
    if (buffer.byteLength !== H_EARTH_PHASE2_SEMANTIC_MATERIAL_FIELD_BYTE_LENGTH) throw new Error(`H_EARTH_BAKED_MATERIAL_BYTE_LENGTH_INVALID:${buffer.byteLength}`);
    const digest = await digestHex(buffer);
    if (digest !== H_EARTH_PHASE2_SEMANTIC_MATERIAL_FIELD_SHA256) throw new Error(`H_EARTH_BAKED_MATERIAL_DIGEST_INVALID:${digest}`);
    return new Uint8Array(buffer);
  })();
  return new Uint8Array(await cachedPromise);
}
export function getHEarthPhase2SemanticMaterialFieldReceipt() {
  return Object.freeze({ fieldId: H_EARTH_PHASE2_SEMANTIC_MATERIAL_FIELD_ID, width: H_EARTH_PHASE2_SEMANTIC_MATERIAL_FIELD_WIDTH, height: H_EARTH_PHASE2_SEMANTIC_MATERIAL_FIELD_HEIGHT, byteLength: H_EARTH_PHASE2_SEMANTIC_MATERIAL_FIELD_BYTE_LENGTH, canonicalSha256: H_EARTH_PHASE2_SEMANTIC_MATERIAL_FIELD_SHA256, domain: H_EARTH_PHASE2_SEMANTIC_MATERIAL_FIELD_DOMAIN, sourceDescriptorSha256: H_EARTH_BAKED_MATERIAL_SOURCE_DESCRIPTOR_SHA256, sourceSegmentationSha256: H_EARTH_BAKED_MATERIAL_SOURCE_SEGMENTATION_SHA256, mipmapsRequired: true, uniqueWorldCoverage: true, runtimeTextureSamplesPerTerrainFragment: 1 });
}
export default loadHEarthPhase2SemanticMaterialField;
