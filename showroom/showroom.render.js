(function () {
  ...
  function syncButtons() {
    ...
    const root = qs("#showroom-globe-mount");
  ...
  function drawStars(svg) {
    const stars = s("g", { "data-layer": "stars" });

    for (let i = 0; i < 170; i += 1) {
      const x = 40 + ((i * 97) % 1520);
      const y = 40 + ((i * 53) % 820);
      const r = i % 11 === 0 ? 1.8 : i % 5 === 0 ? 1.2 : .7;
  ...
  function drawOrbit(svg) {
    const orbit = s("g", { "data-layer": "orbit-rings" });
    ...
    orbit.appendChild(s("ellipse", {
      ...
      ry: 112,
      fill: "none",
      stroke: "rgba(242,199,111,.26)",
      "stroke-width": 2,
      transform: "rotate(-12 800 470)"
    }));
    ...
    orbit.appendChild(s("ellipse", {
      ...
      ry: 142,
      fill: "none",
      stroke: "rgba(118,177,255,.22)",
      "stroke-width": 1.6,
      transform: "rotate(18 800 470)"
    }));
    ...
    orbit.appendChild(s("ellipse", {
      ...
      "stroke-width": 1.2,
      transform: "rotate(-31 800 470)"
    }));

    svg.appendChild(orbit);
  }

  function landPathDemo() {
    return [
  ...
  function drawGlobe(svg) {
    const isMirror = state.universe === "mirror";

    const group = s("g", {
      class: "globe-group",
      "data-universe": state.universe,
    ...
    const paths = isMirror ? landPathMirror() : landPathDemo();
    ...
    if (!isMirror) {
      const clouds = s("g", { class: "globe-cloud", "clip-path": `url(#${clipId})` });
    ...
    if (isMirror) {
      const buildableMark = s("g", { "data-layer": "mirror-buildable-marker" });

      buildableMark.appendChild(s("circle", {
        cx: 800,
        cy: 470,
        r: 74,
        fill: "none",
      ...
      markLabel.textContent = "MANOR / ESTATE CONTEXT";
      buildableMark.appendChild(markLabel);
      group.appendChild(buildableMark);
    }
    ...
    label.textContent = isMirror ? "Mirror / Nine Summits Universe" : "Demo Universe Globe";
  ...
  function render() {
    ...
    scene.setAttribute("aria-label", state.universe === "mirror" ? "Mirror Universe globe" : "Demo Universe globe");

    const svg = s("svg", {
      class: "universe-svg",
      viewBox: "0 0 1600 900",
      role: "img",
      "aria-label": state.universe === "mirror"
  ...
  function setUniverse(universe) {
    if (!["demo", "mirror"].includes(universe)) return;
