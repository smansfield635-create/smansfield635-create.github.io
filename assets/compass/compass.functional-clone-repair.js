...
function init(){
  ...
  // Keep H-Earth on the owner-approved 23949 environment without altering any other room route.
  ...
  // The historical semantic Mirrorland control can drift away from its rendered glass.
  // Retire its hit/text surface and install one clone-only hit target over the actual window.
  if(originalDoor){
  ...
  // Render labels from the actual renderer-driven semantic star positions.
  ...
  // Visible but unobtrusive runtime audit. It verifies construction/mount facts, not user interaction outcomes.
