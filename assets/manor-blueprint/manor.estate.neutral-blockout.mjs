...
export function auditBlockout(){
  ...
  const axisZGableRidgesCorrect=ROOFS.filter((s)=>s.type==='gableRoof'&&s.axis==='z').every((s)=>{const roofMesh=mesh.meshes.find((m)=>m.id===s.id);return roofMesh&&roofMesh.triangles.some((v)=>v[1]===s.ridgeHeight)&&roofMesh.triangles.filter((v)=>v[2]===s.center[2]&&v[1]===s.ridgeHeight).length>=2;});
