...
O=R/'raw-tc-radar-mechanistic-qc-corrected-result.v1.json'
F=R/'raw-tc-radar-mechanistic-qc-corrected-features.v1.csv'
...
# Generic SHIPS/raw numeric cleaner. Pressure values above 900 are physically valid,
# so the SST sentinel rule is intentionally variable-specific below.
def clean(x):
    try:x=float(x)
    except:return np.nan
    return x if np.isfinite(x) and abs(x)<9000 else np.nan

# Technical QC correction only: TC-RADAR/SHIPS SST uses a missing-value sentinel
# near 999.9. The frozen protocol already requires missing BASE variables to be
# UNEVALUABLE with no imputation.
def clean_sst(x):
    try:x=float(x)
    except:return np.nan
    return x if np.isfinite(x) and abs(x)<900 else np.nan
...
def main():
    ...
    if ((p['sst0'].abs()>=900).any()):raise RuntimeError('SST_SENTINEL_SURVIVED_PRIMARY_QC')
    out={'operation':PROTO['operation'],'qc_correction':'SST_SHIPS_ABS_GE_900_TREATED_AS_MISSING_NO_IMPUTATION','protocol_sha256':hashlib.sha256(P.read_bytes()).hexdigest(),'source_receipts':rec,'raw_rows':len(d),'primary_rows':len(p),'calibration_rows':len(tr),'holdout_rows':len(te),'calibration_storms':tr.storm_id.nunique(),'holdout_storms':te.storm_id.nunique(),'calibration_ri':int(tr.ri.sum()) if len(tr) else 0,'holdout_ri':int(te.ri.sum()) if len(te) else 0,'claim_ceiling':PROTO['claim_ceiling']}
