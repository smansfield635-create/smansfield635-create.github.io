...
import { createNavigationOverlayRenderer } from "./navigation_overlay_renderer.js";
...
export async function createScene(canvas, outputs) {
  ...
  const navigationOverlay = createNavigationOverlayRenderer();
  ...
  function drawFrame() {
    ...
    background.draw(ctx, backgroundState);
    ...
    navigationOverlay.drawWorld(ctx, renderState);

    ctx.restore();

    navigationOverlay.drawScreen(ctx, state, {
      width: state.width,
      height: state.height
    });
  }
