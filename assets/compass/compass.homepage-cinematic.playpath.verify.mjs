...
const REPAIR_BASE='0db5d38f475a7ead0bf85991510a0a2fd147eed7';
...
check('S08_RUNTIME_SHIFTED',host.includes("id:'S08',beat:'Return / Handoff',purpose:'Restore visitor agency',startMs:41000,endMs:45000")&&host.includes('const NATURAL_HANDOFF_FADE_START_MS=44540')&&host.includes('elapsedMs>=43800'));
...
check('ENTRY_TESSELLATION_DIRECT_STARFIELD',host.includes("ENTRY_LAW='SELECTED_CONTROL_CELLS_BECOME_SUCCESSOR_STARFIELD_THEN_DIAMOND_GATE_BRIDGE'")&&host.includes('const target=stars[Math.floor(random()*stars.length)]'));
check('ENTRY_2697_MOTION_PROFILE',host.includes('progress=clamp(elapsed/ENTRY_PREROLL_DURATION_MS)')&&host.includes('size=cell.size*mix(1,.24,q)')&&host.includes('alpha=1-q*.76'));
...
check('PREWARM_BEHIND_GATE',host.includes('if(session.rendererPreparationPromise)return session.rendererPreparationPromise')&&host.includes('void prepareRendererForPlayback();'));
