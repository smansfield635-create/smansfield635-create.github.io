...
function mountHomeIdentity(root){
  ...
  const reduced=matchMedia?.('(prefers-reduced-motion: reduce)')?.matches??false; const clock=createCardClock(root); let active=true,raf=0,last=0;
  function frame(now){if(!active&&!reduced)return;if(!reduced&&now-last<30){raf=requestAnimationFrame(frame);return}last=now;const rect=root.getBoundingClientRect(),dpr=Math.min(1.25,devicePixelRatio||1),W=Math.max(1,Math.round(rect.width*dpr)),H=Math.max(1,Math.round(rect.height*dpr));if(canvas.width!==W||canvas.height!==H){canvas.width=W;canvas.height=H;canvas.style.width=`${rect.width}px`;canvas.style.height=`${rect.height}px`}ctx.setTransform(dpr,0,0,dpr,0,0);const cardTime=clock.sample(now);const state=getHomeIdentityState(cardTime.elapsedMs,variant,reduced);render(ctx,rect.width,rect.height,manor.meshes,state,variant);root.dataset.homeIdentityPhase=state.phase;root.dataset.homeIdentityLifecycleGeneration=String(cardTime.generation);if(active&&!reduced)raf=requestAnimationFrame(frame)}
  ...
  const api=Object.freeze({specimenId:SPECIMEN_ID,variant,sourceContract:manor.contract,sourceAudit:audit,cycleMs:HOME_IDENTITY_CYCLE_MS,reducedMotion:reduced,cardClockContract:clock.contract,destroy(){active=false;cancelAnimationFrame(raf);io?.disconnect();clock.destroy();canvas.remove();root.dataset.homeIdentityMounted='false'}});
