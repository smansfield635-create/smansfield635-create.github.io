(()=>{
  ...
  function mountCapability(){
    ...
    let brainPointer=false;stage.addEventListener('pointerdown',event=>{brainPointer=!!event.target.closest?.('[data-human-brain]')},true);stage.addEventListener('click',event=>{if(brainPointer)event.stopPropagation()},true);const clearBrain=()=>setTimeout(()=>brainPointer=false);stage.addEventListener('pointerup',clearBrain,true);stage.addEventListener('pointercancel',clearBrain,true);
