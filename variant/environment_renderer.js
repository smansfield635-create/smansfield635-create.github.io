...
/* TNT — LAND_MASK-DERIVED COASTAL PASS */
/*
WHAT YOU SHOULD SEE AFTER THIS FILE:
- a visible coastal shelf hugging BOTH sides of the peninsula
- a turquoise shallow-water band directly against the coast
- a wet-sand band between shallow water and land
- a dry-sand beach band softening the hard terrain edge
- the Market District / Harbor Village coast should finally read as beach, not raw cut terrain
- harbor / basin interior water should still render normally
*/
...
/* Authoritative land boundary copied from ground_renderer.js */
const LAND_MASK = [
...
/*
  Geometry-owned coastline segments:
  EAST_SHORE  = south tip up along visible east coast
  WEST_SHORE  = north-west coast down to south tip

  This keeps all beach/water derivation owned by LAND_MASK rather than watersById.
*/
...
export function createEnvironmentRenderer() {
  ...
  function drawCoastForPath(ctx, coastPath, tick) {
    /*
      Stronger, clearly visible bands.
      These are intentionally wide enough to read at the current zoom.
    */
    const drySand = offsetPolyline(coastPath, 84);
    const wetSand = offsetPolyline(coastPath, 132);
    const shallow = offsetPolyline(coastPath, 186);
    const shelf = offsetPolyline(coastPath, 252);

    /* dry beach */
    fillBandBetween(ctx, coastPath, drySand, "rgba(238,220,174,0.82)");

    /* wet sand */
    fillBandBetween(ctx, drySand, wetSand, "rgba(216,198,158,0.68)");

    /* shallow turquoise */
    fillBandBetween(ctx, wetSand, shallow, "rgba(124,220,214,0.50)");

    /* outer shelf transition */
    fillBandBetween(ctx, shallow, shelf, "rgba(70,188,206,0.22)");

    /* foam/coast read */
    ctx.save();
    polyline(ctx, coastPath);
    ctx.lineWidth = 1.6;
    ctx.strokeStyle = "rgba(250,246,234,0.26)";
    ctx.lineCap = "round";
    ctx.lineJoin = "round";
    ctx.stroke();
    ctx.restore();

    /* subtle water motion only outside wet sand */
    strokeWaveTrain(ctx, shallow, tick + 10, "rgba(246,252,252,0.05)", 0.7, 24, 0.8);
    strokeWaveTrain(ctx, shelf, tick + 20, "rgba(238,248,250,0.03)", 0.9, 32, 0.7);
  }
