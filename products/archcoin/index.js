(function () {
  ...
  var knobNodes = Array.prototype.slice.call(document.querySelectorAll(".machine-knob"));
  ...
  function createFallingCoin(headIndex) {
    ...
    var coin = {
      ...
      headIndex: headIndex,
      landed: false,
      startedAt: runtime.lastFrameTime ? runtime.lastFrameTime - runtime.sceneStart : 0,
      dropMs: 2850,
      wobbleSeed: Math.random() * 10 + headIndex * 1.3
    };
  ...
  function releaseHead(headIndex) {
    if (runtime.headState[headIndex].busy) return;
    runtime.headState[headIndex].busy = true;

    var knob = knobNodes[headIndex];
    if (knob) knob.classList.add("turning");

    createFallingCoin(headIndex);

    window.setTimeout(function () {
      if (knob) knob.classList.remove("turning");
      runtime.headState[headIndex].busy = false;
    }, 280);
  }
  ...
  function settleCoin(coin, activeTime) {
    ...
    var normalizedHead = (coin.headIndex - 1.5) / 1.5;
    ...
    var y =
      pileHeight * 0.60 +
      runtime.landed.length * 4 +
      coin.headIndex * 5 +
      Math.cos(activeTime * 0.0012 + coin.wobbleSeed) * 4;

    var scale = 0.82 + coin.headIndex * 0.01;
    var rotation = -10 + coin.headIndex * 6;
  ...
  function updateCoin(coin, activeTime) {
    ...
    var sway = Math.sin(progress * Math.PI * 1.15 + coin.wobbleSeed) * 14;
    ...
    var scale = 0.88 + depth * 0.16;
    var opacity = 0.18 + depth * 0.44 + (1 - Math.abs(0.5 - progress) * 2) * 0.28;
  ...
  knobNodes.forEach(function (knob, index) {
