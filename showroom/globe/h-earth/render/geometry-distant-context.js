/** H_EARTH_SINGLE_SPHERICAL_WORLD_MANIFOLD_DISTANT_CONTEXT_v5_COAST_DISTANCE_OPTICS */
...
function buildContinuousOceanField(){
  ...
  const indices=[];let retainedCellCount=0,suppressedLandCellCount=0,mixedCoastCellCount=0;
  for(let zi=0;zi<height-1;zi++)for(let xi=0;xi<width-1;xi++){
    ...
    if(waterVotes===0){suppressedLandCellCount++;continue}if(waterVotes<4)mixedCoastCellCount++;indices.push(a,e,b,b,e,d);retainedCellCount++;
  }
  const maximumRadius=Math.max(...vertices.map(v=>Math.hypot(v.x,v.z)));
  return compact(vertices,indices,{retainedCellCount,suppressedLandCellCount,mixedCoastCellCount,gridWidth:width,gridHeight:height,xMinimum:OCEAN_X[0],xMaximum:OCEAN_X.at(-1),zMinimum:OCEAN_Z[0],zMaximum:OCEAN_Z.at(-1),outerRadius:maximumRadius,waterColorAuthority:'DISTANCE_FROM_CANONICAL_COAST_CONTINUOUS',historical23923ColorAnchorsPreserved:true,visibleWaterAuthority:'ONE_CONTINUOUS_OCEAN_SURFACE',nearCoastTessellation:'WORLD_SPACE_FIELD_NOT_RADIAL_RINGS',lateralColorTerminationPossible:false,visibleRectangularTerminationProhibited:true},vertexRgba);
}
