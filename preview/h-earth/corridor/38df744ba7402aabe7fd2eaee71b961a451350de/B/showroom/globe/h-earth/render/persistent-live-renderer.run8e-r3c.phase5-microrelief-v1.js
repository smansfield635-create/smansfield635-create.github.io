...
} from './persistent-live-renderer.run8e-r3c.cp2-round1-ow01-t4.js';
import {
...
export function createHEarthRun8ER3CPersistentRenderer(options = {}) {
  ...
  const initialize = (packet) => {
    ...
    if (BAKED_FIELD_BYTES.byteLength !== H_EARTH_PHASE2_SEMANTIC_MATERIAL_FIELD_WIDTH * H_EARTH_PHASE2_SEMANTIC_MATERIAL_FIELD_HEIGHT * 4) {
      throw new Error('BM4_BAKED_FIELD_BYTE_LENGTH_INVALID');
    }
    state.microTexture=gl.createTexture();if(!state.microTexture)throw Error('MICRO_TEXTURE_CREATE_FAILED');
    ...
    gl.texImage2D(
      ...
      H_EARTH_PHASE2_SEMANTIC_MATERIAL_FIELD_WIDTH, H_EARTH_PHASE2_SEMANTIC_MATERIAL_FIELD_HEIGHT,
