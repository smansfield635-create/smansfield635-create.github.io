/**
 * H_EARTH_AWARDS_PUBLIC_FACE_PATH_RECOGNITION_v2
 *
 * Audit-only registry successor recognizing the already-live H-Earth Awards
 * public-face occurrence plus its co-located browser validator path.
 * This is exact-path resolution only. It creates no Awards product mutation,
 * validator mutation, award outcome, semantic, canonical, merge, deployment,
 * release, publication, H-Earth runtime, renderer, terrain, geometry, world,
...
const VALIDATOR_GOVERNING_MAIN = '178ebf052e35d8a06c7930432b46b0c7445691d3';
const VALIDATOR_PATH = '/h-earth-3d/validation/instrument-platform/h-earth.public-face-trophy-standard.browser.mjs';
const VALIDATOR_MAIN_BLOB = 'ae3eae6abba0121bd641b53eb932699eb903a1fd';
...
const EVIDENCE_ID = 'EVIDENCE_H_EARTH_AWARDS_PUBLIC_FACE_PATH_RECOGNITION_v2';
...
const OCCURRENCES = Object.freeze([
  ...
  deepFreeze({
    repository: REPOSITORY,
    refType: 'COMMIT',
    refName: VALIDATOR_GOVERNING_MAIN,
    commitSha: VALIDATOR_GOVERNING_MAIN,
    path: VALIDATOR_PATH,
    gitBlobSha: VALIDATOR_MAIN_BLOB,
    contentSha256: null,
    byteCount: null,
    existenceStatus: 'PRESENT',
    fetchbackStatus: 'VERIFIED_PRESENT_AT_EXACT_OPERATION_BASELINE',
...
export const H_EARTH_AWARDS_PUBLIC_FACE_PATH_RECOGNITION_EVIDENCE = deepFreeze({
  ...
  evidenceClass: 'EXISTING_H_EARTH_PUBLIC_FACE_AND_VALIDATOR_PATH_RECOGNITION_WITH_EXACT_BLOB_CUSTODY',
  sourceKind: 'PR935_AUTOMATIC_PREFLIGHT_STOP_PLUS_EXACT_MAIN_FETCHBACK_AND_PR1004_VALIDATOR_PREFLIGHT_STOP',
  sourceIdOrPath: 'PR_935_AWARDS_2027_CONTEXTUAL_RENEWAL_AND_PR_1004_VALIDATOR_RECONCILIATION',
  sourceOccurrenceOrRevision:
    `AWARDS_MAIN=${GOVERNING_MAIN};AWARDS_PR=935;AWARDS_BLOCKED_HEAD=80f31bf8e4fb0582d5c57777107f41bee2cc4d3d;AWARDS_PREFLIGHT_RUN=31655093538;AWARDS_TARGET=${TARGET_MAIN_BLOB};VALIDATOR_MAIN=${VALIDATOR_GOVERNING_MAIN};VALIDATOR_PR=1004;VALIDATOR_BLOCKED_HEAD=6503ac505b5f3aa97fd681003a678a7057f12295;VALIDATOR_PREFLIGHT_RUN=31763000618;VALIDATOR_TARGET=${VALIDATOR_MAIN_BLOB}`,
  governingMain: GOVERNING_MAIN,
  validatorGoverningMain: VALIDATOR_GOVERNING_MAIN,
  blockedPullRequest: 935,
  blockedHead: '80f31bf8e4fb0582d5c57777107f41bee2cc4d3d',
  blockedPreflightRun: 31655093538,
  validatorBlockedPullRequest: 1004,
  validatorBlockedHead: '6503ac505b5f3aa97fd681003a678a7057f12295',
  validatorBlockedPreflightRun: 31763000618,
  exactTargetPathCount: 1,
  exactRecognizedPathCount: 3,
  registrationEffect: 'READ_ONLY_PATH_RECOGNITION_ONLY',
  assertionScope: Object.freeze([
    'EXACT_H_EARTH_AWARDS_PUBLIC_FACE_PATH',
    'EXACT_H_EARTH_AWARDS_BROWSER_VALIDATOR_PATH',
    'EXACT_EXISTING_MAIN_BLOB_IDENTITIES',
    'AUTOMATIC_H_EARTH_PREFLIGHT_PATH_RESOLUTION'
  ]),
  evidenceLimitations: Object.freeze([
    'NO_AWARDS_PRODUCT_MUTATION_AUTHORITY',
    'NO_BROWSER_VALIDATOR_MUTATION_AUTHORITY',
...
export const H_EARTH_AWARDS_PUBLIC_FACE_PATH_RECOGNITION_NODE = deepFreeze({
  ...
  description:
    'Recognizes exactly the current H-Earth Awards public-face occurrence and its co-located browser validator so automatic H-Earth preflight can resolve both before any separately authorized mutation.',
  ...
  authorityPosture: 'READ_ONLY_EXISTING_H_EARTH_AWARDS_PUBLIC_FACE_AND_VALIDATOR_PATH_RECOGNITION',
  registrationEffect: 'PATH_RESOLUTION_ONLY',
  authoritySource: Object.freeze([
    'PR935_AUTOMATIC_H_EARTH_PREFLIGHT_STOP',
    'EXACT_MAIN_FETCHBACK_AT_90AE8C69',
    'PR1004_VALIDATOR_AUTOMATIC_H_EARTH_PREFLIGHT_STOP',
    'EXACT_VALIDATOR_FETCHBACK_AT_178EBF05',
  ...
  orderingRules: Object.freeze([
    'PREFLIGHT_STOP_PRECEDES_PATH_RECOGNITION',
    'PATH_RECOGNITION_PRECEDES_FRESH_PREFLIGHT',
    'FRESH_PREFLIGHT_PRECEDES_CANDIDATE_RETRY'
  ]),
  ...
  prohibitedMutations: Object.freeze([
    ...
    'BROWSER_VALIDATOR_MUTATION_FROM_REGISTRY_AUTHORITY',
  ...
  requiredValidations: Object.freeze([
    'EXACT_ONE_AWARDS_TARGET_PATH',
    'EXACT_AWARDS_TARGET_MAIN_BLOB',
    'EXACT_VALIDATOR_PATH',
    'EXACT_VALIDATOR_BASELINE_BLOB',
  ...
  stoppingBoundaries: Object.freeze([
    ...
    'STOP_ON_VALIDATOR_BASELINE_BLOB_DRIFT',
    'STOP_ON_PATH_OUTSIDE_EXACT_SCOPE',
    'STOP_ON_AUTHORITY_EXPANSION',
    'STOP_BEFORE_UNAUTHORIZED_PRODUCT_OR_VALIDATOR_MUTATION'
  ]),
  currentIdentityReferences: Object.freeze([
    GOVERNING_MAIN,
    `TARGET_MAIN_BLOB=${TARGET_MAIN_BLOB}`,
    VALIDATOR_GOVERNING_MAIN,
    `VALIDATOR_MAIN_BLOB=${VALIDATOR_MAIN_BLOB}`,
    'BLOCKED_PULL_REQUEST=935',
    'BLOCKED_PREFLIGHT_RUN=31655093538',
    'VALIDATOR_BLOCKED_PULL_REQUEST=1004',
    'VALIDATOR_BLOCKED_PREFLIGHT_RUN=31763000618'
  ]),
...
export function verifyHEarthAwardsPublicFacePathRecognition() {
  ...
  const validatorResolution = resolveHEarthRepositoryRegistryPath(VALIDATOR_PATH);
  const targetOccurrence = OCCURRENCES.find((entry) => entry.path === TARGET_PATH);
  const validatorOccurrence = OCCURRENCES.find((entry) => entry.path === VALIDATOR_PATH);
  ...
  const validatorOccurrenceMatched = (validatorResolution.occurrences ?? []).some((entry) =>
    entry.path === validatorOccurrence.path &&
    entry.commitSha === validatorOccurrence.commitSha &&
    entry.existenceStatus === 'PRESENT' &&
    entry.gitBlobSha === VALIDATOR_MAIN_BLOB
  );
  const checks = deepFreeze({
    ...
    validatorPathResolves: validatorResolution.resolved === true,
    validatorOccurrenceMatched,
    exactValidatorMainBlob: validatorOccurrence.gitBlobSha === VALIDATOR_MAIN_BLOB,
    ...
    noValidatorMutationAuthority:
      H_EARTH_AWARDS_PUBLIC_FACE_PATH_RECOGNITION_NODE.authorityLimitations.includes('NO_BROWSER_VALIDATOR_MUTATION_AUTHORITY'),
  ...
  return deepFreeze({
    ...
    validatorMainBlob: VALIDATOR_MAIN_BLOB
  });
}
