 ...
 *
 * H_EARTH_TERRAIN_FORMATIONS_RUN_6B_v1
 *
 * Durable geographic-formation identities. This file defines membership and
 * envelopes; it creates no vertices, chunks, proxies, admission, or rendering.
 */
...
export const H_EARTH_TERRAIN_FORMATIONS = deepFreeze({
  COASTAL_BERM_001: formation({
    ...
    addressRule: { rows: [5, 9], columns: [1, 16] },
    worldBounds: { xMin: -256, xMax: 256, zMin: -138, zMax: -76 },
  ...
  LOWLAND_001: formation({
    ...
    addressRule: { rows: [1, 7], columns: [1, 8] },
    worldBounds: { xMin: -176, xMax: -18, zMin: -224, zMax: -120 },
  ...
  HILL_001: formation({
    ...
    formationClass: 'HILL',
    addressRule: { rows: [1, 7], columns: [9, 13] },
    worldBounds: { xMin: 18, xMax: 132, zMin: -226, zMax: -116 },
  ...
  RIDGE_001: formation({
    ...
    addressRule: { rows: [1, 6], columns: [13, 16] },
    worldBounds: { xMin: 96, xMax: 224, zMin: -256, zMax: -164 },
  ...
  VALLEY_001: formation({
    ...
    addressRule: { rows: [1, 7], columns: [6, 10] },
    worldBounds: { xMin: -54, xMax: 52, zMin: -256, zMax: -128 },
  ...
  DISTANT_HIGHLAND_001: formation({
    ...
    addressRule: { rows: [14, 16], columns: [1, 10] },
...
export function resolveHEarthFormationMembershipForAddress(addressRecord) {
  ...
  return deepFreeze(
    Object.values(H_EARTH_TERRAIN_FORMATIONS)
      .filter((entry) => {
        const [rowMin, rowMax] = entry.addressRule.rows;
        const [columnMin, columnMax] = entry.addressRule.columns;
        return addressRecord.row >= rowMin && addressRecord.row <= rowMax
          && addressRecord.column >= columnMin
          && addressRecord.column <= columnMax;
      })
