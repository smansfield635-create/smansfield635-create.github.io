...
async function readLedgerBlob({repository,token,blobSha}){const g=await req(`${base(repository)}/git/blobs/${dig(blobSha,40,'legacySnapshotBlob','authority-provenance')}`,{headers:H(token)},[200],'AUTHORITY_LEDGER_BLOB');if(g.encoding!=='base64')throw err('AUTHORITY_LEGACY_SNAPSHOT_ENCODING_UNSUPPORTED','encoding','authority-provenance',String(g.encoding));return decodeContent(g.content,'authority-legacy-snapshot')}
