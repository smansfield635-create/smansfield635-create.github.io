...
export const H_EARTH_RENDERER_CORRIDOR_INTEGRATION_HARNESS_CONTRACT_ID =
  'H_EARTH_RENDERER_CORRIDOR_INTEGRATION_HARNESS_v1';

const PROFILES = Object.freeze([
  Object.freeze({
    id: 'SMALL_MOBILE_PORTRAIT_DPR_2',
  ...
  Object.freeze({
    id: 'LARGE_MOBILE_PORTRAIT_DPR_3',
...
async function runProfile(browser, baseUrl, profile, outputDirectory) {
  ...
  await writeJson(
    path.join(outputDirectory, `${profile.id}.receipt.json`),
    receipt
  );
...
async function main() {
  ...
  process.stdout.write(
    `${JSON.stringify({
      status: aggregateReceipt.localIntegrationPassEstablished
        ? 'PASS'
        : 'FAIL',
  ...
  if (!aggregateReceipt.localIntegrationPassEstablished) {
    process.exitCode = 1;
  }
}
