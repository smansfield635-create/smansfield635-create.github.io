...
if(!state||!state.dragonLoop)return 0;
...
function getBannerPair(dragonType,language,state){
const role=PHRASE_ROLE_BY_DRAGON[dragonType]||"alignment";
const cycle=getCycleIndex(state);
const sceneLang=language==="zh"?"zh":language==="es"?"es":"en";
...
function getMoonState(state){
if(
state &&
state.background &&
state.background.moon &&
typeof state.background.moon.x==="number" &&
typeof state.background.moon.y==="number"
){
return state.background.moon;
}
return null;
}
...
ctx.strokeStyle=color;
ctx.globalAlpha=0.16*DRAGON_RULES.WAKE_INTENSITY;
ctx.lineWidth=2.8;
ctx.beginPath();
...
ctx.rotate(a);
ctx.globalAlpha=0.28*DRAGON_RULES.COLOR_DIM;
ctx.fillStyle=color;
ctx.beginPath();
...
ctx.rotate(a);
ctx.globalAlpha=0.40*DRAGON_RULES.GLOW_INTENSITY;
...
ctx.globalAlpha=0.24*DRAGON_RULES.GLOW_INTENSITY;
...
if(dragonType==="fear")drawFearFire(ctx,head,next,tick);
}
...
function drawBanner(ctx,bundle,dragonType,language,state){
...
const phrase=getBannerPair(dragonType,language,state);
...
function drawDragonHalf(ctx,bundle,dragonType,baseFill,glowColor,accent,front,tick,language,state){
...
ctx.save();
ctx.globalAlpha=(0.70-(t*0.14))*clamp(1+p.z*0.10,0.66,1.08)*DRAGON_RULES.COLOR_DIM;
...
if(i%3===0){
drawScalePatch(ctx,p.x+radius*0.10,p.y-radius*0.16,a,radius*0.12,radius*0.06,"rgba(255,244,210,0.20)",0.12);
}
}

if(front){
drawBanner(ctx,bundle,dragonType,language,state);
...
ctx.save();
ctx.globalAlpha=0.36*DRAGON_RULES.REFLECTION_INTENSITY;
...
if(bundles.fear){
drawDragonHalf(ctx,bundles.fear,"fear","rgba(92,20,18,0.82)","rgba(148,58,34,0.34)","rgba(176,98,62,0.48)",true,tick,language,state);
}
if(bundles.wisdom){
drawDragonHalf(ctx,bundles.wisdom,"wisdom","rgba(112,92,24,0.78)","rgba(190,168,84,0.26)","rgba(212,188,112,0.42)",true,tick,language,state);
}
}

window.OPENWORLD_DRAGON_RENDERER=Object.freeze({
version:"OPENWORLD_DRAGON_RENDERER_v4",
getDragonBundles,
