...
vec3 densityAt(vec3 p){
  ...
  for(int i=0;i<8;i++){
    ...
    float den=shape*c.x;mass+=den;iceMass+=den*c.z;precipMass+=den*c.w;
  }
...
export function createExteriorWeatherProjectionGA({renderer,worldCanvas,getSunDirection}={}){
  ...
  const overlay=document.createElement('canvas');overlay.dataset.audraliaExteriorWeather='true';overlay.dataset.fap1GA='true';overlay.setAttribute('aria-hidden','true');Object.assign(overlay.style,{position:'absolute',pointerEvents:'none',zIndex:'2',background:'transparent',imageRendering:'auto'});parent.appendChild(overlay);
  ...
  const program=makeProgram(gl),vao=gl.createVertexArray();let interaction=false,lastPacket=null;
  const names=['uEye','uForward','uRight','uUp','uSunDir','uAspect','uTanHalfFov','uTimeHours','uNearCutoff','uNearFade','uExteriorScale','uOpacity','uSystemCount','uClearCount','uStepCount','uFullDetail','uSysA[0]','uSysB[0]','uSysC[0]','uSysD[0]','uClearA[0]'];
