...
if(JSON.stringify(s.genuineManipulation?.frameInterval)!==JSON.stringify([303,368])||JSON.stringify(s.genuineManipulation?.timelineProgressInterval)!==JSON.stringify([0.59,0.83]))fail('TIMELINE_MANIPULATION_WINDOW');
if(JSON.stringify(s.frameInterval)!==JSON.stringify([144,414])||JSON.stringify(s.secondsInterval)!==JSON.stringify([4.8,13.8])||s.proof?.ownerPreviewDurationSeconds!==9)fail('S01_RETIMING');
if(s.brandDescriptor!=='Independent Interactive Experience & Research Studio'||s.introContextDisposition!=='DEFERRED_TO_REMAINDER_OF_CINEMATIC_NOT_RENDERED_AS_S01_FRAME'||JSON.stringify(s.introPresentationOrder)!==JSON.stringify(['BRAND_IDENTITY_WITH_DESCRIPTOR']))fail('S01_BRAND_PRESENTATION');
...
if(s.proof?.ownerVisibleBrowserProofRequired!==true||s.proof?.ownerAcceptance!=='ACCEPTED'||s.proof?.formalBrowserProofStatus!=='PENDING'||s.proof?.nextShotConstructionAuthorized!==false)fail('OWNER_GATE');
if(s.proof?.ownerAcceptanceBasis?.previewVersion!==3||s.proof?.ownerAcceptanceBasis?.videoSha256!=='14f991099dc4f71b8b43a41377ff15642faa6e4f0ba6c78039e4938eea7c4675'||s.proof?.ownerAcceptanceBasis?.sequencePosition!=='IMMEDIATELY_AFTER_ACCEPTED_S00_DISSOLVE_PIXELATION_AND_SUCCESSOR_MATTER')fail('OWNER_ACCEPTANCE_BASIS');
...
for(const token of [
  ...
  ...ids,'SAME_STARS_FRAME_DGB_IDENTITY','SAME_STARS_CONVERGING','APPROVED_FOUR_CARDINAL_COMPASS','x:star.matter[0],y:star.matter[1]'
]) if(!stage.includes(token))fail('S01_STAGE_TOKEN_'+token);
for(const interactiveToken of ['function manipulate(','pointerdown','pointermove','setPointerCapture','DRAG TO REORIENT','THE COMPASS REORIENTS','Discover the estate.','Discover our ideas, enter the story, or explore your own.','t*.22'])if(stage.includes(interactiveToken))fail('INTERACTIVE_UNFROZEN_OR_DISCONTINUOUS_TOKEN_'+interactiveToken);
...
process.stdout.write(JSON.stringify({schema:'R11A_SOURCE_TRUTH_SHOT_VERIFICATION_RECEIPT_v1',result:'PASS_AT_SOURCE_AND_OWNER_ACCEPTANCE_CONTRACT_LEVEL',shots:['S00','S01'],manifestSha256:digest,acceptedS00Preserved:true,persistentObjectIds:ids,objectReplacementCount:0,genuineManipulation:'TIMELINE_DRIVEN_3D_REORIENTATION',starMotionContinuity:'PASS_NO_PROGRESS_0_24_POSITION_RESET',manipulationFrameInterval:[303,368],ownerPreviewDurationSeconds:9,brandDescriptor:s.brandDescriptor,introContextPresentation:'DEFERRED_TO_REMAINDER_OF_CINEMATIC_NO_S01_FRAME',audienceInteractionRequired:false,interactiveSubsystemCreated:false,terminalForeground:'ORIENTATION',ownerAcceptance:'ACCEPTED',ownerAcceptanceBasis:s.proof.ownerAcceptanceBasis,formalBrowserProofStatus:'PENDING',nextShotConstructionAuthorized:false,fullMasterAuthorized:false,liveMutationAuthorized:false},null,2)+'\n');
