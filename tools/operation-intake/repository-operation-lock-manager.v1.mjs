...
async function readLedgerBlob({repository,token,blobSha}){const sha=dig(blobSha,40,'legacySnapshotBlob','authority-provenance'),url=`${base(repository)}/git/blobs/${sha}`;let z=await fetch(url,{headers:{...H(token),Accept:'application/vnd.github.raw+json'}});if(z.ok){const raw=await z.text();try{return ledger(JSON.parse(raw))}catch(e){if(e.code)throw e;throw err('LEDGER_JSON_DECODE_FAILURE','content','authority-legacy-snapshot-raw',e.message)}}const g=await req(url,{headers:H(token)},[200],'AUTHORITY_LEDGER_BLOB');if(g.encoding!=='base64')throw err('AUTHORITY_LEGACY_SNAPSHOT_ENCODING_UNSUPPORTED','encoding','authority-provenance',String(g.encoding));return decodeContent(g.content,'authority-legacy-snapshot')}
