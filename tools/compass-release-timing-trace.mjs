...
let changed=false;for(const [sx,sy,ex,ey] of patterns){const before=await snap();const x1=b.x+b.width*sx,y1=b.y+b.height*sy,x2=b.x+b.width*ex,y2=b.y+b.height*ey;await client.send('Input.dispatchTouchEvent',{type:'touchStart',touchPoints:point(x1,y1)});for(let i=1;i<=18;i++){const t=i/18;await client.send('Input.dispatchTouchEvent',{type:'touchMove',touchPoints:point(x1+(x2-x1)*t,y1+(y2-y1)*t)});await new Promise(r=>setTimeout(r,22))}await new Promise(r=>setTimeout(r,100));await client.send('Input.dispatchTouchEvent',{type:'touchEnd',touchPoints:[]});await new Promise(r=>setTimeout(r,40));const now=await snap();if(now.focus&&now.focus!==before.focus){changed=true;break}await new Promise(r=>setTimeout(r,1100));}
if(!changed)throw new Error('no committed transition');
const marks=[0,250,500,750,1050,1400,1800,2400];const samples=[];let last=0;for(const ms of marks){await new Promise(r=>setTimeout(r,ms-last));last=ms;const s=await snap();const incoming=s.cards.find(x=>x.wing===s.focus);samples.push({ms,focus:s.focus,phase:s.phase,anchorError:incoming?Math.hypot(incoming.cx-anchor.cx,incoming.cy-anchor.cy):null,cx:incoming?.cx??null,cy:incoming?.cy??null})}
console.log(JSON.stringify({result:'COMPASS_RELEASE_TIMING_TRACE',anchor,samples},null,2));await browser.close();
