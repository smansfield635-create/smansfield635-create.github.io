...
ok('POS_006_P2_BRIDGE_HEAD', selectedP2.bridgeImplementationHead === '3684c3fecf47d1ee4213a9c636cfa15606087cb5');
ok('POS_007_P2_FIVE_PATH_SCOPE', selectedP2.bridgeRequest.operationRequest.allowedPaths.length === 5 && selectedP2.bridgeRequest.constructionProcedure.exactAllowedRepositoryPaths.length === 5);
ok('POS_008_P2_SCOPE_EQUALITY', JSON.stringify(selectedP2.bridgeRequest.operationRequest.allowedPaths) === JSON.stringify(selectedP2.bridgeRequest.constructionProcedure.exactAllowedRepositoryPaths));
ok('POS_009_NO_ARBITRARY_PAYLOAD', selectedP2.arbitraryPayloadAccepted === false);
ok('POS_010_NO_AUTHORITY_CREATED', selectedP2.authorityCreated === false && selectedP2.repositoryMutationPerformed === false);
ok('POS_011_P2_PRODUCT_AUTHORITY_FALSE', selectedP2.bridgeRequest.operationRequest.requestingAuthority.productAuthority === false);
...
const receipt = {schema:'FIXED_FIXTURE_INTAKE_SELF_TEST_RECEIPT_v1', result:'PASS_CLOSED', checkCount:checks.length, checks, fixtureIds:[p1,p2], externalInputSurface:['fixtureId','action'], arbitraryPayloadAccepted:false, canonicalGateModified:false};
