...
export const H_EARTH_GEN307_FINAL_EXPRESSION_PREFLIGHT_REGISTERED_PATHS = Object.freeze([
  ...H_EARTH_GEN307_FINAL_EXPRESSION_PREFLIGHT_TARGET_PATHS,
  AMENDMENT_PATH
]);
...
const OCCURRENCES = Object.freeze([
  ...
  deepFreeze({
    ...
    byteCount: null,
    existenceStatus: 'ABSENT',
    fetchbackStatus: 'VERIFIED_ABSENT_AT_PREDECESSOR_TOOLING_HEAD_BEFORE_GEN307_SCOPE_REPAIR',
...
export const H_EARTH_GEN307_FINAL_EXPRESSION_PREFLIGHT_SCOPE_EVIDENCE = deepFreeze({
  ...
  assertionScope: Object.freeze([
    'EXACT_TWO_GEN307_FINAL_EXPRESSION_FUTURE_PATHS',
    'TRUTHFUL_ABSENCE_AT_FROZEN_GEN306_QUALIFIED_PARENT',
  ...
  evidenceLimitations: Object.freeze([
    ...
    'NO_CONSTRUCTION_AUTHORITY_CREATED_BY_REGISTRY',
    'NO_GEOGRAPHY_NAVIGATION_RENDERER_OR_DEPLOYMENT_AUTHORITY',
    'NO_MERGE_RELEASE_OR_PUBLICATION_AUTHORITY'
  ])
});
...
export const H_EARTH_GEN307_FINAL_EXPRESSION_PREFLIGHT_SCOPE_NODE = deepFreeze({
  ...
  nodeSubtype: 'GEN307_AUTHORIZED_FUTURE_FINAL_EXPRESSION_PATH_PREFLIGHT_SCOPE',
  ...
  repositoryPaths: [...H_EARTH_GEN307_FINAL_EXPRESSION_PREFLIGHT_REGISTERED_PATHS],
  repositoryOccurrences: OCCURRENCES,
  evidenceClass: H_EARTH_GEN307_FINAL_EXPRESSION_PREFLIGHT_SCOPE_EVIDENCE.evidenceClass,
  ...
  authoritySource: Object.freeze([
    'GEN307_LOCK_GENERATION_307',
    OPERATION_ID,
    'GEN307_EXACT_ELEVEN_PATH_MUTATION_ROUTER_PASS_RUN_32085260786',
    'GEN307_NATIVE_PREFLIGHT_REQUESTED_PATH_UNRESOLVED_RUN_32085360964'
  ]),
  ...
  authorityLimitations: Object.freeze([
    'NO_PRODUCT_MUTATION',
    'NO_CONSTRUCTION_AUTHORITY_CREATED_BY_REGISTRY',
    'NO_SCOPE_EXPANSION_BEYOND_TWO_GEN307_TARGETS_AND_THIS_AMENDMENT',
  ...
  orderingRules: Object.freeze([
    'GEN307_ADMISSION_AND_ROUTER_PASS_PRECEDE_THIS_SCOPE_REGISTRATION',
    'NATIVE_PREFLIGHT_PASS_PRECEDES_ANY_GEN307_PRODUCT_MUTATION'
  ]),
  ...
  currentIdentityReferences: Object.freeze([FROZEN_PUBLIC_BASE, TOOLING_PREDECESSOR_HEAD, PRIVATE_GOVERNING_HEAD, 'LOCK_GENERATION=307', OPERATION_ID]),
  unresolvedFields: Object.freeze([])
});

const pathIndex = new Map(H_EARTH_GEN307_FINAL_EXPRESSION_PREFLIGHT_REGISTERED_PATHS.map((repositoryPath) => [repositoryPath, deepFreeze({
  nodes: Object.freeze([H_EARTH_GEN307_FINAL_EXPRESSION_PREFLIGHT_SCOPE_NODE]),
...
const combinedInstance = deepFreeze({
  ...baseInstance,
  evidenceRecords: [...(baseInstance.evidenceRecords ?? []), H_EARTH_GEN307_FINAL_EXPRESSION_PREFLIGHT_SCOPE_EVIDENCE],
  nodes: [...baseInstance.nodes.filter((node) => node.nodeId !== NODE_ID), H_EARTH_GEN307_FINAL_EXPRESSION_PREFLIGHT_SCOPE_NODE]
});
...
export function resolveHEarthRepositoryRegistryOccurrence(input = {}) {
  ...
  const localMatches = OCCURRENCES.filter((entry) => {
    ...
    return true;
  }).map((occurrence) => deepFreeze({nodeId: NODE_ID, node: H_EARTH_GEN307_FINAL_EXPRESSION_PREFLIGHT_SCOPE_NODE, occurrence}));
...
export function verifyHEarthGen307FinalExpressionPreflightScopeRegistration() {
  const pathChecks = H_EARTH_GEN307_FINAL_EXPRESSION_PREFLIGHT_REGISTERED_PATHS.map((repositoryPath) => {
    ...
    const occurrence = (resolution.occurrences ?? []).find((entry) => entry.path === expected.path && entry.commitSha === expected.commitSha && entry.gitBlobSha === expected.gitBlobSha && entry.existenceStatus === expected.existenceStatus);
  ...
  const checks = deepFreeze({
    exactTwoTargetPaths: H_EARTH_GEN307_FINAL_EXPRESSION_PREFLIGHT_TARGET_PATHS.length === 2,
    exactThreeRegisteredPathsIncludingSelf: H_EARTH_GEN307_FINAL_EXPRESSION_PREFLIGHT_REGISTERED_PATHS.length === 3,
    ...
    exactLockGeneration: LOCK_GENERATION === 307,
    exactFrozenPublicBase: FROZEN_PUBLIC_BASE === 'dbf2f3ec90faa59c09193e61a0d141481b68d758',
    exactPredecessorToolingHead: TOOLING_PREDECESSOR_HEAD === '46b2cca200f1db036cae8288329f870a8336505a',
    predecessorRun8EResolutionPreserved: predecessorRun8E.resolved === true,
    predecessorGen306HarnessResolutionPreserved: predecessorGen306Harness.resolved === true,
    noProductAuthority: H_EARTH_GEN307_FINAL_EXPRESSION_PREFLIGHT_SCOPE_NODE.authorityLimitations.includes('NO_PRODUCT_MUTATION'),
    noConstructionAuthorityCreatedByRegistry: H_EARTH_GEN307_FINAL_EXPRESSION_PREFLIGHT_SCOPE_NODE.authorityLimitations.includes('NO_CONSTRUCTION_AUTHORITY_CREATED_BY_REGISTRY')
  });
  const eligible = Object.values(checks).every(Boolean);
  return deepFreeze({eligible, status: eligible ? 'H_EARTH_GEN307_FINAL_EXPRESSION_PREFLIGHT_SCOPE_REGISTRATION_PASS' : 'H_EARTH_GEN307_FINAL_EXPRESSION_PREFLIGHT_SCOPE_REGISTRATION_FAIL', checks, pathChecks, issues: eligible ? [] : Object.entries(checks).filter(([,pass]) => !pass).map(([name]) => name)});
}
...
export default deepFreeze({
  ...
  verifyHEarthGen307FinalExpressionPreflightScopeRegistration
});
