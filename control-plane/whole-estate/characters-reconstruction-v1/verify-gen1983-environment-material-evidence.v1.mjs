...
const CONTRACT=JSON.parse(fs.readFileSync(path.join(HERE,'gen1983-environment-material-evidence-contract.v1.json'),'utf8'));
const args={};
...
check('MANIFEST_SCHEMA',manifest.schema==='MIRRORLAND_GEN1983_ENVIRONMENT_MATERIAL_EVIDENCE_MANIFEST_v1',{observed:manifest.schema});
...
check('GOVERNING_HEAD_EXACT',manifest.governingHead===CONTRACT.governingHead,{observed:manifest.governingHead});
...
check('INDEPENDENT_INTERIOR_REQUIRED_DESTINATION',witness.destinationId===CONTRACT.independentInteriorRule.requiredDestinationId&&witness.siteId===CONTRACT.independentInteriorRule.requiredSiteId,{destinationId:witness.destinationId,siteId:witness.siteId});
...
check('INDEPENDENT_INTERIOR_NOT_MANOR',witness.destinationId!=='manor'&&witness.distinctFromManor===true,{destinationId:witness.destinationId,distinctFromManor:witness.distinctFromManor});
...
check('INDEPENDENT_INTERIOR_SELECTION_LAW',witness.selectionLaw===CONTRACT.independentInteriorRule.selection,{observed:witness.selectionLaw,expected:CONTRACT.independentInteriorRule.selection});

const failed=checks.filter(x=>!x.pass);
const receipt={
  schema:'MIRRORLAND_GEN1983_ENVIRONMENT_MATERIAL_EVIDENCE_VERIFICATION_RECEIPT_v1',
