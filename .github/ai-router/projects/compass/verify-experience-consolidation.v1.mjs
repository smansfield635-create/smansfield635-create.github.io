...
check('COMPACT_STAGE_WITH_CLEARANCE',files.capabilityCss.includes('min-height:28.5rem')&&files.capabilityCss.includes('grid-template-rows:17.6rem auto auto auto')&&files.capabilityCss.includes('.compass-object-caption')&&files.capabilityCss.includes('overflow:visible')&&files.capabilityCss.includes('.compass-action-dock'));
...
check('RESPONSIVE_ROTATION',files.capabilityJs.includes("stage.addEventListener('pointerdown'")&&files.capabilityJs.includes('settleDelay=reduce.matches?0:300')&&!files.capabilityJs.includes('setTimeout(()=>settle(),570)')&&!files.capabilityJs.includes('if(dragging||settling)return'));
...
check('BRAIN_SELF_CONTAINED_WEBGL',files.brainJs.includes(OBJECT_CONTRACT)&&files.brainJs.includes('anatomical-webgl-v6')&&files.brainJs.includes('canvas2d-anatomical-fallback-v1')&&!/compass-capability-orbit|compass-orbit-plaque|compass-object-caption/.test(files.brainJs));
...
check('TROPHY_RENDERER_BOUNDED',files.trophyJs.includes('procedural-webgl-trophy-v3')&&!/compass-capability-orbit|compass-orbit-plaque|compass-object-caption/.test(files.trophyJs));
check('TROPHY_FALLBACK_EXCLUSIVE',files.capabilityJs.includes('class="compass-trophy-fallback" hidden aria-hidden="true"')&&files.capabilityCss.includes('.compass-trophy-fallback{position:absolute;inset:0;z-index:1;display:none!important')&&files.capabilityCss.includes('.compass-award-trophy.is-fallback .compass-trophy-fallback{display:block!important}'));
