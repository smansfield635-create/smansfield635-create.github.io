...
const OPERATION='COMPASS_VOLUMETRIC_ARTIFACTS_BOUNDED_SUCCESSOR_20260817_v1';
const GOVERNING='5166e04086efb2eb1bdd1ab043c18c84cef739ad';
const OUTPUT=process.env.COMPASS_PERFORMANCE_OUTPUT||'/tmp/compass-volumetric-performance.json';
const files={
  input:'assets/compass/compass.orbit-input.js',statement:'assets/compass/compass.statement-carousel.js',statementCss:'assets/compass/compass.statement-carousel.css',brain:'assets/compass/compass.brain-scene.js',trophy:'assets/compass/compass.trophy-scene.js',capability:'assets/compass/compass.capability-carousel.js',capabilityCss:'assets/compass/compass.capability-carousel.css',index:'index.html'
};
const budgets={input:4096,statement:6144,statementCss:4096,brain:16384,trophy:12288,capability:16384,capabilityCss:16384};
...
const legacyKeys=['input','statement','statementCss','brain','capability','capabilityCss'],legacyAggregate=legacyKeys.reduce((sum,key)=>sum+bytes[key],0);
check('SIX_ASSET_AGGREGATE_UNDER_48_KIB',legacyAggregate<=49152,{bytes:legacyAggregate,protectedFloorBytes:48595,max:49152,justification:'generation-1514 six-module protected floor remains within inherited ceiling'});
const baselineFiles=legacyKeys.map(key=>files[key]),show=file=>spawnSync('git',['show',`${GOVERNING}:${file}`],{encoding:null,maxBuffer:2_000_000}),baselineBytes=baselineFiles.reduce((sum,file)=>{const r=show(file);return sum+(r.status===0?r.stdout.length:0)},0),currentProductBytes=legacyAggregate+bytes.trophy,rawProductDelta=currentProductBytes-baselineBytes;
check('VOLUMETRIC_PRODUCT_DELTA_UNDER_12_KIB',rawProductDelta>=0&&rawProductDelta<=12288,{governingHead:GOVERNING,baselineBytes,currentProductBytes,rawProductDelta,maxDelta:12288});
...
check('TROPHY_LOCAL_SCENE_REFERENCE',text.capability.includes('/assets/compass/compass.trophy-scene.js?v=compass-volumetric-artifacts-v1')&&!/https?:\/\//.test(text.trophy),'local procedural scene only');
check('RUNTIME_IDENTITY_BOUND',text.capability.includes("version:'root-award-finish-successor-v1'")&&text.capability.includes("volumetricArtifacts:'generation-1515'"),'protected identity plus generation-1515 volumetric marker');
...
check('TROPHY_CONVENTIONAL_STRUCTURE',['compass-award-trophy','compass-trophy-bowl','compass-trophy-handle--left','compass-trophy-handle--right','compass-trophy-stem','compass-trophy-base','compass-trophy-gleam'].every(value=>text.capability.includes(value))&&text.capability.includes('<div class="compass-award-object" aria-hidden="true">'),'decorative WebGL trophy retains compact recognizable fallback structure');
check('TROPHY_ANTIQUE_GOLD_PALETTE',['#f4d680','#c89e45','#6f4b1e','#fff2be'].every(value=>text.capabilityCss.toLowerCase().includes(value)),'frozen satin/antique gold fallback palette');
...
check('TROPHY_PROCEDURAL_TRUE_DEPTH',text.trophy.includes("version:'procedural-webgl-v1'")&&text.trophy.includes("lathe('bowl-outer'")&&text.trophy.includes("lathe('bowl-inner'")&&text.trophy.includes("handle('handle-left'")&&text.trophy.includes("handle('handle-right'")&&text.trophy.includes("lathe('stem'")&&text.trophy.includes("lathe('base'")&&text.trophy.includes('gl.enable(gl.DEPTH_TEST)')&&text.trophy.includes('gl.readPixels'),'concavity + backside-capable procedural mesh + depth buffer + morphology capture');
check('TROPHY_PBR_LIKE_LIGHT_RESPONSE',text.trophy.includes('reflect(-key,n)')&&text.trophy.includes('spec=pow')&&text.trophy.includes('rim=pow')&&text.trophy.includes("trophyMaterial:'SATIN_ANTIQUE_GOLD_BRONZE_V2'"),'view-dependent specular plus cool environmental rim');
check('TROPHY_FINITE_WEBGL_MOTION',text.trophy.includes('(t-start)/800')&&text.trophy.includes("trophyMotion:reduce.matches?'static-reduced-motion':'finite-arrival-yaw'")&&!text.trophy.includes('setInterval('),'800ms bounded arrival yaw; no perpetual timer');
check('TROPHY_WEBGL_FALLBACK',text.trophy.includes("field?.classList.add('is-fallback')")&&text.capability.includes('compass-trophy-fallback'),'WebGL-denied static trophy remains available');
...
check('BRAIN_DEEP_VARIED_FOLDS_AND_SCALE',text.brain.includes('theta*25.3-phi*15.1')&&text.brain.includes('foldCross')&&text.brain.includes('lateralRadius=.73')&&text.brain.includes('float sx=.86/max(aspect,1.)'),'varied physical sulci/gyri and specimen occupancy');
check('BRAIN_NO_STRIPE_SHADER_DOMINANCE',text.brain.includes("brainVolumetricPass:'generation-1515'")&&text.brain.includes("brainLighting:'warm-key,cool-rim,dark-fill'")&&!text.brain.includes('folds=.78+.22*sin'),'relief normals + directional world lighting replace repetitive fragment stripes');
...
const failures=checks.filter(check=>!check.pass);const receipt={schema:'COMPASS_VOLUMETRIC_ARTIFACTS_STATIC_QUALIFICATION_v1',operationId:OPERATION,lockGeneration:1515,governingHead:GOVERNING,candidateHead,productIdentity:VERSION,volumetricIdentity:'generation-1515',result:failures.length?'FAIL_CLOSED':'PASS_CLOSED',assetBytes:bytes,legacyAggregateBytes:legacyAggregate,currentProductBytes,rawProductDelta,checks,failures:failures.map(check=>check.id)};
