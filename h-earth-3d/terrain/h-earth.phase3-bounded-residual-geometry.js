...
const admittedAt=(x,z,ceiling)=>{if(x%8===0&&z%8===0)return 0;const hood=[];for(let dz=-4;dz<=4;dz+=4)for(let dx=-4;dx<=4;dx+=4)if(!((x+dx)%8===0&&(z+dz)%8===0))hood.push(Math.abs(phase3RawResidual(x+dx,z+dz)));const bound=Math.min(ceiling,3*median(hood)+.35);return clamp(phase3RawResidual(x,z),-bound,bound)}
export function sampleHEarthPhase3BoundedResidual(x,z,{perimeter=false}={}){if((x%8===0&&z%8===0)||perimeter)return 0;const ceiling=ceilingForArticulation(deriveHEarthGen311RegionalArticulation(x,z)),center=admittedAt(x,z,ceiling);const smooth=(admittedAt(x-4,z,ceiling)+2*center+admittedAt(x+4,z,ceiling)+admittedAt(x,z-4,ceiling)+2*center+admittedAt(x,z+4,ceiling))/8;return clamp(smooth,-ceiling,ceiling)}
