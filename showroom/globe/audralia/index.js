...
/* AUDRALIA_CANVAS_SERVED_SOURCE_ALIGNMENT_TNT_v17
   ...
   Jurisdiction:
   - served-source canvas proof
   - adopted canvas authority
   - square-shell obedience
   - no parent/stage expansion
   - intrinsic canvas pixel sizing
   - render radius
...
import RuntimeDefault, {
  ...
  getStatus as getAudraliaRuntimeStatus
} from "./audralia.runtime.js?v=audralia-served-source-v17";

const AUDRALIA_CANVAS_CONTRACT = "AUDRALIA_CANVAS_SERVED_SOURCE_ALIGNMENT_TNT_v17";
...
function getRuntimeAuthority() {
  ...
  runtimeAuthority = window.AudraliaRuntimeTruth || null;
  return runtimeAuthority;
}
...
function sampleSurface(lon, lat) {
  ...
  return {
    ...
    shelfIndex: !land && waterNoise > 0.62 ? 0.8 : 0.1,
    terrainRelief: land ? 0.38 : 0,
    terrainReliefIndex: land ? 0.38 : 0,
    coastBand: waterNoise > 0.64 && waterNoise < 0.78 ? 0.8 : 0.1,
    coastlineIndex: waterNoise > 0.64 && waterNoise < 0.78 ? 0.8 : 0.1,
    runoff: waterNoise > 0.64 && waterNoise < 0.78 ? 0.34 : 0,
    mineral: waveNoise(lon, lat, 18, 6.2),
    mineralIndex: waveNoise(lon, lat, 18, 6.2),
...
function measureContainedShell(shell, mount, canvas, options) {
  const controlBox = shell || mount;
  const rect = controlBox.getBoundingClientRect();
  const viewportWidth = window.innerWidth || document.documentElement.clientWidth || 390;
  const fallbackSize = Math.min(viewportWidth * 0.86, 352);
  const cssSize = Math.max(240, Math.floor(Math.min(rect.width || fallbackSize, rect.height || rect.width || fallbackSize)));

  const profile = resolveProfile(cssSize, options);
  const dpr = clamp(window.devicePixelRatio || 1, 1, options.devicePixelClamp);
  const intrinsicPixelSize = Math.max(240, Math.floor(cssSize * dpr));
...
class AudraliaCanvasController {
  ...
  resize() {
    ...
    this.dimension = measureContainedShell(this.shell, this.mount, this.canvas, this.options);
  ...
  draw() {
    ...
    this.canvas.dataset.visualDefinition = "served-source-contained-square";
...
function renderAudraliaCanvas(options = {}) {
  ...
  const explicitCanvas = resolveElement(options.canvas);
