...
if(!initialAnchor){
  ...
  if(!b){
    ...
    const geometryEvidence=[];let geometryPass=!transitionNavigation&&transitions.length>=3;
    for(const t of transitions){const beforePrimary=primaryOf(t.before);const transitionAnchor=beforePrimary?.rect?{cx:beforePrimary.rect.cx,cy:beforePrimary.rect.cy}:null;const incoming=t.after.cardinals.find(x=>x.wing===t.after.root.focus),outgoing=t.after.cardinals.find(x=>x.wing===t.before.root.focus),incomingEarly=t.early.cardinals.find(x=>x.wing===t.after.root.focus);const anchorError=(incoming?.rect&&transitionAnchor)?Math.hypot(incoming.rect.cx-transitionAnchor.cx,incoming.rect.cy-transitionAnchor.cy):Infinity;const outgoingDistance=(outgoing?.rect&&transitionAnchor)?Math.hypot(outgoing.rect.cx-transitionAnchor.cx,outgoing.rect.cy-transitionAnchor.cy):0;const drift=(incoming?.rect&&incomingEarly?.rect)?Math.hypot(incoming.rect.cx-incomingEarly.rect.cx,incoming.rect.cy-incomingEarly.rect.cy):Infinity;const labelPass=t.after.primary.length===1&&t.after.primary[0]===t.after.root.focus&&t.after.visibleLabels.length===1&&t.after.visibleLabels[0]===t.after.root.focus;const onePass=Boolean(transitionAnchor)&&anchorError<=SETTLED_ANCHOR_TOLERANCE&&outgoingDistance>=OLD_PRIMARY_DEPARTURE_MIN&&drift<=SETTLED_DRIFT_TOLERANCE&&labelPass;geometryPass&&=onePass;geometryEvidence.push({from:t.before.root.focus,to:t.after.root.focus,phase:t.after.root.phase,anchor:transitionAnchor,anchorError,outgoingDistance,settledDrift:drift,primary:t.after.primary,visibleLabels:t.after.visibleLabels,status:onePass?'PASS':'FAIL'});}
    add('AUDIT_SETTLED_GEOMETRY',geometryPass,{requiredDistinctTransitions:3,observedTransitions:transitions.length,anchorPolicy:'PER_TRANSACTION_PRE_DRAG_PRIMARY_STABILIZED_VIEWPORT_FRAME',initialAnchor,tolerances:{anchor:SETTLED_ANCHOR_TOLERANCE,outgoingDeparture:OLD_PRIMARY_DEPARTURE_MIN,drift:SETTLED_DRIFT_TOLERANCE},navigation:transitionNavigation,inputBoundary:'LAWS_STYLE_POINTER_TRANSACTION',stabilizationSamples,transactions:attempts,transitions:geometryEvidence});
