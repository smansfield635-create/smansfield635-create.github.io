    ...
    try {
      for (const route of roomCarouselRoutes) {
        await executePageCase(context, { check: 'room-carousel-runtime', profile: profile.name, route }, async page => {
          ...
          const inspectionIndex = await carouselRoot.getAttribute('data-lrc-index');
          assert(await carouselRoot.locator(':scope > [data-lr-tabs]').count() === 0, `${route}: internal reading tabs escaped the carousel card`);
          ...
          const internalTabGroups = activeCard.locator(':scope > [data-lrc-source-child] [data-lr-tabs]');
          for (let groupIndex = 0; groupIndex < await internalTabGroups.count(); groupIndex += 1) {
            assert(await internalTabGroups.nth(groupIndex).isVisible(), `${route}: internal reading tabs hidden inside opened card`);
          }
          const returnToOrbit = activeCard.locator(':scope > [data-lrc-return]');
          assert(await returnToOrbit.count() === 1, `${route}: Return to Orbit action missing from opened card`);
          assert(/Return to Orbit/i.test((await returnToOrbit.textContent()) || ''), `${route}: Return to Orbit label drift`);
          await measuredPointerClick(page, returnToOrbit, `${route}: Return to Orbit`, { route, profile, inspectionIndex });
          await page.waitForFunction(() => !document.querySelector('[data-laws-room-carousel]')?.dataset.lrcInspecting);
          assert(await carouselRoot.getAttribute('data-lrc-index') === inspectionIndex, `${route}: Return to Orbit failed to restore the same carousel card`);
          ...
          return {
            ...
            internalTabsContainedByOpenedCard: true,
            returnToOrbitRestoredSameCard: true,
...
async function main() {
  ...
  const summary = {
    ...
    roomCarouselInternalTabsInsideOpenedCard: failureIncludes('internal reading tabs') ? 'FAIL' : 'PASS',
    roomCarouselReturnToOrbitRestoresSameCard: failureIncludes('Return to Orbit') ? 'FAIL' : 'PASS',
    ...
    lawsRootCurrentCompassAndDestinationCarousel: failureIncludes('/laws/:') ? 'FAIL' : 'PASS',
    persistentFirstRail: 'NOT_APPLICABLE_CURRENT_ROOT_CONTRACT',
