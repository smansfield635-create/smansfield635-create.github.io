/* TNT — /gauges/gauges_health_runtime.js
   GAUGES HEALTH RUNTIME · GENERATION 2 COMPASS COCKPIT · BRAVO 5
   VERSION = "spine-canopy-parachute-b5"
   PURPOSE:
     - Scan cockpit-aware root source and runtime
     - Verify root-compass-cockpit-b1
     - Verify DGBCompassCockpit
     - Verify visible sun truth
     - Verify canopy truth
     - Verify falseHealthBlocked
*/

(function () {
  "use strict";

  var VERSION = "spine-canopy-parachute-b5";
  ...
  var FILES = [
    ...
    },
    {
      ...
      path: "/gauges/gauges_health_runtime.js?v=spine-canopy-parachute-b5",
      required: [
        "spine-canopy-parachute-b5",
        "root-compass-cockpit-b1",
        "DGBCompassCockpit",
  ...
  function makeRow(a, status, finding) {
  ...
  function scoreFile(file, result) {
    ...
    return {
      ...
      score: Math.max(30, 100 - missing.length * 18),
  ...
  function probeRootVisualTruth() {
    return new Promise(function (resolve) {
      ...
      iframe.src = "/?cockpitGaugeProbe=" + encodeURIComponent(String(Date.now()));
      ...
      iframe.onload = function () {
        window.setTimeout(function () {
          ...
          try {
            ...
            root = doc && (doc.getElementById("door-root") || doc.querySelector("[data-root-door]"));
            canopy = win && win.DGBSpineCanopy && typeof win.DGBSpineCanopy.getPublicState === "function"
              ? win.DGBSpineCanopy.getPublicState()
              : null;
            cockpit = win && win.DGBCompassCockpit && typeof win.DGBCompassCockpit.getPublicState === "function"
              ? win.DGBCompassCockpit.getPublicState()
              : null;
            indexBoot = win && win.DGBIndexBoot && typeof win.DGBIndexBoot.getPublicState === "function"
              ? win.DGBIndexBoot.getPublicState()
              : null;

            mount = doc && doc.querySelector("[data-dgb-sun-mount]");
            ...
            cockpitWindow = doc && doc.querySelector("[data-cockpit-window]");
            controls = doc && doc.querySelector("[data-cockpit-control-panel]");
            title = doc && doc.body && doc.body.textContent.indexOf("Diamond Gate Bridge") !== -1;
            theme = doc && doc.body && doc.body.textContent.indexOf("Learn to Live to Love") !== -1;
            ...
            finish({
              ...
              indexBootExecuted: Boolean(indexBoot || (win && win.DGBIndexBoot)),
              cockpitRuntimePresent: Boolean(cockpit || (win && win.DGBCompassCockpit)),
              ...
              visibleSunPresent: Boolean(
                (fallback && visibleElement(fallback)) ||
                (canvas && visibleElement(canvas)) ||
                (svg && visibleElement(svg)) ||
                (mount && visibleElement(mount))
              ),
              backgroundVisible: Boolean(background && visibleElement(background)),
  ...
  function buildVisualLedger(probe) {
    ...
    push("Root HTML served", probe.rootPresent, "Root document exists in same-origin visual probe.");
    ...
    push("Index boot executed", probe.indexBootExecuted, "window.DGBIndexBoot must exist.");
    push("Cockpit runtime present", probe.cockpitRuntimePresent, "window.DGBCompassCockpit must exist.");
    ...
    push("Visible sun present", probe.visibleSunPresent, "Canvas, SVG, fallback, or visible sun mount must exist.");
    push("Background visible", probe.backgroundVisible, "Universe background must be visible.");
    ...
    push("False health blocked", probe.visibleSunPresent && probe.cockpitRuntimePresent && probe.canopyRuntimePresent, "Strong health is forbidden when cockpit, canopy, or visible sun truth fails.");
  ...
  function computeScores() {
    ...
    var falseHealthBlocked = state.visualResults.some(function (r) {
      return r.check === "False health blocked" && r.status === "STRONG";
    });
    var visibleSunStrong = state.visualResults.some(function (r) {
      return r.check === "Visible sun present" && r.status === "STRONG";
    });
    var cockpitStrong = state.visualResults.some(function (r) {
      return r.check === "Cockpit runtime present" && r.status === "STRONG";
    });
    var canopyStrong = state.visualResults.some(function (r) {
      return r.check === "Canopy runtime present" && r.status === "STRONG";
    });
  ...
  function renderLists() {
    ...
    if (!issues.length) {
      ...
      todo.appendChild(makeLi("Continue controlled refinement under cockpit-aware canopy supervision."));
      return;
    }
    ...
    issues.slice(0, 8).forEach(function (issue) {
      ...
      item.textContent = issue.replace(/^Source issue: /, "Repair source marker or file availability: ").replace(/^Cockpit issue: /, "Repair cockpit visual-control chain: ");
