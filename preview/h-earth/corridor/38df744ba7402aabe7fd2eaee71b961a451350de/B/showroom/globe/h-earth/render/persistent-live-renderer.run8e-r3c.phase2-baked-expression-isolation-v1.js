/** H_EARTH_PHASE2_BAKED_EXPRESSION_ISOLATION_RENDERER_v1 */
import {
...
function transformTerrainFragmentShader(source) {
  if (!source.includes('vRoleCode==1u')) return source;
...
out vec4 outColor;`,'ISOLATION_UNIFORMS');
  const marker=`  if(vRoleCode==1u){`;
  const elseMarker=`  }else if(vRoleCode==2u){`;
  const a=transformed.indexOf(marker),b=transformed.indexOf(elseMarker,a);
  if(a<0||b<0)throw new Error('ISOLATION_TERRAIN_BRANCH_MISSING');
  const replacement=`  if(vRoleCode==1u){
    ...
    base=max(bakedMaterial.rgb,vec3(0.004));
    presentationContact=0.0;
    presentationHighlight=0.0;
`;
  transformed=transformed.slice(0,a)+replacement+transformed.slice(b);
...
`  float ambient=vRoleCode==1u?0.78:(0.26+0.16*clamp(n.y,0.0,1.0)+0.05*materialSignal);
  float directional=diffuse*uSunIntensity*(vRoleCode==1u?0.16:(vRoleCode==2u?0.74:0.82));
  vec3 lit=base*(ambient+directional)*uSunColor;`);
  transformed=transformed.replace(
`  float fog=rawFog*(vRoleCode==1u?0.54:0.68);`,
`  float fog=rawFog*(vRoleCode==1u?0.08:0.68);`);
  transformed=transformed.replace(
`  lit=mix(lit,vec3(luminance),clamp(fog*uDistanceDesaturationStrength*0.48,0.0,0.58));`,
`  lit=mix(lit,vec3(luminance),clamp(fog*uDistanceDesaturationStrength*(vRoleCode==1u?0.08:0.48),0.0,vRoleCode==1u?0.08:0.58));`);
  return transformed;
}
