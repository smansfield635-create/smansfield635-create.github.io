...
def main() -> None:
    ...
    text = source.read_text(encoding="utf-8").replace("003", "005")
    text = replace_once(
        text,
    ...
    correction = r'''# Align the prospective evaluator to the frozen public assertion surfaces.
...
evaluator=evaluator.replace(
    ...
    """  const meshNormalMap=new Map();
  for(let row=0;row<axes.zValues.length;row++)for(let column=0;column<axes.xValues.length;column++)meshNormalMap.set(`${axes.xValues[column]}:${axes.zValues[row]}`,mesh.verticesNormals[row*axes.xValues.length+column]);
  const pointMap=new Map();for(let x=BOUNDS.blend.xMinimum;x<=BOUNDS.blend.xMaximum;x+=2)for(let z=BOUNDS.blend.zMinimum;z<=BOUNDS.blend.zMaximum;z+=2)pointMap.set(`${x}:${z}`,sampleHEarthRun8BSuccessorTerrainField(x,z));"""
)
evaluator=evaluator.replace(
    "angle=angleDegrees(s.normal,n.normal);",
    "angle=angleDegrees(meshNormalMap.get(`${x}:${z}`),meshNormalMap.get(`${x+dx}:${z+dz}`));"
)
evaluator=evaluator.replace(
