...
async function readGitLedgerBlob({repository,token,blobSha,source='resulting-ledger',commitSha=null}){const sha=dig(blobSha,40,'ledgerBlobSha',source);const g=await req(`${base(repository)}/git/blobs/${sha}`,{headers:{...H(token),Accept:'application/vnd.github+json'}},[200],'LEDGER_BLOB_READ');let rawBytes;if(g?.encoding==='base64'&&typeof g?.content==='string')rawBytes=Buffer.from(g.content.replace(/\s/g,''),'base64');else if(typeof g==='string')rawBytes=Buffer.from(g,'utf8');else if(typeof g?.raw==='string')rawBytes=Buffer.from(g.raw,'utf8');else throw err('LEDGER_BLOB_ENCODING_UNSUPPORTED','encoding',source,'EXPECTED_SINGLE_BLOB_PAYLOAD');const actualGitBlobSha=createHash('sha1').update(Buffer.concat([Buffer.from(`blob ${rawBytes.length}\\0`,'utf8'),rawBytes])).digest('hex');if(actualGitBlobSha!==sha)throw err('LEDGER_BLOB_TRANSPORT_CORRESPONDENCE_FAILURE','content',source,JSON.stringify({requestedBlobSha:sha,actualGitBlobSha,rawByteLength:rawBytes.length,rawSha256:createHash('sha256').update(rawBytes).digest('hex'),rawFirst64Hex:rawBytes.subarray(0,64).toString('hex'),commitSha:commitSha||null}));try{return ledger(JSON.parse(rawBytes.toString('utf8')))}catch(e){if(e.code)throw e;throw err('LEDGER_JSON_DECODE_FAILURE','content',source,e.message)}}
