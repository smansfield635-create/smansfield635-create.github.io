...
// HEARTH_DIAGNOSTIC_EAST_HTML_V2_6_EXPECTATION_ALIGNMENT_TNT_v3
...
// Purpose:
// - Renew Diagnostic EAST source-read expectations to the current served Hearth HTML v2_6 contract.
...
// - Preserve Route Conductor v9_4 as current, with v9_3 and v9_2 as accepted lineage.
...
(() => {
  ...
  const ALIGNMENT_CONTRACT = "HEARTH_DIAGNOSTIC_EAST_HTML_V2_6_EXPECTATION_ALIGNMENT_TNT_v3";
  const RECEIPT = "HEARTH_DIAGNOSTIC_EAST_HTML_V2_6_EXPECTATION_ALIGNMENT_RECEIPT_v3";
  ...
  const BASELINE_ALIGNMENT_CONTRACT = "HEARTH_DIAGNOSTIC_EAST_HTML_CONTRACT_ALIGNMENT_TNT_v1";
  const BASELINE_RECEIPT = "HEARTH_DIAGNOSTIC_EAST_HTML_CONTRACT_ALIGNMENT_RECEIPT_v1";
  const VERSION = "2026-06-03.hearth-diagnostic-east-html-v2-6-expectation-alignment-v3";
  ...
  const ACCEPTED_LINEAGE_HTML_CONTRACT =
    ACCEPTED_LINEAGE_HTML_CONTRACTS[0];

  const PREVIOUS_ACCEPTED_HTML_CONTRACT =
    ACCEPTED_LINEAGE_HTML_CONTRACTS[1];
  ...
  const ACCEPTED_LINEAGE_INDEX_JS_CONTRACT =
    ACCEPTED_LINEAGE_INDEX_JS_CONTRACTS[0];
  ...
  const ACCEPTED_LINEAGE_ROUTE_CONDUCTOR_CONTRACT =
    ACCEPTED_LINEAGE_ROUTE_CONDUCTOR_CONTRACTS[0];

  const PREVIOUS_ROUTE_CONDUCTOR_CONTRACT =
    ACCEPTED_LINEAGE_ROUTE_CONDUCTOR_CONTRACTS[1];
  ...
  function includesContract(list, contract) {
    return (list || []).includes(safeString(contract));
  }
  ...
  function lineageText(values) {
    return unique(values).join(" | ") || FALLBACK.UNKNOWN;
  }
  ...
  function makeState() {
    return {
      ...
      baselineAlignmentContract: BASELINE_ALIGNMENT_CONTRACT,
      ...
      htmlV26ExpectationAlignmentActive: true,
      htmlV25AcceptedAsLineage: true,
      htmlV24AcceptedAsLineage: true,
      htmlV23AcceptedAsLineage: true,
      htmlV22AcceptedAsLineage: true,
      htmlV21AcceptedAsLineage: true,
      routeConductorDirectFallbackFetchPreserved: true,
      falseCase5ForAcceptedLineagePrevented: true,
  ...
  function extractJsContract(source, expectedContracts, genericPrefix) {
    const text = safeString(source);
    const expected = Array.isArray(expectedContracts)
      ? expectedContracts
      : [expectedContracts].filter(Boolean);
    ...
    const prefix = safeString(genericPrefix || "HEARTH").replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  ...
  function setRecognitionState(state, scope, values) {
    ...
    if (currentKey in state) state[currentKey] = values.current ? "true" : "false";
    if (lineageKey in state) state[lineageKey] = values.lineage ? "true" : "false";
    if (previousKey in state) state[previousKey] = values.previous ? "true" : "false";
    if (mismatchKey in state) state[mismatchKey] = values.mismatch;
    if (staleKey in state) state[staleKey] = values.stale;
  }

  function evaluateContractRecognition(state, scope, observed, current, lineageContracts, previousContracts) {
    const label = scopeLabel(scope);
    const lineageList = Array.isArray(lineageContracts)
      ? lineageContracts
      : [lineageContracts].filter(Boolean);
    const previousList = Array.isArray(previousContracts)
      ? previousContracts
      : [previousContracts].filter(Boolean);
    ...
    if (includesContract(lineageList, observed)) {
      const lineageIndex = lineageList.indexOf(observed) + 1;
      ...
      addNote(state, `${label}_LINEAGE_${lineageIndex}_NOT_TREATED_AS_CURRENT_FAILURE`);
      addNote(state, `${label}_LINEAGE_NOT_TREATED_AS_CACHE_MISMATCH`);
    ...
    if (includesContract(previousList, observed)) {
      const previousIndex = previousList.indexOf(observed) + 1;
      ...
      addNote(state, `PREVIOUS_${label}_LINEAGE_${previousIndex}_NOT_TREATED_AS_CURRENT_FAILURE`);
      addNote(state, `PREVIOUS_${label}_LINEAGE_NOT_TREATED_AS_CACHE_MISMATCH`);
  ...
  function evaluateReadStatus(state) {
    ...
    if (anyIncomplete || !allContractsConfident) {
  ...
  function makeEvidencePacket(state) {
    return {
      ...
      ACCEPTED_LINEAGE_HTML_CONTRACTS: lineageText(state.acceptedLineageHtmlContracts),
      ...
      ACCEPTED_LINEAGE_INDEX_JS_CONTRACTS: lineageText(state.acceptedLineageIndexJsContracts),
      ...
      ACCEPTED_LINEAGE_ROUTE_CONDUCTOR_CONTRACTS: lineageText(state.acceptedLineageRouteConductorContracts),
      ...
      HTML_V2_6_EXPECTATION_ALIGNMENT_ACTIVE: String(state.htmlV26ExpectationAlignmentActive),
      HTML_V2_5_ACCEPTED_AS_LINEAGE: String(state.htmlV25AcceptedAsLineage),
      HTML_V2_4_ACCEPTED_AS_LINEAGE: String(state.htmlV24AcceptedAsLineage),
      HTML_V2_3_ACCEPTED_AS_LINEAGE: String(state.htmlV23AcceptedAsLineage),
      HTML_V2_2_ACCEPTED_AS_LINEAGE: String(state.htmlV22AcceptedAsLineage),
      HTML_V2_1_ACCEPTED_AS_LINEAGE: String(state.htmlV21AcceptedAsLineage),
      ROUTE_CONDUCTOR_DIRECT_FALLBACK_FETCH_PRESERVED: String(state.routeConductorDirectFallbackFetchPreserved),
      FALSE_CASE_5_FOR_ACCEPTED_LINEAGE_PREVENTED: String(state.falseCase5ForAcceptedLineagePrevented),
  ...
  function getEastReceipt() {
    ...
    return {
      ...
      acceptedLineageHtmlContracts: lineageText(state.acceptedLineageHtmlContracts),
      ...
      acceptedLineageIndexJsContracts: lineageText(state.acceptedLineageIndexJsContracts),
      ...
      acceptedLineageRouteConductorContracts: lineageText(state.acceptedLineageRouteConductorContracts),
