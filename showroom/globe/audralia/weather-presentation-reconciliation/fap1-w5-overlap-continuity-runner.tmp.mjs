...
const receipts=objects.map(object=>{
  const receipt=evaluateMacroLocalContinuity(object,{canonicalTimeHours:0});
  const verification=verifyMacroLocalContinuity(receipt);
  return {weatherId:object.ID_i,weatherClass:object.weatherClass,genus:object.W_i.genus,receipt,verification};
});
const summary={schema:'FAP1_W5_OVERLAP_CONTINUITY_EXECUTION_v1',canonicalTimeHours:0,systemCount:receipts.length,pass:receipts.every(x=>x.receipt.pass&&x.verification.pass),eligibleCount:receipts.filter(x=>x.receipt.pass&&x.verification.pass).length,heldCount:receipts.filter(x=>!(x.receipt.pass&&x.verification.pass)).length,receipts};
