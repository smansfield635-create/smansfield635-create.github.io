...
try{
 ...
 for(const [id,width,height] of VIEWS){
  ...
  const execution=await page.evaluate(async({GROUPS,CADENCE,COALESCING})=>{
   ...
   const correspondence=proposals.map(p=>{const f=map.get(p.sequence),changed=Boolean(f)&&f.colorSummary.byteHash!==previous;if(f)previous=f.colorSummary.byteHash;return{proposalSequence:p.sequence,inputClass:p.inputClass,action:p.intent.action,navigationSequence:p.afterNavigationSequence,frameSequence:f?.frameSequence,frameNavigationSequence:f?.navigationSequence,cameraUniformUpdated:Boolean(f&&f.cameraPosition&&[f.cameraPosition.x,f.cameraPosition.y,f.cameraPosition.z].every(Number.isFinite)&&f.viewProjectionMatrix?.length===16&&f.viewProjectionMatrix.every(Number.isFinite)&&Number.isFinite(f.verticalFovDegrees)),presented:f?.sourceKind==='ACCEPTED_NAVIGATION_PROPOSAL',frameHash:f?.colorSummary.byteHash,hashChanged:changed}});
