/** H_EARTH_FAP1_WEATHER_STATE_CANDIDATE_A_v1
...
const REGIMES = freeze([
  freeze({
    id: 'GRATITUDE_CLEAR_CORRIDOR',
    weatherClass: 'CLEAR',
    center: freeze({ latitudeDeg: 26, longitudeDeg: -18 }),
    radiusDeg: 23,
    occupancy: 0.08,
    hazeSupport: 0.08,
    families: freeze([]),
    wind: freeze({ eastKmH: 38, northKmH: 6 })
  }),
  freeze({
    id: 'NORTHERN_HIGH_ICE_FIELD',
    weatherClass: 'HIGH_ICE',
    center: freeze({ latitudeDeg: 57, longitudeDeg: 26 }),
    radiusDeg: 34,
    occupancy: 0.54,
    hazeSupport: 0.12,
    families: freeze(['HIGH']),
    wind: freeze({ eastKmH: 118, northKmH: -10 })
  }),
  freeze({
    id: 'WESTERN_FRONTAL_FIELD',
    weatherClass: 'MID_FRONTAL',
    center: freeze({ latitudeDeg: 34, longitudeDeg: -73 }),
    radiusDeg: 31,
    occupancy: 0.67,
    hazeSupport: 0.28,
    families: freeze(['HIGH', 'MID']),
    wind: freeze({ eastKmH: 72, northKmH: 18 })
  }),
  freeze({
    id: 'EQUATORIAL_CUMULUS_BELT',
    weatherClass: 'LOW_CUMULIFORM',
    center: freeze({ latitudeDeg: 4, longitudeDeg: 48 }),
    radiusDeg: 39,
    occupancy: 0.58,
    hazeSupport: 0.18,
    families: freeze(['LOW']),
    wind: freeze({ eastKmH: -31, northKmH: 8 })
  }),
  freeze({
    id: 'SOUTHEAST_DEEP_CONVECTION',
    weatherClass: 'DEEP_CONVECTION',
    center: freeze({ latitudeDeg: -19, longitudeDeg: 82 }),
    radiusDeg: 27,
    occupancy: 0.76,
    hazeSupport: 0.35,
    families: freeze(['LOW', 'DEEP', 'HIGH']),
    wind: freeze({ eastKmH: -46, northKmH: -16 })
  }),
  freeze({
    id: 'SOUTHERN_OCEAN_CYCLONE',
    weatherClass: 'CYCLONE',
    center: freeze({ latitudeDeg: -36, longitudeDeg: -126 }),
    radiusDeg: 24,
    occupancy: 0.84,
    hazeSupport: 0.42,
    families: freeze(['LOW', 'MID', 'DEEP', 'HIGH']),
    wind: freeze({ eastKmH: 42, northKmH: 12 }),
    cyclone: freeze({
      eyeRadiusDeg: 2.4,
      eyewallRadiusDeg: 5.8,
      outerRainbandRadiusDeg: 21,
      spiralTightness: 2.15,
      rotationDirection: 'CCW',
      upperOutflowRadiusDeg: 28,
      maximumConvectiveSupport: 1
    })
  })
]);
...
function supportFor(regime, latitudeDeg, longitudeDeg) {
  const distance = angularDistanceDeg(
    latitudeDeg,
    longitudeDeg,
    regime.center.latitudeDeg,
    regime.center.longitudeDeg
  );
  return clamp01(1 - distance / regime.radiusDeg);
}

export function sampleHEarthFAP1WeatherState({
  latitudeDeg,
  longitudeDeg,
  ...
  const weighted = REGIMES.map((regime) => ({ regime, support: supportFor(regime, lat, lon) }))
  ...
  return freeze({
    ...
    stateIdentity: `${primary.regime.id}:${Math.round(lat * 10)}:${Math.round(lon * 10)}`
  });
}
...
export function sampleHEarthFAP1AltitudeVelocity(weatherState, altitudeKm) {
  ...
  const family = altitudeKm >= 7
    ? H_EARTH_FAP1_ALTITUDE_FAMILIES.HIGH
    : altitudeKm >= 2.5
      ? H_EARTH_FAP1_ALTITUDE_FAMILIES.MID
      : H_EARTH_FAP1_ALTITUDE_FAMILIES.LOW;
  const scale = family.velocityScale;
  const cycloneBoost = weatherState.weatherClass === 'CYCLONE' ? clamp01(altitudeKm / 12) * 22 : 0;
  return freeze({
    familyId: family.id,
    eastKmH: weatherState.baseWind.eastKmH * scale + cycloneBoost,
    northKmH: weatherState.baseWind.northKmH * scale
  });
}
...
export function evaluateHEarthFAP1WeatherContract() {
  ...
  if (!cyclone?.eyeRadiusDeg || !cyclone?.eyewallRadiusDeg || !cyclone?.outerRainbandRadiusDeg || !cyclone?.upperOutflowRadiusDeg) {
    issues.push('STRUCTURED_CYCLONE_INCOMPLETE');
  }
  return freeze({
    eligible: issues.length === 0,
    status: issues.length === 0 ? 'FAP1_WEATHER_CONTRACT_PASS' : 'FAP1_WEATHER_CONTRACT_FAIL',
