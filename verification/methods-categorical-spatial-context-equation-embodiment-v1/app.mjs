import { loadCanonicalCatalog } from "../methods-models-categorical-spatial-template-v1/catalog-source.mjs";
import { assertSpatialRegistry } from "../methods-models-categorical-spatial-template-v1/descriptor-schema.mjs";
import { resolveSceneState, createReturnSnapshot } from "../methods-models-categorical-spatial-template-v1/resolver.mjs";
import { MethodsNativeBridge } from "../methods-models-categorical-spatial-template-v1/native-bridge.mjs";
import { buildSemanticSpatialRegistry, assertSemanticRegistry } from "./semantic-descriptor.mjs";
import { MethodsSemanticSpatialRenderer } from "./renderer.mjs";
...
function descriptorById(modelId) {
  return registry?.descriptors.find(candidate => candidate.MODEL_ID === modelId) || null;
}

function familyById(familyId) {
  return catalog?.find(candidate => candidate.id === familyId) || null;
}
...
function focusIdentityForElement(element) {
  if (!element || element === document.body || element === document.documentElement) return freezeFocusTarget("body", "BODY");
  ...
  if (element.matches?.(".spatial-model-node[data-model-id]")) return freezeFocusTarget("model", element.dataset.modelId);
...
function resolveFocusTarget(target) {
  ...
  if (target.kind === "control") return document.querySelector(`[data-control="${CSS.escape(target.value)}"]`);
  if (target.kind === "model") return document.querySelector(`.spatial-model-node[data-model-id="${CSS.escape(target.value)}"]`);
...
function pointsEqual(left, right, tolerance = 1) {
  return Math.abs(Number(left?.x || 0) - Number(right?.x || 0)) <= tolerance && Math.abs(Number(left?.y || 0) - Number(right?.y || 0)) <= tolerance;
}
...
function resolveCurrent() {
  ...
  currentResolved = resolveSceneState({ registry, nativeState, cameraMode, inspectionOpen, viewport: viewport() });
...
function setText(selector, value) {
  const element = inspection.querySelector(selector);
...
function renderSymbols(symbols) {
  const list = inspection.querySelector("[data-inspection-symbols]");
  if (!list) return;
  list.replaceChildren(...(Array.isArray(symbols) ? symbols : []).map(symbol => {
...
function openInspectionView() {
  const descriptor = descriptorById(nativeState.x.modelId);
  const family = familyById(nativeState.z.familyId);
  if (!descriptor || !family) throw new Error(`METHODS_INSPECTION_MODEL_UNRESOLVED:${nativeState.x.modelId}`);

  inspection.dataset.sourceState = descriptor.SOURCE_STATE;
  inspection.dataset.formClass = descriptor.EQUATION_FORM_CLASS;
  inspection.dataset.modelId = descriptor.MODEL_ID;
  setText("[data-inspection-family-title]", family.title);
  setText("[data-inspection-family-question]", family.question);
  setText("[data-inspection-model-title]", descriptor.TITLE);
  setText("[data-inspection-model-question]", descriptor.QUESTION);
  setText("[data-inspection-statement]", descriptor.STATEMENT);
  setText("[data-inspection-equation-label]", descriptor.EQUATION_LABEL);
  setHtml("[data-inspection-equation]", descriptor.EQUATION);
  setText("[data-inspection-source-state]", descriptor.SOURCE_STATE === "hold" ? "Architecture preserved under source hold" : "Preserved confirmed architecture");
  setText("[data-inspection-status]", descriptor.STATUS);
  setText("[data-inspection-practical]", descriptor.PRACTICAL);
  setText("[data-inspection-engineering]", descriptor.ENGINEERING);
  setText("[data-inspection-evidence]", descriptor.EVIDENCE);
  setText("[data-inspection-evidence-standing]", `${descriptor.SOURCE_STATE} · ${descriptor.STATUS}`);
  setText("[data-inspection-purpose]", descriptor.PURPOSE);
  setText("[data-inspection-architecture]", descriptor.ARCHITECTURE);
  setText("[data-inspection-operation]", descriptor.OPERATION);
  setText("[data-inspection-failure]", descriptor.FAILURE);
  setText("[data-inspection-limits]", descriptor.LIMITS);
  setText("[data-inspection-form-class]", descriptor.EQUATION_FORM_CLASS);
  setText("[data-inspection-source-reference]", `${descriptor.CANONICAL_SOURCE_REFERENCE.path} · ${descriptor.CANONICAL_SOURCE_REFERENCE.selector}`);
  renderSymbols(descriptor.SYMBOLS);
...
function emitReturnReceipt(reached) {
  ...
  const reachedDetailClasses = Object.freeze(Object.fromEntries(reached.nodes.filter(node => node.visible).map(node => [node.modelId, node.detailClass])));
  const receipt = Object.freeze({
    ...
    exactNativeReturn: reached.native.familyId === returnSnapshot.nativeFamily && reached.native.modelId === returnSnapshot.nativeModel && reached.native.lensId === returnSnapshot.nativeLens && reached.native.displayState === returnSnapshot.displayState,
...
function bindControls() {
  ...
  document.querySelector("[data-control='inspect']").addEventListener("click", () => { captureSnapshot(); bridge.openInspection(); });
...
async function initialize() {
  ...
  registry = assertSemanticRegistry(assertSpatialRegistry(buildSemanticSpatialRegistry(catalog)), catalog);
  renderer = new MethodsSemanticSpatialRenderer({ stage, registry, catalog });
  ...
  globalThis.__METHODS_SPATIAL_APP = Object.freeze({
    contract: "METHODS_CATEGORICAL_SPATIAL_CONTEXT_AND_EQUATION_EMBODIMENT_PASS_v1",
    baselineContract: "METHODS_MODELS_CATEGORICAL_SPATIAL_TEMPLATE_VERTICAL_SLICE_v1",
    registry,
    catalog,
    ...
    get activeSemanticDescriptor() { return descriptorById(nativeState?.x?.modelId); },
    setCameraMode(mode) {
    ...
    inspect: () => { captureSnapshot(); bridge.openInspection(); },
    closeInspection: () => bridge.closeInspection(),
    whenStable: () => renderQueue
  });
  document.documentElement.dataset.methodsSpatialReady = "true";
  dispatchEvent(new CustomEvent("METHODS_MODELS_SPATIAL_TEMPLATE_READY", { detail: { registryCount: registry.modelCount, semanticContract: registry.semanticRegistryContract } }));
}
