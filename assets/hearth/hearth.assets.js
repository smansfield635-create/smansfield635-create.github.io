...
// HEARTH_ASSETS_CLIMATE_BIOME_REGION_COLORING_TNT_v11
// Full-file replacement.
// Assets authority consuming terrain base + dedicated elevation + climate/biome authority.
// Purpose:
// - Remove body-mass-assigned color logic.
// - Color land by climate and region: forest, desert, plains, mountains, tundra, wetland, savanna, steppe, coast, ice, highlands.
...
(() => {
  ...
  const CONTRACT = "HEARTH_ASSETS_CLIMATE_BIOME_REGION_COLORING_TNT_v11";
  const RECEIPT = "HEARTH_ASSETS_CLIMATE_BIOME_REGION_COLORING_RECEIPT_v11";
  const PREVIOUS_CONTRACT = "HEARTH_ASSETS_CONSUME_DEDICATED_ELEVATION_TNT_v10";
  ...
  function elevationAuthority() {
    const elevation = window.HEARTH_ELEVATION;
    if (!elevation || !String(elevation.contract || "").includes(REQUIRED_ELEVATION)) return null;
    if (typeof elevation.sampleElevation !== "function") return null;
    return elevation;
  }

  function climateAuthority() {
    const climate = window.HEARTH_CLIMATE;
    if (!climate || !String(climate.contract || "").includes(REQUIRED_CLIMATE)) return null;
    if (typeof climate.sampleClimate !== "function") return null;
    return climate;
  }
  ...
  function classify(u, v) {
    ...
    const elevationSample = elevation ? elevation.sampleElevation(u, v, base, terrain || {}) : null;
    const climateSample = climate ? climate.sampleClimate(u, v, base, terrain || {}, elevationSample || {}) : null;
  ...
  function sample(u, v) {
    ...
    let color = biomeBaseColor(climate);
    ...
    color = mix(color, C.opal, (climate.biome === "coastal-shelf" ? 0.14 : 0.04) * (elevation.mineralFace || 0));
  ...
  function createTextureCanvas(options = {}) {
    ...
    canvas.dataset.hearthElevationAuthorityContract = REQUIRED_ELEVATION;
    canvas.dataset.hearthClimateAuthorityContract = REQUIRED_CLIMATE;
    canvas.dataset.hearthTerrainExtensionLoaded = String(Boolean(terrainExtension()));
    canvas.dataset.hearthElevationAuthorityLoaded = String(Boolean(elevationAuthority()));
    canvas.dataset.hearthClimateAuthorityLoaded = String(Boolean(climateAuthority()));
  ...
  document.documentElement.dataset.hearthClimateAuthorityLoaded = String(Boolean(climateAuthority()));
