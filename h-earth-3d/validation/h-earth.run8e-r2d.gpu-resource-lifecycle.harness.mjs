...
try {
  ...
  assert.equal(receipt.status, 'RUN_8E_R2D_GPU_RESOURCE_LIFECYCLE_PASS');
  assert.equal(receipt.packageIdentity, 'H_EARTH_RUN_8E_R2_LIVE_RENDER_PACKAGE_FD913C25');
  assert.equal(receipt.contentDigest, 'fnv1a32:fd913c25');
  ...
  for (const cycle of cycles) {
    ...
    assert.deepEqual(cycle.resources.map(({ key }) => key), expectedKeys, `R2D_RESOURCE_ORDER:${cycle.cycleId}`);
  ...
  assert.equal([...totalByteLengths][0], 2145444, 'R2D_TOTAL_UPLOAD_BYTE_LENGTH_UNEXPECTED');
