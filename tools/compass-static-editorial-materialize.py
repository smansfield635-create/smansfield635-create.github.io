...
.compass-built{margin-top:clamp(2rem,5vw,4rem)!important}
.compass-capability-cue{margin-top:clamp(2rem,4vw,3.2rem)!important}
.compass-capability-orbit{margin-top:clamp(.7rem,2vw,1.2rem)!important}
.compass-build-cta{margin-top:clamp(2.5rem,6vw,5rem)!important;padding-top:clamp(1.6rem,3vw,2.4rem)!important;border-top:1px solid rgba(244,214,128,.18)}
...
pattern = re.compile(r'(?P<cap><p class="compass-capability-cue">.*?</p>\s*<section class="compass-monuments".*?</section>)\s*(?P<built><section class="compass-built".*?</section>)(?P<cta><section class="compass-build-cta".*?</section>)', re.S)
m = pattern.search(s)
assert m, 'LOWER_SEQUENCE_NOT_FOUND'
s = s[:m.start()] + m.group('built') + '\n' + m.group('cap') + '\n' + m.group('cta') + s[m.end():]
...
assert 'compass-selective-routes' not in s
assert 'compass-accessibility-note' not in s
assert 'compass.track-b.js' not in s
assert 'compass.track-b.css' not in s
assert 'positionCapability' not in s
...
p.write_text(s)
print('STATIC_EDITORIAL_MATERIALIZED')
