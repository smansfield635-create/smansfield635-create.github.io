...
function shouldUseFirstPartySiteContract(prompt) {
  const text = normalize(prompt).toLowerCase();
  if (!text) return false;
  const asksAboutCharacterOrNarrative = /\b(jeeves|elara|auren(?:\s+vale)?|soren|character|characters|persona|narrative|relationship|relationships)\b/.test(text);
  if (asksAboutCharacterOrNarrative) return false;
