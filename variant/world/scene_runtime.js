...
export async function createScene(canvas, outputs) {
  ...
  function updateViewportOffset(forceSnap = false) {
    ...
    const projectedPlayer = projector.point(state.player.x, state.player.y);

    const followX = targetScreenX - ((projectedPlayer.x * state.renderScale) + baseX + dynamicForwardBiasX);
    const followY = targetScreenY - ((projectedPlayer.y * state.renderScale) + baseY + dynamicForwardBiasY);
