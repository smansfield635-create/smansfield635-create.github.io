(() => {
  ...
  const CONTRACT = 'LAWS_CONTEXTUAL_READABLE_TABS_FOUR_MODEL_EUCLIDEAN_CAROUSEL_V1';
  ...
  const FOCUSED_HORIZONTAL_LAW = 'NO_PAGE_ADVANCE_NO_CAROUSEL_ROTATION';
  ...
  dynamicStyle.dataset.euclideanCarouselRuntime = 'true';
  ...
  const setStatus = () => {
    ...
    if (state.focused) {
      ...
      status.textContent = `${id} · same Euclidean object · ${state.categories.get(id)} · carousel frozen`;
    } else {
      status.textContent = `Four surfaces · shared Y axis · θ ${state.thetaWheel.toFixed(1)}°`;
    }
  };
  ...
  window.__LAWS_EUCLIDEAN_CAROUSEL__ = Object.freeze({
    ...
    orbitInputDomain: 'BACKGROUND_AND_PAGE_SURFACES',
