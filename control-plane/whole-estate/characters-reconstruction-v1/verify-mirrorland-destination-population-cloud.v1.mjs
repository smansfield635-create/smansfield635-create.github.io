...
const verifierPath='control-plane/whole-estate/characters-reconstruction-v1/verify-mirrorland-destination-population-cloud.v1.mjs';
const dossierPath='control-plane/whole-estate/characters-reconstruction-v1/dossier-preservation-manifest.v1.json';
const requiredProductFiles=['characters/destination-registry.mjs','characters/population-registry.mjs','characters/orientation-shell.mjs','characters/cloud-traversal.mjs','characters/scene-transition.mjs'];
...
const dossier=json(dossierPath),dossierText=JSON.stringify(dossier);
const protectedCount=Number(dossier.requiredStateCount??dossier.protectedStateCount??dossier.stateCount??0);
assert(protectedCount===256||/256/.test(dossierText),'LEGACY_PRESERVATION_FAILURE','256-state surface not declared');
...
contains('characters/step9-regional-geography.mjs','canonicalGeographyReadOnly:true','GEOGRAPHY_AUTHORITY_DRIFT');
contains('characters/step9-regional-geography.mjs','STEP9_SAFE_INTERIOR','SAFE_INTERIOR_CAMERA_ENVELOPE_FAILURE');
contains('characters/step9-regional-geography.mjs','canonicalWorld','GEOGRAPHY_AUTHORITY_DRIFT');
...
contains('characters/orientation-shell.mjs','createCloudTraversalController','CLOUD_TRAVERSAL_STATE_FAILURE');
contains('characters/orientation-shell.mjs','createSceneTransitionController','SCENE_ENTRY_FADE_STATE_FAILURE');
...
contains('characters/index.html','./orientation-shell.mjs','SCENE_ENTRY_FADE_STATE_FAILURE');
...
for(const marker of ['COMPASS','SHOWROOM','H_EARTH','AUDRALIA'])assert(registry.includes(marker),'CONNECTED_WORLD_ROUTE_REGRESSION',marker);

const state=read('characters/narrative-world-state.mjs');
assert(/if\(id==='clock'\|\|!revealed\.has\(id\)\)signalState='UNSEEN'/.test(state),'DESTINATION_CLASSIFICATION_FAILURE','clock-signal-visible');
...
assert(read('characters/index.html').includes('Return to orbit'),'RETURN_PATH_FAILURE');

let changed='';
try{changed=execFileSync('git',['diff','--name-only',`${BASE}...HEAD`],{encoding:'utf8'});}catch{}
if(changed){
  const allowed=new Set(['characters/index.html','characters/app.mjs','characters/narrative-world-state.mjs','characters/night-renderer.mjs','characters/NARRATIVE_WORLD_STATE_CONTRACT.md','characters/step9-regional-geography.mjs','characters/gratitude-geography.adapter.mjs','characters/cardinal-scenes.data.mjs','characters/cardinal-scene-state.mjs','characters/cardinal-scene-geometry.mjs','characters/coast-map.mjs','characters/knowledge-card.mjs','characters/cinematic-intro.mjs','characters/cardinal-scenes.css','characters/destination-registry.mjs','characters/population-registry.mjs','characters/orientation-shell.mjs','characters/cloud-traversal.mjs','characters/scene-transition.mjs',contractPath,verifierPath]);
  for(const rel of changed.trim().split('\n').filter(Boolean))assert(allowed.has(rel),'PATH_SCOPE_VIOLATION',rel);
}

console.log(JSON.stringify({schema:'MIRRORLAND_DESTINATION_POPULATION_CLOUD_VERIFICATION_RECEIPT_v1',result:'STATIC_CONTRACT_PASS',governingHead:BASE,invariants:{geographyReadOnly:true,canonicalCoordinatesPreservedAsProvenance:true,legacy256Preserved:true,destinationClassesFrozen:true,clockStoryOrientationOnly:true,safeInteriorRequired:true,worldAnchoredClouds:true,travelOrderFrozen:true,fadeOrderFrozen:true,returnRecoveryRequired:true}},null,2));
