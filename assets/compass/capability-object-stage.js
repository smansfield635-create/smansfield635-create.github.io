(()=>{
'use strict';
const VERSION='CAPABILITY_OBJECT_STAGE_v3';
const M=Math;
...
function program(gl,mode='material'){
 ...
 const fs=geometry?`precision mediump float;varying vec3 vN,vP,vC;void main(){vec3 n=normalize(vN);vec3 view=normalize(vec3(0.,.04,2.35)-vP);vec3 key=normalize(vec3(-.62,.72,.44));vec3 fill=normalize(vec3(.58,.12,.66));vec3 low=normalize(vec3(-.15,-.65,.36));float kd=max(dot(n,key),0.);float fd=max(dot(n,fill),0.);float ld=max(dot(n,low),0.);float rim=pow(1.-max(dot(n,view),0.),2.1);float depth=.48+.26*clamp(vP.z+.8,0.,1.5);vec3 base=vec3(.57,.59,.61)*(.28+.64*kd+.11*fd+.06*ld)*depth;vec3 edge=vec3(.91,.93,.94)*rim*.17;gl_FragColor=vec4(base+edge,1.0);}`:`precision mediump float;varying vec3 vN,vP,vC;void main(){vec3 n=normalize(vN);vec3 view=normalize(vec3(0.,.10,2.2)-vP);vec3 key=normalize(vec3(-.52,.74,.58));vec3 fill=normalize(vec3(.62,.08,.78));float kd=max(dot(n,key),0.);float fd=max(dot(n,fill),0.);float rim=pow(1.-max(dot(n,view),0.),2.35);vec3 base=mix(vec3(.052,.015,.027),vC,.31+.54*kd+.10*fd);vec3 cool=vec3(.30,.64,.74)*(fd*.045+rim*.15);gl_FragColor=vec4(base+cool,.996);}`;
...
function mount(canvas,{meshFactory,foreground=()=>true,initialYaw=.32,initialPitch=-.05,spin=0,scale=1,dataset={},inspectionMode='material'}={}){
 ...
 const anim=()=>!reduce.matches&&visible&&pageOn&&foreground()&&spin!==0;
 function draw(t=performance.now()){raf=0;resize();const dt=M.min(40,t-last||16);last=t;if(!drag&&anim())targetYaw+=dt*spin;yaw+=(targetYaw-yaw)*.16;pitch+=(targetPitch-pitch)*.16;gl.clearColor(0,0,0,0);gl.clear(gl.COLOR_BUFFER_BIT|gl.DEPTH_BUFFER_BIT);gl.enable(gl.DEPTH_TEST);gl.disable(gl.CULL_FACE);gl.useProgram(pr);gl.uniformMatrix4fv(loc.m,false,matrix(yaw,pitch,scale));gl.uniform1f(loc.aspect,canvas.width/M.max(1,canvas.height));bind(mesh.p,loc.p);bind(mesh.n,loc.n);bind(mesh.c,loc.c);gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER,mesh.i);gl.drawElements(gl.TRIANGLES,mesh.count,gl.UNSIGNED_SHORT,0);canvas.dataset.objectFrames=String((+canvas.dataset.objectFrames||0)+1);if(anim()||drag||M.abs(targetYaw-yaw)>.001||M.abs(targetPitch-pitch)>.001)raf=requestAnimationFrame(draw)}
 const schedule=()=>{if(!raf)raf=requestAnimationFrame(draw)};
 const setView=(y,p)=>{targetYaw=y;targetPitch=p;schedule()};
 const down=e=>{drag=true;lastX=e.clientX;lastY=e.clientY;canvas.setPointerCapture?.(e.pointerId);schedule()};
 const move=e=>{if(!drag)return;const dx=e.clientX-lastX,dy=e.clientY-lastY;lastX=e.clientX;lastY=e.clientY;targetYaw+=dx*.0085;targetPitch=M.max(-1.35,M.min(1.20,targetPitch+dy*.0065));schedule()};
 ...
 const api=Object.freeze({canvas,renderer:VERSION,setView,capture:()=>{draw();return{renderer:VERSION,yaw,pitch}},inspect:()=>({renderer:VERSION,inspectionMode,triangles:mesh.triangles||mesh.count/3,yaw,pitch,foreground:foreground()}),destroy:()=>{cancelAnimationFrame(raf);ro.disconnect();io.disconnect()}});scenes.set(canvas,api);return api
}
