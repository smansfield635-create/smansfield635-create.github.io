...
const GRID=freeze({columns:120,rows:88,insetFraction:.025,jitterFraction:.34,seed:1689746977});
...
export const V4_UNDERSTORY_CONTRACT=freeze({
  ...
  stage:'GROUND_CLOSURE_AND_HYDROLOGY_DIFFERENTIATION_SUCCESSOR',
  predecessorBoundary:'STAND_TOPOLOGY_EDGE_ECOLOGY_NEGATIVE_SPACE_PASS',
  targetBoundary:'MATERIAL_WORLD_READING_SUCCESSOR_MECHANICAL_PASS',
  ...
  grid:GRID,
  compactPresentationSampling:.68,
...
function baseClassDensity(type,ecology){
  ...
  switch(type){
    case 'GRASS_SEDGE':if(drainage!=='LAND'||shore<6||slopeClass==='STEEP_NONCLIMBING')return 0;return clamp(.28+.30*forest+.32*wet+.22*(['LOWLAND_SOIL','COASTAL_SOIL','FOREST_SOIL'].includes(material)?1:0)-.10*(slopeClass==='MODERATE'?1:0),0,.96);
    case 'LOW_SHRUB':if(drainage!=='LAND'||shore<12||forest<.14||slopeClass==='STEEP_NONCLIMBING')return 0;return clamp(.10+.50*forest+.10*(curvatureClass==='CONCAVE'?1:0),0,.72);
    ...
    case 'REED_WET_MARGIN':if(drainage!=='LAND'||shore<2||wet<.08||!['LEVEL','GENTLE'].includes(slopeClass))return 0;return clamp(.24+1.04*wet,0,.99);
    case 'DEAD_SPARSE_GROUND':if(drainage!=='LAND'||shore<10||!['STONE_AND_SPARSE_SOIL','COASTAL_SOIL'].includes(material)||forest>=.36)return 0;return clamp(.10+.28*(material==='STONE_AND_SPARSE_SOIL'?1:0)+.16*(slopeClass==='MODERATE'?1:0),0,.52);
    case 'FOREST_FLOOR_CLUSTER':if(drainage!=='LAND'||shore<14||forest<.24||!(material==='FOREST_SOIL'||['WOODLAND','FOREST'].includes(biomeClass)))return 0;return clamp(.18+.68*forest+.10*(curvatureClass==='CONCAVE'?1:0),0,.94);
    default:return 0;
  }
}
function zoneMultiplier(type,environment){const z=environment.spatialZone;if(z==='OPENING'){if(type==='LOW_SHRUB'||type==='SAPLING_YOUNG_GROWTH')return 0;if(type==='GRASS_SEDGE')return 1.72;if(type==='REED_WET_MARGIN')return 1.45;if(type==='DEAD_SPARSE_GROUND')return 1.04;return .16;}if(z==='EDGE'){if(type==='LOW_SHRUB')return 1.78;if(type==='SAPLING_YOUNG_GROWTH')return 1.90;if(type==='GRASS_SEDGE')return 1.22;if(type==='REED_WET_MARGIN')return 1.34;if(type==='FOREST_FLOOR_CLUSTER')return .66;return 1.08;}if(z==='TRANSITION'){if(type==='LOW_SHRUB')return 1.38;if(type==='SAPLING_YOUNG_GROWTH')return 1.34;if(type==='GRASS_SEDGE')return 1.52;if(type==='REED_WET_MARGIN')return 1.42;if(type==='FOREST_FLOOR_CLUSTER')return .78;return 1.08;}if(type==='LOW_SHRUB')return .54;if(type==='SAPLING_YOUNG_GROWTH')return .42;if(type==='GRASS_SEDGE')return .52;if(type==='FOREST_FLOOR_CLUSTER')return 1.48;if(type==='DEAD_SPARSE_GROUND')return .62;return 1.08;}
function standMultiplier(type,environment){switch(environment.standClass){case 'DENSE_WOODLAND':return type==='FOREST_FLOOR_CLUSTER'?1.34:(type==='GRASS_SEDGE'?.70:1);case 'GROVE':return type==='FOREST_FLOOR_CLUSTER'?1.18:1;case 'COASTAL_SCRUB':return type==='LOW_SHRUB'?1.24:(type==='GRASS_SEDGE'?1.18:1);case 'WET_MARGIN_RIPARIAN':return type==='REED_WET_MARGIN'?2.18:(type==='GRASS_SEDGE'?1.34:(type==='LOW_SHRUB'?.78:1));case 'EXPOSED_UPLAND':return type==='DEAD_SPARSE_GROUND'?1.34:(type==='FOREST_FLOOR_CLUSTER'?.44:.92);case 'SPARSE_TRANSITION':return type==='GRASS_SEDGE'?1.26:(type==='FOREST_FLOOR_CLUSTER'?.58:1);case 'ECOLOGICAL_OPEN':return type==='GRASS_SEDGE'?1.36:(type==='LOW_SHRUB'||type==='SAPLING_YOUNG_GROWTH'?0:.84);default:return 1;}}
...
function coverageFloor(type,ecology,environment){
  ...
  if(type==='REED_WET_MARGIN'&&(environment.standClass==='WET_MARGIN_RIPARIAN'||wet>=.18))return .78;
  if(type==='FOREST_FLOOR_CLUSTER'&&environment.spatialZone==='INTERIOR'&&['DENSE_WOODLAND','GROVE'].includes(environment.standClass))return .72;
  if(type==='GRASS_SEDGE'&&environment.spatialZone==='OPENING')return .64;
  if(type==='GRASS_SEDGE'&&environment.spatialZone==='TRANSITION')return .58;
  if(['LOW_SHRUB','SAPLING_YOUNG_GROWTH'].includes(type)&&environment.spatialZone==='EDGE')return .48;
  return 0;
}
function selectClass(seed,ecology,environment,trees){
  ...
  if(reed){const wet=Math.max(Number(ecology.hydrology?.riverWeight)||0,Number(ecology.hydrology?.lakeWeight)||0),shore=Math.max(0,Number(ecology.shorelineDistance)||0),wetAffinity=clamp((wet-.08)/.37,0,1),shoreAffinity=clamp((260-shore)/180,0,1),riparianBoost=environment.standClass==='WET_MARGIN_RIPARIAN'?1.35:1;const reedOpportunity=Math.max(coverageFloor('REED_WET_MARGIN',ecology,environment),clamp(reed.density*riparianBoost*(1.08+.38*wetAffinity+.18*shoreAffinity),0,.99));if(rand(seed,CLASS_SALT.REED_WET_MARGIN+211)<=reedOpportunity)return {type:'REED_WET_MARGIN',density:reed.density,coverageProbability:reedOpportunity};}
  ...
  for(const candidate of eligible){const salt=CLASS_SALT[candidate.type],densityScore=candidate.density*(.84+.16*rand(seed,salt)),preference=candidate.type===preferredType?.72:0,edgeYoungBonus=environment.spatialZone==='EDGE'&&['LOW_SHRUB','SAPLING_YOUNG_GROWTH'].includes(candidate.type)?.74:0,transitionGroundBonus=environment.spatialZone==='TRANSITION'&&['GRASS_SEDGE','LOW_SHRUB','SAPLING_YOUNG_GROWTH'].includes(candidate.type)?.30:0,openingGroundBonus=environment.spatialZone==='OPENING'&&candidate.type==='GRASS_SEDGE'?.68:0,interiorFloorBonus=environment.spatialZone==='INTERIOR'&&candidate.type==='FOREST_FLOOR_CLUSTER'?.72:0,score=densityScore+preference+edgeYoungBonus+transitionGroundBonus+openingGroundBonus+interiorFloorBonus;if(score>selectedScore){selected=candidate.type;selectedDensity=candidate.density;selectedScore=score;}}
...
function scaleFor(type,seed){switch(type){case 'GRASS_SEDGE':return .82+.86*rand(seed,151);case 'LOW_SHRUB':return .78+.82*rand(seed,151);case 'SAPLING_YOUNG_GROWTH':return 1.20+1.35*rand(seed,151);case 'REED_WET_MARGIN':return 1.02+.95*rand(seed,151);case 'DEAD_SPARSE_GROUND':return .70+1.10*rand(seed,151);case 'FOREST_FLOOR_CLUSTER':return 1.05+1.05*rand(seed,151);default:return 1;}}
...
const GEOMETRY=freeze({GRASS_SEDGE:new Float32Array([-.07,0,0,.07,0,0,0,.95,0,0,0,-.07,0,0,.07,0,.82,0,-.18,0,.06,-.05,0,.06,-.10,.68,.03]),LOW_SHRUB:new Float32Array([-.55,0,0,.55,0,0,0,.72,0,0,0,-.50,0,0,.50,0,.68,0,-.38,.12,-.28,.38,.12,.28,0,.82,0]),SAPLING_YOUNG_GROWTH:new Float32Array([-.055,0,0,.055,0,0,.04,.90,0,-.42,.66,0,.42,.66,0,0,1.55,0,0,.68,-.38,0,.68,.38,0,1.42,0]),REED_WET_MARGIN:new Float32Array([-.035,0,-.18,.035,0,-.18,0,1.20,-.15,-.035,0,0,.035,0,0,0,1.42,.03,-.035,0,.18,.035,0,.18,0,1.08,.22]),DEAD_SPARSE_GROUND:new Float32Array([-.025,0,-.20,.025,0,-.20,0,1.05,-.12,-.025,0,.14,.025,0,.14,.12,.78,.08,-.22,0,.02,-.16,0,.02,-.05,.58,.04]),FOREST_FLOOR_CLUSTER:new Float32Array([-.62,0,0,.62,0,0,0,.30,.02,0,0,-.58,0,0,.58,.02,.26,0,-.46,0,-.32,.46,0,.32,0,.34,0])});
const TINT=freeze({GRASS_SEDGE:freeze([.105,.205,.145]),LOW_SHRUB:freeze([.075,.155,.105]),SAPLING_YOUNG_GROWTH:freeze([.085,.175,.115]),REED_WET_MARGIN:freeze([.11,.25,.18]),DEAD_SPARSE_GROUND:freeze([.19,.17,.135]),FOREST_FLOOR_CLUSTER:freeze([.065,.125,.085])});
