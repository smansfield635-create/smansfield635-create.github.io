...
const VERSION='mirror-manor-gothic-phase3-carousel-v1';
...
function mount(canvas,{foreground=()=>true}={}){
 ...
 Object.assign(canvas.dataset,{houseRenderer:VERSION,houseContract:'MIRROR_MANOR_GOTHIC_ARCHITECTURAL_DETAIL_PHASE3_v1',houseGeometry:'canonical-phase3-estate-mesh,true-facade-openings,open-pointed-portals,true-dormer-apertures,tower-crowns,owned-gothic-enrichment',houseMotion:'static-exhibit',houseRoofWindows:'none',houseWindowFloors:'two',houseCarouselDistance:'94',housePrincipalSpan:'27',objectContinuity:CONTINUITY,legacyContinuity:LEGACY_CONTINUITY,houseCullPolicy:'two-sided-mixed-winding'});
 const api={fallback:false,setForeground:on=>{canvas.dataset.manorForeground=on?'true':'false';if(on)drawImpl()},draw:()=>drawImpl(),inspect:()=>({fallback:false,renderer:VERSION,contract:canvas.dataset.houseContract,foreground:foreground(),geometry:canvas.dataset.houseGeometry,carouselDistance:94,principalSpan:27,motion:'static-exhibit',continuity:CONTINUITY,ready,cullPolicy:canvas.dataset.houseCullPolicy})};
 ...
 (async()=>{
  try{
   ...
   if(disposed)return;
   const audit=p3.auditPhase3();if(!audit.passStatic)throw new Error('MIRROR_MANOR_PHASE3_AUDIT_FAILED');
   ...
   const build=p3.buildPhase3DetailMesh();for(const m of neutral.buildNeutralMesh({omitFaceMap:p1b.OMIT_FACE_MAP}).meshes.filter(m=>m.id!==build.suppressedNeutralRoofId))add(m,[.39,.40,.42]);for(const m of build.replacementRoof)add(m,p2.PHASE2_MATERIALS.DORMER_SLATE.rgb);for(const m of build.meshes){const z=p3.PHASE3_MATERIALS[m.material]||p2.PHASE2_MATERIALS[m.material]||p1.MATERIAL_ZONES[m.material]||p1.MATERIAL_ZONES.GRAND_STONE;add(m,z.rgb)}
   ...
   drawImpl=()=>{if(disposed||!gl||!foreground())return;const d=Math.min(devicePixelRatio||1,2),r=canvas.getBoundingClientRect(),w=Math.max(2,Math.round(r.width*d)),h=Math.max(2,Math.round(r.height*d));if(canvas.width!==w||canvas.height!==h){canvas.width=w;canvas.height=h}gl.viewport(0,0,w,h);const C=neutral.CAMERA,a=rad(C.azimuthDeg),e=rad(C.elevationDeg),t=[0,7,7],eye=[Math.sin(a)*Math.cos(e)*C.distance,t[1]+Math.sin(e)*C.distance,t[2]+Math.cos(a)*Math.cos(e)*C.distance];gl.uniformMatrix4fv(u,false,mul(perspective(rad(C.fieldOfViewDegrees),w/h,.1,260),lookAt(eye,t)));gl.enable(gl.DEPTH_TEST);gl.disable(gl.CULL_FACE);gl.clearColor(0,0,0,0);gl.clear(gl.COLOR_BUFFER_BIT|gl.DEPTH_BUFFER_BIT);gl.drawArrays(gl.TRIANGLES,0,pos.length/3);canvas.dataset.houseFrames=String((Number(canvas.dataset.houseFrames)||0)+1)};
   ready=true;canvas.dataset.houseAssetReady='true';drawImpl();new ResizeObserver(()=>{if(!drawQueued){drawQueued=true;requestAnimationFrame(()=>{drawQueued=false;drawImpl()})}}).observe(canvas);
