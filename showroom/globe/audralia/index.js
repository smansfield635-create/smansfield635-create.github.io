...
// - Import the current v6 canvas authority with a hard cache key.
// - Call the canonical mountAudraliaCanvas export against the explicit Audralia mount.
// - Expose route status, canvas status, and receipts into DOM for Gauges.
// - Do not own HTML shell, runtime truth, terrain, hydration, oceans, canvas painting, GraphicBox, image generation, or visual pass.

const ROUTE_RECEIPT = "AUDRALIA_ROUTE_V6_HARD_BIND_CANVAS_CALLER_TNT_v1";
const REQUIRED_CANVAS_CONTRACT = "AUDRALIA_CANVAS_INTERACTION_SAFE_ORTHOGRAPHIC_4K_TNT_v6";
...
const CANVAS_AUTHORITY_PATH =
  "/assets/audralia/audralia.canvas.js?route=AUDRALIA_ROUTE_V6_HARD_BIND_CANVAS_CALLER_TNT_v1&canvas=AUDRALIA_CANVAS_INTERACTION_SAFE_ORTHOGRAPHIC_4K_TNT_v6";

const RUNTIME_AUTHORITY_PATH =
  "/assets/audralia/audralia.runtime.js?route=AUDRALIA_ROUTE_V6_HARD_BIND_CANVAS_CALLER_TNT_v1&runtime=AUDRALIA_RUNTIME_LAND_WATER_NORMALIZATION_TNT_v8";
...
function ensureProofPanel(mount) {
  let panel = document.querySelector("[data-audralia-route-v6-proof='true']");

  if (!panel) {
    panel = document.createElement("section");
    panel.dataset.audraliaRouteV6Proof = "true";
...
function writeProofPanel(mount, canvasStatus = null) {
  ...
  const status = canvasStatus || window.__AUDRALIA_CANVAS_STATUS__ || null;
...
async function importAndMountCanvas(mount) {
  try {
    ...
    writeProofPanel(mount, window.__AUDRALIA_CANVAS_STATUS__);
...
async function bootAudraliaRoute() {
  cleanRouteResidue();
  setStatusMessage("Audralia doorway loading v6 canvas authority.", "loading");
