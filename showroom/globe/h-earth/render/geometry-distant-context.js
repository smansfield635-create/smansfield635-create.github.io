/** H_EARTH_WORLD_MANIFOLD_FAR_CONTEXT_GEOMETRY_GEN306_v1 */
import {
...
export const H_EARTH_GEOMETRY_DISTANT_CONTEXT_CONTRACT_ID='H_EARTH_WORLD_MANIFOLD_FAR_CONTEXT_GEOMETRY_GEN306_v1';
...
export function constructHEarthDistantContextGeometry({cameraWorld={x:0,y:8,z:-40},rings=[520,820,1180,1680,2200],sectorCount=64}={}){
  ...
  for(let r=0;r<rows-1;r++){
    for(let c=0;c<cols;c++){
      ...
      if(cell.some(v=>v.terrainSilhouettePermitted!==true)){suppressedOceanCellCount++;continue;}
      indices.push(a,e,b,b,e,d);
    }
  }
  if(indices.length===0)issues.push('FAR_LAND_REPRESENTATION_EMPTY');
