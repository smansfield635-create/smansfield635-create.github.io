(()=>{
'use strict';
const VERSION='CAPABILITY_OBJECT_STAGE_v1';
const M=Math;
...
function mount(canvas,{meshFactory,foreground=()=>true,initialYaw=.32,initialPitch=-.05,spin=.000105,scale=1,dataset={}}={}){
 ...
 const pr=program(gl),mesh=meshFactory(gl);const loc={p:gl.getAttribLocation(pr,'aP'),n:gl.getAttribLocation(pr,'aN'),c:gl.getAttribLocation(pr,'aC'),m:gl.getUniformLocation(pr,'uM'),aspect:gl.getUniformLocation(pr,'uAspect')};
 Object.assign(canvas.dataset,{objectStage:VERSION,objectDepthModel:'TRUE_WEBGL_GEOMETRY',objectTriangleCount:String(mesh.triangles||mesh.count/3),...dataset});
 ...
 const api=Object.freeze({canvas,renderer:VERSION,capture:()=>{draw();return{renderer:VERSION,yaw,pitch}},inspect:()=>({renderer:VERSION,triangles:mesh.triangles||mesh.count/3,yaw,pitch,foreground:foreground()}),destroy:()=>{cancelAnimationFrame(raf);ro.disconnect();io.disconnect()}});scenes.set(canvas,api);return api
}
