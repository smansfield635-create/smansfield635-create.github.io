(function () {
  ...
  function update(state, step) {
    ...
    const dt = (typeof step === "number" && Number.isFinite(step)) ? step : 0.08;
  ...
  function getViewportTransform(state, width, height, focusPoint) {
    ...
    const focusX = (focusPoint && Number.isFinite(focusPoint.x)) ? focusPoint.x : 0;
    const focusY = (focusPoint && Number.isFinite(focusPoint.y)) ? focusPoint.y : 0;
