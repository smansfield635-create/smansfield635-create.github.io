(()=>{
  ...
  const interactive='a,button,input,select,textarea,[data-human-brain],[data-award-trophy]';
  ...
  function loadTrophy(done){if(window.CompassTrophyScene){done();return}const existing=document.querySelector('script[data-compass-trophy-scene]');if(existing){existing.addEventListener('load',done,{once:true});return}const script=document.createElement('script');script.src='/assets/compass/compass.trophy-scene.js?v=compass-volumetric-artifacts-v1';script.dataset.compassTrophyScene='true';script.addEventListener('load',done,{once:true});document.head.append(script)}
  ...
  function mountCapability(){
    ...
    const stage=element('section','compass-capability-orbit');stage.dataset.capabilityOrbit='true';stage.dataset.capabilityMode='orbit';stage.dataset.volumetricArtifacts='generation-1515';region(stage,'Signature Diamond Gate capabilities','compass-capability-guidance');
    ...
    function illuminateAwards(){clearTimeout(awardsTimer);awards.classList.remove('is-illuminating');if(reduce.matches||cardIndex!==1||mode!=='orbit')return;void awards.offsetWidth;awards.classList.add('is-illuminating');window.CompassTrophyScene?.activate();awardsTimer=setTimeout(()=>awards.classList.remove('is-illuminating'),800)}
    ...
    renderMembers();renderCards();settleCards();const canvas=diagnostic.querySelector('[data-human-brain] canvas');window.CompassBrainScene?.mount(canvas,{foreground:()=>mode==='orbit'&&cards[cardIndex].dataset.capability==='diagnostic'});const mountTrophy=()=>{const trophyCanvas=awards.querySelector('[data-award-trophy] canvas');window.CompassTrophyScene?.mount(trophyCanvas,{foreground:()=>mode==='orbit'&&cards[cardIndex].dataset.capability==='awards'})};loadTrophy(mountTrophy);
  }
  function mountProof(){
  ...
  const publicApi=Object.freeze({version:'root-award-finish-successor-v1',volumetricArtifacts:'generation-1515',mount});
