...
function activeCitiesForContinent(profile) {
  ...
  const baselineCitiesPerContinent =
    baselineStatesPerContinent * WORLD_KERNEL.hierarchy.citiesPerState;
...
function activeMetrosForContinent(profile) {
  ...
  const baselineMetrosPerContinent =
    baselineStatesPerContinent * WORLD_KERNEL.hierarchy.metrosPerState;
...
function resolveNode(sample) {
  ...
  const hasLatLon = Number.isFinite(source.latDeg) && Number.isFinite(source.lonDeg);
  if (hasLatLon) {
    const { best } = findBestContinent(source.latDeg, source.lonDeg);
    const profile = best?.profile ?? WORLD_KERNEL.continents[0];
    const local = continentNormalizedCoordinates(source.latDeg, source.lonDeg, profile);
    const structure = resolveCountryRegionState(local.u, local.v);

    return Object.freeze({
      id: structure.stateIndex,
      provisional: false,
      relation_type: "nearest_state_projection",
      lattice_scope: WORLD_KERNEL.hierarchy.latticeScope,
      node_scope: WORLD_KERNEL.hierarchy.nodeScope,
      nearest_id: structure.stateIndex,
      distance: 0,
      anchor: profile.id,
      transition_face: null
    });
  ...
  throw new Error("WORLD_KERNEL.resolveNode(sample) requires sample.stateIndex or sample.latDeg/lonDeg");
}
...
function buildSample(latDeg, lonDeg) {
  ...
  if (!best || !isLand) {
    ...
    const nodeRelation = resolveNode({ latDeg, lonDeg });
  ...
  const shorelineBand =
    elevation >= -WORLD_KERNEL.constants.shorelineBand &&
    elevation <= WORLD_KERNEL.constants.shorelineBand;
  ...
  const nodeRelation = resolveNode({
