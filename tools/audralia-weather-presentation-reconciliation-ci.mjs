...
async function waitForAudraliaReadinessAcrossFrameLifecycle(page,{timeout=105000}={}){
  const started=Date.now();
  let transientContextLosses=0;
  let lastTransientError=null;
  while(Date.now()-started<timeout){
    if(page.isClosed())throw new Error('AUDRALIA_GESTURE_PAGE_CLOSED_BEFORE_READY');
    try{
      const ready=await page.evaluate(()=>window.__AUDRALIA_WEATHER_PRESENTATION_RECONCILIATION__?.getRuntime?.()?.invariants?.pass===true);
      if(ready){
        const evidence=Object.freeze({result:'READY',transientContextLosses,elapsedMs:Date.now()-started,lastTransientError});
        fs.writeFileSync('/tmp/audralia-cdp-stability-evidence.txt',`${JSON.stringify(evidence)}\n`);
        return evidence;
      }
    }catch(error){
      const message=String(error?.message||error);
      if(!/(frame got detached|Execution context was destroyed|Cannot find context with specified id)/i.test(message))throw error;
      transientContextLosses++;
      lastTransientError=message;
    }
    await sleep(250);
  }
  throw new Error(`AUDRALIA_GESTURE_READINESS_TIMEOUT_AFTER_FRAME_LIFECYCLE_RECOVERY:${JSON.stringify({transientContextLosses,lastTransientError})}`);
}
...
try{
  ...
  fs.writeFileSync('/tmp/audralia-browser-process-evidence.txt',`${JSON.stringify({pid:browserProcess?.pid??null,exitCode:browserProcess?.exitCode??null})}\n`);
  ...
  await waitForAudraliaReadinessAcrossFrameLifecycle(page,{timeout:105000});
